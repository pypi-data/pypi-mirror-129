...
import io
import re
from skitai.tasks import Task

rx_prohibit = re.compile ("[\s\\\"':;|><&?]+")
def _name_securing (name):
    ...
    if not name:
        return 'noname'
    return rx_prohibit.sub ("_", name)


class FileWrapper:
    def __init__ (self, part):
        self.part = part
        self.name = _name_securing (part.get_remote_filename ())
        parts = self.name.split ('.')
        self.ext = ('.' + parts [-1]) if len (parts) > 1 else None
    ...
    def flush (self):
        if self.path is not None:
            return self.path
        with tempfile.NamedTemporaryFile (delete = False, suffix = self.ext, prefix = 'atila-') as f:
...
class File:
    ...
    def __init__ (self, filename, max_size, content_length = 0):
        ...
        self._buffer = None
        parts = _name_securing (filename).split (".")
        self.ext = ('.' + parts [-1]) if len (parts) > 1 else None
    ...
    def write (self, data):
        ...
        if not self._physical and self.size > self.in_memory:
            ...
            self.descriptor = tempfile.NamedTemporaryFile (delete = False, suffix = self.ext, prefix = 'atila-')
...
class Part:
    def __init__ (self, header, max_size):
        ...
        if self.content_type.startswith ("multipart/"):
            self.boundary = attr ["boundary"].replace ('"', "").encode ("utf8")
            self.value = []
            ...
            if val:
                ...
                if "filename" in attr and attr ["filename"]:
                    ...
                    if self.filename:
                        ...
                        self.value = File (self.filename, self.max_size, cl)
...
class MultipartCollector (collectors.FormCollector):
    ...
    def collect_incoming_data (self, data):
        ...
        if self.current_part:
            ...
            self.buffer += data
            if self.buffer == b"--" or self.buffer == b"--\r\n":
