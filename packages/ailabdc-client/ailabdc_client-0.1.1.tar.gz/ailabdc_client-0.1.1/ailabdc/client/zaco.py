...
class LabelTask:
    def __init__(self, annotation_dict=None, num_worker=4):
        '''
        ...
        tic = time.time()
        assert type(annotation_dict)==dict, 'annotation file format {} not supported'.format(type(annotation_dict))
        print('Done (t={:0.2f}s)'.format(time.time() - tic))
        self.label_task_data = annotation_dict
    ...
    def get_path_by_id(self, item_id):
        ...
        return self.get_path_by_item(item)
