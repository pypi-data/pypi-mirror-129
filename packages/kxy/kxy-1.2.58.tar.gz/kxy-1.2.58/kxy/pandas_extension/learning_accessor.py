...
class LearningAccessor(BaseAccessor):
	...
	def fit(self, target_column, learner_func, problem_type=None, snr='auto', train_frac=0.8, random_state=0, \
		...
		if getattr(self, 'models', None) is None or force_redo:
			...
			spinner.text = 'Lean Boosting: Baseline score (%s): %.3f' % (score, previous_score)
			...
			for i in range(self.start_n_features, n_variables+1):
				...
				if val_score > previous_score or (min_n_features and i<=min_n_features):
					...
					spinner.text = 'Lean Boosting: Variable #%d (%s) increased validation performance from %.3f to %.3f' % (i, self.variables[i-1], previous_score, val_score)
					...
					if n_down_perf >= n_down_perf_before_stop:
						...
						spinner.text = 'Lean Boosting: Stopping training as validation performance did not increase %d consecutive times.' % n_down_perf_before_stop
						spinner.succeed()
						break
				...
				if max_n_features and (i==max_n_features):
					...
					spinner.text = 'Lean Boosting: Stopping training as the maximum number of variables (%d) has been reached' % max_n_features
					spinner.succeed()
					break				
