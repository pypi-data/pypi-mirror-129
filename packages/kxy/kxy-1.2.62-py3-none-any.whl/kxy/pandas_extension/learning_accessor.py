...
class LearningAccessor(BaseAccessor):
	...
	def _non_additive_fit(self):
		pass


	def _additive_fit(self):
		pass
		...
		if getattr(self, 'models', None) is None or force_redo:
			...
			for i in range(self.start_n_features, n_variables+1):
				...
				if val_score > previous_score or (min_n_features and i<=min_n_features):
					...
					spinner.text = 'Lean Boosting -- %d Variables, Val %s: %.4f' % (i, score, val_score)
	...
	def batch_predict(self, obj):
		"""
		Same as :code:`predict(self, obj)`, but more memory friendly.
		"""
		n = obj.shape[0]
		max_n = 100000
		res = []
		i = 0
		while i < n:
			res +=[self.predict(obj.iloc[i:i+max_n])]
			i += max_n

		res = pd.concat(res, axis=0) if len(res) > 1 else res[0]
		return res
