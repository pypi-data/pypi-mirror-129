...
def representative_card(parsed, source_url):
    ...
    if match := _check_uid_and_url_match_source_url(cards):
        return match
    if match := _check_url_matches_rel_me(cards):
        return match
    if match := _check_url_matches_source_url(cards):
        return match
...
def _check_uid_and_url_match_source_url(cards):  # FIXME same as below?
    for card in cards:
...
def _check_url_matches_source_url(cards):  # FIXME same as above?
    found = []
    count = 0
    for card in cards:
