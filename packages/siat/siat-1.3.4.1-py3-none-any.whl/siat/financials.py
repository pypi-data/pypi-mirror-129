...
def prepare_fin_stmt_ak(ticker,start,end,period_type='all'):
    ...
    fs5['ticker']=ticker
    fs5['endDate']=fs5.index.strftime('%Y-%m-%d')
    fs5['periodType']=fs5['Month'].apply(lambda x: '年报' if x==12 else '季报')
    
    #转换数值类型
    for i in fs5.columns:
        try:
            fs5[i]=fs5[i].astype('float')
        except:
            continue
...
def calc_dupont_china(ticker,start,end,period_type='all'):
    """
    功能：计算股票ticker的杜邦分析项目，从akshare抓取财报，仅限于中国A股
    """
    fsr2=prepare_fin_stmt_ak(ticker,start,end,period_type)
    if fsr2 is None:
        print("  #Error(calc_dupont_china): failed to retrieved reports for",ticker)
        return None   
    
    if period_type=='annual':
        fsr3=fsr2[fsr2['periodType']=='年报']
    else:
        fsr3=fsr2
    ...
    fsr3['Total Asset Turnover']=fsr3['一、营业总收入']/fsr3['资产总计']
    fsr3['Equity Multiplier']=fsr3['资产总计']/fsr3['所有者权益(或股东权益)合计']
    
    dpidf=fsr3[['ticker','endDate','periodType','ROE','Profit Margin','Total Asset Turnover','Equity Multiplier']]    
...
def compare_dupont_china(tickerlist,fsdate='latest',scale1 = 10,scale2 = 10,hatchlist=['.', 'o', '\\']):
    """
    ...
    for t in tickerlist:
        try:
            ...
            print("  #Warning(compare_dupont_china): lack of some accounting items for",t)
            continue
        ...
        if len(dpidf)==0:
            ...
            continue
        dpi=dpidf.tail(1)
        #rptdate=dpi['endDate'].values[0]
        #rpttype=dpi['periodType'].values[0]
    ...
    for i in range(num):
        code=df.loc[i,'财报类型']
        if code == '季报': df.loc[i,'财报类型']='季报'
        else: df.loc[i,'财报类型']='年报'
    print(df.to_string(index=False))
    print("*** 数据来源：新浪/东方财富，"+str(today))
