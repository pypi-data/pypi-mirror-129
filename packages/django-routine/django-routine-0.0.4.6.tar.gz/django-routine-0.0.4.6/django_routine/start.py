...
class UpdateFiles:
    ...
    # function that find in file name INSTALLED_APPS and return list of installed apps

    # function that wwrite in file list of lines from list
    def write_in_file(self, list):
        with open('config/settings.py', 'w') as f:
            for line in list:
                f.write(line + '\n')

    # find in list 'INSTALLED_APPS' and add after this word 'drf_yasg' and 'rest_framework_swagger'
    def add_drf_yasg_rest_framework_swagger(self, list):
        for i in range(len(list)):
            if list[i] == 'INSTALLED_APPS = [':
                list.insert(i + 1, '    \'drf_yasg\',')
                list.insert(i + 2, '    \'rest_framework_swagger\',')
        self.write_in_file(list)

    def find_installed_apps(self):
        with open('config/settings.py') as f:
            data = f.read()
        self.add_drf_yasg_rest_framework_swagger(data.split('\n'))
...
def start():
    ...
    UpdateFiles().find_installed_apps()
    if len(sys.argv):
        for arg in sys.argv:
            if arg != sys.argv[0]:
