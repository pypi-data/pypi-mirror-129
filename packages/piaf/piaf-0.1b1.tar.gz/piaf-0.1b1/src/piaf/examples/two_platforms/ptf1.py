...
class FindOtherAgent(Behavior):
    ...
    def __init__(self, agent: "Agent", ams: "AID"):
        """
    ...
    async def action(self) -> None:
        ...
        # Read response
        agree_or_refuse = await self.agent.receive(
            MT_AND(
        ...
        if len(resp.acl_message.content) == 0:
...
if __name__ == "__main__":
    ...
    ap = platform.AgentPlatform("ptf1")
    ...
    async def main():
        ...
        aid = await ap.agent_manager.create(
            MyAgent, "my-agent", other=AID("other@ptf2", ["amqp://ptf2/acc"])
        )
