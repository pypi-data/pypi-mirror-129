...
class Counter():
    ...
    def snapshot(self, stop_ts=None, interval=None, update_rates=False):
        ...
        jsonData['uptime'] = round(time.time() - self._start_ts, INTERVAL_PRECISION)
        jsonData['time_active'] = round(self._stop_ts - self._start_ts, INTERVAL_PRECISION)
    ...
    @property
    def uptime(self):
        return round(time.time() - self._start_ts, INTERVAL_PRECISION)
...
class CounterEvent():
    ...
    def snapshot_table(self, stop_ts=None, interval=None, update_stats=False,
        ...
        o.append(f"Start: {start_ts_str:>22}     Stop: {stop_ts_str:>22}    Duration: {s.time_active:8.3f} (s)")
        if summary_only:
