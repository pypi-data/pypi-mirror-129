...
from pdx.git import UtilityFunctions
from pdx.git import RepoFunctions
import os
import re
...
def list_remotes():
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "remote"], trace_level="debug")
...
def add_remote(url, remote_name="origin"):
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "remote", "add", remote_name, url])
...
def get_remote_url(remote_name="origin"):
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "config", "--get", "remote.{0}.url".format(remote_name)], "debug")
...
def set_remote_url(url, remote_name="origin"):
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "remote", "set-url", remote_name, url])
    return exe.return_code == 0
...
def clone(repo_url, repo_dir_name=None):
    ...
    if ("pdx.edu" in repo_url) and (not RepoFunctions.on_psu_network):
        RepoFunctions.remote_disabled(skip_remote_check=True)
        return False

    # Get name of repo from parameter, or from the URL
    repo_name = repo_dir_name if repo_dir_name else UtilityFunctions.get_repo_from_url(repo_url)
    ...
    # Clone the repo
    exe = UtilityFunctions.run_git_cmd(["git", "clone", repo_url, repo_name])
...
def fetch():
    ...
    if not RepoFunctions.remote_disabled():
        exe = UtilityFunctions.run_git_cmd(['git', 'fetch', '--all'])
...
def branch_tracking():
    ...
    exe = UtilityFunctions.run_git_cmd(['git', 'branch', '-vv'])
...
def track_remote(remote_name="origin"):
    ...
    current = RepoFunctions.current_branch()
    ...
    if current in remotes:
        ...
        if current not in tracking:
            # set up tracking now
            exe = UtilityFunctions.run_git_cmd(
...
def pull_needed(status_output=None, warnings_in_session=False):
    ...
    if RepoFunctions.remote_disabled():
        ...
        if status_output is None:
            status_output = RepoFunctions.status()
    ...
    if answer is None:
        ...
        try:
            tracked_remote = branch_tracking()[RepoFunctions.current_branch()]
...
def pull(local_branch=None, remote_branch=None, remote_name="origin", rollback_conflicts=False, suppress_stderr=True):
    ...
    start_branch = RepoFunctions.current_branch()
    ...
    if local_branch is None:
        local_branch = RepoFunctions.current_branch()
    if remote_branch is None:
        remote_branch = local_branch

    # Make sure we're on the specified branch
    if RepoFunctions.checkout_branch(local_branch):

        # Pull changes
        exe = UtilityFunctions.run_git_cmd(["git", "pull", remote_name, remote_branch])
        ...
        if not success:
            RepoFunctions.checkout_branch(start_branch)
            return None
        ...
        if "CONFLICT " in output or "CONFLICT " in stderr:
            status = RepoFunctions.status_dict()
            ...
            if rollback_conflicts:
                ...
                if status['rebasing']['items']:
                    UtilityFunctions.run_git_cmd(['git', 'rebase', '--abort'])

                # Reset any modifications
                if RepoFunctions.reset():
...
def push(local_branch=None, remote_name="origin", force=None):
    ...
    if RepoFunctions.checkout_branch(local_branch):
        ...
        if force:
            exe = UtilityFunctions.run_git_cmd(["git", "push", "-uf", remote_name, local_branch])
        else:
            exe = UtilityFunctions.run_git_cmd(["git", "push", "-u", remote_name, local_branch])
...
def push_tags(remote_name="origin"):
    ...
    # Push changes
    exe = UtilityFunctions.run_git_cmd(['git', 'push', '--tags'], suppress_errors=True)
    stderr = exe.get_stderr()
...
def list_remote_branches(remote_name="origin"):
    ...
    exe = UtilityFunctions.run_git_cmd(git_cmd, "debug")
...
def delete_remote_branch(remote_branch, remote_name="origin"):
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "push", remote_name, "--delete", remote_branch])
...
def prune_remote(remote_name="origin"):
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "remote", "prune", remote_name])
    return exe.return_code == 0
