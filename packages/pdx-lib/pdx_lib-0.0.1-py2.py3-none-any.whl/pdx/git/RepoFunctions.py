...
on_psu_network = Utilities.is_on_psu_network()
...
def checkout_branch(branch_name, create_from="master"):
    ...
    UtilityFunctions.run_git_cmd(['git', 'checkout', branch_name])
...
def create_branch(branch_name, branch_from=None):
    ...
    UtilityFunctions.run_git_cmd(['git', 'checkout', '-b', branch_name])
...
def delete_branch(rm_branch):
    ...
    if this_branch == rm_branch:
        # Use master branch, if possible
        switch_to = 'master'
        # If master is not an option
    ...
    # Delete the branch
    if this_branch != rm_branch:
        exe = UtilityFunctions.run_git_cmd(["git", "branch", "-D", rm_branch])
        return exe.return_code == 0
...
def list_branches():
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "branch"], 'debug')
...
def list_all_branches(remote_name="origin"):
    ...
    exe = UtilityFunctions.run_git_cmd(git_cmd, trace_level="debug")
...
def list_tags():
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "tag"], 'debug')
...
def delete_tag(tag_name, remote_name="origin"):
    ...
    # Delete local tag
    exe = UtilityFunctions.run_git_cmd(["git", "tag", "-d", tag_name])
    ...
    if exe.return_code == 0 and not remote_disabled(remote_name):
        UtilityFunctions.run_git_cmd(["git", "push", remote_name, ":refs/tags/{0}".format(tag_name)])
...
def is_clean(allow_untracked=False):
    ...
    # Get status
    exe = UtilityFunctions.run_git_cmd(["git", "status"], trace_level="debug")
...
def status(branch_name=None):
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "status"])
...
def diff_branches(into_branch=None, from_branch=None):
    ...
    # Run the command
    exe = UtilityFunctions.run_git_cmd(git_cmd)
...
def merged_branches(target_branch="master"):
...
def get_commits(branch_name=None, max_commits=None, since_commit=None):
    ...
    for line in exe.get_output().splitlines():
        ...
        if line.startswith('commit'):
            ...
            commit['date'] = UtilityFunctions.git_string_to_date(line.replace('Date:', '').strip())
...
def get_common_ancestor_details(branch_1, branch_2):
    ...
    for line in exe.get_output().splitlines():
        ...
        if line.startswith('commit'):
            ...
            commit_details['date'] = UtilityFunctions.git_string_to_date(line.replace('Date:', '').strip())
