...
def add(path="."):
    ...
    # Add to Git
    exe = UtilityFunctions.run_git_cmd(["git", "add", "--all", path])
...
def remove(path):
    ...
    # Remove from Git
    exe = UtilityFunctions.run_git_cmd(["git", "rm", path])
...
def commit(commit_msg, amend=False):
    ...
    current_branch = RepoFunctions.current_branch()
    ...
    # Commit changes
    exe = UtilityFunctions.run_git_cmd(cmd)
...
def merge_preview(into_branch, from_branch, rollback_conflicts=True):
    ...
    if "origin/" in from_branch:
        if not RemoteFunctions.remote_branch_exists(from_branch.replace("origin/", "")):
    ...
    elif not RepoFunctions.branch_exists(from_branch):
...
def merge(into_branch, from_branch, rollback_conflicts=True, file_list_on_success=False):
    ...
    if "origin/" in from_branch:
        ...
        if RemoteFunctions.remote_branch_exists(from_branch) and RepoFunctions.checkout_branch(from_branch, None):
            RepoFunctions.checkout_branch(start_branch)
        else:
            out.put_error("Cannot merge non-existing branch: {0}".format(from_branch))
            return None
...
def rebase(into_branch, from_branch, rollback_conflicts=True):
    ...
    if "origin/" in into_branch:
        ...
        return None
    elif not RepoFunctions.branch_exists(into_branch):
    ...
    # Rebase branch
    exe = UtilityFunctions.run_git_cmd(["git", "rebase", from_branch])
...
def tag(version, remote_name="origin", push_tags=True):
    ...
    # Create tag
    exe = UtilityFunctions.run_git_cmd(["git", "tag", str(version)])

    if push_tags and exe.return_code == 0:
        RemoteFunctions.push_tags(remote_name)
...
def most_recent_tag():
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "describe", "--abbrev=0", "--tags"], suppress_errors=True)
    if exe.return_code == 0:
...
def stash():
    """Stash all changes"""
    out.trace("stash", [])
    exe = UtilityFunctions.run_git_cmd(["git", "stash"])
    return exe.return_code == 0
...
def pop():
    ...
    out.trace("pop", [])
    exe = UtilityFunctions.run_git_cmd(["git", "stash", "pop"])
    return exe.return_code == 0
...
def get_previous_version(file_path, tag_or_commit="HEAD~1"):
    ...
    exe = UtilityFunctions.run_git_cmd(["git", "checkout", tag_or_commit, '--', file_path])
    return exe.return_code == 0
