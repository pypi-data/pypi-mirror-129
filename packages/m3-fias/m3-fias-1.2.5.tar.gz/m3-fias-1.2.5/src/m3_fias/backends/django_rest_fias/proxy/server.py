...
from requests.packages.urllib3.util.retry import Retry  # noqa E0401
...
# Количество переотправок запросов в ФИАС
FIAS_RETRY_COUNT = settings.FIAS.get('RETRY_COUNT', 0)
...
class ServerBase(with_metaclass(ABCMeta, object)):
    ...
    @staticmethod
    def set_session_retries(session, retry_count=FIAS_RETRY_COUNT):
        """Устанавливает количество переотправок запросов для сессии.

        :param session: Текущая сессия
        :type session: requests.sessions.Session
        :param retry_count: Количество переотправок запросов при ошибках
        :type retry_count: int
        """
        assert retry_count >= 0, 'Количество переотправок должно быть >= 0'
...
class SimpleServer(ServerBase):
    ...
    def _session(self):
        ...
        self.set_session_retries(result)
