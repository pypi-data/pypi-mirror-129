"""
pycord.features.root_command
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The pycord root command.
...
from pycord.modules import package_version
...
class RootCommand(Feature):
    """
    Feature containing the root pyc command
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.pyc.hidden = Flags.HIDE

    @Feature.Command(
        name="pycord", aliases=["pyc"], invoke_without_command=True, ignore_extra=False
    )
    async def pyc(self, ctx: commands.Context):  # pylint: disable=too-many-branches
        """
        The pycord debug and diagnostic commands.
        ...
        summary = [
            f"pycord v{package_version('pycord')}, {package_name} `{package_version(package_name)}`, "
    ...
    @Feature.Command(parent="pyc", name="hide")
    async def pyc_hide(self, ctx: commands.Context):
        """
        Hides pycord from the help command.
        """

        if self.pyc.hidden:
            return await ctx.send("pycord is already hidden.")

        self.pyc.hidden = True
        await ctx.send("pycord is now hidden.")

    @Feature.Command(parent="pyc", name="show")
    async def pyc_show(self, ctx: commands.Context):
        """
        Shows pycord in the help command.
        """

        if not self.pyc.hidden:
            return await ctx.send("pycord is already visible.")

        self.pyc.hidden = False
        await ctx.send("pycord is now visible.")

    # pylint: enable=no-member

    @Feature.Command(parent="pyc", name="tasks")
    async def pyc_tasks(self, ctx: commands.Context):
        """
        Shows the currently running pycord tasks.
        """
    ...
    @Feature.Command(parent="pyc", name="cancel")
    async def pyc_cancel(self, ctx: commands.Context, *, index: typing.Union[int, str]):
        """
