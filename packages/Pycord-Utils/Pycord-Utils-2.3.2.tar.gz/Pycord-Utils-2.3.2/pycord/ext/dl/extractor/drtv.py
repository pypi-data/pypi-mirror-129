    ...
    def _real_extract(self, url):
        ...
        if video_id:
            ...
            page = self._parse_json(
                self._search_regex(
                    r"data\s*=\s*({.+?})\s*(?:;|</script)", webpage, "data"
                ),
                "1",
            )["cache"]["page"]
            page = page[list(page.keys())[0]]
            item = try_get(
                page, (lambda x: x["item"], lambda x: x["entries"][0]["item"]), dict
            )
        ...
        title = str_or_none(data.get("Title")) or re.sub(
            r"\s*\|\s*(?:TV\s*\|\s*DR|DRTV)$", "", self._og_search_title(webpage)
        )
        description = self._og_search_description(webpage, default=None) or data.get(
            "Description"
        )
        ...
        def decrypt_uri(e):
            ...
            key = bytes_to_intlist(
                hashlib.sha256(
                    ("%s:sRBzYNXBzkKgnjj8pGtkACch" % a).encode("utf-8")
                ).digest()
            )
            iv = bytes_to_intlist(hex_to_bytes(a))
            decrypted = aes_cbc_decrypt(data, key, iv)
            return (
                intlist_to_bytes(decrypted[: -decrypted[-1]])
                .decode("utf-8")
                .split("?")[0]
            )
