...
class Channel9IE(InfoExtractor):
    ...
    def _extract_list(self, video_id, rss_url=None):
        ...
        entries = [
            self.url_result(session_url.text, "Channel9")
    ...
    def _real_extract(self, url):
        ...
        if episode_data:
            ...
            content_url = (
                "https://channel9.msdn.com/odata"
                + episode_data["api"]
                + "?$select=Captions,CommentCount,MediaLengthInSeconds,PublishedDate,Rating,RatingCount,Title,VideoMP4High,VideoMP4Low,VideoMP4Medium,VideoPlayerPreviewImage,VideoWMV,VideoWMVHQ,Views,"
            )
            if is_session:
                content_url += (
                    "Code,Description,Room,Slides,Speakers,ZipFile&$expand=Speakers"
                )
            else:
