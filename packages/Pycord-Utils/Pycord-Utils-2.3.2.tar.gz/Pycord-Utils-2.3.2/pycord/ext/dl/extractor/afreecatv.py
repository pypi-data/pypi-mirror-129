...
class AfreecaTVIE(InfoExtractor):
    ...
    def _login(self):
        ...
        if result != 1:
            ...
            raise ExtractorError(
                "Unable to login: %s said: %s" % (self.IE_NAME, error), expected=True
            )
    ...
    def _real_extract(self, url):
        ...
        if re.search(r'alert\(["\']This video has been deleted', webpage):
            raise ExtractorError("Video %s has been deleted" % video_id, expected=True)

        station_id = self._search_regex(r"nStationNo\s*=\s*(\d+)", webpage, "station")
        bbs_id = self._search_regex(r"nBbsNo\s*=\s*(\d+)", webpage, "bbs")
