...
class TuneInBaseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        streams = self._download_json(
            ...
            transform_source=lambda s: re.sub(r"^\s*\((.*)\);\s*$", r"\1", s),
        )["Streams"]
        ...
        for stream in streams:
            ...
            format_note = (
                "Reliability: %d%%" % reliability if reliability is not None else None
            )
            formats.append(
                {
...
class TuneInStationIE(TuneInBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if TuneInClipIE.suitable(url)
            else super(TuneInStationIE, cls).suitable(url)
        )
