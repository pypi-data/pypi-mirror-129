...
class KUSIIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?kusi\.com/(?P<path>story/.+|video\?clipId=(?P<clipId>\d+))"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        xml_url = update_url_query(
            "http://www.kusi.com/build.asp",
            {
        ...
        quality_options = doc.find("{http://search.yahoo.com/mrss/}group").findall(
