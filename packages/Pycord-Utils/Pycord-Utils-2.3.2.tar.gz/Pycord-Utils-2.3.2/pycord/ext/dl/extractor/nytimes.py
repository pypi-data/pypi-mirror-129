...
class NYTimesBaseIE(InfoExtractor):
    ...
    def _extract_video_from_id(self, video_id):
        ...
        hm = hmac.new(
            self._SECRET, (path + ":vhs").encode(), hashlib.sha512
        ).hexdigest()
                ...
                formats.append(
                    {
                        ...
                        "filesize": get_file_size(
                            video.get("file_size") or video.get("fileSize")
                        ),
        ...
        self._sort_formats(
            formats, ("height", "width", "filesize", "tbr", "fps", "format_id")
        )
...
class NYTimesArticleIE(NYTimesBaseIE):
    ...
    def _extract_podcast_from_json(self, json, page_id, webpage):
        ...
        title = (
            "%s: %s" % (podcast_title, episode_title)
            if podcast_title
            else episode_title
        )
    ...
    def _real_extract(self, url):
        ...
        video_id = self._search_regex(
            r'data-videoid=["\'](\d+)', webpage, "video id", default=None, fatal=False
        )
