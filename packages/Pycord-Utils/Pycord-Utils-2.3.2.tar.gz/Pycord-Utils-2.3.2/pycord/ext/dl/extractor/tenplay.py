...
class TenPlayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        m3u8_url = self._request_webpage(
            HEADRequest(self._FASTLY_URL_TEMPL % brightcove_id), brightcove_id
        ).geturl()
        ...
        return {
            ...
            or metadata.get("pageContentName")
            or metadata["showContentName"],
            "description": video.get("description"),
            "age_limit": parse_age_limit(
                video.get("showRatingClassification")
                or metadata.get("showProgramClassification")
            ),
            "series": metadata.get("showName"),
            "season": metadata.get("showContentSeason"),
            "timestamp": parse_iso8601(
                metadata.get("contentPublishDate")
                or metadata.get("pageContentPublishDate")
            ),
