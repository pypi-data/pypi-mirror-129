...
class LivestreamIE(InfoExtractor):
    ...
    def _parse_smil_formats(
        ...
        for vn in video_nodes:
            ...
            furl = update_url_query(
                compat_urlparse.urljoin(base, vn.attrib["src"]),
                {
                    "v": "3.0.3",
                    "fp": "WIN% 14,0,0,145",
                },
    ...
    def _extract_stream_info(self, stream_info):
        ...
        return {
            "id": broadcast_id,
            "formats": formats,
            "title": self._live_title(stream_info["stream_title"])
            if is_live
            else stream_info["stream_title"],
    ...
    def _extract_event(self, event_data):
        ...
        for i in itertools.count(1):
            ...
            videos_info = self._download_json(
                info_url, event_id, "Downloading page {0}".format(i)
            )["data"]
...
class LivestreamOriginalIE(InfoExtractor):
    ...
    def _extract_video_info(self, user, video_id):
        api_url = (
            "http://x%sx.api.channel.livestream.com/2.0/clipdetails?extendedInfo=true&id=%s"
            % (user, video_id)
        )
