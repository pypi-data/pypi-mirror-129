...
class CBCWatchBaseIE(InfoExtractor):
    ...
    def _signature(self, email, password):
        ...
        resp = self._download_json(
            self._LOGIN_URL, None, data=data, headers=headers, query=query
        )
        ...
        for _ in range(2):
            try:
                ...
                if isinstance(e.cause, compat_HTTPError) and e.cause.code == 401:
                    ...
                    continue
                raise
        error_message = xpath_text(result, "userMessage") or xpath_text(
    ...
    def _real_initialize(self):
        if self._valid_device_token():
            return
        device = self._downloader.cache.load("cbcwatch", self._cache_device_key()) or {}
    ...
    def _cache_device_key(self):
        ...
        return (
            "%s_device" % hashlib.sha256(email.encode()).hexdigest()
            if email
            else "device"
        )
    ...
    def _register_device(self):
        ...
        if email and password:
            ...
            data = "<login><token>{0}</token><device><deviceId>{1}</deviceId><type>web</type></device></login>".format(
                escape(signature), escape(self._device_id)
            ).encode()
            url = self._API_BASE_URL + "device/login"
            result = self._download_xml(
                url, None, data=data, headers={"content-type": "application/xml"}
            )
            ...
            entries.append(
                {
                    ...
                    "age_limit": parse_age_limit(
                        xpath_text(item, _add_ns("media:rating"))
                    ),
                    "episode": xpath_text(item, _add_ns("clearleap:episode")),
                    "episode_number": int_or_none(
                        xpath_text(item, _add_ns("clearleap:episodeInSeason"))
                    ),
                    "series": xpath_text(item, _add_ns("clearleap:series")),
                    "season_number": int_or_none(
                        xpath_text(item, _add_ns("clearleap:season"))
                    ),
...
class CBCWatchVideoIE(CBCWatchBaseIE):
    ...
    def _real_extract(self, url):
        ...
        formats = self._extract_m3u8_formats(
            re.sub(r"/([^/]+)/[^/?]+\.m3u8", r"/\1/\1.m3u8", m3u8_url),
            video_id,
            "mp4",
            fatal=False,
        )
