...
class SoundcloudIE(InfoExtractor):
    ...
    def _download_json(self, *args, **kwargs):
        ...
        for _ in range(2):
            ...
            kwargs["query"] = query
            try:
                return super(SoundcloudIE, self)._download_json(
    ...
    def _real_initialize(self):
        self._CLIENT_ID = (
            self._downloader.cache.load("soundcloud", "client_id")
            or "YUKXoArFcqrlQn9tfNHvvyfnDISj04zk"
        )
    ...
    def _extract_info_dict(self, info, full_title=None, secret_token=None):
        ...
        if info.get("downloadable") and info.get("has_downloads_left"):
            ...
            redirect_url = (
                self._download_json(download_url, track_id, fatal=False) or {}
            ).get("redirectUri")
            if redirect_url:
        ...
        def add_format(f, protocol, is_preview=False):
            mobj = re.search(
                r"\.(?P<abr>\d+)\.(?P<ext>[0-9a-z]{3,4})(?=[/?])", stream_url
            )
            if mobj:
        ...
        for t in transcodings:
            ...
            if not format_url:
                continue
            stream = self._download_json(format_url, track_id, query=query, fatal=False)
            ...
            if re.search(self._IMAGE_REPL_RE, thumbnail):
                for image_id, size in self._ARTWORK_MAP.items():
                    i = {
                        "id": image_id,
                        "url": re.sub(
                            self._IMAGE_REPL_RE, "-%s.jpg" % image_id, thumbnail
                        ),
                    }
...
class SoundcloudSetIE(SoundcloudPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info = self._download_json(
            self._resolv_url(self._BASE_URL + full_title), full_title
        )

        if "errors" in info:
            msgs = (compat_str(err["error_message"]) for err in info["errors"])
            raise ExtractorError(
                "unable to download video webpage: %s" % ",".join(msgs)
            )
