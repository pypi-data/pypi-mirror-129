...
class VierIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if r'id="user-login"' in webpage:
            self.report_warning("Log in to extract metadata", video_id=display_id)
            webpage = self._download_webpage(
                "http://www.%s.be/video/v3/embed/%s" % (site, video_id), display_id
            )
        ...
        video_id = self._search_regex(
            ...
            webpage,
            "video id",
            default=video_id or display_id,
        )
        ...
        if not playlist_url:
            ...
            playlist_url = (
                "http://vod.streamcloud.be/%s/_definst_/mp4:%s.mp4/playlist.m3u8"
