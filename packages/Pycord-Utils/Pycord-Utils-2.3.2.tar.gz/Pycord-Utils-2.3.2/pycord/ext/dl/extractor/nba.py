...
class NBAWatchIE(NBAWatchBaseIE):
    ...
    def _real_extract(self, url):
        ...
        collection_id = compat_parse_qs(compat_urllib_parse_urlparse(url).query).get(
            "collection", [None]
        )[0]
        if collection_id:
            if self._downloader.params.get("noplaylist"):
                self.to_screen(
                    "Downloading just video %s because of --no-playlist" % display_id
                )
            else:
                self.to_screen(
