...
class VRTIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        attrs = extract_attributes(
            self._search_regex(r'(<[^>]+class="vrtvideo"[^>]*>)', webpage, "vrt video")
        )
        ...
        title = strip_or_none(
            get_element_by_class("vrt-title", webpage)
            or self._html_search_meta(["og:title", "twitter:title", "name"], webpage)
        )
