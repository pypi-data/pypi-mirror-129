...
class AnimeOnDemandIE(InfoExtractor):
    ...
    def _login(self):
        ...
        if (
            ">Our licensing terms allow the distribution of animes only to German-speaking countries of Europe"
            in login_page
        ):
    ...
    def _real_extract(self, url):
        ...
        if "data-playlist=" not in webpage:
            self._download_webpage(
                self._APPLY_HTML5_URL,
                anime_id,
                "Activating HTML5 beta",
                "Unable to apply HTML5 beta",
            )
            webpage = self._download_webpage(url, anime_id)
        ...
        anime_title = self._html_search_regex(
            r'(?s)<h1[^>]+itemprop="name"[^>]*>(.+?)</h1>', webpage, "anime name"
        )
        ...
        def extract_episodes(html):
            for num, episode_html in enumerate(
                re.findall(
                    r'(?s)<h3[^>]+class="episodebox-title".+?>Episodeninhalt<', html
                ),
                1,
            ):
