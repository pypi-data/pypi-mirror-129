...
class SixPlayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            "https://pc.middleware.6play.fr/6play/v2/platforms/m6group_web/services/%s/videos/clip_%s"
            % (service, video_id),
            video_id,
        ...
        for asset in assets:
            ...
            if (
                not asset_url
                or (
                    (protocol == "primetime" or asset.get("type") == "usp_hlsfp_h264")
                    and not ("_drmnp.ism/" in asset_url or "_unpnp.ism/" in asset_url)
                )
            ...
            if container == "m3u8" or ext == "m3u8":
                if protocol == "usp":
                    if compat_parse_qs(
                        compat_urllib_parse_urlparse(asset_url).query
                    ).get("token", [None])[0]:
