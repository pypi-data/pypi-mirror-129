...
class FacebookIE(InfoExtractor):
    ...
    _api_config = {"graphURI": "/api/graphql/"}
    ...
    def _login(self):
        ...
        try:
            ...
            self._downloader.report_warning(
                "unable to log in: %s" % error_to_compat_str(err)
            )
            return
    ...
    def _extract_from_url(self, url, video_id):
        ...
        def extract_relay_prefetched_data(_filter):
            ...
            for require in replay_data.get("require") or []:
                if require[0] == "RelayPrefetchedStreamCache":
                    return (
                        try_get(
                            require, lambda x: x[3][1]["__bbox"]["result"]["data"], dict
                        )
                        or {}
                    )
            ...
            if data:
                ...
                def parse_graphql_video(video):
                    ...
                    info = {
                        ...
                        "formats": formats,
                        "thumbnail": try_get(
                            video, lambda x: x["thumbnailImage"]["uri"]
                        ),
                ...
                for node in nodes:
                    story = (
                        try_get(
                            node,
                            lambda x: x["comet_sections"]["content"]["story"],
                            dict,
                        )
                        or {}
                    )
        ...
        uploader = (
            ...
            or self._search_regex(
                r'ownerName\s*:\s*"([^"]+)"', webpage, "uploader", default=None
            )
            or self._og_search_title(webpage, fatal=False)
        )
        timestamp = int_or_none(
            self._search_regex(
                r'<abbr[^>]+data-utime=["\'](\d+)', webpage, "timestamp", default=None
            )
        )
...
class FacebookPluginsVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        return self.url_result(
            compat_urllib_parse_unquote(self._match_id(url)), FacebookIE.ie_key()
        )
