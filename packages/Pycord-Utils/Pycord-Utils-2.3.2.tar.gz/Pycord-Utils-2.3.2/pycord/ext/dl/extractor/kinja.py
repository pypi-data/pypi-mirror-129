...
class KinjaEmbedIE(InfoExtractor):
    ...
    def _extract_urls(webpage, url):
        return [
            urljoin(url, unescapeHTML(mobj.group("url")))
            for mobj in re.finditer(
                r'(?x)<iframe[^>]+?src=(?P<q>["\'])(?P<url>(?:(?:https?:)?//%s)?%s(?:(?!\1).)+)\1'
                % (KinjaEmbedIE._DOMAIN_REGEX, KinjaEmbedIE._COMMON_REGEX),
                webpage,
            )
        ]
    ...
    def _real_extract(self, url):
        ...
        if video_type == "kinjavideo":
            ...
            if poster_id:
                thumbnail = (
                    "https://i.kinja-img.com/gawker-media/image/upload/%s.%s"
            ...
            return {
                ...
                "title": title,
                "thumbnail": try_get(
                    iptc, lambda x: x["cloudinaryLink"]["link"], compat_str
                ),
                "uploader": fmg.get("network"),
                "duration": int_or_none(iptc.get("fileDuration")),
                "formats": formats,
                "description": try_get(
                    iptc, lambda x: x["description"]["en"], compat_str
                ),
