    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, compat_HTTPError) and e.cause.code == 403:
                msg = self._parse_json(e.cause.read().decode("utf-8"), lecture_id)
        ...
        if parts:
            ...
            def extract_part(part_id):
                ...
                info["id"] = (
                    lecture_id if not multipart else "%s_part%s" % (lecture_id, part_id)
                )
                info["display_id"] = (
                    lecture_slug
                    if not multipart
                    else "%s_part%s" % (lecture_slug, part_id)
                )
                if multipart:
