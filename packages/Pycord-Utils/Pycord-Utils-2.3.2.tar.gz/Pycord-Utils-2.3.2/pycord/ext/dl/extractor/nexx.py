...
class NexxIE(InfoExtractor):
    ...
    def _call_api(self, domain_id, path, video_id, data=None, headers={}):
        ...
        result = self._download_json(
            "https://api.nexx.cloud/v3/%s/%s" % (domain_id, path),
            video_id,
            "Downloading %s JSON" % path,
        ...
        def get_cdn_shield_base(shield_type="", static=False):
            for secure in ("", "s"):
                cdn_shield = stream_data.get(
                    "cdnShield%sHTTP%s" % (shield_type, secure.upper())
                )
                if cdn_shield:
    ...
    def _real_extract(self, url):
        ...
        response = self._download_json(
            "https://arc.nexx.cloud/api/video/%s.json" % video_id, video_id, fatal=False
        )
        ...
        if not video:
            ...
            request_token = hashlib.md5(
                "".join((op, domain_id, secret)).encode("utf-8")
            ).hexdigest()

            result = self._call_api(
                domain_id,
                "videos/%s/%s" % (op, video_id),
                video_id,
                data={
        ...
        return {
            ...
            "thumbnail": try_get(video, lambda x: x["imagedata"]["thumb"], compat_str),
            "duration": parse_duration(general.get("runtime")),
            "timestamp": int_or_none(general.get("uploaded")),
            "episode_number": int_or_none(
                try_get(video, lambda x: x["episodedata"]["episode"])
            ),
            "season_number": int_or_none(
                try_get(video, lambda x: x["episodedata"]["season"])
            ),
            "formats": formats,
        }
