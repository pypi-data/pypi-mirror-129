...
class YouNowLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            "https://api.younow.com/php/api/broadcast/info/curId=0/user=%s" % username,
            username,
        )
        ...
        uploader = (
            try_get(data, lambda x: x["user"]["profileUrlString"], compat_str)
            or username
        )
...
class YouNowChannelIE(InfoExtractor):
    ...
    _TEST = {
        ...
        "info_dict": {"id": "14629760", "title": "its_Kateee_ moments"},
        "playlist_mincount": 8,
    }
    ...
    def _entries(self, username, channel_id):
        ...
        for page_num in itertools.count(1):
            ...
            info = self._download_json(
                ...
                % (CDN_API_BASE, channel_id, created_before),
                username,
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(username, channel_id), channel_id, "%s moments" % username
        )
