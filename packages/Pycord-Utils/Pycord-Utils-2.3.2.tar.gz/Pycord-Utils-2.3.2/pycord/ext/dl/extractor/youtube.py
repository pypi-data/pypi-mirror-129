...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def _login(self):
        ...
        lookup_results = req(
            ...
            "Unable to look up account info",
        )
        ...
        challenge_results = req(
            self._CHALLENGE_URL, challenge_req, "Logging in", "Unable to log in"
        )
        ...
        if login_challenge:
            ...
            if challenge_str == "TWO_STEP_VERIFICATION":
                ...
                tfa_results = req(
                    self._TFA_URL.format(tl),
                    tfa_req,
                    "Submitting TFA code",
                    "Unable to submit TFA code",
                )
    ...
    def _extract_yt_initial_data(self, video_id, webpage):
        return self._parse_json(
            self._search_regex(
                (
                    r"%s\s*%s"
                    % (self._YT_INITIAL_DATA_RE, self._YT_INITIAL_BOUNDARY_RE),
    ...
    def _extract_video(self, renderer):
        ...
        description = try_get(
            renderer, lambda x: x["descriptionSnippet"]["runs"][0]["text"], compat_str
        )
        duration = parse_duration(
            try_get(renderer, lambda x: x["lengthText"]["simpleText"], compat_str)
        )
        view_count_text = (
            try_get(renderer, lambda x: x["viewCountText"]["simpleText"], compat_str)
            or ""
        )
        view_count = str_to_int(
            self._search_regex(
                r"^([\d,]+)",
                re.sub(r"\s", "", view_count_text),
                "view count",
                default=None,
            )
        )
...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _extract_signature_function(self, video_id, player_url, example_sig):
        ...
        func_id = "js_%s_%s" % (player_id, self._signature_cache_id(example_sig))
    ...
    def _extract_chapters_from_json(self, data, video_id, duration):
        chapters_list = try_get(
            data,
            lambda x: x["playerOverlays"]["playerOverlayRenderer"][
        ...
        def chapter_time(chapter):
            return float_or_none(
                try_get(
                    chapter, lambda x: x["chapterRenderer"]["timeRangeStartMillis"], int
                ),
                scale=1000,
            )
        ...
        for next_num, chapter in enumerate(chapters_list, start=1):
            ...
            if start_time is None:
                continue
            end_time = (
                chapter_time(chapters_list[next_num])
                if next_num < len(chapters_list)
                else duration
            )
            if end_time is None:
                continue
            title = try_get(
                chapter,
                lambda x: x["chapterRenderer"]["title"]["simpleText"],
                compat_str,
            )
            chapters.append(
                {
    ...
    def _extract_yt_initial_variable(self, webpage, regex, video_id, name):
        return self._parse_json(
            self._search_regex(
                (r"%s\s*%s" % (regex, self._YT_INITIAL_BOUNDARY_RE), regex),
                webpage,
                name,
                default="{}",
            ),
    ...
    def _real_extract(self, url):
        ...
        if not player_response:
            player_response = self._call_api("player", {"videoId": video_id}, video_id)

        playability_status = player_response.get("playabilityStatus") or {}
        if playability_status.get("reason") == "Sign in to confirm your age":
            video_info = self._download_webpage(
                base_url + "get_video_info",
                video_id,
        ...
        trailer_video_id = try_get(
            playability_status,
            lambda x: x["errorScreen"]["playerLegacyDesktopYpcTrailerRenderer"][
                "trailerVideoId"
            ],
            compat_str,
        )
        ...
        if not smuggled_data.get("force_singlefeed", False):
            if not self._downloader.params.get("noplaylist"):
                multifeed_metadata_list = try_get(
                    player_response,
                    lambda x: x["multicamera"]["playerLegacyMulticameraRenderer"][
                        "metadataList"
                    ],
                    compat_str,
                )
        ...
        for container in (video_details, microformat):
            for thumbnail in (
                try_get(container, lambda x: x["thumbnail"]["thumbnails"], list) or []
            ):
        ...
        duration = int_or_none(
            video_details.get("lengthSeconds") or microformat.get("lengthSeconds")
        ...
        info = {
            ...
            "upload_date": unified_strdate(
                microformat.get("uploadDate") or search_meta("uploadDate")
            ),
            "uploader": video_details["author"],
            "uploader_id": self._search_regex(
                r"/(?:channel|user)/([^/?&#]+)", owner_profile_url, "uploader id"
            )
        ...
        pctr = try_get(
            player_response,
            lambda x: x["captions"]["playerCaptionsTracklistRenderer"],
            dict,
        )
        if pctr:
                ...
                info.update(
                    {
                        ...
                        "artist": mobj.group("clean_artist")
                        or ", ".join(
        ...
        if webpage:
            initial_data = self._extract_yt_initial_variable(
                webpage, self._YT_INITIAL_DATA_RE, video_id, "yt initial data"
            )
        if not initial_data:
        ...
        if initial_data:
            ...
            if not chapters:
                for engagment_pannel in initial_data.get("engagementPanels") or []:
                    contents = try_get(
                        engagment_pannel,
                        lambda x: x["engagementPanelSectionListRenderer"]["content"][
                    ...
                    def chapter_time(mmlir):
                        return parse_duration(get_text(mmlir.get("timeDescription")))
            ...
            contents = (
                try_get(
                    initial_data,
                    lambda x: x["contents"]["twoColumnWatchNextResults"]["results"][
                        "results"
                    ]["contents"],
                    list,
                )
            ...
            for content in contents:
                ...
                if vpir:
                    ...
                    sbr_tooltip = try_get(
                        vpir,
                        lambda x: x["sentimentBar"]["sentimentBarRenderer"]["tooltip"],
                    )
                    if sbr_tooltip:
...
class YoutubeTabIE(YoutubeBaseInfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False if YoutubeIE.suitable(url) else super(YoutubeTabIE, cls).suitable(url)
        )
    ...
    def _extract_continuation(cls, renderer):
        ...
        for content in contents:
            ...
            continuation_ep = try_get(
                content,
                lambda x: x["continuationItemRenderer"]["continuationEndpoint"],
                dict,
            )
    ...
    def _entries(self, tab, item_id, webpage):
        ...
        for page_num in itertools.count(1):
            ...
            visitor_data = (
                try_get(
                    response, lambda x: x["responseContext"]["visitorData"], compat_str
                )
                or visitor_data
            )
            ...
            if continuation_items:
                ...
                ) or continuation_item.get("itemSectionRenderer")
                if renderer:
    ...
    def _extract_uploader(data):
        ...
        if sidebar_renderer:
            for item in sidebar_renderer:
                ...
                owner = try_get(
                    renderer,
                    lambda x: x["videoOwner"]["videoOwnerRenderer"]["title"]["runs"][0],
                    dict,
                )
                if owner:
                    ...
                    uploader["uploader_id"] = try_get(
                        owner,
                        lambda x: x["navigationEndpoint"]["browseEndpoint"]["browseId"],
                        compat_str,
                    )
                    uploader["uploader_url"] = urljoin(
                        ...
                        try_get(
                            owner,
                            lambda x: x["navigationEndpoint"]["browseEndpoint"][
                                "canonicalBaseUrl"
                            ],
                            compat_str,
                        ),
    ...
    def _extract_alert(data):
        ...
        for alert in try_get(data, lambda x: x["alerts"], list) or []:
            if not isinstance(alert, dict):
                continue
            alert_text = try_get(alert, lambda x: x["alertRenderer"]["text"], dict)
            if not alert_text:
                continue
            text = try_get(
                alert_text,
    ...
    def _extract_from_playlist(self, item_id, url, data, playlist):
        ...
        return self.playlist_result(
            self._playlist_entries(playlist),
            playlist_id=playlist_id,
            playlist_title=title,
        )
        ...
        for page_num in itertools.count(1):
            ...
            slr_contents = try_get(
                ...
                (
                    ...
                    lambda x: x["onResponseReceivedCommands"][0][
            ...
            for slr_content in slr_contents:
                isr_contents = try_get(
                    slr_content, lambda x: x["itemSectionRenderer"]["contents"], list
                )
                if not isr_contents:
                    continue
            ...
            token = try_get(
                slr_contents,
                lambda x: x[-1]["continuationItemRenderer"]["continuationEndpoint"][
                    "continuationCommand"
                ]["token"],
                compat_str,
            )
