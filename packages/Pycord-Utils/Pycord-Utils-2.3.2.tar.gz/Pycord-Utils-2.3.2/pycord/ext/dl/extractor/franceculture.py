...
class FranceCultureIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?franceculture\.fr/emissions/(?:[^/]+/)*(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        title = (
            video_data.get("data-asset-title")
            or video_data.get("data-diffusion-title")
        ...
        return {
            ...
            "uploader": uploader,
            "timestamp": int_or_none(video_data.get("data-start-time"))
            or int_or_none(video_data.get("data-asset-created-date")),
