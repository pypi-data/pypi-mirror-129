...
class TBSIE(TurnerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_data = next(
            v for v in drupal_settings["turner_playlist"] if v.get("url") == path
        )

        media_id = video_data["mediaID"]
        title = video_data["title"]
        tokenizer_query = compat_parse_qs(
            compat_urllib_parse_urlparse(drupal_settings["ngtv_token_url"]).query
        )
