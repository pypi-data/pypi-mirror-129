...
class RTBFIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._parse_json(
            self._html_search_regex(r'data-media="([^"]+)"', embed_page, "media data"),
            media_id,
        )
        ...
        fix_url = (
            lambda x: x.replace("//rtbf-vod.", "//rtbf.") if "/geo/drm/" in x else x
        )
        ...
        if not data.get("drm") and mpd_url:
            formats.extend(
                self._extract_mpd_formats(mpd_url, media_id, mpd_id="dash", fatal=False)
            )
