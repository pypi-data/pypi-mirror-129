...
class SteamIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if (
            re.search("<h2>Please enter your birth date to continue:</h2>", webpage)
            is not None
        ):
        ...
        flash_vars = self._parse_json(
            self._search_regex(
                r"(?s)rgMovieFlashvars\s*=\s*({.+?});", webpage, "flash vars"
            ),
            playlist_id,
            js_to_json,
        )
        ...
        if fileID:
            ...
            for movie_id, movie in flash_vars.items():
                ...
                video_id = self._search_regex(
                    r"movie_(\d+)", movie_id, "video id", fatal=False
                )
                ...
                if highlight_element:
                    ...
                    if highlight_attribs:
                        ...
                        for quality in ("", "-hd"):
                            for ext in ("webm", "mp4"):
                                video_url = highlight_attribs.get(
                                    "data-%s%s-source" % (ext, quality)
                                )
                                if video_url:
                                    formats.append(
                                        {
