...
class EllenTubeBaseIE(InfoExtractor):
    def _extract_data_config(self, webpage, video_id):
        details = self._search_regex(
            r'(<[^>]+\bdata-component=(["\'])[Dd]etails.+?></div>)', webpage, "details"
        )
...
class EllenTubeVideoIE(EllenTubeBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            "ellentube:%s" % video_id, ie=EllenTubeIE.ie_key(), video_id=video_id
        )


class EllenTubePlaylistIE(EllenTubeBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?ellentube\.com/(?:episode|studios)/(?P<id>.+?)\.html"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        feed = self._download_json(
            "https://api-prod.ellentube.com/ellenapi/api/feed/?%s" % data["filter"],
            display_id,
        )
        entries = [
            self._extract_video(elem, elem["id"])
            for elem in feed
            if elem.get("type") == "VIDEO" and elem.get("id")
        ]
