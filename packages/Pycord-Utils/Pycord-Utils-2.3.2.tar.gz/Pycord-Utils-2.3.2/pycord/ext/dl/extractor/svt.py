    ...
    def _extract_video(self, video_info, video_id):
        ...
        if not formats and rights.get("geoBlockedSweden"):
            self.raise_geo_restricted(
                "This video is only available in Sweden", countries=self._GEO_COUNTRIES
            )
...
class SVTPlayIE(SVTPlayBaseIE):
    ...
    def _extract_by_video_id(self, video_id, webpage=None):
        ...
        if not info_dict.get("title"):
            ...
            if not title and webpage:
                title = re.sub(r"\s*\|\s*.+?$", "", self._og_search_title(webpage))
            if not title:
                title = video_id
    ...
    def _real_extract(self, url):
        ...
        data = self._parse_json(
            self._search_regex(
                self._SVTPLAY_RE, webpage, "embedded data", default="{}", group="json"
            ),
            video_id,
            fatal=False,
        )
        ...
        if data:
            video_info = try_get(
                data,
                lambda x: x["context"]["dispatcher"]["stores"]["VideoTitlePageStore"][
                    "data"
                ]["video"],
                dict,
            )
            if video_info:
                ...
                info_dict.update(
                    {
                        "title": data["context"]["dispatcher"]["stores"]["MetaStore"][
                            "title"
                        ],
                        "thumbnail": thumbnail,
                    }
            ...
            svt_id = try_get(
                data, lambda x: x["statistics"]["dataLake"]["content"]["id"], compat_str
            )
...
class SVTSeriesIE(SVTPlayBaseIE):
    ...
    def suitable(cls, url):
        return (
            ...
            else super(SVTSeriesIE, cls).suitable(url)
        )
...
class SVTPageIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if SVTIE.suitable(url) or SVTPlayIE.suitable(url)
            else super(SVTPageIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        def _process_content(content):
            if content.get("_type") in ("VIDEOCLIP", "VIDEOEPISODE"):
                ...
                entries.append(
                    self.url_result("svt:" + video_id, SVTPlayIE.ie_key(), video_id)
                )
        ...
        return self.playlist_result(
            entries, str_or_none(article.get("id")), strip_or_none(article.get("title"))
        )
