    ...
    def _real_extract(self, url):
        ...
        if isinstance(videos, dict):
            for format_id, format_url in videos.items():
                ...
                m = re.search(r"/(?P<height>\d+)[pP]_(?P<tbr>\d+)[kK]", video_url)
                if m:
                    tbr = int(m.group("tbr"))
        ...
        webpage = self._download_webpage(
            "https://www.spankwire.com/_/video%s/" % video_id, video_id, fatal=False
        )
        if webpage:
            ...
            if not view_count:
                view_count = str_to_int(
                    self._search_regex(
                        r'data-views=["\']([\d,.]+)', webpage, "view count", fatal=False
                    )
                )
