...
class MedalTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        hydration_data = self._parse_json(
            self._search_regex(
        ...
        def add_item(container, item_url, height, id_key="format_id", item_id=None):
            ...
            container.append(
                {"url": item_url, id_key: item_id, "width": width, "height": height}
            )
        ...
        if not formats and error:
            if error == 404:
                ...
                raise ExtractorError(
                    "An unknown error occurred ({0}).".format(error), video_id=video_id
                )
        ...
        author = (
            try_get(hydration_data, lambda x: list(x["profiles"].values())[0], dict)
            or {}
        )
        author_id = str_or_none(author.get("id"))
        author_url = (
            "https://medal.tv/users/{0}".format(author_id) if author_id else None
        )
