...
class ViceIE(ViceBaseIE, AdobePassIE):
    ...
    title""",
        ...
        for subtitle in preplay.get("subtitleURLs", []):
            ...
            language_code = (
                try_get(
                    subtitle, lambda x: x["languages"][0]["language_code"], compat_str
                )
                or "en"
            )
        ...
        return {
            ...
            "series": try_get(
                video_data, lambda x: x["show"]["base"]["display_title"], compat_str
            ),
...
class ViceShowIE(ViceBaseIE):
    ...
    id
    url""",
            ", page: %d, per_page: %d" % (page + 1, self._PAGE_SIZE),
        )
        for video in videos:
...
class ViceArticleIE(ViceBaseIE):
    ...
    embed_code""",
        ...
        embed_code = self._search_regex(
            r'embedCode=([^&\'"]+)', body, "ooyala embed code", default=None
        )
        if embed_code:
