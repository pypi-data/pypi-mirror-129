...
class PBSIE(InfoExtractor):
    ...
    IE_NAME = "pbs"
    IE_DESC = "Public Broadcasting Service (PBS) and member stations: %s" % ", ".join(
        list(zip(*_STATIONS))[1]
    )
    ...
    def _real_extract(self, url):
        ...
        if isinstance(video_id, list):
            entries = [
                self.url_result("http://video.pbs.org/video/%s" % vid_id, "PBS", vid_id)
        ...
        if http_url:
            for m3u8_format in m3u8_formats:
                ...
                if not self._is_valid_url(
                    f_url, display_id, "http-%sk video" % bitrate
                ):
                    continue
                f = m3u8_format.copy()
                f.update(
                    ...
                    [
                        {
                            "url": closed_captions_url.replace(
                                ttml_caption_suffix,
                                "/%d_Encoded.srt" % (ttml_caption_id + 1),
                            ),
                            "ext": "srt",
                        },
                        {
                            "url": closed_captions_url.replace(
                                ttml_caption_suffix,
                                "/%d_Encoded.vtt" % (ttml_caption_id + 2),
                            ),
                            "ext": "vtt",
                        },
                    ]
