...
class LBRYBaseIE(InfoExtractor):
    ...
    def _parse_stream(self, stream, url):
        ...
        info = {
            "thumbnail": try_get(
                stream_value, lambda x: x["thumbnail"]["url"], compat_str
            ),
            ...
            "ext": determine_ext(source.get("name"))
            or mimetype2ext(source.get("media_type")),
...
class LBRYIE(LBRYBaseIE):
    ...
    _VALID_URL = (
        ...
        + r"(?P<id>\$/[^/]+/[^/]+/{1}|@{0}/{0}|(?!@){0})".format(
            LBRYBaseIE._OPT_CLAIM_ID, LBRYBaseIE._CLAIM_ID_REGEX
        )
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        streaming_url = self._call_api_proxy(
            "get", claim_id, {"uri": uri}, "streaming url"
        )["streaming_url"]
...
class LBRYChannelIE(LBRYBaseIE):
    ...
    def _real_extract(self, url):
        ...
        result = self._resolve_url("lbry://" + display_id, display_id, "channel")
        claim_id = result["claim_id"]
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, claim_id, url, params), self._PAGE_SIZE
        )
        result_value = result.get("value") or {}
        return self.playlist_result(
            entries,
            claim_id,
            result_value.get("title"),
