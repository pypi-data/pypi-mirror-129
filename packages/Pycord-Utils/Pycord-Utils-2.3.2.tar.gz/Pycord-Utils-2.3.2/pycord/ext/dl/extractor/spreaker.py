...
def _extract_episode(data, episode_id=None):
    ...
    def stats(key):
        return int_or_none(
            try_get(
                data,
                (lambda x: x["%ss_count" % key], lambda x: x["stats"]["%ss" % key]),
            )
        )
...
class SpreakerIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            "https://api.spreaker.com/v2/episodes/%s" % episode_id, episode_id
        )["response"]["episode"]
...
class SpreakerPageIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        episode_id = self._search_regex(
            (r'data-episode_id=["\'](?P<id>\d+)', r"episode_id\s*:\s*(?P<id>\d+)"),
            webpage,
            "episode id",
        )
        return self.url_result(
            "https://api.spreaker.com/episode/%s" % episode_id,
            ie=SpreakerIE.ie_key(),
            video_id=episode_id,
        )
    ...
    def _entries(self, show_id):
        for page_num in itertools.count(1):
            episodes = self._download_json(
                ...
                show_id,
                note="Downloading JSON page %d" % page_num,
                query={
                    "page": page_num,
                    "max_per_page": 100,
                },
...
class SpreakerShowPageIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        display_id = self._match_id(url)
        webpage = self._download_webpage(url, display_id)
        show_id = self._search_regex(r"show_id\s*:\s*(?P<id>\d+)", webpage, "show id")
        return self.url_result(
            "https://api.spreaker.com/show/%s" % show_id,
            ie=SpreakerShowIE.ie_key(),
            video_id=show_id,
        )
