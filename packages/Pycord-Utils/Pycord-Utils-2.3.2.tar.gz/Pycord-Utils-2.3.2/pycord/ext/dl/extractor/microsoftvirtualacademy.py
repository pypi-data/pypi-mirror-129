...
class MicrosoftVirtualAcademyIE(MicrosoftVirtualAcademyBaseIE):
    ...
    _VALID_URL = (
        r"(?:%s:|https?://(?:mva\.microsoft|(?:www\.)?microsoftvirtualacademy)\.com/[^/]+/training-courses/[^/?#&]+-)(?P<course_id>\d+)(?::|\?l=)(?P<id>[\da-zA-Z]+_\d+)"
        % IE_NAME
    )
    ...
    def _real_extract(self, url):
        ...
        base_url = smuggled_data.get("base_url") or self._extract_base_url(
            course_id, video_id
        )
        ...
        _, title = self._extract_chapter_and_title(
            xpath_text(settings, ".//Title", "title", fatal=True)
        )
...
class MicrosoftVirtualAcademyCourseIE(MicrosoftVirtualAcademyBaseIE):
    ...
    _VALID_URL = (
        r"(?:%s:|https?://(?:mva\.microsoft|(?:www\.)?microsoftvirtualacademy)\.com/[^/]+/training-courses/(?P<display_id>[^/?#&]+)-)(?P<id>\d+)"
        % IE_NAME
    )
    ...
    def suitable(cls, url):
        return (
            False
            if MicrosoftVirtualAcademyIE.suitable(url)
            else super(MicrosoftVirtualAcademyCourseIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        manifest = self._download_json(
            ...
            display_id,
            "Downloading course manifest JSON",
        )["manifest"]
