...
class TVNetIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        title = (
            ...
            or self._search_regex(r"<title>([^<]+)<", webpage, "title")
        )
        ...
        data_file = unescapeHTML(
            self._search_regex(
                r'data-file=(["\'])(?P<url>(?:https?:)?//.+?)\1',
                webpage,
                "data file",
                group="url",
            )
        ...
        thumbnail = self._og_search_thumbnail(webpage, default=None) or unescapeHTML(
            self._search_regex(
                r'data-image=(["\'])(?P<url>(?:https?:)?//.+?)\1',
                webpage,
                "thumbnail",
                default=None,
                group="url",
