...
class YandexMusicBaseIE(InfoExtractor):
    ...
    def _download_webpage_handle(self, *args, **kwargs):
        webpage = super(YandexMusicBaseIE, self)._download_webpage_handle(
            *args, **kwargs
        )
        if (
...
class YandexMusicTrackIE(YandexMusicBaseIE):
    ...
    _VALID_URL = (
        r"%s/album/(?P<album_id>\d+)/track/(?P<id>\d+)"
        % YandexMusicBaseIE._VALID_URL_BASE
    )
    ...
    def _extract_tracks(self, source, item_id, url, tld):
        ...
        if len(tracks) < len(track_ids):
            present_track_ids = set(
                [compat_str(track["id"]) for track in tracks if track.get("id")]
            )
            missing_track_ids = [
...
class YandexMusicAlbumIE(YandexMusicPlaylistBaseIE):
    ...
    def suitable(cls, url):
        return (
            ...
            else super(YandexMusicAlbumIE, cls).suitable(url)
        )
...
class YandexMusicPlaylistIE(YandexMusicPlaylistBaseIE):
    ...
    _VALID_URL = (
        r"%s/users/(?P<user>[^/]+)/playlists/(?P<id>\d+)"
        % YandexMusicBaseIE._VALID_URL_BASE
    )
