...
class NetzkinoIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?netzkino\.de/\#!/(?P<category>[^/]+)/(?P<id>[^/]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        api_url = (
            "http://api.netzkino.de.simplecache.net/capi-2.0a/categories/%s.json?d=www"
            % category_id
        )
        api_info = self._download_json(api_url, video_id)
        info = next(p for p in api_info["posts"] if p["slug"] == video_id)
