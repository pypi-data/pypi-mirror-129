...
class TVerIE(InfoExtractor):
    ...
    def _real_initialize(self):
        self._TOKEN = self._download_json("https://tver.jp/api/access_token.php", None)[
            "token"
        ]
    ...
    def _real_extract(self, url):
        ...
        main = self._download_json(
            "https://api.tver.jp/v4/" + path, video_id, query={"token": self._TOKEN}
        )["main"]
        ...
        return {
            ...
            "episode_number": int_or_none(
                try_get(main, lambda x: x["ext"]["episode_number"])
            ),
            "url": bc_url,
