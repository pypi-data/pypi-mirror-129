...
class BiliBiliIE(InfoExtractor):
    ...
    def _report_error(self, result):
        if "message" in result:
            raise ExtractorError(
                "%s said: %s" % (self.IE_NAME, result["message"]), expected=True
            )
        elif "code" in result:
            raise ExtractorError(
                "%s returns error %d" % (self.IE_NAME, result["code"]), expected=True
            )
        else:
    ...
    def _real_extract(self, url):
        ...
        if "anime/" not in url:
            cid = (
                self._search_regex(
                    r'\bcid(?:["\']:|=)(\d+)', webpage, "cid", default=None
                )
                or compat_parse_qs(
                    self._search_regex(
                        [
                ...
                self.to_screen(
                    ...
                    % (
                        ...
                        compat_urlparse.urljoin(
                            url, "//bangumi.bilibili.com/anime/%s" % anime_id
                        ),
                    )
                )
            headers = {
        ...
        for num, rendition in enumerate(RENDITIONS, start=1):
            ...
            sign = hashlib.md5(
                (payload + self._BILIBILI_KEY).encode("utf-8")
            ).hexdigest()
        ...
        timestamp = unified_timestamp(
            ...
            or self._html_search_meta("uploadDate", webpage, "timestamp", default=None)
        )
...
class BiliBiliBangumiIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if BiliBiliIE.suitable(url)
            else super(BiliBiliBangumiIE, cls).suitable(url)
        )
