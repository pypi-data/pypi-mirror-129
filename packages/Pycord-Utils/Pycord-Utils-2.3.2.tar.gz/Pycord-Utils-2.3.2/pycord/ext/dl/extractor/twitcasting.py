...
class TwitCastingIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:[^/]+\.)?twitcasting\.tv/(?P<uploader_id>[^/]+)/movie/(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if not m3u8_url:
            video_js_data = self._parse_json(
                self._search_regex(
                    r"data-movie-playlist='(\[[^']+\])'", webpage, "movie playlist"
                ),
                video_id,
            )[0]
        ...
        thumbnail = video_js_data.get("thumbnailUrl") or self._og_search_thumbnail(
            webpage
        )
        ...
        duration = float_or_none(video_js_data.get("duration"), 1000) or parse_duration(
            clean_html(get_element_by_class("tw-player-duration-time", webpage))
        )
        view_count = str_to_int(
            self._search_regex(r"Total\s*:\s*([\d,]+)\s*Views", webpage, "views", None)
        )
