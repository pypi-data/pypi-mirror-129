        ...
        for site in (353, 183):
            ...
            if site == 353:
                ...
                timestamp = (
                    self._download_json(
                        ...
                        data=json.dumps(
                            {"method": "da.timestamp.get", "params": []}
                        ).encode(),
                        fatal=False,
                    )
                    or {}
                ...
                query = {
                    "ts": timestamp,
                    "sign": CMAC.new(
                        self._LIGHT_KEY, timestamp.encode() + content_data, Blowfish
                    ).hexdigest(),
                }
            else:
                query = {}
            ...
            if error:
                ...
                if origin == "NotAllowedForLocation":
                    ...
                    raise ExtractorError(
                        "pycryptodomex not found. Please install it.", expected=True
                    )
                elif message:
...
class IviCompilationIE(InfoExtractor):
    ...
    def _extract_entries(self, html, compilation_id):
        return [
            self.url_result(
                "http://www.ivi.ru/watch/%s/%s" % (compilation_id, serie),
                IviIE.ie_key(),
            )
