    ...
    def _real_extract(self, url):
        ...
        try:
            response = self._download_json(
                "https://www.filmon.com/api/vod/movie?id=%s" % video_id, video_id
            )["response"]
        except ExtractorError as e:
            if isinstance(e.cause, compat_HTTPError):
                errmsg = self._parse_json(e.cause.read().decode(), video_id)["reason"]
                raise ExtractorError(
                    "%s said: %s" % (self.IE_NAME, errmsg), expected=True
                )
            raise
...
class FilmOnChannelIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, compat_HTTPError):
                errmsg = self._parse_json(e.cause.read().decode(), channel_id)[
                    "message"
                ]
                raise ExtractorError(
                    "%s said: %s" % (self.IE_NAME, errmsg), expected=True
