...
class TestURLIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        matching_extractors = [e for e in all_extractors if rex.search(e.IE_NAME)]

        if len(matching_extractors) == 0:
            raise ExtractorError(
                "No extractors matching %r found" % extractor_id, expected=True
            )
        ...
        try:
            ...
            raise ExtractorError(
                ("Test case %d not found, got only %d tests" % (num, len(testcases))),
                expected=True,
            )
