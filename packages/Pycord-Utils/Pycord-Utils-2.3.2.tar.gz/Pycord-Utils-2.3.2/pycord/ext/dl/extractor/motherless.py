...
class MotherlessIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?motherless\.com/(?:g/[a-z0-9_]+/)?(?P<id>[A-Z0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if ">The content you are trying to view is for friends only." in webpage:
            raise ExtractorError(
                "Video %s is for friends only" % video_id, expected=True
            )
        ...
        video_url = (
            ...
            or "http://cdn4.videos.motherlessmedia.com/videos/%s.mp4?fs=opencloud"
            % video_id
        )
        ...
        if not upload_date:
            ...
            if uploaded_ago:
                ...
                upload_date = (
                    datetime.datetime.utcnow() - datetime.timedelta(**kwargs)
                ).strftime("%Y%m%d")

        comment_count = webpage.count('class="media-comment-contents"')
        uploader_id = self._html_search_regex(
            r'"thumb-member-username">\s+<a href="/m/([^"]+)"', webpage, "uploader_id"
        )
...
class MotherlessGroupIE(InfoExtractor):
    ...
    def _extract_entries(self, webpage, base):
        ...
        if not entries:
            entries = [
                ...
                for entry_id in orderedSet(
                    re.findall(r'data-codename=["\']([A-Z0-9]+)', webpage)
                )
            ]
        return entries
    ...
    def _real_extract(self, url):
        ...
        page_count = self._int(
            self._search_regex(
                r"(\d+)</(?:a|span)><(?:a|span)[^>]+>\s*NEXT", webpage, "page_count"
            ),
            "page_count",
        )
        PAGE_SIZE = 80
