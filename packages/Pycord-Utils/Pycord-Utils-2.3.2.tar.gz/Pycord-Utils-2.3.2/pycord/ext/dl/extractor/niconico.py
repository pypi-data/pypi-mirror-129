...
class NiconicoIE(InfoExtractor):
    ...
    def _extract_format_for_quality(
        self, api_data, video_id, audio_quality, video_quality
    ):
        def yesno(boolean):
        ...
        format_id = "-".join(
            map(
                lambda s: remove_start(s["id"], "archive_"),
            ...
            data=json.dumps(
                {
                    "session": {
                        ...
                        "content_auth": {
                            "auth_type": session_api_data["auth_types"][
                                session_api_data["protocols"][0]
                            ],
                            "content_key_timeout": session_api_data[
                                "content_key_timeout"
                            ],
            ...
            if dmc_info:  # "New" HTML5 videos
                ...
                for audio_quality in quality_info["audios"]:
                    for video_quality in quality_info["videos"]:
                        if (
                            not audio_quality["available"]
                            or not video_quality["available"]
                        ):
                            continue
                        formats.append(
                            self._extract_format_for_quality(
...
class NiconicoPlaylistIE(InfoExtractor):
    ...
    def _call_api(self, list_id, resource, query):
        return self._download_json(
            "https://nvapi.nicovideo.jp/v2/mylists/" + list_id,
            list_id,
            "Downloading %s JSON metatdata" % resource,
            query=query,
