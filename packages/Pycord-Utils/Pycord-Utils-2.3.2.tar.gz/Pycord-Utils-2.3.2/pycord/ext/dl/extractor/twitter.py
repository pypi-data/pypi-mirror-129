...
class TwitterBaseIE(InfoExtractor):
    ...
    def _extract_formats_from_vmap_url(self, vmap_url, video_id):
        ...
        for video_variant in vmap_data.findall(
            ...
            formats.extend(
                self._extract_variant_formats(video_variant.attrib, video_id)
            )
        ...
        try:
            ...
            if isinstance(e.cause, compat_HTTPError) and e.cause.code == 403:
                raise ExtractorError(
                    self._parse_json(e.cause.read().decode(), video_id)["errors"][0][
                        "message"
                    ],
                    expected=True,
                )
...
class TwitterIE(TwitterBaseIE):
    IE_NAME = "twitter"
    _VALID_URL = (
        TwitterBaseIE._BASE_REGEX + r"(?:(?:i/web|[^/]+)/status|statuses)/(?P<id>\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        info = {
            ...
            "uploader_url": "https://twitter.com/" + uploader_id
            if uploader_id
            else None,
...
class TwitterBroadcastIE(TwitterBaseIE, PeriscopeBaseIE):
    ...
    def _real_extract(self, url):
        ...
        source = self._call_api("live_video_stream/status/" + media_key, media_key)[
            "source"
        ]
        m3u8_url = source.get("noRedirectPlaybackUrl") or source["location"]
        if "/live_video_stream/geoblocked/" in m3u8_url:
            self.raise_geo_restricted()
        m3u8_id = compat_parse_qs(compat_urllib_parse_urlparse(m3u8_url).query).get(
            "type", [None]
        )[0]
