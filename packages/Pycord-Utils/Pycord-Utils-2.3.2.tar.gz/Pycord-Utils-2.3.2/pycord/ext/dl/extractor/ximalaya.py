...
class XimalayaIE(XimalayaBaseIE):
    ...
    IE_DESC = "喜马拉雅FM"
    _VALID_URL = (
        r"https?://(?:www\.|m\.)?ximalaya\.com/(?P<uid>[0-9]+)/sound/(?P<id>[0-9]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        if is_m:
            audio_description = self._html_search_regex(
                r'(?s)<section\s+class=["\']content[^>]+>(.+?)</section>',
                webpage,
                "audio_description",
                fatal=False,
            )
            ...
            audio_description = self._html_search_regex(
                r'(?s)<div\s+class=["\']rich_intro[^>]*>(.+?</article>)',
                webpage,
                "audio_description",
                fatal=False,
            )
        ...
        if not audio_description:
            audio_description_file = "%s://www.ximalaya.com/sounds/%s/rich_intro" % (
                scheme,
                audio_id,
            )
            audio_description = self._download_webpage(
        ...
        return {
            ...
            "categories": list(
                filter(
                    None,
                    (audio_info.get("category_name"), audio_info.get("category_title")),
                )
            ),
...
class XimalayaAlbumIE(XimalayaBaseIE):
    ...
    IE_DESC = "喜马拉雅FM 专辑"
    _VALID_URL = (
        r"https?://(?:www\.|m\.)?ximalaya\.com/(?P<uid>[0-9]+)/album/(?P<id>[0-9]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            self._TEMPLATE_URL % (scheme, uid, playlist_id),
            playlist_id,
        ...
        return self.playlist_result(
            self._entries(webpage, playlist_id, uid), playlist_id, title
        )
    ...
    def _entries(self, page, playlist_id, uid):
        ...
        for page_num in itertools.count(1):
            ...
            next_url = self._search_regex(
                r'<a\s+href=(["\'])(?P<more>[\S]+)\1[^>]+rel=(["\'])next\3',
                html,
                "list_next_url",
                default=None,
                group="more",
    ...
    def _process_page(self, html, uid):
        ...
        for mobj in re.finditer(self._LIST_VIDEO_RE % uid, html[find_from:]):
            yield self.url_result(
                self._BASE_URL_TEMPL % (self.scheme, mobj.group("url")),
                XimalayaIE.ie_key(),
                mobj.group("id"),
                mobj.group("title"),
            )
