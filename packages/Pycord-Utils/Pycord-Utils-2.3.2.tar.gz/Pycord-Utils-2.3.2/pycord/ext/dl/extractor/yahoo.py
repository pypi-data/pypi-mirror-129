...
class YahooIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        item = self._download_json(
            "https://%s.yahoo.com/caas/content/article" % country,
            display_id,
...
class YahooSearchIE(SearchInfoExtractor):
    ...
    def _get_n_results(self, query, n):
        ...
        for pagenum in itertools.count(0):
            result_url = (
                "http://video.search.yahoo.com/search/?p=%s&fr=screen&o=js&gs=0&b=%d"
...
class YahooGyaOIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        videos = self._download_json(
            "https://gyao.yahoo.co.jp/api/programs/%s/videos" % program_id, program_id
        )["videos"]
        entries = []
        for video in videos:
...
class YahooJapanNewsIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        {"url": "https://news.yahoo.co.jp/feature/1356", "only_matching": True},
    ]
    ...
    def _real_extract(self, url):
        ...
        if display_id == host:
            ...
            stream_plists = re.findall(r"plist=(\d+)", webpage) or re.findall(
                r'plist["\']:\s*["\']([^"\']+)', webpage
            )
            entries = [
                self.url_result(
