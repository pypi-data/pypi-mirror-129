...
class ZingMp3BaseIE(InfoExtractor):
    _VALID_URL_TMPL = (
        r"https?://(?:mp3\.zing|zingmp3)\.vn/(?:%s)/[^/]+/(?P<id>\w+)\.html"
    )
    _GEO_COUNTRIES = ["VN"]
    ...
    def _real_extract(self, url):
        ...
        data_path = self._search_regex(r'data-xml="([^"]+)', webpage, "data path")
        return self._process_data(
            self._download_json("https://mp3.zing.vn/xhr" + data_path, page_id)["data"]
        )
