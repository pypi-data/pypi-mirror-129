                    ...
                    if supplier == "asx":
                        ...
                        if protocol in ("http", "https"):
                            ...
                            fmt.update(
                                {
                                    "url": "%s://%s/%s?%s"
                                    % (protocol, server, application, auth_string),
    ...
    def _real_extract(self, url):
        ...
        if group_id:
            return self.url_result(
                "https://www.bbc.co.uk/programmes/%s" % group_id, ie=BBCCoUkIE.ie_key()
            )
        ...
        programme_id = self._search_regex(
            [
                ...
                r'<param[^>]+name="externalIdentifier"[^>]+value="(%s)"'
                % self._ID_REGEX,
        ...
        if morph_payload:
            components = (
                try_get(morph_payload, lambda x: x["body"]["components"], list) or []
            )
        ...
        if preload_state:
            ...
            if (
                current_programme
                and programme_id
                and current_programme.get("type") == "playable_item"
            ):
                title = (
                    current_programme.get("titles", {}).get("tertiary")
                    or playlist_title
                )
                formats, subtitles = self._download_media_selector(programme_id)
                self._sort_formats(formats)
        ...
        if initial_data:
            ...
            for resp in (initial_data.get("data") or {}).values():
                ...
                if name == "media-experience":
                    ...
                    for block in (
                        try_get(resp, lambda x: x["data"]["blocks"], list) or []
                    ):
        ...
        EMBED_URL = (
            r'https?://(?:www\.)?bbc\.co\.uk/(?:[^/]+/)+%s(?:\b[^"]+)?' % self._ID_REGEX
        )
        entries = []
        ...
        entries.extend(re.findall(r'setPlaylist\("(%s)"\)' % EMBED_URL, webpage))
        if entries:
            return self.playlist_result(
        ...
        for num, media_meta in enumerate(medias, start=1):
            ...
            if not video_id:
                video_id = (
                    playlist_id if len(medias) == 1 else "%s-%s" % (playlist_id, num)
                )
            ...
            if not title:
                title = (
                    playlist_title
                    if len(medias) == 1
                    else "%s - Video %s" % (playlist_title, num)
                )
    ...
    def _entries(self, webpage, url, playlist_id):
        ...
        for page_num in itertools.count(2):
            ...
            webpage = self._download_webpage(
                compat_urlparse.urljoin(url, next_page),
                playlist_id,
...
class BBCCoUkIPlayerPlaylistBaseIE(InfoExtractor):
    _VALID_URL_TMPL = (
        r"https?://(?:www\.)?bbc\.co\.uk/iplayer/%%s/(?P<id>%s)" % BBCCoUkIE._ID_REGEX
    )
...
class BBCCoUkPlaylistIE(BBCCoUkPlaylistBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?bbc\.co\.uk/programmes/(?P<id>%s)/(?:episodes|broadcasts|clips)"
        % BBCCoUkIE._ID_REGEX
    )
