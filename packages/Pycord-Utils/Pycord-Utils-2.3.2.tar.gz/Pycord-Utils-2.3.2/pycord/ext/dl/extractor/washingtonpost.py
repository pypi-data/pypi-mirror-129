...
class WashingtonPostIE(InfoExtractor):
    ...
    def _extract_urls(cls, webpage):
        return re.findall(r'<iframe[^>]+\bsrc=["\'](%s)' % cls._EMBED_URL, webpage)
...
class WashingtonPostArticleIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if WashingtonPostIE.suitable(url)
            else super(WashingtonPostArticleIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result("washingtonpost:%s" % uuid, "WashingtonPost", uuid)
            for uuid in uuids
        ]
