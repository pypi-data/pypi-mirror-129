...
class AsianCrushBaseIE(InfoExtractor):
    ...
    _KALTURA_KEYS = [
        ...
        "download_url",
        "thumbnail_url",
        "widescreen_thumbnail_url",
        "screencap_widescreen",
    ]
    _API_SUFFIX = {"retrocrush.tv": "-ott"}

    def _call_api(self, host, endpoint, video_id, query, resource):
        return self._download_json(
            "https://api%s.%s/%s" % (self._API_SUFFIX.get(host, ""), host, endpoint),
            video_id,
    ...
    def _download_object_data(self, host, object_id, resource):
        return self._call_api(host, "search", object_id, {"id": object_id}, resource)[0]

    def _get_object_description(self, obj):
        return strip_or_none(
            obj.get("long_description") or obj.get("short_description")
        )
...
class AsianCrushIE(AsianCrushBaseIE):
    _VALID_URL = (
        r"%s/video/(?:[^/]+/)?0+(?P<id>\d+)v\b" % AsianCrushBaseIE._VALID_URL_BASE
    )
    _TESTS = [
        {
        ...
        if host == "cocoro.tv":
            ...
            for mobj in re.finditer(
                ...
                if attrs.get("class") == "clearfix":
                    entries.append(
                        self.url_result(mobj.group("url"), ie=AsianCrushIE.ie_key())
                    )

            title = (
                self._html_search_regex(
                    r'(?s)<h1\b[^>]\bid=["\']movieTitle[^>]+>(.+?)</h1>',
                    webpage,
                    "title",
                    default=None,
                )
            ...
            entries = OnDemandPagedList(
                functools.partial(self._fetch_page, host, playlist_id), self._PAGE_SIZE
            )
