...
class NPOBaseIE(InfoExtractor):
    def _get_token(self, video_id):
        return self._download_json(
            "http://ida.omroep.nl/app.php/auth", video_id, note="Downloading token"
        )["token"]
...
class NPOIE(NPOBaseIE):
    ...
    def _get_info(self, url, video_id):
        ...
        player = self._download_json(
            "https://www.npostart.nl/player/%s" % video_id,
            video_id,
    ...
    def _get_old_info(self, video_id):
        ...
        def is_legal_url(format_url):
            return (
                format_url
                and format_url not in urls
                and re.match(r"^(?:https?:)?//", format_url)
            )
        ...
        for num, item in enumerate(items):
            ...
            format_id = self._search_regex(
                r"video/ida/([^/]+)", item_url, "format id", default=None
            )
            ...
            try:
                stream_info = self._download_json(
                    item_url + "&type=json",
                    video_id,
                    "Downloading %s stream JSON" % item_label
                    or item.get("format")
                    or format_id
                    or num,
                )
                ...
                if isinstance(ee.cause, compat_HTTPError) and ee.cause.code == 404:
                    error = (
                        self._parse_json(
                            ee.cause.read().decode(), video_id, fatal=False
                        )
                        or {}
                    ).get("errorstring")
                    if error:
...
class NPORadioIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            ...
            else super(NPORadioIE, cls).suitable(url)
        )
...
class NPOPlaylistBaseIE(NPOIE):
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                "npo:%s" % video_id if not video_id.startswith("http") else video_id
            )
