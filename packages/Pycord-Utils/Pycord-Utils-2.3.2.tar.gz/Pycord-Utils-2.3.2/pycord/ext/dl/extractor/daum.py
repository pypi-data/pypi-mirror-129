...
class DaumClipIE(DaumBaseIE):
    ...
    def suitable(cls, url):
        return (
            ...
            else super(DaumClipIE, cls).suitable(url)
        )
...
class DaumListIE(InfoExtractor):
    def _get_entries(self, list_id, list_id_type):
        ...
        for pagenum in itertools.count(1):
            ...
            entries.extend(
                [
                    self.url_result("http://tvpot.daum.net/v/%s" % clip["vid"])
    ...
    def _check_clip(self, url, list_id):
        ...
        if "clipid" in query_dict:
            ...
            if self._downloader.params.get("noplaylist"):
                self.to_screen(
                    "Downloading just video %s because of --no-playlist" % clip_id
                )
...
class DaumPlaylistIE(DaumListIE):
    ...
    def suitable(cls, url):
        return (
            False
            if DaumUserIE.suitable(url)
            else super(DaumPlaylistIE, cls).suitable(url)
        )
