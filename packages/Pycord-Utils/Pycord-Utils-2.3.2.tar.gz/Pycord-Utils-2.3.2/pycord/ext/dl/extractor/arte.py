...
class ArteTVIE(ArteTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not upload_date_str:
            upload_date_str = (
                player_info.get("VRA") or player_info.get("VDA") or ""
            ).split(" ")[0]
        ...
        for format_id, format_dict in vsr.items():
            ...
            format = {
                ...
                "format_note": "%s, %s"
                % (f.get("versionCode"), f.get("versionLibelle")),
        ...
        return {
            ...
            "thumbnail": player_info.get("programImage")
            or player_info.get("VTU", {}).get("IUR"),
            "formats": formats,
        }
...
class ArteTVPlaylistIE(ArteTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        collection = self._download_json(
            ...
            % (self._API_BASE, lang, playlist_id),
            playlist_id,
        )
        entries = []
        for video in collection["videos"]:
            if not isinstance(video, dict):
                continue
            video_url = url_or_none(video.get("url")) or url_or_none(
                video.get("jsonUrl")
            )
            if not video_url:
                continue
            ...
            entries.append(
                {
                    ...
                    "thumbnail": url_or_none(
                        try_get(video, lambda x: x["mainImage"]["url"], compat_str)
                    ),
