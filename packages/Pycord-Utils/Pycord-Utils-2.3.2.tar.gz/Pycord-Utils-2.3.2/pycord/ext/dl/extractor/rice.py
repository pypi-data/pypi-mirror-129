...
class RICEIE(InfoExtractor):
    _VALID_URL = (
        r"https?://mediahub\.rice\.edu/app/[Pp]ortal/video\.aspx\?(?P<query>.+)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        if (
            not qs.get("PortalID")
            or not qs.get("DestinationID")
            or not qs.get("ContentID")
        ):
        ...
        content_data = self._download_xml(
            "https://mediahub.rice.edu/api/portal/GetContentTitle",
            content_id,
            query={
                "portalId": portal_id,
        ...
        player_data = self._download_xml(
            "https://mediahub.rice.edu/api/player/GetPlayerConfig",
            content_id,
            query={
