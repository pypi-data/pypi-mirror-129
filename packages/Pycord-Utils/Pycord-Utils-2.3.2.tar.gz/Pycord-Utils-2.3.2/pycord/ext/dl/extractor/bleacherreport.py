...
class BleacherReportIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        article_data = self._download_json(
            "http://api.bleacherreport.com/api/v1/articles/%s" % article_id, article_id
        )["article"]
        ...
        if video:
            ...
            if video_type in ("cms.bleacherreport.com", "vid.bleacherreport.com"):
                info["url"] = (
                    "http://bleacherreport.com/video_embed?id=%s" % video["id"]
                )
...
class BleacherReportCMSIE(AMPIE):
    ...
    def _real_extract(self, url):
        ...
        info = self._extract_feed_info(
            "http://vid.bleacherreport.com/videos/%s.akamai" % video_id
        )
        info["id"] = video_id
        return info
