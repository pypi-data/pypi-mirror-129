...
class LineLiveBaseIE(InfoExtractor):
    ...
    def _parse_broadcast_item(self, item):
        ...
        return {
            ...
            "channel_url": "https://live.line.me/channels/" + channel_id
            if channel_id
            else None,
...
class LineLiveIE(LineLiveBaseIE):
    ...
    def _real_extract(self, url):
        ...
        formats = []
        for k, v in (
            broadcast.get(("live" if info["is_live"] else "archived") + "HLSURLs") or {}
        ).items():
            if not v:
                continue
            if k == "abr":
                formats.extend(
                    self._extract_m3u8_formats(
                        v, broadcast_id, "mp4", protocol, m3u8_id="hls", fatal=False
                    )
                )
                continue
            f = {
        ...
        if not formats:
            ...
            if archive_status != "ARCHIVED":
                raise ExtractorError(
                    "this video has been " + archive_status.lower(), expected=True
                )
        self._sort_formats(formats)
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._archived_broadcasts_entries(
                channel.get("archivedBroadcasts") or {}, channel_id
            ),
            channel_id,
            channel.get("title"),
