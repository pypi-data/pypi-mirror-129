...
class MyviIE(SprutoBaseIE):
    ...
    def _extract_url(cls, webpage):
        mobj = re.search(
            r'<iframe[^>]+?src=(["\'])(?P<url>(?:https?:)?//myvi\.(?:ru/player|tv)/(?:embed/html|flash)/[^"]+)\1',
            webpage,
        )
        if mobj:
    ...
    def _real_extract(self, url):
        ...
        spruto = self._download_json(
            "http://myvi.ru/player/api/Video/Get/%s?sig" % video_id, video_id
        )["sprutoData"]
...
class MyviEmbedIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?myvi\.tv/(?:[^?]+\?.*?\bv=|embed/)(?P<id>[\da-z]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        myvi_id = self._search_regex(
            r'CreatePlayer\s*\(\s*["\'].*?\bv=([\da-zA-Z_]+)', webpage, "video id"
        )
