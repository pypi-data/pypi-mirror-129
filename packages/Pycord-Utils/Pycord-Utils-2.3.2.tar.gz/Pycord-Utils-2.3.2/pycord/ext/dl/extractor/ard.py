...
class ARDMediathekIE(ARDMediathekBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if ARDBetaMediathekIE.suitable(url)
            else super(ARDMediathekIE, cls).suitable(url)
        )
    ...
    tracking {
      atiCustomVars {
        ...
        content_id = str_or_none(
            try_get(player_page, lambda x: x["tracking"]["atiCustomVars"]["contentId"])
        )
        ...
        info = self._parse_media_info(
            media_collection, content_id or video_id, player_page.get("blockedByFsk")
        )
        age_limit = None
