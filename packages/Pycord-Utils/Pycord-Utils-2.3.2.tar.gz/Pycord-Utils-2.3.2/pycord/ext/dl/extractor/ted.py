...
class TEDIE(InfoExtractor):
    ...
    def _extract_info(self, webpage):
        info_json = self._search_regex(
            r'(?s)q\(\s*"\w+.init"\s*,\s*({.+?})\)\s*</script>', webpage, "info json"
        )
    ...
    def _playlist_videos_info(self, url, name):
        ...
        for entry in re.findall(
            r'(?s)<[^>]+data-ga-context=["\']playlist["\'][^>]*>', webpage
        ):
    ...
    def _talk_info(self, url, video_name):
        ...
        native_downloads = (
            downloads.get("nativeDownloads") or talk_info.get("nativeDownloads") or {}
        )
        ...
        formats = [
            ...
            for (format_id, format_url) in native_downloads.items()
            ...
            if format_id == "hls":
                ...
                formats.extend(
                    self._extract_m3u8_formats(
                        stream_url, video_name, "mp4", m3u8_id=format_id, fatal=False
                    )
                )
            else:
    ...
    def _get_subtitles(self, video_id, talk_info):
        ...
        for language in try_get(
            talk_info,
            (lambda x: x["downloads"]["languages"], lambda x: x["languages"]),
            list,
        ):
            ...
            sub_lang_list[lang_code] = [
                {
                    "url": "http://www.ted.com/talks/subtitles/id/%s/lang/%s/format/%s"
                    % (video_id, lang_code, ext),
                    "ext": ext,
                }
                for ext in ["ted", "srt"]
            ]
