...
class VLiveIE(VLiveBaseIE):
    ...
    def _login(self):
        ...
        def is_logged_in():
            login_info = self._download_json(
                "https://www.vlive.tv/auth/loginInfo",
                None,
                note="Downloading login info",
                headers={"Referer": "https://www.vlive.tv/home"},
            )
            return try_get(login_info, lambda x: x["message"]["login"], bool) or False

        LOGIN_URL = "https://www.vlive.tv/auth/email/login"
        self._request_webpage(LOGIN_URL, None, note="Downloading login cookies")
    ...
    def _call_api(self, path_template, video_id, fields=None):
        ...
        try:
            return self._download_json(
                "https://www.vlive.tv/globalv-web/vam-web/" + path_template % video_id,
                video_id,
            ...
            if isinstance(e.cause, compat_HTTPError) and e.cause.code == 403:
                self.raise_login_required(
                    json.loads(e.cause.read().decode("utf-8"))["message"]
                )
            raise
    ...
    def _real_extract(self, url):
        ...
        if video_type == "VOD":
            ...
            vod_id = video["vodId"]
            return merge_dicts(
                get_common_fields(), self._extract_video_info(video_id, vod_id, inkey)
            )
        elif video_type == "LIVE":
            status = video.get("status")
            if status == "ON_AIR":
                stream_url = self._call_api("old/v3/live/%s/playInfo", video_id)[
                    "result"
                ]["adaptiveStreamUrl"]
...
class VLivePostIE(VLiveIE):
    ...
    def _real_extract(self, url):
        ...
        video_seq = str_or_none(try_get(post, lambda x: x["officialVideo"]["videoSeq"]))
        if video_seq:
            return self.url_result(
                "http://www.vlive.tv/video/" + video_seq, VLiveIE.ie_key(), video_seq
            )
        ...
        for idx, video in enumerate(post["attachments"]["video"].values()):
            ...
            entry = None
            if upload_type == "SOS":
                download = self._call_api(self._SOS_TMPL, video_id)["videoUrl"][
                    "download"
                ]
                formats = []
...
class VLiveChannelIE(VLiveBaseIE):
    ...
    def _real_extract(self, url):
        ...
        channel_seq = self._call_api(
            "decodeChannelCode", "Code", channel_code, "decode channel code", {}
        )["channelSeq"]
        ...
        for page_num in itertools.count(1):
            ...
            if not channel_name:
                channel_name = try_get(
                    video_list, lambda x: x["channelInfo"]["channelName"], compat_str
                )

            videos = try_get(video_list, lambda x: x["videoList"], list)
            if not videos:
                break
        ...
        return self.playlist_result(entries, channel_code, channel_name)
