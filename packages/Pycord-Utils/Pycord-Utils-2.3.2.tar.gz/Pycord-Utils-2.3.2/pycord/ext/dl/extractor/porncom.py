...
class PornComIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        config = self._parse_json(
            self._search_regex(
                (r"=\s*({.+?})\s*;\s*v1ar\b", r"=\s*({.+?})\s*,\s*[\da-zA-Z_]+\s*="),
                webpage,
                "config",
                default="{}",
            ),
        ...
        if config:
            ...
            thumbnail = (
                compat_urlparse.urljoin(config["thumbCDN"], config["poster"])
                if config.get("thumbCDN") and config.get("poster")
                else None
            )
            duration = int_or_none(config.get("length"))
        else:
            title = self._search_regex(
                (r"<title>([^<]+)</title>", r"<h1[^>]*>([^<]+)</h1>"), webpage, "title"
            )
            formats = [
                {
