...
"""
    ...
    def _get_subtitles(self, video_id, webpage):
        subtitles = {}
        for sub_id, sub_name in re.findall(
            r'\bssid=([0-9]+)"[^>]+?\btitle="([^"]+)', webpage
        ):
            ...
            lang_code = self._search_regex(
                r'lang_code=["\']([^"\']+)', subtitle, "subtitle_lang_code", fatal=False
            )
            if not lang_code:
                continue
    ...
    def _real_extract(self, url):
        ...
        if mobj.group("prefix") == "m":
            mobile_webpage = self._download_webpage(
                url, video_id, "Downloading mobile webpage"
            )
        ...
        webpage = self._download_webpage(
            self._add_skip_wall(webpage_url),
            video_id,
        ...
        mobj = re.search(
            r"Page\.messaging_box_controller\.addItems\(\[(?P<msg>{.+?})\]\)", webpage
        )
        if mobj:
            msg = json.loads(mobj.group("msg"))
            if msg.get("type") == "error":
                raise ExtractorError(
                    "crunchyroll returned error: %s" % msg["message_body"],
                    expected=True,
                )
        ...
        if video_description:
            video_description = lowercase_escape(
                video_description.replace(r"\r\n", "\n")
            )
        ...
        if not formats:
            available_fmts = []
            for a, fmt in re.findall(
                r'(<a[^>]+token=["\']showmedia\.([0-9]{3,4})p["\'][^>]+>)', webpage
            ):
            ...
            for fmt in available_fmts:
                ...
                streamdata = self._call_rpc_api(
                    "VideoPlayer_GetStandardConfig",
                    video_id,
                    "Downloading media info for %s" % video_format,
                    data={
                        "media_id": video_id,
                ...
                stream_info = self._call_rpc_api(
                    "VideoEncode_GetStreamInfo",
                    video_id,
                    "Downloading stream info for %s" % video_format,
                    data={
                        "media_id": video_id,
                        "video_format": stream_format,
                ...
                for stream_info in stream_infos:
                    ...
                    if ".fplive.net/" in video_url:
                        ...
                        direct_video_url = compat_urlparse.urlunparse(
                            parsed_video_url._replace(
                                netloc="v.lvlt.crcdn.net",
                                path="%s/%s"
                                % (
                                    remove_end(parsed_video_url.path, "/"),
                                    video_file.split(":")[-1],
                                ),
                            )
                        )
        ...
        self._sort_formats(
            formats,
            ("preference", "language_preference", "height", "width", "tbr", "fps"),
        )

        metadata = self._call_rpc_api(
            "VideoPlayer_GetMediaMetadata",
            video_id,
...
class CrunchyrollShowPlaylistIE(CrunchyrollBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {"id": "cosplay-complex-ova", "title": "Cosplay Complex OVA"},
            "playlist_count": 3,
            "skip": "Georestricted",
        },
        {
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            self._add_skip_wall(url), show_id, headers=self.geo_verification_headers()
        )
