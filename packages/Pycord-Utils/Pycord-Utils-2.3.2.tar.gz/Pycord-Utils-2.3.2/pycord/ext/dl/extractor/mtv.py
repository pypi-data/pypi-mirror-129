...
class MTVServicesInfoExtractor(InfoExtractor):
    ...
    def _extract_mobile_video_formats(self, mtvn_id):
        ...
        metrics_url = unescapeHTML(
            self._search_regex(r'<a href="(http://metrics.+?)"', webpage, "url")
        )
        ...
        for transcript in mdoc.findall(".//transcript"):
            ...
            for typographic in transcript.findall("./typographic"):
                ...
                subtitles.setdefault(lang, []).append(
                    {"url": compat_str(sub_src), "ext": ext}
                )
        return subtitles
    ...
    def _get_video_info(self, itemdoc, use_hls=True):
        ...
        content_el = itemdoc.find(
            "%s/%s" % (_media_xml_tag("group"), _media_xml_tag("content"))
        )
    ...
    def _get_videos_info_from_url(self, url, video_id, use_hls=True):
        idoc = self._download_xml(
            url, video_id, "Downloading info", transform_source=fix_xml_ampersands
        )
    ...
    def _extract_mgid(self, webpage):
        ...
        if not mgid:
            data = self._parse_json(
                self._search_regex(r"__DATA__\s*=\s*({.+?});", webpage, "data"), None
            )
...
class MTVServicesEmbeddedIE(MTVServicesInfoExtractor):
    ...
    def _get_feed_url(self, uri):
        ...
        config = self._download_json(
            "http://media.mtvnservices.com/pmt/e1/access/index.html?uri=%s&configtype=edge"
            % uri,
            video_id,
        )
...
class MTVDEIE(MTVServicesInfoExtractor):
    IE_NAME = "mtv.de"
    _VALID_URL = (
        r"https?://(?:www\.)?mtv\.de/(?:musik/videoclips|folgen|news)/(?P<id>[0-9a-z]+)"
    )
    _TESTS = [
        {
