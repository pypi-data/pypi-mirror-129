...
class Vbox7IE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        response = self._download_json(
            "https://www.vbox7.com/ajax/video/nextvideo.php?vid=%s" % video_id, video_id
        )
        ...
        if webpage:
            info = self._search_json_ld(
                webpage.replace('"/*@context"', '"@context"'), video_id, fatal=False
            )
        ...
        info.update(
            {
                ...
                "thumbnail": self._proto_relative_url(
                    info.get("thumbnail") or self._og_search_thumbnail(webpage), "http:"
                ),
            }
        )
        return info
