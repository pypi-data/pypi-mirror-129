...
class PlayStuffIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        account_id = (
            try_get(state, lambda x: x["configurations"]["accountId"], compat_str)
            or "6005208634001"
        )
        player_id = (
            try_get(state, lambda x: x["configurations"]["playerId"], compat_str)
            or "default"
        )
        ...
        for item_id, video in state["items"].items():
            if not isinstance(video, dict):
                continue
            asset_id = try_get(
                video, lambda x: x["content"]["attributes"]["assetId"], compat_str
            )
            if not asset_id:
                continue
            entries.append(
