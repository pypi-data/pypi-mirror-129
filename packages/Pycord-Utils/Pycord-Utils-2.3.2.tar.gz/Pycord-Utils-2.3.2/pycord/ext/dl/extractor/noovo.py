...
class NoovoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._parse_json(
            self._search_regex(
                r"(?s)dataLayer\.push\(\s*({.+?})\s*\);", webpage, "data", default="{}"
            ),
            video_id,
        ...
        title = try_get(
            data, lambda x: x["video"]["nom"], compat_str
        ) or self._html_search_meta("dcterms.Title", webpage, "title", fatal=True)

        description = self._html_search_meta(
            ("dcterms.Description", "description"), webpage, "description"
        )

        series = try_get(data, lambda x: x["emission"]["nom"]) or self._search_regex(
