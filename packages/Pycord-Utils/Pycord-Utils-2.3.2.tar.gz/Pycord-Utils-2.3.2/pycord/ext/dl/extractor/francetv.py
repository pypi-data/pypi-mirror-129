...
class FranceTVBaseInfoExtractor(InfoExtractor):
    def _make_url_result(self, video_or_full_id, catalog=None):
        ...
        return self.url_result(
            full_id, ie=FranceTVIE.ie_key(), video_id=video_or_full_id.split("@")[0]
        )
    ...
    def _extract_video(self, video_id, catalogue=None):
        ...
        if allowed_countries:
            ...
            geo_info = self._download_json(
                "http://geo.francetv.fr/ws/edgescape.json",
                video_id,
            ...
            if country not in allowed_countries:
                raise ExtractorError(
                    "The video is not available from your location", expected=True
                )
        else:
            georestricted = False
        ...
        if not videos:
            for device_type in ["desktop", "mobile"]:
                fallback_info = self._download_json(
                    ...
                    % video_id,
                    video_id,
                    "Downloading fallback %s video JSON" % device_type,
                    query={
        ...
        for video in videos:
            ...
            if is_live is None:
                is_live = (
                    try_get(video, lambda x: x["plages_ouverture"][0]["direct"], bool)
                    is True
        ...
        subtitles_list = [
            ...
            for subformat in info.get("subtitles", [])
            if subformat.get("url")
        ]
        if subtitles_list:
        ...
        return {
            ...
            "thumbnail": urljoin(
                "https://sivideo.webservices.francetelevisions.fr", info.get("image")
            ),
            "duration": int_or_none(info.get("real_duration"))
            or parse_duration(info.get("duree")),
            "timestamp": int_or_none(
                try_get(info, lambda x: x["diffusion"]["timestamp"])
            ),
            "is_live": is_live,
            "formats": formats,
            "subtitles": subtitles,
...
class GenerationWhatIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        youtube_id = self._search_regex(
            r"window\.videoURL\s*=\s*'([0-9A-Za-z_-]{11})';", webpage, "youtube id"
        )
...
class CultureboxIE(FranceTVBaseInfoExtractor):
    _VALID_URL = (
        r"https?://(?:m\.)?culturebox\.francetvinfo\.fr/(?:[^/]+/)*(?P<id>[^/?#&]+)"
    )
...
class FranceTVJeunesseIE(FranceTVBaseInfoExtractor):
    _VALID_URL = (
        r"(?P<url>https?://(?:www\.)?(?:zouzous|ludo)\.fr/heros/(?P<id>[^/?#&]+))"
    )
    ...
    def _real_extract(self, url):
        ...
        if not playlist.get("count"):
            raise ExtractorError("%s is not available" % playlist_id, expected=True)
