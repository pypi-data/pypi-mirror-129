...
class AdobeTVPlaylistBaseIE(AdobeTVBaseIE):
    ...
    def _extract_playlist_entries(self, display_id, query):
        return OnDemandPagedList(
            functools.partial(self._fetch_page, display_id, query), self._PAGE_SIZE
        )
...
class AdobeTVShowIE(AdobeTVPlaylistBaseIE):
    ...
    _VALID_URL = (
        r"https?://tv\.adobe\.com/(?:(?P<language>fr|de|es|jp)/)?show/(?P<id>[^/]+)"
    )
...
class AdobeTVChannelIE(AdobeTVPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._extract_playlist_entries(channel_urlname, query), channel_urlname
        )
...
class AdobeTVVideoIE(AdobeTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_data = self._parse_json(
            self._search_regex(r"var\s+bridge\s*=\s*([^;]+);", webpage, "bridged data"),
            video_id,
        )
        title = video_data["title"]
            ...
            formats.append(
                {
                    ...
                    "format_id": "-".join(
                        filter(None, [source.get("format"), source.get("label")])
                    ),
