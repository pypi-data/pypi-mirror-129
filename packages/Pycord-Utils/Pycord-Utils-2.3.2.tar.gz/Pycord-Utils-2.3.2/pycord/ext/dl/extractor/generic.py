...
class GenericIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        # arte embed
        {
            ...
            "params": {"skip_download": "Requires rtmpdump"},
            "skip": "video gone",
        },
        # francetv embed
        {
        ...
        },
        {
            ...
            "add_ie": ["Kaltura"],
            "expected_warnings": ["Could not send HEAD request"],
            "params": {
                "skip_download": True,
            },
        },
    ...
    def _extract_rss(self, url, video_id, doc):
        ...
        for it in doc.findall("./channel/item"):
            ...
            def itunes(key):
                return xpath_text(
                    it, xpath_with_ns("./itunes:%s" % key, NS_MAP), default=None
                )
    ...
    def _real_extract(self, url):
        ...
        mobj = (
            ...
            or re.search(
                r'SBN\.VideoLinkset\.ooyala\([\'"](?P<ec>.{32})[\'"]\)', webpage
            )
        ...
        mobj = re.search(
            r'<iframe .*?src="(http://www\.aparat\.com/video/[^"]+)"', webpage
        )
        if mobj is not None:
        ...
        if simplecast_urls:
            return self.playlist_from_matches(simplecast_urls, video_id, video_title)
        ...
        if eagleplatform_url:
            return self.url_result(
                smuggle_url(eagleplatform_url, {"referrer": url}),
        ...
        if kinja_embed_urls:
            return self.playlist_from_matches(kinja_embed_urls, video_id, video_title)
        ...
        sharevideos_urls = [
            sharevideos_mobj.group("url")
            for sharevideos_mobj in re.finditer(
                r'<iframe[^>]+?\bsrc\s*=\s*(["\'])(?P<url>(?:https?:)?//embed\.share-videos\.se/auto/embed/\d+\?.*?\buid=\d+.*?)\1',
                webpage,
            )
        ]
        if sharevideos_urls:
            return self.playlist_from_matches(sharevideos_urls, video_id, video_title)
        ...
        # Video.js embed
        mobj = re.search(
            r"(?s)\bvideojs\s*\(.+?\.src\s*\(\s*((?:\[.+?\]|{.+?}))\s*\)\s*;", webpage
        )
        if mobj is not None:
            sources = (
                self._parse_json(
                    mobj.group(1), video_id, transform_source=js_to_json, fatal=False
                )
                or []
            )
