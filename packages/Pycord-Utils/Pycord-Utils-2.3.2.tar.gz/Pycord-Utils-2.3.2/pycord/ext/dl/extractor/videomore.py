...
class VideomoreIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        partner_id = (
            ...
            or compat_parse_qs(compat_urllib_parse_urlparse(url).query).get(
                "partner_id", [None]
            )[0]
            or "97"
        )
...
class VideomoreVideoIE(VideomoreBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if VideomoreIE.suitable(url)
            else super(VideomoreVideoIE, cls).suitable(url)
        )
...
class VideomoreSeasonIE(VideomoreBaseIE):
    ...
    def _real_extract(self, url):
        ...
        tracks = self._download_json(
            self._API_BASE_URL + "seasons/%s/tracks" % season_id, season_id
        )["data"]
        entries = []
        for track in tracks:
