...
class VimeoBaseInfoExtractor(InfoExtractor):
    ...
    def _parse_config(self, config, video_id):
                ...
                if re.search(sep_pattern, manifest_url):
                    for suffix, repl in (("", "video"), ("_sep", "sep/video")):
                        sep_manifest_urls.append(
                            (
                                format_id + suffix,
                                re.sub(sep_pattern, "/%s/" % repl, manifest_url),
                            )
                        )
                else:
        ...
        return {
            ...
            "uploader_id": video_uploader_url.split("/")[-1]
            ...
            if isinstance(source_file, dict):
                ...
                if (
                    ...
                    if self._is_valid_url(
                        download_url, video_id, "%s video" % source_name
                    ):
                        ext = (
                            try_get(source_file, lambda x: x["extension"], compat_str)
...
class VimeoChannelIE(VimeoBaseInfoExtractor):
    ...
    def _title_and_entries(self, list_id, base_url):
        for pagenum in itertools.count(1):
            ...
            webpage = self._download_webpage(
                page_url, list_id, "Downloading page %s" % pagenum
            )
...
class VimeoAlbumIE(VimeoBaseInfoExtractor):
    IE_NAME = "vimeo:album"
    _VALID_URL = (
        r"https://vimeo\.com/(?:album|showcase)/(?P<id>\d+)(?:$|[?#]|/(?!video))"
    )
    ...
    def _fetch_page(self, album_id, authorization, hashed_pass, page):
        ...
        for video in videos:
            ...
            uri = video.get("uri")
            video_id = (
                self._search_regex(r"/videos/(\d+)", uri, "video_id", default=None)
                if uri
                else None
            )
    ...
    def _real_extract(self, url):
        ...
        if not viewer:
            ...
            viewer = self._parse_json(
                self._search_regex(
                    r"bootstrap_data\s*=\s*({.+?})</script>", webpage, "bootstrap data"
                ),
                album_id,
            )["viewer"]
        jwt = viewer["jwt"]
...
class VimeoReviewIE(VimeoBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if data.get("isLocked") is True:
            video_password = self._get_video_password()
            viewer = self._download_json("https://vimeo.com/_rv/viewer", video_id)
            webpage = self._verify_video_password(
            ...
            clip_page_config = self._parse_json(
                self._search_regex(
        ...
        source_format = self._extract_original_format(page_url + "/action", video_id)
        if source_format:
