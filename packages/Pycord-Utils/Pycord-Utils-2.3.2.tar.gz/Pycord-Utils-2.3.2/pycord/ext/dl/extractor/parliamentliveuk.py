...
class ParliamentLiveUKIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        widget_config = self._parse_json(
            self._search_regex(
                r"(?s)kWidgetConfig\s*=\s*({.+});", webpage, "kaltura widget config"
            ),
            video_id,
        )
        kaltura_url = "kaltura:%s:%s" % (
            widget_config["wid"][1:],
            widget_config["entry_id"],
        )
        event_title = self._download_json(
            "http://parliamentlive.tv/Event/GetShareVideo/" + video_id, video_id
        )["event"]["title"]
        return {
