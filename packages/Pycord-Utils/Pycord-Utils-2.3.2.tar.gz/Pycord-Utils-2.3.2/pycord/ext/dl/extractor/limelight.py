...
class LimelightBaseIE(InfoExtractor):
    ...
    def _call_playlist_service(self, item_id, method, fatal=True, referer=None):
        ...
        try:
            ...
            if isinstance(e.cause, compat_HTTPError) and e.cause.code == 403:
                error = self._parse_json(e.cause.read().decode(), item_id)["detail"][
    ...
    def _extract_info(self, pc, mobile, i, referer):
        ...
        for mobile_url in mobile_item.get("mobileUrls", []):
            ...
            if (
                not media_url
                or format_id in ("Widevine", "SmoothStreaming")
                or media_url in urls
            ):
                continue
            ...
            if flag == "ClosedCaptions":
                closed_captions = (
                    self._call_playlist_service(
                        video_id, "getClosedCaptionsDetailsByMediaId", False, referer
                    )
                    or []
                )
...
class LimelightMediaIE(LimelightBaseIE):
    ...
    def _real_extract(self, url):
        ...
        pc, mobile = self._extract(
            video_id, "getPlaylistByMediaId", "getMobilePlaylistByMediaId", source_url
        )
