    ...
    def _get_formats(self, song_id, tolerate_ip_deny=False):
        ...
        for file_format in self._FORMATS:
            ...
            song_url = self._download_webpage(
                ...
                song_id,
                note="Download %s url info" % file_format["format"],
                query=query,
    ...
    def _real_extract(self, url):
        ...
        if album_id is not None:
            album_info_page = self._download_webpage(
                "http://www.kuwo.cn/album/%s/" % album_id,
                song_id,
...
class KuwoAlbumIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        album_intro = remove_start(
            clean_html(get_element_by_id("intro", webpage)), "%s简介：" % album_name
        )
    ...
    def _real_extract(self, url):
        ...
        def page_func(page_num):
            webpage = self._download_webpage(
                ...
                singer_id,
                note="Download song list page #%d" % (page_num + 1),
...
class KuwoCategoryIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        category_desc = remove_start(
            get_element_by_id("intro", webpage).strip(), "%s简介：" % category_name
        )
        if category_desc == "暂无":
            category_desc = None
...
class KuwoMvIE(KuwoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            url,
            song_id,
            note="Download mv detail info: %s" % song_id,
        ...
        mv_url = self._download_webpage(
            ...
            song_id,
            note="Download %s MV URL" % song_id,
        )
        formats.append(
            {
                "url": mv_url,
