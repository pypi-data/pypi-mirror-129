...
class ImdbIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_metadata = self._parse_json(
            self._search_regex(
                r"args\.push\(\s*({.+?})\s*\)\s*;", webpage, "video metadata"
            ),
            video_id,
        )
        ...
        if video_info and isinstance(video_info, dict):
            info = try_get(video_info, lambda x: x[list(video_info.keys())[0]][0], dict)
        else:
            info = {}
        ...
        return {
            ...
            "thumbnail": url_or_none(
                try_get(video_metadata, lambda x: x["videoSlate"]["source"])
            ),
...
class ImdbListIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            ...
            for m in re.findall(
                r'href="(/list/ls%s/videoplayer/vi[^"]+)"' % list_id, webpage
            )
        ]
