...
class AirMozillaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        jwconfig = self._parse_json(
            self._search_regex(r"initCallback\((.*)\);", embed_script, "metadata"),
            video_id,
        )["config"]
        ...
        view_count = int_or_none(
            self._html_search_regex(
                r"Views since archived: ([0-9]+)", webpage, "view count", fatal=False
            )
        )
        ...
        info_dict.update(
            {
                ...
                "location": self._html_search_regex(
                    r"Location: (.*)", webpage, "location", default=None
                ),
                "duration": duration,
                "view_count": view_count,
                "categories": re.findall(
                    r'<a href=".*?" class="channel">(.*?)</a>', webpage
                ),
            }
        )
