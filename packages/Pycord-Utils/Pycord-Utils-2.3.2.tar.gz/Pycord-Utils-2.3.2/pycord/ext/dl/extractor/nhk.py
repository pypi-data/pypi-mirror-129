            ...
            info.update(
                {
                    ...
                    "ie_key": "Piksel",
                    "url": "https://player.piksel.com/v/refid/nhkworld/prefid/"
                    + vod_id,
                    "id": vod_id,
                }
            )
...
class NhkVodIE(NhkBaseIE):
    _VALID_URL = r"%s%s(?P<id>\d{7}|[^/]+?-\d{8}-[0-9a-z]+)" % (
        NhkBaseIE._BASE_URL_REGEX,
        NhkBaseIE._TYPE_REGEX,
    )
...
class NhkVodProgramIE(NhkBaseIE):
    _VALID_URL = (
        r"%s/program%s(?P<id>[0-9a-z]+)(?:.+?\btype=(?P<episode_type>clip|(?:radio|tv)Episode))?"
    ...
    def _real_extract(self, url):
        ...
        for episode in episodes:
            ...
            if not episode_path:
                continue
            entries.append(
                self._extract_episode_info(urljoin(url, episode_path), episode)
            )
