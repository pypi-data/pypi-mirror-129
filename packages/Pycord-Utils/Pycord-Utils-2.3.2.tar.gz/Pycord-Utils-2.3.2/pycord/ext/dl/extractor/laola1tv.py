...
class Laola1TvEmbedIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?laola1\.tv/titanplayer\.php\?.*?\bvideoid=(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _extract_token_url(self, stream_access_url, video_id, data):
        return self._download_json(
            self._proto_relative_url(stream_access_url, "https:"),
            video_id,
            headers={
                "Content-Type": "application/json",
            },
            data=json.dumps(data).encode(),
        )["data"]["stream-access"][0]
    ...
    def _extract_formats(self, token_url, video_id):
        ...
        formats = self._extract_akamai_formats(
            "%s?hdnea=%s" % (token_attrib["url"], token_attrib["auth"]), video_id
        )
    ...
    def _real_extract(self, url):
        ...
        def get_flashvar(x, *args, **kwargs):
            flash_var = self._search_regex(
                r'%s\s*:\s*"([^"]+)"' % x, flash_vars, x, default=None
            )
            if not flash_var:
                flash_var = self._search_regex(
                    [r'flashvars\.%s\s*=\s*"([^"]+)"' % x, r'%s\s*=\s*"([^"]+)"' % x],
                    webpage,
                    x,
                    *args,
                    **kwargs
            ...
            stream_access_url = update_url_query(
                "https://club.laola1.tv/sp/laola1/api/v3/user/session/premium/player/stream-access",
                {
                    ...
                    "target": self._search_regex(
                        r"vs_target = (\d+);", webpage, "vs target"
                    ),
                    "label": _v("label"),
                    "area": _v("area"),
                },
...
class Laola1TvBaseIE(Laola1TvEmbedIE):
    def _extract_video(self, url):
        ...
        if "Dieser Livestream ist bereits beendet." in webpage:
            raise ExtractorError(
                "This live stream has already finished.", expected=True
            )

        conf = self._parse_json(
            self._search_regex(r"(?s)conf\s*=\s*({.+?});", webpage, "conf"),
            display_id,
        ...
        token_url = self._extract_token_url(
            video_data["streamAccess"], video_id, video_data["abo"]["required"]
        )
...
class EHFTVIE(Laola1TvBaseIE):
    IE_NAME = "ehftv"
    _VALID_URL = (
        r"https?://(?:www\.)?ehftv\.com/[a-z]+(?:-[a-z]+)?/[^/]+/(?P<id>[^/?#&]+)"
    )
