...
class BoxIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:[^.]+\.)?app\.box\.com/s/(?P<shared_name>[^/]+)/file/(?P<id>\d+)"
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        request_token = self._parse_json(
            self._search_regex(r"Box\.config\s*=\s*({.+?});", webpage, "Box config"),
            file_id,
        )["requestToken"]
        ...
        query = {"access_token": access_token, "shared_link": shared_link}
