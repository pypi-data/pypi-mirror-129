...
class PinterestCollectionIE(PinterestBaseIE):
    _VALID_URL = (
        r"%s/(?P<username>[^/]+)/(?P<id>[^/?#&]+)" % PinterestBaseIE._VALID_URL_BASE
    )
    _TESTS = [
        {
    ...
    def suitable(cls, url):
        return (
            False
            if PinterestIE.suitable(url)
            else super(PinterestCollectionIE, cls).suitable(url)
        )

    def _real_extract(self, url):
        username, slug = re.match(self._VALID_URL, url).groups()
        board = self._call_api("Board", slug, {"slug": slug, "username": username})[
            "data"
        ]
        board_id = board["id"]
        options = {
