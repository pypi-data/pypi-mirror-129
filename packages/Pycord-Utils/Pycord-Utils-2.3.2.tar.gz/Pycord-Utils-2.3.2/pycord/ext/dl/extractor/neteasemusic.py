...
class NetEaseMusicIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        params = {"id": song_id, "ids": "[%s]" % song_id}
        info = self.query_api(
        ...
        lyrics_info = self.query_api(
            "song/lyric?id=%s&lv=-1&tv=-1" % song_id, song_id, "Downloading lyrics data"
        )
...
class NetEaseMusicAlbumIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info = self.query_api(
            "album/%s?id=%s" % (album_id, album_id), album_id, "Downloading album data"
        )["album"]
        ...
        entries = [
            self.url_result(
                "http://music.163.com/#/song?id=%s" % song["id"],
                "NetEaseMusic",
                song["id"],
            )
...
class NetEaseMusicListIE(NetEaseMusicBaseIE):
    ...
    IE_DESC = "网易云音乐 - 歌单"
    _VALID_URL = (
        r"https?://music\.163\.com/(#/)?(playlist|discover/toplist)\?id=(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if info.get("specialType") == 10:  # is a chart/toplist
            datestamp = datetime.fromtimestamp(
                self.convert_milliseconds(info["updateTime"])
            ).strftime("%Y-%m-%d")
...
class NetEaseMusicMvIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        info = self.query_api(
            "mv/detail?id=%s&type=mp4" % mv_id, mv_id, "Downloading mv info"
        )["data"]
...
class NetEaseMusicProgramIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                "http://music.163.com/#/song?id=%s" % song_id, "NetEaseMusic", song_id
            )
