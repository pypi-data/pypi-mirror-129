...
class CrackleIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        countries = (
            [country_code]
            if country_code
            else ("US", "AU", "CA", "AS", "FM", "GU", "MP", "PR", "PW", "MH", "VI")
        )
        ...
        for country in countries:
            try:
                ...
                media_detail_url = (
                    "https://web-api-us.crackle.com/Service.svc/details/media/%s/%s?disableProtocols=true"
                    % (video_id, country)
                )
                ...
                h = (
                    hmac.new(
                        b"IGSLUQCBDFHEOIFM",
                        "|".join([media_detail_url, timestamp]).encode(),
                        hashlib.sha1,
                    )
                    .hexdigest()
                    .upper()
            ...
            duration = int_or_none(media.get("DurationInSeconds")) or parse_duration(
                media.get("Duration")
            )
