...
class ImgurIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for m in re.finditer(
            r'<source\s+src="(?P<src>[^"]+)"\s+type="(?P<type>[^"]+)"', video_elements
        ):
            formats.append(
                {
        ...
        gif_json = self._search_regex(
            r"(?s)var\s+videoItem\s*=\s*(\{.*?\})", webpage, "GIF code", fatal=False
        )
        if gif_json:
...
class ImgurGalleryIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._download_json(
            "https://imgur.com/gallery/%s.json" % gallery_id, gallery_id
        )["data"]["image"]

        if data.get("is_album"):
            entries = [
                self.url_result(
                    "http://imgur.com/%s" % image["hash"],
                    ImgurIE.ie_key(),
                    image["hash"],
                )
            ...
            return self.playlist_result(
                entries, gallery_id, data.get("title"), data.get("description")
            )

        return self.url_result(
            "http://imgur.com/%s" % gallery_id, ImgurIE.ie_key(), gallery_id
        )
