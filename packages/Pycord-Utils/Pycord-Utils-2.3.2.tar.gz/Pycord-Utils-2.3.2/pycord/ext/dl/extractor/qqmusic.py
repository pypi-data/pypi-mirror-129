...
class QQMusicIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        publish_time = self._html_search_regex(
            r"发行时间：(\d{4}-\d{2}-\d{2})", detail_info_page, "publish time", default=None
        )
        if publish_time:
        ...
        if albummid:
            thumbnail_url = (
                "http://i.gtimg.cn/music/photo/mid_album_500/%s/%s/%s.jpg"
    ...
    def get_entries_from_page(self, singmid):
        ...
        for item in json_obj_all_songs["data"]["list"]:
            if item["musicData"].get("songmid") is not None:
                ...
                entries.append(
                    self.url_result(
                        r"https://y.qq.com/n/yqq/song/%s.html" % songmid,
                        "QQMusic",
                        songmid,
                    )
                )
...
class QQMusicToplistIE(QQPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                "https://y.qq.com/n/yqq/song/" + song["data"]["songmid"] + ".html",
                "QQMusic",
                song["data"]["songmid"],
            )
