    ...
    def _login(self):
        ...
        try:
            login_page = self._download_webpage(
                "https://login.linuxacademy.com/usernamepassword/login",
                None,
        ...
        callback_page, urlh = self._download_webpage_handle(
            "https://login.linuxacademy.com/login/callback",
            None,
        ...
        access_token = self._search_regex(
            r"access_token=([^=&]+)", urlh.geturl(), "access token", default=None
        )
        if not access_token:
            access_token = self._parse_json(
                self._search_regex(
                    r"authorizationResponse\s*=\s*({.+?})\s*;",
                    callback_page,
    ...
    def _real_extract(self, url):
        ...
        if course_id:
            ...
            for item in module["items"]:
                ...
                def type_field(key):
                    return (
                        try_get(item, lambda x: x["type"][key], compat_str) or ""
                    ).lower()
                ...
                description = (
                    item.get("md_desc")
                    or clean_html(item.get("description"))
                    or clean_html(item.get("text"))
                )
                entries.append(
                    {
                        ...
                        "timestamp": unified_timestamp(item.get("date"))
                        or unified_timestamp(item.get("created_on")),
        ...
        formats = self._extract_m3u8_formats(
            m3u8_url, item_id, "mp4", entry_protocol="m3u8_native", m3u8_id="hls"
        )
            ...
            info.update(
                {
                    ...
                    "description": lesson.get("md_desc")
                    or clean_html(lesson.get("desc")),
                    "timestamp": unified_timestamp(lesson.get("date"))
                    or unified_timestamp(lesson.get("created_on")),
