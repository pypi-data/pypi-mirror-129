...
class OoyalaBaseIE(InfoExtractor):
    ...
    _AUTHORIZATION_URL_TEMPLATE = (
        _PLAYER_BASE + "sas/player_api/v2/authorization/embed_code/%s/%s"
    )
    ...
    def _extract(
        ...
        for stream in streams:
            ...
            if delivery_type == "hls" or ext == "m3u8":
                ...
                formats.extend(
                    self._extract_smil_formats(s_url, embed_code, fatal=False)
                )
            else:
                formats.append(
                    {
        ...
        if not formats and not auth_data.get("authorized"):
            raise ExtractorError(
                "%s said: %s" % (self.IE_NAME, auth_data["message"]), expected=True
            )
        self._sort_formats(formats)

        subtitles = {}
        for lang, sub in (
            metadata.get("closed_captions_vtt", {}).get("captions", {}).items()
        ):
            sub_url = sub.get("url")
            if not sub_url:
                continue
...
class OoyalaIE(OoyalaBaseIE):
    _VALID_URL = (
        r"(?:ooyala:|https?://.+?\.ooyala\.com/.*?(?:embedCode|ec)=)(?P<id>.+?)(&|$)"
    )
    ...
    def _real_extract(self, url):
        ...
        content_tree_url = self._CONTENT_TREE_BASE + "embed_code/%s/%s" % (
            embed_code,
            embed_code,
        )
        return self._extract(
            content_tree_url, embed_code, domain, supportedformats, embed_token
        )
...
class OoyalaExternalIE(OoyalaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        content_tree_url = self._CONTENT_TREE_BASE + "external_id/%s/%s:%s" % (
            pcode,
            partner_id,
            video_id,
        )
