...
class DailymotionBaseInfoExtractor(InfoExtractor):
    ...
    def _real_initialize(self):
        ...
        self._FAMILY_FILTER = (
            ff == "on"
            if ff
            else age_restricted(18, self._downloader.params.get("age_limit"))
        )
                ...
                try:
                    ...
                    if isinstance(e.cause, compat_HTTPError) and e.cause.code == 400:
                        raise ExtractorError(
                            self._parse_json(e.cause.read().decode(), xid)[
                                "error_description"
                            ],
                            expected=True,
                        )
...
class DailymotionIE(DailymotionBaseInfoExtractor):
    ...
    ... on Live {
      ...
      isOnAir
            ...
            'password: "%s"' % self._downloader.params.get("videopassword")
            if password
            else None,
        )
        xid = media["xid"]
        ...
        if error:
            ...
            raise ExtractorError("%s said: %s" % (self.IE_NAME, title), expected=True)
