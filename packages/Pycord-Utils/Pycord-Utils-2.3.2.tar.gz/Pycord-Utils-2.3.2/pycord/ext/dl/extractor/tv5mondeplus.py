...
class TV5MondePlusIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_files = self._parse_json(vpl_data["data-broadcast"], display_id).get(
            "files", []
        )
        formats = []
        ...
        description = self._html_search_regex(
            r'(?s)<div[^>]+class=["\']episode-texte[^>]+>(.+?)</div>',
            webpage,
            "description",
            fatal=False,
        )
        ...
        return {
            ...
            "duration": int_or_none(vpl_data.get("data-duration"))
