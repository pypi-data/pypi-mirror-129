...
class NBCSportsVPlayerIE(InfoExtractor):
    ...
    def _extract_url(webpage):
        iframe_m = re.search(
            ...
            % NBCSportsVPlayerIE._VALID_URL_BASE,
            webpage,
        )
        if iframe_m:
...
class NBCSportsStreamIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        live_source = self._download_json(
            "http://stream.nbcsports.com/data/live_sources_%s.json" % video_id, video_id
        )
...
class NBCOlympicsStreamIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        resource = self._search_regex(
            r"resource\s*=\s*'(.+)';", webpage, "resource"
        ).replace("' + pid + '", pid)
        event_config = self._download_json(
            self._DATA_URL_TEMPLATE % ("event_config", pid), pid
        )["eventConfig"]
        title = self._live_title(event_config["eventTitle"])
        source_url = self._download_json(
            self._DATA_URL_TEMPLATE % ("live_sources", pid), pid
        ...
        formats = self._extract_m3u8_formats(
            self._download_webpage(
                "http://sp.auth.adobe.com/tvs/v1/sign",
                pid,
                query={
                    "cdn": "akamai",
