...
class NewgroundsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        media_url = self._parse_json(
            self._search_regex(r'"url"\s*:\s*("[^"]+"),', webpage, ""), media_id
        )
        ...
        max_resolution = int_or_none(
            self._search_regex(
                r'max_resolution["\']\s*:\s*(\d+)',
                webpage,
                "max resolution",
                default=None,
            )
        ...
        duration = parse_duration(
            self._search_regex(
                r"(?s)<dd>\s*Song\s*</dd>\s*<dd>.+?</dd>\s*<dd>([^<]+)",
                webpage,
                "duration",
                default=None,
            )
