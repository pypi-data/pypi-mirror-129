...
class VidmeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if video.get("state") == "deleted":
            raise ExtractorError(
                "Vidme said: Sorry, this video has been deleted.", expected=True
            )
...
class VidmeListBaseIE(InfoExtractor):
    ...
    def _entries(self, user_id, user_name):
        for page_num in itertools.count(1):
            page = self._download_json(
                ...
                user_name,
                "Downloading user %s page %d" % (self._API_ITEM, page_num),
            )
    ...
    def _real_extract(self, url):
        ...
        user_id = self._download_json(
            "https://api.vid.me/userByUsername?username=%s" % user_name, user_name
        )["user"]["user_id"]

        return self.playlist_result(
            self._entries(user_id, user_name),
            user_id,
...
class VidmeUserIE(VidmeListBaseIE):
    IE_NAME = "vidme:user"
    _VALID_URL = (
        r"https?://vid\.me/(?:e/)?(?P<id>[\da-zA-Z_-]{6,})(?!/likes)(?:[^\da-zA-Z_-]|$)"
    )
    _API_ITEM = "list"
    _TITLE = "Videos"
    _TESTS = [
