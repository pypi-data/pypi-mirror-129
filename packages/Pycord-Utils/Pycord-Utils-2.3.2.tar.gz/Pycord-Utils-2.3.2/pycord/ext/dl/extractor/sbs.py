...
class SBSIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        player_params = self._download_json(
            "http://www.sbs.com.au/api/video_pdkvars/id/%s?form=json" % video_id,
            video_id,
        )
        ...
        if error:
            ...
            if error_code == "ComingSoon":
                ...
                error_message = "Sorry, %s is no longer available." % video_data.get(
                    "title", ""
                )
            raise ExtractorError(
                "%s said: %s" % (self.IE_NAME, error_message), expected=True
            )

        urls = player_params["releaseUrls"]
        theplatform_url = (
            urls.get("progressive")
            or urls.get("html")
            or urls.get("standard")
            or player_params["relatedItemsURL"]
        )
        ...
        return {
            ...
            "id": video_id,
            "url": smuggle_url(
                self._proto_relative_url(theplatform_url), {"force_smil_url": True}
            ),
        }
