...
class TVNowIE(TVNowBaseIE):
    ...
    def suitable(cls, url):
        return (
            ...
            or TVNowSeasonIE.suitable(url)
            or TVNowAnnualIE.suitable(url)
            or TVNowShowIE.suitable(url)
...
class TVNowListBaseIE(TVNowNewBaseIE):
    ...
    def _extract_items(self, url, show_id, list_id, query):
        items = self._call_api(
            "teaserrow/format/episode/" + show_id, list_id, query=query
        )["items"]
...
class TVNowAnnualIE(TVNowListBaseIE):
    _VALID_URL = (
        r"%s/(?P<year>\d{4})-(?P<month>\d{2})" % TVNowListBaseIE._SHOW_VALID_URL
    )
    _TESTS = [
        {
