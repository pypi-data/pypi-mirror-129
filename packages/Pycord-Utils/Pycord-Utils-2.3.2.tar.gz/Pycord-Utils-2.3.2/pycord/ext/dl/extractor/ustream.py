...
class UstreamIE(InfoExtractor):
    ...
    def _get_stream_info(self, url, video_id, app_id_ver, extra_note=None):
        ...
        conn_info = self._download_json(
            "http://r%d-1-%s-recorded-lp-live.ums.ustream.tv/1/ustream"
            % (rnd(1e8), video_id),
            video_id,
    ...
    def _parse_segmented_mp4(self, dash_stream_info):
        ...
        for stream in dash_stream_info["streams"]:
            ...
            f = {
                "format_id": "-".join(
                    filter(None, ["dash", kind, str_or_none(stream.get("bitrate"))])
                ),
    ...
    def _real_extract(self, url):
        ...
        while next_url:
            reply = self._download_json(
                compat_urlparse.urljoin(BASE, next_url),
                display_id,
