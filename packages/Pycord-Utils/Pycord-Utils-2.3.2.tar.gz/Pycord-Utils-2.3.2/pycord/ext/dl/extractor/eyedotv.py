...
class EyedoTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_data = self._download_xml(
            "http://eyedo.tv/api/live/GetLive/%s" % video_id, video_id
        )
        ...
        if is_live:
            if xpath_text(video_data, "Cdn") == "true":
                m3u8_url = (
                    "http://rrr.sz.xlcdn.com/?account=eyedo&file=A%s&type=live&service=wowza&protocol=http&output=playlist.m3u8"
                    % video_id
                )
            else:
                m3u8_url = self._ROOT_URL + "w/%s/eyedo_720p/playlist.m3u8" % video_id
        else:
            m3u8_url = self._ROOT_URL + "replay-w/%s/mp4:%s.mp4/playlist.m3u8" % (
                video_id,
                video_id,
            )
