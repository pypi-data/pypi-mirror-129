...
class DouyuTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        room = self._download_json(
            "http://m.douyu.com/html5/live?roomId=%s" % room_id,
            video_id,
        ...
        tt = int(time.time())
        signContent = (
            "lapi/live/thirdPart/getPlay/%s?aid=pcclient&rate=0&time=%d9TUk5fjjUjg9qIMH3sdnh"
            % (room_id, tt)
        )
...
class DouyuShowIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        room_info = self._parse_json(
            self._search_regex(r"var\s+\$ROOM\s*=\s*({.+});", webpage, "room info"),
            video_id,
        )
        ...
        for trial in range(5):
            ...
            try:
                video_info = self._download_json(
                    "https://vmobile.douyu.com/video/getInfo",
                    video_id,
                    query={"vid": video_id},
                    headers={
                        "Referer": url,
