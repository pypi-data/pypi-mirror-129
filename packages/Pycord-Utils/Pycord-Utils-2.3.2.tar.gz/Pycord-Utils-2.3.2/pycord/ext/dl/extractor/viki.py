...
class VikiBaseIE(InfoExtractor):
    ...
    def _prepare_call(self, path, timestamp=None, post_data=None):
        ...
        sig = hmac.new(
            self._APP_SECRET.encode("ascii"), query.encode("ascii"), hashlib.sha1
        ).hexdigest()
        url = self._API_URL_TEMPLATE % (query, sig)
        return (
            sanitized_Request(url, json.dumps(post_data).encode("utf-8"))
            if post_data
            else url
        )
    ...
    def _login(self):
        ...
        login = self._call_api(
            "sessions.json", None, "Logging in", post_data=login_form
        )
    ...
    def _real_extract(self, url):
            ...
            subtitles[subtitle_lang] = [
                {
                    ...
                    "url": self._prepare_call(
                        "videos/%s/subtitles/%s.%s"
                        % (video_id, subtitle_lang, subtitles_format)
                    ),
                }
        ...
        if not formats:
            streams = self._call_api(
                "videos/%s/streams.json" % video_id,
                video_id,
...
class VikiChannelIE(VikiBaseIE):
    ...
    _VALID_URL = (
        r"%s(?:tv|news|movies|artists)/(?P<id>[0-9]+c)" % VikiBaseIE._VALID_URL_BASE
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        channel = self._call_api(
            "containers/%s.json" % channel_id, channel_id, "Downloading channel JSON"
        )
        ...
        for video_type in ("episodes", "clips", "movies"):
            for page_num in itertools.count(1):
                page = self._call_api(
                    ...
                    % (channel_id, video_type, self._PER_PAGE, page_num),
                    channel_id,
