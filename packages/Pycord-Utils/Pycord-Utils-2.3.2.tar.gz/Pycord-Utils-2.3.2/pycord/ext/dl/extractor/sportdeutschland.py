...
class SportDeutschlandIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        info = {
            "id": asset_id,
            "title": title,
            "description": clean_html(asset.get("body") or asset.get("description"))
            or asset.get("teaser"),
            "duration": int_or_none(asset.get("seconds")),
        }
        videos = asset.get("videos") or []
        if len(videos) > 1:
            playlist_id = compat_parse_qs(compat_urllib_parse_urlparse(url).query).get(
                "playlistId", [None]
            )[0]
            if playlist_id:
            ...
            info.update(
                {
                    ...
                    "timestamp": parse_iso8601(
                        asset.get("date") or asset.get("published_at")
                    ),
                }
            )
        return info
