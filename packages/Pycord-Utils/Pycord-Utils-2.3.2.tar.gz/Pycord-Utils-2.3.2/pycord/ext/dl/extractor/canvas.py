    ...
    def _real_extract(self, url):
        ...
        if site_id != "vrtvideo":
            ...
            data = self._download_json(
                "https://mediazone.vrt.be/api/v1/%s/assets/%s" % (site_id, video_id),
                video_id,
        ...
        for target in data["targetUrls"]:
            format_url, format_type = url_or_none(target.get("url")), str_or_none(
                target.get("type")
            )
            ...
            if format_type in self._HLS_ENTRY_PROTOCOLS_MAP:
                formats.extend(
                    self._extract_m3u8_formats(
                        ...
                        video_id,
                        "mp4",
                        self._HLS_ENTRY_PROTOCOLS_MAP[format_type],
                        m3u8_id=format_type,
                        fatal=False,
                    )
                )
...
class CanvasEenIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?(?P<site_id>canvas|een)\.be/(?:[^/]+/)*(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
...
class VrtNUIE(GigyaBaseIE):
            ...
            try:
                ...
                if isinstance(e.cause, compat_HTTPError) and e.cause.code == 401:
                    ...
                    self._sleep(
                        1,
                        None,
                        msg_template="Waiting for %(timeout)s seconds before trying again",
                    )
                else:
                    raise e
            else:
    ...
    def _real_extract(self, url):
        ...
        attrs = extract_attributes(
            self._search_regex(r"(<nui-media[^>]+>)", webpage, "media element")
        )
        ...
        page = (
            self._parse_json(
                self._search_regex(
                    r"digitalData\s*=\s*({.+?});", webpage, "digial data", default="{}"
                ),
                video_id,
                fatal=False,
            )
