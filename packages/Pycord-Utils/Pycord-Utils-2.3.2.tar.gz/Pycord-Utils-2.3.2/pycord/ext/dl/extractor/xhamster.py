...
class XHamsterEmbedIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:.+?\.)?%s/xembed\.php\?video=(?P<id>\d+)" % XHamsterIE._DOMAINS
    )
    _TEST = {
    ...
    def _real_extract(self, url):
        ...
        video_url = self._search_regex(
            r'href="(https?://xhamster\.com/(?:movies/{0}/[^"]*\.html|videos/[^/]*-{0})[^"]*)"'.format(
                video_id
            ),
            webpage,
            "xhamster url",
        ...
        if not video_url:
            ...
            video_url = dict_get(
                vars, ("downloadLink", "homepageLink", "commentsLink", "shareUrl")
            )
