...
class TwitchBaseIE(InfoExtractor):
    ...
    def _download_access_token(self, video_id, token_kind, param_name):
        ...
        return self._download_base_gql(
            video_id, ops, "Downloading %s access token GraphQL" % token_kind
        )["data"][method]
...
class TwitchVodIE(TwitchBaseIE):
    ...
    def _download_info(self, item_id):
        ...
        if video is None:
            raise ExtractorError("Video %s does not exist" % item_id, expected=True)
...
class TwitchStreamIE(TwitchBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not user:
            raise ExtractorError("%s does not exist" % channel_name, expected=True)
  ...
  clip(slug: "%s") {
    ...
    viewCount
        ...
        return {
            ...
            "thumbnails": thumbnails,
            "creator": try_get(
                clip, lambda x: x["broadcaster"]["displayName"], compat_str
            ),
            "uploader": try_get(
                clip, lambda x: x["curator"]["displayName"], compat_str
            ),
