...
class DiscoveryGoBaseIE(InfoExtractor):
    ...
    def _extract_video_info(self, video, stream, display_id):
        ...
        for stream_kind in ("", "hds"):
            suffix = (
                STREAM_URL_SUFFIX.capitalize() if stream_kind else STREAM_URL_SUFFIX
            )
...
class DiscoveryGoIE(DiscoveryGoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video = self._parse_json(
            container.get("data-video") or container.get("data-json"), display_id
        )
...
class DiscoveryGoPlaylistIE(DiscoveryGoBaseIE):
    ...
    def suitable(cls, url):
        return (
            False
            if DiscoveryGoIE.suitable(url)
            else super(DiscoveryGoPlaylistIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        for mobj in re.finditer(r'data-json=(["\'])(?P<json>{.+?})\1', webpage):
            ...
            entries.append(
                self.url_result(
                    episode_url, ie=DiscoveryGoIE.ie_key(), video_id=data.get("id")
                )
            )
        ...
        return self.playlist_result(
            entries,
            display_id,
            remove_end(self._og_search_title(webpage, fatal=False), " | Discovery GO"),
