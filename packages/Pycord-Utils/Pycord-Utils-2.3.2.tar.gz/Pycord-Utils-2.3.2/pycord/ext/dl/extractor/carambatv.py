...
class CarambaTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video = self._download_json(
            "http://video1.carambatv.ru/v/%s/videoinfo.js" % video_id, video_id
        )
        ...
        duration = float_or_none(
            try_get(video, lambda x: x["annotations"][0]["end_time"], compat_str)
        )
    ...
    def _real_extract(self, url):
        ...
        if not videomore_url:
            videomore_id = self._search_regex(
                r'getVMCode\s*\(\s*["\']?(\d+)', webpage, "videomore id", default=None
            )
            if videomore_id:
        ...
        if not video_url:
            video_id = self._search_regex(
                r'(?:video_id|crmb_vuid)\s*[:=]\s*["\']?(\d+)', webpage, "video id"
            )
