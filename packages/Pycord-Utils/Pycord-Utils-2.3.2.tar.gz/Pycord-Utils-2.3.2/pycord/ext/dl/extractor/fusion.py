...
class FusionIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?fusion\.(?:net|tv)/(?:video/|show/.+?\bvideo=)(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        video = self._download_json(
            "https://platform.fusion.net/wp-json/fusiondotnet/v1/video/" + video_id,
            video_id,
        )
        ...
        for f_id, f in src.items():
            for q_id, q in f.items():
                ...
                if ext == "smil":
                    formats.extend(
                        self._extract_smil_formats(q_url, video_id, fatal=False)
                    )
