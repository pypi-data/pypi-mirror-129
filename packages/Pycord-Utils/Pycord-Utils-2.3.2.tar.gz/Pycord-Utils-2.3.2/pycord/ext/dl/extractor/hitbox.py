...
class HitboxIE(InfoExtractor):
    IE_NAME = "hitbox"
    _VALID_URL = (
        r"https?://(?:www\.)?(?:hitbox|smashcast)\.tv/(?:[^/]+/)*videos?/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
...
class HitboxLiveIE(HitboxIE):
    ...
    def suitable(cls, url):
        return (
            False if HitboxIE.suitable(url) else super(HitboxLiveIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        player_config = self._download_json(
            "https://www.smashcast.tv/api/player/config/live/%s" % video_id, video_id
        )
        ...
        for cdn in cdns:
            ...
            if base_url not in servers:
                ...
                for stream in cdn.get("bitrates"):
                    ...
                    if (
                        stream.get("provider") == "hls"
                        or determine_ext(stream_url) == "m3u8"
                    ):
