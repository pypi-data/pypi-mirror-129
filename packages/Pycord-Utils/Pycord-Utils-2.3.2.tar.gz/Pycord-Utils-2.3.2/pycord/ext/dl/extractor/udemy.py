...
class UdemyIE(InfoExtractor):
    ...
    def _extract_course_info(self, webpage, video_id):
        ...
        course_id = course.get("id") or self._search_regex(
            [r'data-course-id=["\'](\d+)', r"&quot;courseId&quot;\s*:\s*(\d+)"],
            webpage,
            "course id",
        )
    ...
    def _enroll_course(self, base_url, webpage, course_id):
        def combine_url(base_url, url):
            return (
                compat_urlparse.urljoin(base_url, url)
    ...
    def _handle_error(self, response):
        ...
        if error:
            error_str = "Udemy returned error #%s: %s" % (
                error.get("code"),
                error.get("message"),
            )
...
class UdemyCourseIE(UdemyIE):
    ...
    def suitable(cls, url):
        return (
            False if UdemyIE.suitable(url) else super(UdemyCourseIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        response = self._download_json(
            "https://www.udemy.com/api-2.0/courses/%s/cached-subscriber-curriculum-items"
            % course_id,
            course_id,
            ...
            if clazz == "lecture":
                ...
                if lecture_id:
                    entry = {
                        ...
                        "url": "https://www.udemy.com/%s/learn/v4/t/lecture/%s"
                        % (course_path, entry["id"]),
