...
class MetacafeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if m_external is not None:
            ...
            if prefix == "yt":
                return self.url_result(
                    "http://www.youtube.com/watch?v=%s" % ext_id, "Youtube"
                )
        ...
        ) or self._search_regex(r"<h1>(.*?)</h1>", webpage, "title")
        ...
        if video_url is None:
            flashvars = self._search_regex(
                r' name="flashvars" value="(.*?)"', webpage, "flashvars", default=None
            )
            if flashvars:
        ...
        if video_url is None:
            player_url = self._search_regex(
                r"swfobject\.embedSWF\('([^']+)'", webpage, "config URL", default=None
            )
            if player_url:
                ...
                config_doc = self._download_xml(
                    config_url, video_id, note="Downloading video config"
                )
                smil_url = config_doc.find(".//properties").attrib["smil_file"]
                smil_doc = self._download_xml(
                    smil_url, video_id, note="Downloading SMIL document"
                )
        ...
        if video_url is None:
            flashvars = self._parse_json(
                self._search_regex(
                    r"flashvars\s*=\s*({.*});", webpage, "flashvars", default=None
                ),
                video_id,
                fatal=False,
            )
