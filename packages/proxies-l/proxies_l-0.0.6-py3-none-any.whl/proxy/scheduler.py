            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    async with session.get(
                            self.config.get("TSET_API"),
                            headers=Headers().headers(),
                            proxy=real_proxy,
                            timeout=self.config.get("TEST_TIME_OUT")
                    ) as response:
...
class PoolAdder(object):
    ...
    def add_to_pool(self):
        while True:
            ...
            logging.info(f"{str(datetime.now())[:-7]}-{proxies}")
            print(f"{str(datetime.now())[:-7]}-{proxies}")
            if proxies:
...
class Scheduler(object):
    ...
    def check_pool_add(config):
        ...
        while True:
            if datetime.now().hour in [1, 2, 3, 4, 5, 6, 20, 21, 22, 23, 0]:
...
#     s = Scheduler('47.107.86.234', 27017)
#     s.run(s.config)
