...
class CentralReachRequestsCore:
    ...
    def check_response(self, response,
                       ...
                       exc_message: str = '',
                       re_authorize: bool = True) -> None:
        """
        This method check response and raise exception 'BadRequestError' or 'ScheduledMaintenanceError' with exc_message,
        ...
        if self.__is_scheduled_maintenance(response):
            ...
            self._login_to_central_reach(self.__login, self.__password)
            raise BadRequestError(f"{exc_message}Status Code: {response.status_code} (Unauthorized request), "
        ...
        if response.status_code != 200 or (mandatory_json and not self._is_json_response(response)):
            ...
            if self._is_json_response(response):
                raise BadRequestError(f"{exc_message}Status Code: {response.status_code}, "
                                 f"Json content: {response.json()}, Headers: {response.headers}")
            else:
                raise BadRequestError(f"{exc_message}Status Code: {response.status_code}, "
