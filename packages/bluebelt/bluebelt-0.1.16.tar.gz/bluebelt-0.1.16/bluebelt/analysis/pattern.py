...
class Polynomial():
    ...
    def calculate(self):
        ...
        self.result, self.residuals, self.shape, self.polyfit, self.statistic, self.p_value, self.rsq = _poly_hand_granade(series=self.series, shape=self.shape, validation=self.validation, threshold=self.threshold)
        ...
        if self.adjust:
            ...
            self.result, self.residuals, self.shape, self.polyfit, self.statistic, self.p_value, self.rsq = _poly_hand_granade(series=values, shape=self.shape, validation=self.validation, threshold=self.threshold)
...
def _poly_hand_granade(series, shape=(0, 6), validation='p_value', threshold=0.05, **kwargs):
    ...
    # get the index
    index = series.index.view(int)-series.index.view(int).min()
    index = index / np.gcd.reduce(index)

    # drop nan values
    _index = series.dropna().index.view(int)-series.index.view(int).min()
    ...
    if isinstance(shape, int):
        polyfit = np.polynomial.polynomial.polyfit(_index, values, shape)
        pattern = pd.Series(index=series.index, data=np.polynomial.polynomial.polyval(index, polyfit), name=f'{_get_nice_polynomial_name(shape)}')
    ...
    return pattern, residuals, shape, polyfit, statistic, p_value, rsq
...
def _pattern_plot(_obj, **kwargs):
    ...
    # pattern
    ax1.plot(_obj.result, **style.patterns.pattern)
    ...
    # labels
    ax1.set_title(f'{_obj.series.name} {_obj.result.name}', **style.patterns.title)
