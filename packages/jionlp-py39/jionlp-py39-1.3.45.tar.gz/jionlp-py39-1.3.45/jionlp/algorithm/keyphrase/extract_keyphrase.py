...
class ChineseKeyPhrasesExtractor(object):
    ...
    def _prepare(self):
        ...
        # 去除冗余短语的规则
        self.redundant_strict_pattern = re.compile(
            '[\*\|`\;:丨－\<\>]')  # 有一个字符即抛弃
        self.redundant_loose_pattern = re.compile(
            '[/\d\.\-:=a-z+,%]+')  # 全部是该字符即抛弃
        self.extra_date_ptn = re.compile('\d{1,2}[月|日]')

        # 加载 idf，计算其 oov 均值
        self.idf_dict = idf_loader()
        self.median_idf = sorted(self.idf_dict.values())[
            len(self.idf_dict) // 2]
        self.seg = spacy_pkuseg.pkuseg(postag=True)  # 北大分词器
    ...
    def _lda_prob_matrix(self):
        ...
        dict_dir_path = os.path.join(os.path.dirname(
            os.path.dirname(DIR_PATH)), 'dictionary')
            ...
            for sen_segs, sen_segs_weights in zip(sentences_segs_list, sentences_segs_weights_list):
                ...
                for n in range(1, sen_length + 1):  # n-grams
                    for i in range(0, sen_length - n + 1):
                        ...
                        candidate_phrase_weight = sum(
                            sen_segs_weights[i: i + n])
    ...
    def _topic_prominence(self):
        ...
        for word in self.topic_word_weight:
            ...
            for i in range(self.topic_num):
                if str(i) in self.topic_word_weight[word]:
                    conditional_prob_list.append(
                        self.topic_word_weight[word][str(i)])
                else:
                    conditional_prob_list.append(1e-5)
            conditional_prob = np.array(conditional_prob_list)

            tmp_dot_log_res = np.log2(np.multiply(
                conditional_prob, init_prob_distribution))
            kl_div_sum = np.dot(
                conditional_prob, tmp_dot_log_res)  # kl divergence
        ...
        for k, v in topic_prominence_dict.items():
            topic_prominence_dict[k] = (
                v - min_prominence) / (max_prominence - min_prominence)

        self.topic_prominence_dict = topic_prominence_dict

        # 计算未知词汇的主题突出度，由于停用词已经预先过滤，所以这里不需要再考停用词无突出度
        tmp_prominence_list = [item[1]
                               for item in self.topic_prominence_dict.items()]
        self.unk_topic_prominence_value = sum(
            tmp_prominence_list) / (2 * len(tmp_prominence_list))
...
if __name__ == '__main__':
    ...
    key_phrases = ckpe_obj(text, allow_length_weight=False,
                           topic_theta=0.5, max_phrase_len=8)
