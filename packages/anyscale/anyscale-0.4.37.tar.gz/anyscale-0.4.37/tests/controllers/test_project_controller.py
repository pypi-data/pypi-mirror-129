...
def test_list(mock_api_client: Mock, name: str, json_format: bool, any_creator: bool):
    ...
    mock_anyscale_client = Mock()
    mock_anyscale_client.search_projects = mock_search_projects
    ...
    project_controller.list(name, json_format, any_creator)

    if any_creator:
        mock_get_user_info_api_v2_userinfo_get.assert_not_called()
    else:
        mock_get_user_info_api_v2_userinfo_get.assert_called_once_with()

    mock_search_projects.assert_called_once()
    (projects_query,) = mock_search_projects.call_args[0]
    if name:
