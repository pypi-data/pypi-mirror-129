...
from anyscale.project import get_project_id, load_project_or_throw, validate_project_id
...
class ClusterController:
    ...
    def start(
        ...
        project_id, cluster_name = self._get_project_id_and_cluster_name(
            cluster_id, project_id, cluster_name
        )
        ...
        if needs_start:
            ...
            self.log.info(f"Cluster {cluster_name} finished starting. View at {url}")
        else:
            self.log.info(
    ...
    def list(
        ...
        if project_id is not None and include_all_projects:
            self.log.warning(
    ...
    def _get_cluster_compute(
        ...
        if cluster_compute_name:
            ...
            cluster_compute = register_compute_template(config_object, self.api_client)
        else:
