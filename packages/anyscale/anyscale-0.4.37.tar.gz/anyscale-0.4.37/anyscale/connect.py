...
class ClientBuilder:
    ...
    def _set_runtime_env_in_job_config(
        self, project_dir: Optional[str], default_project_tempdir: str
    ) -> None:
        ...
        if "working_dir" not in runtime_env:
            ...
            elif not self._in_shell:
                # Default project case. Only the script itself
                # should be uploaded to the cluster. Don't do this
                # if running in a shell. You can't copy a script if
                # there is no script.

                # Note: sys.argv[0] may be a path and not just a filename.
                # Running os.path.join on two paths clobbers the first argument.
                # So be sure to join with os.path.basename(sys.argv[0]) which does not
                # have this problem.
                shutil.copyfile(
                    sys.argv[0],
                    os.path.join(
                        default_project_tempdir, os.path.basename(sys.argv[0])
                    ),
                )
                runtime_env["working_dir"] = os.path.expanduser(default_project_tempdir)
    ...
    def connect(self) -> AnyscaleClientContext:  # noqa
        ...
        with tempfile.TemporaryDirectory() as default_project_tmpdir:
            self._set_runtime_env_in_job_config(
                self._project_dir, default_project_tmpdir
            )
