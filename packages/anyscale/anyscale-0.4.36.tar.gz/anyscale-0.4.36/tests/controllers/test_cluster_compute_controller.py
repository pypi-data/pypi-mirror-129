...
def test_delete(
    ...
    if cluster_compute_name is None and id is None:
        ...
        mock_sdk_client.delete_cluster_compute.assert_not_called()
        ...
        cluster_compute_controller.delete(cluster_compute_name, id)
        mock_sdk_client.search_cluster_computes.assert_called_once_with(
            ClusterComputesQuery(name=TextQuery(equals=cluster_compute_name))
        )
        mock_sdk_client.delete_cluster_compute.assert_called_once_with(
            cluster_compute_test_data.id
        )
    elif cluster_compute_name is None and id:
        cluster_compute_controller.delete(cluster_compute_name, id)
        mock_sdk_client.get_cluster_compute.assert_called_once_with(id)
        mock_sdk_client.delete_cluster_compute.assert_called_once_with(id)
