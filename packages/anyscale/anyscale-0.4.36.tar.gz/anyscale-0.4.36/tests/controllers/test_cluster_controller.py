...
def test_get_project_id_and_cluster_name_given_cluster_id(
    ...
    assert cluster_controller._get_project_id_and_cluster_name(
        cluster_id="mock_cluster_id", project_id=None, cluster_name=None
    ...
    with patch.multiple(
        "anyscale.controllers.cluster_controller", validate_project_id=Mock(),
    ):
        assert cluster_controller._get_project_id_and_cluster_name(
            cluster_id=None, project_id="mock_project_id1", cluster_name=None
        ) == ("mock_project_id1", "mock_cluster_name1")

    # Test using default project
    cluster_controller._get_or_generate_cluster_name = Mock(  # type: ignore
        return_value="mock_cluster_name2"
    )
    cluster_controller.anyscale_api_client.get_default_project = Mock(
    ...
    with patch.multiple(
        ...
        assert cluster_controller._get_project_id_and_cluster_name(
            cluster_id=None, project_id=None, cluster_name=None
        ) == ("mock_project_id2", "mock_cluster_name2")


def test_get_cluster_env_and_build(
    base_mock_api_client: Mock, base_mock_anyscale_api_client: Mock,
):
    cluster_controller = ClusterController(
        api_client=base_mock_api_client,
...
def test_get_cluster_compute(
    ...
    mock_register_compute_template = Mock(return_value=Mock(id="cluster_compute2"))
...
def test_start(
    base_mock_api_client: Mock, base_mock_anyscale_api_client: Mock, needs_start: bool
) -> None:
    cluster_controller = ClusterController(
        api_client=base_mock_api_client,
        anyscale_api_client=base_mock_anyscale_api_client,
...
def test_terminate(
    ...
    with pytest.raises(click.ClickException):
        cluster_controller.terminate(None, None, None)
    with pytest.raises(click.ClickException):
        cluster_controller.terminate("cluster_name", "cluster_id", None)
    if len(cluster_list) == 0:
        with pytest.raises(click.ClickException):
            cluster_controller.terminate("cluster_name", None, "project_id")
    else:
        cluster_controller.terminate("cluster_name", None, "project_id")
        cluster_controller._get_project_id_and_cluster_name.assert_called_once_with(
            None, "project_id", "cluster_name"
        )
