...
def _format_pred(dfpred,colname,Tdim):
    x = pd.DataFrame(dfpred).rename(columns = {t:f't{t}' for t in range(Tdim)})
    x = x.reset_index().rename(columns = {'index':'ind'})
    return pd.wide_to_long(x,stubnames = ['t'], i="ind", j="time").reset_index().rename(columns = {'t':colname})
...
def fn_add_lags(df,idvar,timevar,varlist,lagorders):
    ...
    for lag in lagorders:
        ...
        renameDict = {i:f'{i}_l{lag}' for i in varlist}
        df_lagged = dfl.groupby(level = [0])[varlist].shift(lag).reset_index().rename(columns = renameDict)
