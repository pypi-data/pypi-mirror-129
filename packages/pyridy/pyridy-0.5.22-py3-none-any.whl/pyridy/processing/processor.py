...
class PostProcessor:
    def __init__(self, campaign: Campaign, thres_lo: float = 0.15, thres_hi: float = 0.25, v_thres: float = 1.0,
        ...
        for f in tqdm(self.campaign.files):
            self.trk_conds.append(self.con_proc.evaluate_track_condition(f, method))
    ...
    def add_track_condition_to_map(self, m: Map, hide_good_sections: bool = False) -> Map:
        for trk_con in self.trk_conds:
            ...
            if not hide_good_sections:
                ...
                good_markers = [CircleMarker(location=(row[0], row[1]), radius=5, fill_color='#57AB27', color='#000000')
                                for row in trk_con_good[['lat', 'lon']]]
            ...
            okay_markers = [CircleMarker(location=(row[0], row[1]), radius=5, fill_color='#57AB27', color='#000000')
                            for row in trk_con_okay[['lat', 'lon']]]
            ...
            bad_markers = [CircleMarker(location=(row[0], row[1]), radius=5, fill_color='#57AB27', color='#000000')
                           for row in trk_con_bad[['lat', 'lon']]]
