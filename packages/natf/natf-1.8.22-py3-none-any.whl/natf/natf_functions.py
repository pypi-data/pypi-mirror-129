...
# global variables
supported_nuc_sets = ('TENDL2019', 'FENDL3')
...
def get_nuclide_sets(config):
    ...
    tokens = config.get('mcnp', 'NUCLIDE_SETS').split(',')
    nuclide_sets = []
...
def natf_cell_mat_evo(config):
    ...
    # init record_file
    if moniter_cell:
        record_file = f"c{moniter_cell}_drop.csv"
    ...
    if stat == 'POST' and current_step < total_steps - 1:
        ...
        for c in cells_need_cal:
            ...
            mat.fispact2mcnp(ntrans_avail_nucs, record_drop=record_drop,
                             record_file=record_file, iter_step=current_step)
