...
from domain.distribution.distribution_cell_value_types import DistributionCellValueType
from domain.distribution.distribution_data import DistributionData
...
class DistributionSheet(Sheet):
    distribution_images: List[BytesIO] = []
    max_col: int = 3
    start_col: int
    ...
    def __init__(
        ...
        for image in worksheet.images:
            self.distribution_images.append(image.ref)
