...
def adjoint(matrix1):
	...
	if order[0] == 2:
		...
		res = [[((-1)**(i+j+2))*(matrix1[i][j])*(determinantOf(matrix1.minorOfValueAt(i+1, j+1))) for j in range(order[1])] for i in range(order[0])]
...
def determinantOf(matrix):
	...
	if order[0] == order[1]:
		if order[0] == 1:
			...
			M11 = matrix[0][0]*(determinantOf(matrix.minorOfValueAt(1, 1)))
			M12 = matrix[0][1]*(determinantOf(matrix.minorOfValueAt(1, 2)))
			M13 = matrix[0][2]*(determinantOf(matrix.minorOfValueAt(1, 3)))
