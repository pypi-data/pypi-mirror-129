...
from freem_bots.configuration import Configuration
...
class DiscordBot(discord.ext.commands.Bot):
	...
	async def get_audio_for_text(self, text: str, voice: TTSVoice, username: str):
	...
	async def get_audio_for_text_simple(self, text: str, voice: TTSVoice):
		'''Gets PCM for a simple string'''
		voiceline = TTSVoiceline([TTSVoicelinePart(voice, text)])
		return await self.get_voiceline_pcm(voiceline)

	async def get_voiceline_pcm(self, voiceline: TTSVoiceline):
	...
	def get_pcm_from_file(self, file):
		...
		audio_stream = wave.open(file, 'rb')
		num_frames = audio_stream.getnframes()
		audio_raw = audio_stream.readframes(num_frames)
		audio_stream.close()

		return audio_raw

	@staticmethod
	def get_pcm_from_bytes(sound_bytes: bytes):
		'''Gets PCM from bytes'''
	...
	def send_log_to_log_channel(self, message: str):
		self._log_queue.put_nowait(message)

	async def locate_user_in_voice_channel(self, target_user, guild):
