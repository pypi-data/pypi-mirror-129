...
class TTSVoicelinePart:
	...
	def _scrub_duplicate_voice_definitions(input_ssml: str) -> str:
		...
		opening_matches = []
		for match in re.finditer(r'<voice name=\"(?P<voice>.*?)\">', input_ssml):
			start = match.start()
			voice = match.group('voice')
			opening_matches.append([start, voice])

		closing_match_positions = []
		for match in re.finditer(r'</voice>', input_ssml):
			start = match.start()
			closing_match_positions.append(start)

		marked_opening_matches_for_removal = []

		last_match = []
		for match in opening_matches:
			if len(last_match) > 0 and last_match[1] == match[1]:
				# same voice, can remove here
				marked_opening_matches_for_removal.append(match)
			last_match = match
