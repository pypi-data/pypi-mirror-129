...
from azure.ml.component._util._utils import _ensure_target_folder_exist, _sanitize_python_variable_name, \
    _get_param_in_group_from_pipeline_parameters, _download_snapshot_by_id, _relative_path, _relative_to
...
class ComponentDefinition(Component):
    ...
    def load(cls, yaml_file, *, _construct_local_runsettings=False) -> Union[
        ...
        try:
            ...
            err_msg = f"Failed to load yaml file {yaml_file!r}, only local file with YAML format is supported"
            raise UserErrorException(err_msg, inner_exception=e) from e
...
class CommandComponentDefinition(ComponentDefinition):
    ...
    def register(workspace, spec_file, package_zip, anonymous_registration: bool,
        ...
        if anonymous_registration:
            ...
            # \r Ensure that the terminal output statement starts printing from the lead
            print("\rCreated Anonymous Component: %s" % repr_dict)
        return definition
    ...
    def _save_to_code_folder(self, folder, spec_file=None):
        ...
        source_dir_2_spec_folder = _relative_to(spec_file.parent, source_directory)
        spec_under_source = True
        if not source_dir_2_spec_folder:
            # Get the relative path from spec to code (may not under same directory).
            source_dir_2_spec_folder = _relative_path(spec_file.parent, source_directory)
            spec_under_source = False
        if not source_dir_2_spec_folder:
            ...
            source_dir_2_spec_folder = Path(spec_file.parent).absolute()
        ...
        if env.conda is not None and env.conda.conda_dependencies is None:
            conda_dir = source_dir_2_spec_folder if spec_under_source else Path('.')
            conda_file = (conda_dir / self.DEFAULT_CONDA_FILE).as_posix()
            ...
            if not (Path(source_directory) / conda_dir / self.DEFAULT_CONDA_FILE).exists():
                msg = "Conda file %s doesn't exist in the folder %s, a default one is dumped at %s." % (
                    self.DEFAULT_CONDA_FILE, conda_dir, Path(spec_file.parent) / self.DEFAULT_CONDA_FILE)
                logging.info(msg)
                # Generate conda and spec at same folder.
                YAML._dump_yaml_file(
                    _Dependencies.create_default().conda_dependency_dict,
                    Path(spec_file.parent) / self.DEFAULT_CONDA_FILE
                )
                if not spec_under_source:
                    # If spec not under source, specify path as 'conda.yaml'
                    # as we will copy it into the same level folder as the code.
                    spec_dict['environment']['conda'][env.conda.CONDA_FILE_KEY] = self.DEFAULT_CONDA_FILE
