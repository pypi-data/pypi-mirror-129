...
class Run(Visible, WorkspaceTelemetryMixin):
    ...
    @track(_get_logger, activity_type=_PUBLIC_API, activity_name="Run_debug")
    def _debug(self, compute=None):
        ...
        debugger = OnlineStepRunDebugger(run_id=self._id,
    ...
    @track(_get_logger, activity_type=_PUBLIC_API, activity_name="Run_cancel")
    def cancel(self):
    ...
    def _repr_html_(self):
        if enabled_subpipeline_registration():
            ...
            info = self._get_base_info_dict()
            info.update([
                ('Details Page', make_link(self._get_portal_url(), "Link to Azure Machine Learning studio")),
