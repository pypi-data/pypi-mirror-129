    ...
    def do_GET(self):
        ...
        try:
            if designer_service_caller is None:
                ...
                run_graph = _get_graph_from_query_dict(query_dict)
                ...
                run_status = _get_run_status_from_query_dict(query_dict)
                ...
                graph_no_status = _get_graph_no_staus_from_query_dict(query_dict)
                ...
                if 'runId' not in query_dict:
                    self.send_error(400, 'missing required parameter runId')
                result = _get_child_runs_from_query_dict(query_dict)
                self._set_response()
                self.wfile.write(result.response.content)
            elif parse_result.path == PIPELINE_RUN_ROUTE:
                if 'runId' not in query_dict:
                    self.send_error(400, 'missing required parameter runId')
                run = _get_pipeline_run_from_query_dict(query_dict)
                self._set_response()
                ...
                step_details = _get_step_run_details_from_run(run, query_dict)
