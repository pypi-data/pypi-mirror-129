...
class DB_Pool:
    ...
    def __init__(self, con, init_num=0, num=15, encoding=None,is_make_dsn=False):
        self.con = con
        db_engine = self.get_db_api(self.con)
        self.db_type = db_engine[0]
        self.db_config = db_engine[1]
        self.is_make_dsn = is_make_dsn
        self.config_dict = {
            "mincached": init_num,
            "maxcached": num,
            "maxshared": num,
...
class Sqlalchemy_Pool(DB_Pool):
    def __init__(self, con, num=15, encoding="utf8",is_make_dsn=False):
        self.con = con
        db_engine = self.get_db_api(self.con)
        self.db_type = db_engine[0]
        self.db_config = db_engine[1]
