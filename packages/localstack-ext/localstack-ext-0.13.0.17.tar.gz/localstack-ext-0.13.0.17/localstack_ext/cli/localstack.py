...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return jaktq if e else jakty
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->jaktx:
...
@click.option("--revert",is_flag=jaktq,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:jakte):
...
def _cpvcs_initialized(pod_name:jaktP)->jakte:
...
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_init(name:jaktP):
...
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=jaktq,default=jakty)
def cmd_cpvcs_delete(name:jaktP,remote:jakte):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_register_remote(name:jaktP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=jaktq)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=jaktq)
def cmd_cpvcs_rename(name:jaktP,new_name:jaktP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_commit(message:jaktP,name:jaktP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=jaktq,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=jaktq,default=jakty,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_push(squash:jakte,message:jaktP,name:jaktP,three_way:jakte):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
@click.option("-v","--version",type=jaktL)
@click.option("-m","--message",required=jakty)
def cmd_cpvcs_push_overwrite(version:jaktL,message:jaktP,name:jaktP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=jaktL,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=jaktq,default=jakty,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_inject(version:jaktL,reset:jakte,name:jaktP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=jaktq,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=jaktq,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:jaktP,inject:jakte,reset:jakte):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=jaktq,default=jakty)
def cmd_cpvcs_list_pods(remote:jakte):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_versions(name:jaktP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=jaktq,type=jaktL)
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_version_info(version:jaktL,name:jaktP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=jaktL,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_version_metamodel(version:jaktL,name:jaktP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=jaktq,type=jaktL,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=jaktq,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=jaktq,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=jakty,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_set_version(version:jaktL,inject:jakte,reset:jakte,commit_before:jakte,name:jaktP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_commits(version:jaktL,name:jaktP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=jaktq)
@click.option("--commit","-c",required=jaktq)
@click.option("-n","--name",help="Name of the cloud pod",required=jaktq)
def cmd_cpvcs_commit_diff(version:jaktL,commit:jaktL,name:jaktP):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(jaktx)
  status.stop()
...
def print_table(columns:Dict[jaktP,jaktP],rows:List[Dict[jaktP,Any]],formatters:Dict[jaktP,Callable[[Any],jaktP]]=jaktx):
 from rich.table import Table
 if formatters is jaktx:
  formatters=jaktD()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  jaktF(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->jaktx:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->jaktx:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->jaktx:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->jaktx:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
