...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=IbeuL(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not IbeuJ
   return result
  except Ibeud:
   props=self.props
   ...
   if parts[0]in candidates and IbeuU(parts)>1:
