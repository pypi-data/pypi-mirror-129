...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or NKokJ
 @NKokA
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return NKokL(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @NKokh
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if NKokl(origins,NKokU)else origins.get("Items")
  origins_to_create=NKokd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if NKokl(aliases,NKokU)else aliases.get("Items")
  aliases_to_create=NKokd(aliases)
  for distr in distrs:
   origins=NKokd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=NKokd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",NKokv)
   config["Enabled"]=NKoku(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if NKokl(custom_errors,NKokF)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=NKoku(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":NKokv}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=NKoku(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=NKokJ):
    parent=config if parent is NKokJ else parent
    items=parent.get(key)or[]
    if NKokl(items,NKokF)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":NKokY(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if NKokl(cache_behavs,NKokF)else cache_behavs)
