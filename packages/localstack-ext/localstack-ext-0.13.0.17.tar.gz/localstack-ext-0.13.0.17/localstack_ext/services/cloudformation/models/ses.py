...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[sXcdI])[0]
 def get_physical_resource_id(self,attribute=sXcdI,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if sXcdD(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return sXcdo(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @sXcdr
 def add_defaults(resource,stack_name:sXcdh):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return sXcdo(SESReceiptRule,self).get_cfn_attribute(attribute)
 @sXcdQ
