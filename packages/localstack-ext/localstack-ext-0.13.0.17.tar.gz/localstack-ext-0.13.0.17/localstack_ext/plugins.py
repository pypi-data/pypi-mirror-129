...
def api_key_configured()->dwWhv:
 return dwWhF if os.environ.get("LOCALSTACK_API_KEY")else dwWhf
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if dwWhg([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
