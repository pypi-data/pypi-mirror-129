...
SENT_EMAILS=[]
EMAIL_BLACKLIST=beBEc()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",beBEw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=beBEv,from_name=beBEv,smtp_host=beBEv,smtp_user=beBEv,smtp_pass=beBEv,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if beBEr(recipients,beBEF)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=beBEv):
 ...
 result["From"]=formataddr((beBEw(Header(from_name,"utf-8")),from_email))
 return result
