...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[KlJoI],KlJoI],api_name:KlJoI):
 if not(is_debian()and in_docker()):
  ...
  return
 if KlJox(packages,KlJoI):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=KlJoG)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=KlJoG)
def check_or_install(command:KlJoI,packages:Union[List[KlJoI],KlJoI],api:KlJoI):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=KlJoG):
  try:
