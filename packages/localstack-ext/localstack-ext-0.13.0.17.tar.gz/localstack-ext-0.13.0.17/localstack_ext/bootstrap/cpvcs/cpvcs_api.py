...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=fVHwl,state_files=fVHwG(),version_number=0)
 ...
 with fVHwb(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with fVHwb(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with fVHwb(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:fVHwP,meta_archives:Dict[fVHwP,fVHwP],state_archives:Dict[fVHwP,fVHwP],remote_info:Dict[fVHwP,fVHwP],pod_name:fVHwP):
...
def merge_from_remote(version_space_archive:fVHwP,meta_archives:Dict[fVHwP,fVHwP],state_archives:Dict[fVHwP,fVHwP]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->fVHwv:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:fVHwP):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:fVHwP)->fVHwv:
 ...
 return fVHwx
def list_locally_available_pods(show_remote_or_local:fVHwv=fVHwx)->Set[fVHwP]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:fVHwP,file_name:fVHwP,service:fVHwP,region:fVHwP)->fVHwP:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->fVHwP:
 ...
 return key
def _get_state_file_path(key:fVHwP)->fVHwP:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=fVHwG(fVHwn(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:fVHwP)->Optional[fVHwP]:
...
def get_version_info(version_no:fVHwo)->Dict[fVHwP,fVHwP]:
...
def read_file_from_archive(archive_path:fVHwP,file_name:fVHwP):
 try:
...
def commit(message:fVHwP=fVHwl)->Revision:
 ...
 curr_expansion_point_parent_state_files=fVHwG()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=fVHwl
...
def zip_directories(zip_dest:fVHwP,directories:List[fVHwP])->fVHwP:
 ...
 return zip_dest
def create_version_space_archive()->fVHwP:
...
def get_max_version_no()->fVHwo:
 with fVHwb(config_context.get_max_ver_path())as fp:
  return fVHwo(os.path.basename(fp.readline()))
...
def push_overwrite(version:fVHwo,comment:fVHwP)->fVHwv:
 ...
 _create_metadata_archive(version_node=version_node,overwrite=fVHwx,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return fVHwx
def push(comment:fVHwP=fVHwl,three_way:fVHwv=fVHwO)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=fVHwG(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,get_version_state_archive(new_max_version_no)),state_files=fVHwG(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 ...
 return new_version
def get_version_meta_archive(version_no:fVHwo)->fVHwP:
...
def get_version_state_archive(version_no:fVHwo)->fVHwP:
...
def get_version_metamodel(version_no:fVHwo)->fVHwP:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:fVHwv=fVHwO,overwrite:fVHwv=fVHwO,metamodels_file:fVHwP=fVHwl):
 ...
 with fVHwb(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with fVHwb(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:fVHwo,state_file_refs:Set[StateFileRef],delete_files=fVHwO,archive=fVHwx):
...
def set_active_version(version_no:fVHwo,commit_before=fVHwO)->fVHwv:
 ...
 return fVHwO
def _set_active_version(key:fVHwP):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=fVHwG(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:fVHwo)->Version:
 versions=load_version_references()
 version_ref=fVHwc((version[1]for version in versions if version[0]==version_no),fVHwl)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[fVHwo,fVHwP]]:
 result={}
 with fVHwb(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with fVHwb(symlink,"r")as sp:
    result[fVHwo(os.path.basename(symlink))]=sp.readline()
 return fVHwT(result.items(),key=lambda x:x[0],reverse=fVHwx)
def list_versions()->List[fVHwP]:
 ...
 return result
def list_version_commits(version_no:fVHwo)->List[fVHwP]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:fVHwo,commit_no:fVHwo)->fVHwP:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->fVHwP:
 with fVHwb(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->fVHwP:
 with fVHwb(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->fVHwP:
 with fVHwb(config_context.get_known_ver_path(),"a")as fp:
...
def _is_special_case(obj)->fVHwv:
 return fVHwS(obj,(Queue,FakeBucket))
...
def _merge_two_state_dir_func(**kwargs):
 ...
 if os.path.isfile(dst_state_file_path):
  ...
  mkdir(dst_state_dir)
  persist_object(src_state,dst_state_file_path)
def _merge_expansion_point_with_max(three_way=fVHwO):
 ...
 common_ancestor_state_archive=fVHwl
 tmp_common_ancestor_state_dir=fVHwl
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=fVHwO)
...
def _infer_backend_init(clazz:Type,sf:StateFileRef)->fVHwa:
 if fVHwS(clazz,fVHwX):
  return{}
 constructor=fVHwA(clazz,"__init__",fVHwl)
 ...
 if "region" in sig_args.args:
  ...
  backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->fVHwP:
 try:
  ...
  save_file(dest,"{}")
  return key
def _do_create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->fVHwP:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[fVHwP,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,fVHwl)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(fVHwl,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=fVHwr(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=fVHwr(backend1)
