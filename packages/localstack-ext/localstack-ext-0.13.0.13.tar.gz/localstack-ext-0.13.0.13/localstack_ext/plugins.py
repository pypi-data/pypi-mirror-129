...
def api_key_configured()->KNsJX:
 return KNsJM if os.environ.get("LOCALSTACK_API_KEY")else KNsJE
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if KNsJq([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
