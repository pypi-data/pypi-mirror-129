...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return vsDgq if e else vsDgB
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->vsDgK:
...
@click.option("--revert",is_flag=vsDgq,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:vsDgt):
...
def _cpvcs_initialized(pod_name:vsDgk)->vsDgt:
...
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_init(name:vsDgk):
...
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=vsDgq,default=vsDgB)
def cmd_cpvcs_delete(name:vsDgk,remote:vsDgt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_register_remote(name:vsDgk):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=vsDgq)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=vsDgq)
def cmd_cpvcs_rename(name:vsDgk,new_name:vsDgk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_commit(message:vsDgk,name:vsDgk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=vsDgq,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=vsDgq,default=vsDgB,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_push(squash:vsDgt,message:vsDgk,name:vsDgk,three_way:vsDgt):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
@click.option("-v","--version",type=vsDgX)
@click.option("-m","--message",required=vsDgB)
def cmd_cpvcs_push_overwrite(version:vsDgX,message:vsDgk,name:vsDgk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=vsDgX,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=vsDgq,default=vsDgB,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_inject(version:vsDgX,reset:vsDgt,name:vsDgk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=vsDgq,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=vsDgq,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:vsDgk,inject:vsDgt,reset:vsDgt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=vsDgq,default=vsDgB)
def cmd_cpvcs_list_pods(remote:vsDgt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_versions(name:vsDgk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=vsDgq,type=vsDgX)
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_version_info(version:vsDgX,name:vsDgk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=vsDgX,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_version_metamodel(version:vsDgX,name:vsDgk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=vsDgq,type=vsDgX,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=vsDgq,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=vsDgq,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=vsDgB,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_set_version(version:vsDgX,inject:vsDgt,reset:vsDgt,commit_before:vsDgt,name:vsDgk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_commits(version:vsDgX,name:vsDgk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=vsDgq)
@click.option("--commit","-c",required=vsDgq)
@click.option("-n","--name",help="Name of the cloud pod",required=vsDgq)
def cmd_cpvcs_commit_diff(version:vsDgX,commit:vsDgX,name:vsDgk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(vsDgK)
  status.stop()
...
def print_table(columns:Dict[vsDgk,vsDgk],rows:List[Dict[vsDgk,Any]],formatters:Dict[vsDgk,Callable[[Any],vsDgk]]=vsDgK):
 from rich.table import Table
 if formatters is vsDgK:
  formatters=vsDgP()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  vsDgY(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->vsDgK:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->vsDgK:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->vsDgK:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->vsDgK:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
