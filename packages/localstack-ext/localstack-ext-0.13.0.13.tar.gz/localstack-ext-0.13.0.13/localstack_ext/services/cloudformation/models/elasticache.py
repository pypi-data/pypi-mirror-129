...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=TVurj(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not TVurE
   return result
  except TVurO:
   props=self.props
   ...
   if parts[0]in candidates and TVurs(parts)>1:
