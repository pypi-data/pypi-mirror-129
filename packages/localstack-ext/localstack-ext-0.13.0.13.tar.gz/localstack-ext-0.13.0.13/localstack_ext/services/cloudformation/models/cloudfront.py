...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or xiSFR
 @xiSFU
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return xiSFB(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @xiSFY
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if xiSFv(origins,xiSFK)else origins.get("Items")
  origins_to_create=xiSFI([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if xiSFv(aliases,xiSFK)else aliases.get("Items")
  aliases_to_create=xiSFI(aliases)
  for distr in distrs:
   origins=xiSFI([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=xiSFI(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",xiSFD)
   config["Enabled"]=xiSFX(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if xiSFv(custom_errors,xiSFw)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=xiSFX(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":xiSFD}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=xiSFX(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=xiSFR):
    parent=config if parent is xiSFR else parent
    items=parent.get(key)or[]
    if xiSFv(items,xiSFw)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":xiSFV(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if xiSFv(cache_behavs,xiSFw)else cache_behavs)
