...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=PgUaK
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:PgUar)->PgUar:
  pass
 @PgUaM
 def get(cls,requested_file_locator:PgUar):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=PgUaK
 @PgUaM
 def get(cls,state_file_locator:PgUar,requested_storage:PgUar,serializers:Dict[PgUar,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[PgUar,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:PgUar)->PgUar:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:PgUar)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:PgUar)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:PgUar)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:PgUar)->PgUar:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[PgUar]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:PgUar,new_key:PgUar,referenced_by_version:PgUar=PgUaK):
  pass
 @abc.abstractmethod
 def version_exists(self,key:PgUar)->PgUax:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:PgUar,key:PgUar):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:PgUar,new_key:PgUar,base_path:PgUar)->PgUax:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[PgUar]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[PgUar,PgUar],remote_location:PgUar=PgUaK)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[PgUar]:
  ...
  for cpvcs_obj in args:
   if PgUav(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif PgUav(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[PgUar,PgUar],remote_location:PgUar=PgUaK)->List[Union[Revision,Version]]:
