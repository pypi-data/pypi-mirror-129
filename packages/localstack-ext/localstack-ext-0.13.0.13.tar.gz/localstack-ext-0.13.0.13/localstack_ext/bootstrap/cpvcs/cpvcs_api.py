...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=oRVyX,state_files=oRVyn(),version_number=0)
 ...
 with oRVyc(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with oRVyc(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with oRVyc(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:oRVyE,meta_archives:Dict[oRVyE,oRVyE],state_archives:Dict[oRVyE,oRVyE],remote_info:Dict[oRVyE,oRVyE],pod_name:oRVyE):
...
def merge_from_remote(version_space_archive:oRVyE,meta_archives:Dict[oRVyE,oRVyE],state_archives:Dict[oRVyE,oRVyE]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->oRVyM:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:oRVyE):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:oRVyE)->oRVyM:
 ...
 return oRVyG
def list_locally_available_pods(show_remote_or_local:oRVyM=oRVyG)->Set[oRVyE]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:oRVyE,file_name:oRVyE,service:oRVyE,region:oRVyE)->oRVyE:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->oRVyE:
 ...
 return key
def _get_state_file_path(key:oRVyE)->oRVyE:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=oRVyn(oRVyB(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:oRVyE)->Optional[oRVyE]:
...
def get_version_info(version_no:oRVym)->Dict[oRVyE,oRVyE]:
...
def read_file_from_archive(archive_path:oRVyE,file_name:oRVyE):
 try:
...
def commit(message:oRVyE=oRVyX)->Revision:
 ...
 curr_expansion_point_parent_state_files=oRVyn()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=oRVyX
...
def zip_directories(zip_dest:oRVyE,directories:List[oRVyE])->oRVyE:
 ...
 return zip_dest
def create_version_space_archive()->oRVyE:
...
def get_max_version_no()->oRVym:
 with oRVyc(config_context.get_max_ver_path())as fp:
  return oRVym(os.path.basename(fp.readline()))
...
def push_overwrite(version:oRVym,comment:oRVyE)->oRVyM:
 ...
 _create_metadata_archive(version_node=version_node,overwrite=oRVyG,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return oRVyG
def push(comment:oRVyE=oRVyX,three_way:oRVyM=oRVyr)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=oRVyn(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,get_version_state_archive(new_max_version_no)),state_files=oRVyn(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 ...
 return new_version
def get_version_meta_archive(version_no:oRVym)->oRVyE:
...
def get_version_state_archive(version_no:oRVym)->oRVyE:
...
def get_version_metamodel(version_no:oRVym)->oRVyE:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:oRVyM=oRVyr,overwrite:oRVyM=oRVyr,metamodels_file:oRVyE=oRVyX):
 ...
 with oRVyc(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with oRVyc(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:oRVym,state_file_refs:Set[StateFileRef],delete_files=oRVyr,archive=oRVyG):
...
def set_active_version(version_no:oRVym,commit_before=oRVyr)->oRVyM:
 ...
 return oRVyr
def _set_active_version(key:oRVyE):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=oRVyn(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:oRVym)->Version:
 versions=load_version_references()
 version_ref=oRVyf((version[1]for version in versions if version[0]==version_no),oRVyX)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[oRVym,oRVyE]]:
 result={}
 with oRVyc(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with oRVyc(symlink,"r")as sp:
    result[oRVym(os.path.basename(symlink))]=sp.readline()
 return oRVyJ(result.items(),key=lambda x:x[0],reverse=oRVyG)
def list_versions()->List[oRVyE]:
 ...
 return result
def list_version_commits(version_no:oRVym)->List[oRVyE]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:oRVym,commit_no:oRVym)->oRVyE:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->oRVyE:
 with oRVyc(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->oRVyE:
 with oRVyc(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->oRVyE:
 with oRVyc(config_context.get_known_ver_path(),"a")as fp:
...
def _is_special_case(obj)->oRVyM:
 return oRVyA(obj,(Queue,FakeBucket))
...
def _merge_two_state_dir_func(**kwargs):
 ...
 if os.path.isfile(dst_state_file_path):
  ...
  mkdir(dst_state_dir)
  persist_object(src_state,dst_state_file_path)
def _merge_expansion_point_with_max(three_way=oRVyr):
 ...
 common_ancestor_state_archive=oRVyX
 tmp_common_ancestor_state_dir=oRVyX
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=oRVyr)
...
def _infer_backend_init(clazz,sf):
 if oRVyA(clazz,oRVyj):
  backend={}
 else:
  constructor=oRVyI(clazz,"__init__",oRVyX)
  ...
  if "region" in sig_args.args:
   ...
   backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->oRVyE:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[oRVyE,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,oRVyX)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(oRVyX,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=oRVyC(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=oRVyC(backend1)
