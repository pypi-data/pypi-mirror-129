...
class PodInfo:
 def __init__(self,name=ahjfQ,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(ahjfF):
 BACKEND="_none_"
 def __init__(self,pod_name=ahjfQ,config=ahjfQ):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise ahjfp("Not implemented")
 def delete(self,remote:ahjfc)->ahjfc:
  raise ahjfp("Not implemented")
 def push(self,comment:ahjfV=ahjfQ,three_way:ahjfc=ahjfy)->PodInfo:
  raise ahjfp("Not implemented")
 def push_overwrite(self,version:ahjfq,comment:ahjfV=ahjfQ):
  pass
 def pull(self,inject_version_state:ahjfc=ahjfy,reset_state_before:ahjfc=ahjfy,fetch_all:ahjfc=ahjfQ):
  raise ahjfp("Not implemented")
 def commit(self,message:ahjfV=ahjfQ):
  raise ahjfp("Not implemented")
 def inject(self,version:ahjfq,reset_state:ahjfc):
  raise ahjfp("Not implemented")
 def list_versions(self)->List[ahjfV]:
  raise ahjfp("Not implemented")
 def version_info(self,version:ahjfq):
  raise ahjfp("Not implemented")
 def version_metamodel(self,version:ahjfq):
  raise ahjfp("Not implemented")
 def set_version(self,version:ahjfq,inject_version_state:ahjfc,reset_state:ahjfc,commit_before:ahjfc):
  raise ahjfp("Not implemented")
 def list_version_commits(self,version:ahjfq)->List[ahjfV]:
  raise ahjfp("Not implemented")
 def get_commit_diff(self,version:ahjfq,commit:ahjfq)->ahjfV:
  raise ahjfp("Not implemented")
 def register_remote(self,pod_name:ahjfV,ci_pod:ahjfc)->ahjfc:
  raise ahjfp("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->ahjfc:
  raise ahjfp("Not implemented")
 def list_pods(self,fetch_remote:ahjfc)->Set[ahjfV]:
  raise ahjfp("Not implemented")
 ...
 def get(cls,pod_name,pre_config=ahjfQ):
  ...
  raise ahjfp('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=ahjfy
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=ahjfm)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=ahjfm
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise ahjfp("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @ahjfd
 def get_state_zip_from_instance(get_content=ahjfy):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise ahjfp("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:ahjfV=ahjfQ):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @ahjfd
 def parse_pod_name_from_qualifying_name(qualifying_name:ahjfV)->ahjfV:
 ...
 @ahjfd
 def _get_max_version_for_pod_from_platform(pod_name:ahjfV,auth_headers):
  ...
  remote_max_ver=ahjfq(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with ahjfJ(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with ahjfJ(meta_archive,"rb")as meta_archive_content:
   ...
   with ahjfJ(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=ahjfQ)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:ahjfV):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:ahjfc)->ahjfc:
  ...
  if remote:
   pass
  return ahjfy
 def _push_to_remote(self,url:ahjfV):
 ...
 def push_overwrite(self,version:ahjfq,comment:ahjfV=ahjfQ):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return ahjfm
 def pull(self,inject_version_state:ahjfc=ahjfy,reset_state_before:ahjfc=ahjfy,fetch_all:ahjfc=ahjfm):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=ahjfy):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(ahjfM(lambda ver:ahjfV(ver),ahjfs(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:ahjfq,reset_state:ahjfc):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=ahjfm,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def get_commit_diff(self,version:ahjfq,commit:ahjfq)->ahjfV:
  ...
  return commit_diff
 def register_remote(self,pod_name:ahjfV,ci_pod:ahjfc=ahjfy)->ahjfc:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:ahjfc)->Set[ahjfV]:
  ...
  return result
 @ahjfT
 def upload_content(cls,presigned_url:ahjfV,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise ahjfp("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @ahjfd
 def create_platform_url(request:ahjfV)->ahjfV:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:ahjfV=ahjfQ,three_way:ahjfc=ahjfy)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=ahjfm)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(ahjfz(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=ahjfz(zip_data_content)
  return result
 def pull(self,inject_version_state:ahjfc=ahjfy,reset_state_before:ahjfc=ahjfy,fetch_all:ahjfc=ahjfQ):
  ...
  rm_rf(zip_path)
 @ahjfd
 def presigned_url(pod_name:ahjfV,mode:ahjfV)->ahjfV:
  ...
  if response.status_code>=400:
   raise ahjfp("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise ahjfp("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:ahjfV=ahjfQ,three_way:ahjfc=ahjfy):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=ahjfm)
 ...
 def pull(self,inject_version_state:ahjfc=ahjfy,reset_state_before:ahjfc=ahjfy,fetch_all:ahjfc=ahjfQ):
  ...
  if remote_ref not in remote_refs:
   raise ahjfp('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=ahjfe(self,"clone_dir",ahjfQ)
...
class PodConfigManagerMeta(ahjfg):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=ahjfQ
   for manager in cls.CHAIN:
    try:
     tmp=ahjfe(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif ahjfO(tmp,ahjfI)and ahjfO(result,ahjfI):
       result.extend(tmp)
    except ahjfp:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not ahjfQ:
    return result
   raise ahjfp('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(ahjfF,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @ahjfT
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise ahjfp('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(ahjfF):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=ahjfm).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=ahjfP([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=ahjfy):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except ahjfp:
   if safe:
    return{}
   raise ahjfp('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(ahjfF):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise ahjfp("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise ahjfp("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:ahjfV,remote:ahjfc,pre_config:Dict[ahjfV,ahjfV])->ahjfc:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],**kwargs)->ahjfc:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",ahjfy))
 return result
def rename_pod(current_pod_name:ahjfV,new_pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:ahjfc,pre_config:Dict[ahjfV,ahjfV],**kwargs)->List[ahjfV]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],message:ahjfV=ahjfQ,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:ahjfV,version:ahjfq,reset_state:ahjfc,pre_config:Dict[ahjfV,ahjfV],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],**kwargs)->List[ahjfV]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:ahjfq,pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:ahjfq,pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:ahjfq,inject_version_state:ahjfc,reset_state:ahjfc,commit_before:ahjfc,pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV],**kwargs)->ahjfc:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:ahjfq,pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV])->List[ahjfV]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:ahjfq,commit:ahjfq,pod_name:ahjfV,pre_config:Dict[ahjfV,ahjfV])->ahjfV:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:ahjfq,pod_name:ahjfV,comment:ahjfV,pre_config:Dict[ahjfV,ahjfV]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=ahjfQ,squash_commits=ahjfy,comment=ahjfQ,three_way=ahjfy,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=ahjfy,reset_state_before=ahjfy,**kwargs):
 pre_config=kwargs.get("pre_config",ahjfQ)
 if not pod_name:
  raise ahjfp("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=ahjfy,exclude_from_reset:List[ahjfV]=ahjfQ):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise ahjfp("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->ahjfV:
 try:
...
def get_persisted_resource_names(data_dir)->List[ahjfV]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and ahjfz(os.listdir(entry.path))>0:
    names.append(entry.name)
