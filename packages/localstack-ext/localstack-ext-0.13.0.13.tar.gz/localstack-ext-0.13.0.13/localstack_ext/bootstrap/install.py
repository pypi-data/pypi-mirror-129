...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[hDMuS],hDMuS],api_name:hDMuS):
 if not(is_debian()and in_docker()):
  ...
  return
 if hDMuO(packages,hDMuS):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=hDMue)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=hDMue)
def check_or_install(command:hDMuS,packages:Union[List[hDMuS],hDMuS],api:hDMuS):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=hDMue):
  try:
