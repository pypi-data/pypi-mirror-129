...
class DecryptionHandler:
 decryption_key:LpEMu
 def __init__(self,decryption_key:LpEMu):
  self.decryption_key=decryption_key
 def decrypt(self,content)->LpEMu:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=LpEMO):
  if path and not LpEMg(path,LpEMA):
   path=LpEMA(LpEMQ(path,"_path",[]))
  if not path:
   return LpEMO
...
def patch_traceback_lines():
 if LpEMQ(traceback.FrameSummary,"_ls_patch_applied",LpEMO):
  return
 @LpEMz
 def line(self):
  try:
 ...
 LpEMv(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=LpEMU
def patch_inspect_findsource():
 if LpEMQ(inspect,"_ls_patch_applied",LpEMO):
  return
 ...
 inspect._ls_patch_applied=LpEMU
