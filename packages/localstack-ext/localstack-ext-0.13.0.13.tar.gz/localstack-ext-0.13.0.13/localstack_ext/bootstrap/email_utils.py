...
SENT_EMAILS=[]
EMAIL_BLACKLIST=NnrhT()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",Nnrhw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=NnrhM,from_name=NnrhM,smtp_host=NnrhM,smtp_user=NnrhM,smtp_pass=NnrhM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if NnrhR(recipients,NnrhI)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=NnrhM):
 ...
 result["From"]=formataddr((Nnrhw(Header(from_name,"utf-8")),from_email))
 return result
