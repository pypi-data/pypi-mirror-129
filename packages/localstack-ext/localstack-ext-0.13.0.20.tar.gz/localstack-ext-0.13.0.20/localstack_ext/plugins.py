...
def api_key_configured()->SOfUG:
 return SOfUP if os.environ.get("LOCALSTACK_API_KEY")else SOfUs
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if SOfUL([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
