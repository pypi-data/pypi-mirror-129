...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[pyMKm])[0]
 def get_physical_resource_id(self,attribute=pyMKm,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if pyMKn(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return pyMKr(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @pyMKd
 def add_defaults(resource,stack_name:pyMKx):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return pyMKr(SESReceiptRule,self).get_cfn_attribute(attribute)
 @pyMKl
