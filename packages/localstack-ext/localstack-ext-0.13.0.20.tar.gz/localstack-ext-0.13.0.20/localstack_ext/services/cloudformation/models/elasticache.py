...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=OfJGA(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not OfJGL
   return result
  except OfJGW:
   props=self.props
   ...
   if parts[0]in candidates and OfJGa(parts)>1:
