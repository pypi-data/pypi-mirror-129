...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or EFwVA
 @EFwVi
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return EFwVy(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @EFwVg
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if EFwVh(origins,EFwVM)else origins.get("Items")
  origins_to_create=EFwVL([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if EFwVh(aliases,EFwVM)else aliases.get("Items")
  aliases_to_create=EFwVL(aliases)
  for distr in distrs:
   origins=EFwVL([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=EFwVL(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",EFwVN)
   config["Enabled"]=EFwVs(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if EFwVh(custom_errors,EFwVW)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=EFwVs(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":EFwVN}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=EFwVs(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=EFwVA):
    parent=config if parent is EFwVA else parent
    items=parent.get(key)or[]
    if EFwVh(items,EFwVW)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":EFwVm(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if EFwVh(cache_behavs,EFwVW)else cache_behavs)
