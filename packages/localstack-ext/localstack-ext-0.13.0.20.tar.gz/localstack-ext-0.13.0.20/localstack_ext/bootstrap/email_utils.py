...
SENT_EMAILS=[]
EMAIL_BLACKLIST=moSwl()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",moSwX(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=moSwH,from_name=moSwH,smtp_host=moSwH,smtp_user=moSwH,smtp_pass=moSwH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if moSwN(recipients,moSwO)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=moSwH):
 ...
 result["From"]=formataddr((moSwX(Header(from_name,"utf-8")),from_email))
 return result
