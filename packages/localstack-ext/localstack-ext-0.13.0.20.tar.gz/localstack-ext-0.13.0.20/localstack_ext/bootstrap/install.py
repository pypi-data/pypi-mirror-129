...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[YWmjn],YWmjn],api_name:YWmjn):
 if not(is_debian()and in_docker()):
  ...
  return
 if YWmjg(packages,YWmjn):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=YWmjc)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=YWmjc)
def check_or_install(command:YWmjn,packages:Union[List[YWmjn],YWmjn],api:YWmjn):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:YWmjn,target_file:YWmjn,timeout:int=YWmjT):
 def do_download(url,print_error=YWmjc):
  try:
