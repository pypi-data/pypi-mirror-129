...
def random_hash()->NQGEB:
 return NQGEM(random.getrandbits(160))
def compute_file_hash(file_path:NQGEB,accum=NQGEK)->Optional[NQGEB]:
 try:
  with NQGEr(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:NQGEV,state_archive:NQGEB)->NQGEB:
 ...
 m.update(NQGEB(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->NQGEB:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=NQGEH(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with NQGEr(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except NQGEm as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if NQGEe(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(NQGEB(cpvcs_node.revision_number).encode("utf-8"))
 elif NQGEe(cpvcs_node,Version):
  m.update(NQGEB(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
