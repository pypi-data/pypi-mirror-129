...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version_no,with_format:pECTX=pECTx)->pECTM:
 ...
 def update_ver_log(self,author:pECTM,ver_no:pECTg,rev_id:pECTM,rev_no:pECTg):
  with pECTs(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:pECTM,key:pECTM=pECTF)->pECTM:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:pECTM,key:pECTM=pECTF)->pECTM:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->pECTX:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:pECTM,key:pECTM,path:pECTM)->pECTM:
