...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=ABegw
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:ABegL)->ABegL:
  pass
 @ABegM
 def get(cls,requested_file_locator:ABegL):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=ABegw
 @ABegM
 def get(cls,state_file_locator:ABegL,requested_storage:ABegL,serializers:Dict[ABegL,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[ABegL,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:ABegL)->ABegL:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:ABegL)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:ABegL)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:ABegL)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:ABegL)->ABegL:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[ABegL]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:ABegL,new_key:ABegL,referenced_by_version:ABegL=ABegw):
  pass
 @abc.abstractmethod
 def version_exists(self,key:ABegL)->ABegf:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:ABegL,key:ABegL):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:ABegL,new_key:ABegL,base_path:ABegL)->ABegf:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[ABegL]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[ABegL,ABegL],remote_location:ABegL=ABegw)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[ABegL]:
  ...
  for cpvcs_obj in args:
   if ABegT(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif ABegT(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[ABegL,ABegL],remote_location:ABegL=ABegw)->List[Union[Revision,Version]]:
