...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return xEcIt if e else xEcIF
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->xEcIG:
...
@click.option("--revert",is_flag=xEcIt,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:xEcIy):
...
def _cpvcs_initialized(pod_name:xEcIV)->xEcIy:
...
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_init(name:xEcIV):
...
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=xEcIt,default=xEcIF)
def cmd_cpvcs_delete(name:xEcIV,remote:xEcIy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_register_remote(name:xEcIV):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=xEcIt)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=xEcIt)
def cmd_cpvcs_rename(name:xEcIV,new_name:xEcIV):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_commit(message:xEcIV,name:xEcIV):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=xEcIt,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=xEcIt,default=xEcIF,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_push(squash:xEcIy,message:xEcIV,name:xEcIV,three_way:xEcIy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
@click.option("-v","--version",type=xEcIa)
@click.option("-m","--message",required=xEcIF)
def cmd_cpvcs_push_overwrite(version:xEcIa,message:xEcIV,name:xEcIV):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=xEcIa,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=xEcIt,default=xEcIF,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_inject(version:xEcIa,reset:xEcIy,name:xEcIV):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=xEcIt,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=xEcIt,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:xEcIV,inject:xEcIy,reset:xEcIy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=xEcIt,default=xEcIF)
def cmd_cpvcs_list_pods(remote:xEcIy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_versions(name:xEcIV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=xEcIt,type=xEcIa)
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_version_info(version:xEcIa,name:xEcIV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=xEcIa,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_version_metamodel(version:xEcIa,name:xEcIV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=xEcIt,type=xEcIa,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=xEcIt,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=xEcIt,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=xEcIF,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_set_version(version:xEcIa,inject:xEcIy,reset:xEcIy,commit_before:xEcIy,name:xEcIV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_commits(version:xEcIa,name:xEcIV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=xEcIt)
@click.option("--commit","-c",required=xEcIt)
@click.option("-n","--name",help="Name of the cloud pod",required=xEcIt)
def cmd_cpvcs_commit_diff(version:xEcIa,commit:xEcIa,name:xEcIV):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(xEcIG)
  status.stop()
...
def print_table(columns:Dict[xEcIV,xEcIV],rows:List[Dict[xEcIV,Any]],formatters:Dict[xEcIV,Callable[[Any],xEcIV]]=xEcIG):
 from rich.table import Table
 if formatters is xEcIG:
  formatters=xEcIu()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  xEcIj(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->xEcIG:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->xEcIG:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->xEcIG:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->xEcIG:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
