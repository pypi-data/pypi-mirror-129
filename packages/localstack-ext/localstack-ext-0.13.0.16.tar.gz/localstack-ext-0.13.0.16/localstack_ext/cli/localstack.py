...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return rDuHP if e else rDuHL
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->rDuHX:
...
@click.option("--revert",is_flag=rDuHP,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:rDuHF):
...
def _cpvcs_initialized(pod_name:rDuHx)->rDuHF:
...
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_init(name:rDuHx):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=rDuHP,default=rDuHL)
def cmd_cpvcs_delete(name:rDuHx,remote:rDuHF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_register_remote(name:rDuHx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=rDuHP)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=rDuHP)
def cmd_cpvcs_rename(name:rDuHx,new_name:rDuHx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_commit(message:rDuHx,name:rDuHx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=rDuHP,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=rDuHP,default=rDuHL,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_push(squash:rDuHF,message:rDuHx,name:rDuHx,three_way:rDuHF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
@click.option("-v","--version",type=rDuHt)
@click.option("-m","--message",required=rDuHL)
def cmd_cpvcs_push_overwrite(version:rDuHt,message:rDuHx,name:rDuHx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=rDuHt,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=rDuHP,default=rDuHL,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_inject(version:rDuHt,reset:rDuHF,name:rDuHx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=rDuHP,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=rDuHP,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:rDuHx,inject:rDuHF,reset:rDuHF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=rDuHP,default=rDuHL)
def cmd_cpvcs_list_pods(remote:rDuHF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_versions(name:rDuHx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=rDuHP,type=rDuHt)
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_version_info(version:rDuHt,name:rDuHx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=rDuHt,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_version_metamodel(version:rDuHt,name:rDuHx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=rDuHP,type=rDuHt,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=rDuHP,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=rDuHP,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=rDuHL,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_set_version(version:rDuHt,inject:rDuHF,reset:rDuHF,commit_before:rDuHF,name:rDuHx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_commits(version:rDuHt,name:rDuHx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=rDuHP)
@click.option("--commit","-c",required=rDuHP)
@click.option("-n","--name",help="Name of the cloud pod",required=rDuHP)
def cmd_cpvcs_commit_diff(version:rDuHt,commit:rDuHt,name:rDuHx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(rDuHX)
  status.stop()
...
def print_table(columns:Dict[rDuHx,rDuHx],rows:List[Dict[rDuHx,Any]],formatters:Dict[rDuHx,Callable[[Any],rDuHx]]=rDuHX):
 from rich.table import Table
 if formatters is rDuHX:
  formatters=rDuHd()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  rDuHp(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->rDuHX:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->rDuHX:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->rDuHX:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->rDuHX:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
