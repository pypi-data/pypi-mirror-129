...
def api_key_configured()->IbaxK:
 return IbaxP if os.environ.get("LOCALSTACK_API_KEY")else Ibaxq
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if IbaxV([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
