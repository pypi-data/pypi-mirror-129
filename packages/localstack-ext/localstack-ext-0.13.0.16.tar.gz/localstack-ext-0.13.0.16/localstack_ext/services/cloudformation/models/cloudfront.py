...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or WcdOs
 @WcdOh
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return WcdOf(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @WcdOB
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if WcdOY(origins,WcdOC)else origins.get("Items")
  origins_to_create=WcdOe([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if WcdOY(aliases,WcdOC)else aliases.get("Items")
  aliases_to_create=WcdOe(aliases)
  for distr in distrs:
   origins=WcdOe([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=WcdOe(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",WcdOp)
   config["Enabled"]=WcdOX(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if WcdOY(custom_errors,WcdOw)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=WcdOX(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":WcdOp}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=WcdOX(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=WcdOs):
    parent=config if parent is WcdOs else parent
    items=parent.get(key)or[]
    if WcdOY(items,WcdOw)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":WcdOG(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if WcdOY(cache_behavs,WcdOw)else cache_behavs)
