...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=nsHfL(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not nsHfY
   return result
  except nsHfE:
   props=self.props
   ...
   if parts[0]in candidates and nsHfq(parts)>1:
