...
SENT_EMAILS=[]
EMAIL_BLACKLIST=JVDwj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",JVDwP(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=JVDwX,from_name=JVDwX,smtp_host=JVDwX,smtp_user=JVDwX,smtp_pass=JVDwX,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if JVDwH(recipients,JVDwK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=JVDwX):
 ...
 result["From"]=formataddr((JVDwP(Header(from_name,"utf-8")),from_email))
 return result
