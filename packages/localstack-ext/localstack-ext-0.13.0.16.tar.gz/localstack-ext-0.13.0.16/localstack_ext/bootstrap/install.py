...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[SWNIb],SWNIb],api_name:SWNIb):
 if not(is_debian()and in_docker()):
  ...
  return
 if SWNIK(packages,SWNIb):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=SWNIg)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=SWNIg)
def check_or_install(command:SWNIb,packages:Union[List[SWNIb],SWNIb],api:SWNIb):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=SWNIg):
  try:
