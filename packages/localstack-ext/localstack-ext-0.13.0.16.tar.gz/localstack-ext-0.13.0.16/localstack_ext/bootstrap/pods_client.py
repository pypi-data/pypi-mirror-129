...
class PodInfo:
 def __init__(self,name=IMyQU,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(IMyQk):
 BACKEND="_none_"
 def __init__(self,pod_name=IMyQU,config=IMyQU):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise IMyQu("Not implemented")
 def delete(self,remote:IMyQx)->IMyQx:
  raise IMyQu("Not implemented")
 def push(self,comment:IMyQg=IMyQU,three_way:IMyQx=IMyQT)->PodInfo:
  raise IMyQu("Not implemented")
 def push_overwrite(self,version:IMyQG,comment:IMyQg=IMyQU):
  pass
 def pull(self,inject_version_state:IMyQx=IMyQT,reset_state_before:IMyQx=IMyQT,fetch_all:IMyQx=IMyQU):
  raise IMyQu("Not implemented")
 def commit(self,message:IMyQg=IMyQU):
  raise IMyQu("Not implemented")
 def inject(self,version:IMyQG,reset_state:IMyQx):
  raise IMyQu("Not implemented")
 def list_versions(self)->List[IMyQg]:
  raise IMyQu("Not implemented")
 def version_info(self,version:IMyQG):
  raise IMyQu("Not implemented")
 def version_metamodel(self,version:IMyQG):
  raise IMyQu("Not implemented")
 def set_version(self,version:IMyQG,inject_version_state:IMyQx,reset_state:IMyQx,commit_before:IMyQx):
  raise IMyQu("Not implemented")
 def list_version_commits(self,version:IMyQG)->List[IMyQg]:
  raise IMyQu("Not implemented")
 def get_commit_diff(self,version:IMyQG,commit:IMyQG)->IMyQg:
  raise IMyQu("Not implemented")
 def register_remote(self,pod_name:IMyQg,ci_pod:IMyQx)->IMyQx:
  raise IMyQu("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->IMyQx:
  raise IMyQu("Not implemented")
 def list_pods(self,fetch_remote:IMyQx)->Set[IMyQg]:
  raise IMyQu("Not implemented")
 ...
 def get(cls,pod_name,pre_config=IMyQU):
  ...
  raise IMyQu('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=IMyQT
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=IMyQj)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=IMyQj
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise IMyQu("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @IMyQP
 def get_state_zip_from_instance(get_content=IMyQT):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise IMyQu("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:IMyQg=IMyQU):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @IMyQP
 def parse_pod_name_from_qualifying_name(qualifying_name:IMyQg)->IMyQg:
 ...
 @IMyQP
 def _get_max_version_for_pod_from_platform(pod_name:IMyQg,auth_headers):
  ...
  remote_max_ver=IMyQG(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with IMyQV(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with IMyQV(meta_archive,"rb")as meta_archive_content:
   ...
   with IMyQV(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=IMyQU)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:IMyQg):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:IMyQx)->IMyQx:
  ...
  if remote:
   pass
  return IMyQT
 def _push_to_remote(self,url:IMyQg):
 ...
 def push_overwrite(self,version:IMyQG,comment:IMyQg=IMyQU):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return IMyQj
 def pull(self,inject_version_state:IMyQx=IMyQT,reset_state_before:IMyQx=IMyQT,fetch_all:IMyQx=IMyQj):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=IMyQT):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(IMyQA(lambda ver:IMyQg(ver),IMyQX(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:IMyQG,reset_state:IMyQx):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=IMyQj,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def get_commit_diff(self,version:IMyQG,commit:IMyQG)->IMyQg:
  ...
  return commit_diff
 def register_remote(self,pod_name:IMyQg,ci_pod:IMyQx=IMyQT)->IMyQx:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:IMyQx)->Set[IMyQg]:
  ...
  return result
 @IMyQH
 def upload_content(cls,presigned_url:IMyQg,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise IMyQu("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @IMyQP
 def create_platform_url(request:IMyQg)->IMyQg:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:IMyQg=IMyQU,three_way:IMyQx=IMyQT)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=IMyQj)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(IMyQO(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=IMyQO(zip_data_content)
  return result
 def pull(self,inject_version_state:IMyQx=IMyQT,reset_state_before:IMyQx=IMyQT,fetch_all:IMyQx=IMyQU):
  ...
  rm_rf(zip_path)
 @IMyQP
 def presigned_url(pod_name:IMyQg,mode:IMyQg)->IMyQg:
  ...
  if response.status_code>=400:
   raise IMyQu("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise IMyQu("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:IMyQg=IMyQU,three_way:IMyQx=IMyQT):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=IMyQj)
 ...
 def pull(self,inject_version_state:IMyQx=IMyQT,reset_state_before:IMyQx=IMyQT,fetch_all:IMyQx=IMyQU):
  ...
  if remote_ref not in remote_refs:
   raise IMyQu('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=IMyQt(self,"clone_dir",IMyQU)
...
class PodConfigManagerMeta(IMyQS):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=IMyQU
   for manager in cls.CHAIN:
    try:
     tmp=IMyQt(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif IMyQe(tmp,IMyQB)and IMyQe(result,IMyQB):
       result.extend(tmp)
    except IMyQu:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not IMyQU:
    return result
   raise IMyQu('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(IMyQk,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @IMyQH
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise IMyQu('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(IMyQk):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=IMyQj).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=IMyQc([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=IMyQT):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except IMyQu:
   if safe:
    return{}
   raise IMyQu('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(IMyQk):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise IMyQu("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise IMyQu("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:IMyQg,remote:IMyQx,pre_config:Dict[IMyQg,IMyQg])->IMyQx:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],**kwargs)->IMyQx:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",IMyQT))
 return result
def rename_pod(current_pod_name:IMyQg,new_pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:IMyQx,pre_config:Dict[IMyQg,IMyQg],**kwargs)->List[IMyQg]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],message:IMyQg=IMyQU,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:IMyQg,version:IMyQG,reset_state:IMyQx,pre_config:Dict[IMyQg,IMyQg],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],**kwargs)->List[IMyQg]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:IMyQG,pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:IMyQG,pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:IMyQG,inject_version_state:IMyQx,reset_state:IMyQx,commit_before:IMyQx,pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg],**kwargs)->IMyQx:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:IMyQG,pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg])->List[IMyQg]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:IMyQG,commit:IMyQG,pod_name:IMyQg,pre_config:Dict[IMyQg,IMyQg])->IMyQg:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:IMyQG,pod_name:IMyQg,comment:IMyQg,pre_config:Dict[IMyQg,IMyQg]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=IMyQU,squash_commits=IMyQT,comment=IMyQU,three_way=IMyQT,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=IMyQT,reset_state_before=IMyQT,**kwargs):
 pre_config=kwargs.get("pre_config",IMyQU)
 if not pod_name:
  raise IMyQu("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=IMyQT,exclude_from_reset:List[IMyQg]=IMyQU):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise IMyQu("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->IMyQg:
 try:
...
def get_persisted_resource_names(data_dir)->List[IMyQg]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and IMyQO(os.listdir(entry.path))>0:
    names.append(entry.name)
