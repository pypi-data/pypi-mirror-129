...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return ksXjc if e else ksXjl
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->ksXjB:
...
@click.option("--revert",is_flag=ksXjc,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:ksXji):
...
def _cpvcs_initialized(pod_name:ksXjK)->ksXji:
...
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_init(name:ksXjK):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=ksXjc,default=ksXjl)
def cmd_cpvcs_delete(name:ksXjK,remote:ksXji):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_register_remote(name:ksXjK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=ksXjc)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=ksXjc)
def cmd_cpvcs_rename(name:ksXjK,new_name:ksXjK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_commit(message:ksXjK,name:ksXjK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=ksXjc,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=ksXjc,default=ksXjl,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_push(squash:ksXji,message:ksXjK,name:ksXjK,three_way:ksXji):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
@click.option("-v","--version",type=ksXjh)
@click.option("-m","--message",required=ksXjl)
def cmd_cpvcs_push_overwrite(version:ksXjh,message:ksXjK,name:ksXjK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=ksXjh,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=ksXjc,default=ksXjl,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_inject(version:ksXjh,reset:ksXji,name:ksXjK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=ksXjc,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=ksXjc,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:ksXjK,inject:ksXji,reset:ksXji):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=ksXjc,default=ksXjl)
def cmd_cpvcs_list_pods(remote:ksXji):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_versions(name:ksXjK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ksXjc,type=ksXjh)
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_version_info(version:ksXjh,name:ksXjK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=ksXjh,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_version_metamodel(version:ksXjh,name:ksXjK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ksXjc,type=ksXjh,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=ksXjc,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=ksXjc,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=ksXjl,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_set_version(version:ksXjh,inject:ksXji,reset:ksXji,commit_before:ksXji,name:ksXjK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_commits(version:ksXjh,name:ksXjK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=ksXjc)
@click.option("--commit","-c",required=ksXjc)
@click.option("-n","--name",help="Name of the cloud pod",required=ksXjc)
def cmd_cpvcs_commit_diff(version:ksXjh,commit:ksXjh,name:ksXjK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(ksXjB)
  status.stop()
...
def print_table(columns:Dict[ksXjK,ksXjK],rows:List[Dict[ksXjK,Any]],formatters:Dict[ksXjK,Callable[[Any],ksXjK]]=ksXjB):
 from rich.table import Table
 if formatters is ksXjB:
  formatters=ksXjx()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  ksXja(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->ksXjB:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->ksXjB:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->ksXjB:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->ksXjB:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
