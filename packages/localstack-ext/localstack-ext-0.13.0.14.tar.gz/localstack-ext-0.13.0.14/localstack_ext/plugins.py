...
def api_key_configured()->foJVO:
 return foJVX if os.environ.get("LOCALSTACK_API_KEY")else foJVy
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if foJVm([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
