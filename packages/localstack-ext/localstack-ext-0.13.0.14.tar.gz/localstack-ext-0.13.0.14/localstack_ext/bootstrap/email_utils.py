...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DcTzL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DcTzb(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DcTzX,from_name=DcTzX,smtp_host=DcTzX,smtp_user=DcTzX,smtp_pass=DcTzX,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DcTzA(recipients,DcTzB)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DcTzX):
 ...
 result["From"]=formataddr((DcTzb(Header(from_name,"utf-8")),from_email))
 return result
