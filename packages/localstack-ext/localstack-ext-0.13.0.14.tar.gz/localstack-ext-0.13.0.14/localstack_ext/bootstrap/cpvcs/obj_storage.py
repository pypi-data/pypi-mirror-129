...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=bAovJ
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:bAovc)->bAovc:
  pass
 @bAovF
 def get(cls,requested_file_locator:bAovc):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=bAovJ
 @bAovF
 def get(cls,state_file_locator:bAovc,requested_storage:bAovc,serializers:Dict[bAovc,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[bAovc,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:bAovc)->bAovc:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:bAovc)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:bAovc)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:bAovc)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:bAovc)->bAovc:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[bAovc]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:bAovc,new_key:bAovc,referenced_by_version:bAovc=bAovJ):
  pass
 @abc.abstractmethod
 def version_exists(self,key:bAovc)->bAovN:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:bAovc,key:bAovc):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:bAovc,new_key:bAovc,base_path:bAovc)->bAovN:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[bAovc]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[bAovc,bAovc],remote_location:bAovc=bAovJ)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[bAovc]:
  ...
  for cpvcs_obj in args:
   if bAovm(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif bAovm(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[bAovc,bAovc],remote_location:bAovc=bAovJ)->List[Union[Revision,Version]]:
