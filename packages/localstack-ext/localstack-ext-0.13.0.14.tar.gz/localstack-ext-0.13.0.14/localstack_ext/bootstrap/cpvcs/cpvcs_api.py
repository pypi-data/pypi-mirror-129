...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=tRmHf,state_files=tRmHU(),version_number=0)
 ...
 with tRmHS(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with tRmHS(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with tRmHS(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:tRmHL,meta_archives:Dict[tRmHL,tRmHL],state_archives:Dict[tRmHL,tRmHL],remote_info:Dict[tRmHL,tRmHL],pod_name:tRmHL):
...
def merge_from_remote(version_space_archive:tRmHL,meta_archives:Dict[tRmHL,tRmHL],state_archives:Dict[tRmHL,tRmHL]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->tRmHA:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:tRmHL):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:tRmHL)->tRmHA:
 ...
 return tRmHx
def list_locally_available_pods(show_remote_or_local:tRmHA=tRmHx)->Set[tRmHL]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:tRmHL,file_name:tRmHL,service:tRmHL,region:tRmHL)->tRmHL:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->tRmHL:
 ...
 return key
def _get_state_file_path(key:tRmHL)->tRmHL:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=tRmHU(tRmHQ(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:tRmHL)->Optional[tRmHL]:
...
def get_version_info(version_no:tRmHp)->Dict[tRmHL,tRmHL]:
...
def read_file_from_archive(archive_path:tRmHL,file_name:tRmHL):
 try:
...
def commit(message:tRmHL=tRmHf)->Revision:
 ...
 curr_expansion_point_parent_state_files=tRmHU()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=tRmHf
...
def zip_directories(zip_dest:tRmHL,directories:List[tRmHL])->tRmHL:
 ...
 return zip_dest
def create_version_space_archive()->tRmHL:
...
def get_max_version_no()->tRmHp:
 with tRmHS(config_context.get_max_ver_path())as fp:
  return tRmHp(os.path.basename(fp.readline()))
...
def push_overwrite(version:tRmHp,comment:tRmHL)->tRmHA:
 ...
 _create_metadata_archive(version_node=version_node,overwrite=tRmHx,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return tRmHx
def push(comment:tRmHL=tRmHf,three_way:tRmHA=tRmHY)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=tRmHU(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,get_version_state_archive(new_max_version_no)),state_files=tRmHU(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 ...
 return new_version
def get_version_meta_archive(version_no:tRmHp)->tRmHL:
...
def get_version_state_archive(version_no:tRmHp)->tRmHL:
...
def get_version_metamodel(version_no:tRmHp)->tRmHL:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:tRmHA=tRmHY,overwrite:tRmHA=tRmHY,metamodels_file:tRmHL=tRmHf):
 ...
 with tRmHS(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with tRmHS(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:tRmHp,state_file_refs:Set[StateFileRef],delete_files=tRmHY,archive=tRmHx):
...
def set_active_version(version_no:tRmHp,commit_before=tRmHY)->tRmHA:
 ...
 return tRmHY
def _set_active_version(key:tRmHL):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=tRmHU(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:tRmHp)->Version:
 versions=load_version_references()
 version_ref=tRmHT((version[1]for version in versions if version[0]==version_no),tRmHf)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[tRmHp,tRmHL]]:
 result={}
 with tRmHS(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with tRmHS(symlink,"r")as sp:
    result[tRmHp(os.path.basename(symlink))]=sp.readline()
 return tRmHN(result.items(),key=lambda x:x[0],reverse=tRmHx)
def list_versions()->List[tRmHL]:
 ...
 return result
def list_version_commits(version_no:tRmHp)->List[tRmHL]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:tRmHp,commit_no:tRmHp)->tRmHL:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->tRmHL:
 with tRmHS(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->tRmHL:
 with tRmHS(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->tRmHL:
 with tRmHS(config_context.get_known_ver_path(),"a")as fp:
...
def _is_special_case(obj)->tRmHA:
 return tRmHF(obj,(Queue,FakeBucket))
...
def _merge_two_state_dir_func(**kwargs):
 ...
 if os.path.isfile(dst_state_file_path):
  ...
  mkdir(dst_state_dir)
  persist_object(src_state,dst_state_file_path)
def _merge_expansion_point_with_max(three_way=tRmHY):
 ...
 common_ancestor_state_archive=tRmHf
 tmp_common_ancestor_state_dir=tRmHf
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=tRmHY)
...
def _infer_backend_init(clazz,sf):
 if tRmHF(clazz,tRmHq):
  backend={}
 else:
  constructor=tRmHk(clazz,"__init__",tRmHf)
  ...
  if "region" in sig_args.args:
   ...
   backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->tRmHL:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[tRmHL,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,tRmHf)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(tRmHf,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=tRmHX(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=tRmHX(backend1)
