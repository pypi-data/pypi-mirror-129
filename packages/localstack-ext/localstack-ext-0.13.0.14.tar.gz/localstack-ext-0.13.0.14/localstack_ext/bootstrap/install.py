...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[IUxLE],IUxLE],api_name:IUxLE):
 if not(is_debian()and in_docker()):
  ...
  return
 if IUxLt(packages,IUxLE):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=IUxLP)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=IUxLP)
def check_or_install(command:IUxLE,packages:Union[List[IUxLE],IUxLE],api:IUxLE):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=IUxLP):
  try:
