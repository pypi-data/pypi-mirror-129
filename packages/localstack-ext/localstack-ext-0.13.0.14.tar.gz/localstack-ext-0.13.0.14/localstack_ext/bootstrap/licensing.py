...
class KeyActivationError(Icgmf):
 pass
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def read_api_key(raise_if_missing=Icgmw):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise Icgmf("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:IcgmJ):
 return '"%s..."(%s)'%(api_key[:3],IcgmH(api_key))
def fetch_key()->Icgmn:
 ...
 try:
  ...
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=Icgmd)
  ...
  if log_license_issues():
   api_key=IcgmJ(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 timestamp=IcgmJ(IcgmF(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in Icgmu(IcgmH(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 cache=get_key_cache()
 cache.update({"timestamp":IcgmF(timestamp),"key_hash":md5(api_key),"key":key_b64})
 cache.save()
def load_cached_key(api_key)->IcgmJ:
 cache=get_key_cache()
 ...
 now=now_utc()
 if(now-cache["timestamp"])>MAX_KEY_CACHE_DURATION_SECS:
  raise CachedKeyError("Cached key expired")
 timestamp=IcgmJ(cache["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:Icgmn):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise IcgmN("Decryption resulted in invalid python file!")
 except Icgmf:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise Icgmf("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=Icgmd)
def is_logged_in():
 return Icgmw if get_auth_cache().get("token")else Icgmd
def get_auth_headers()->Dict:
 ...
 api_key=read_api_key(raise_if_missing=Icgmd)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise Icgmf("Please log in first")
