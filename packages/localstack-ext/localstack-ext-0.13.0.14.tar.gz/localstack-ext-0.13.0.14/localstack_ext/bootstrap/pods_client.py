...
class PodInfo:
 def __init__(self,name=WoEwF,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(WoEwT):
 BACKEND="_none_"
 def __init__(self,pod_name=WoEwF,config=WoEwF):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise WoEwP("Not implemented")
 def delete(self,remote:WoEwd)->WoEwd:
  raise WoEwP("Not implemented")
 def push(self,comment:WoEwB=WoEwF,three_way:WoEwd=WoEwj)->PodInfo:
  raise WoEwP("Not implemented")
 def push_overwrite(self,version:WoEwQ,comment:WoEwB=WoEwF):
  pass
 def pull(self,inject_version_state:WoEwd=WoEwj,reset_state_before:WoEwd=WoEwj,fetch_all:WoEwd=WoEwF):
  raise WoEwP("Not implemented")
 def commit(self,message:WoEwB=WoEwF):
  raise WoEwP("Not implemented")
 def inject(self,version:WoEwQ,reset_state:WoEwd):
  raise WoEwP("Not implemented")
 def list_versions(self)->List[WoEwB]:
  raise WoEwP("Not implemented")
 def version_info(self,version:WoEwQ):
  raise WoEwP("Not implemented")
 def version_metamodel(self,version:WoEwQ):
  raise WoEwP("Not implemented")
 def set_version(self,version:WoEwQ,inject_version_state:WoEwd,reset_state:WoEwd,commit_before:WoEwd):
  raise WoEwP("Not implemented")
 def list_version_commits(self,version:WoEwQ)->List[WoEwB]:
  raise WoEwP("Not implemented")
 def get_commit_diff(self,version:WoEwQ,commit:WoEwQ)->WoEwB:
  raise WoEwP("Not implemented")
 def register_remote(self,pod_name:WoEwB,ci_pod:WoEwd)->WoEwd:
  raise WoEwP("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->WoEwd:
  raise WoEwP("Not implemented")
 def list_pods(self,fetch_remote:WoEwd)->Set[WoEwB]:
  raise WoEwP("Not implemented")
 ...
 def get(cls,pod_name,pre_config=WoEwF):
  ...
  raise WoEwP('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=WoEwj
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=WoEwV)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=WoEwV
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise WoEwP("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @WoEwI
 def get_state_zip_from_instance(get_content=WoEwj):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise WoEwP("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:WoEwB=WoEwF):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @WoEwI
 def parse_pod_name_from_qualifying_name(qualifying_name:WoEwB)->WoEwB:
 ...
 @WoEwI
 def _get_max_version_for_pod_from_platform(pod_name:WoEwB,auth_headers):
  ...
  remote_max_ver=WoEwQ(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with WoEwu(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with WoEwu(meta_archive,"rb")as meta_archive_content:
   ...
   with WoEwu(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=WoEwF)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:WoEwB):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:WoEwd)->WoEwd:
  ...
  if remote:
   pass
  return WoEwj
 def _push_to_remote(self,url:WoEwB):
 ...
 def push_overwrite(self,version:WoEwQ,comment:WoEwB=WoEwF):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return WoEwV
 def pull(self,inject_version_state:WoEwd=WoEwj,reset_state_before:WoEwd=WoEwj,fetch_all:WoEwd=WoEwV):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=WoEwj):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(WoEwt(lambda ver:WoEwB(ver),WoEwb(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:WoEwQ,reset_state:WoEwd):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=WoEwV,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def get_commit_diff(self,version:WoEwQ,commit:WoEwQ)->WoEwB:
  ...
  return commit_diff
 def register_remote(self,pod_name:WoEwB,ci_pod:WoEwd=WoEwj)->WoEwd:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:WoEwd)->Set[WoEwB]:
  ...
  return result
 @WoEwk
 def upload_content(cls,presigned_url:WoEwB,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise WoEwP("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @WoEwI
 def create_platform_url(request:WoEwB)->WoEwB:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:WoEwB=WoEwF,three_way:WoEwd=WoEwj)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=WoEwV)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(WoEwq(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=WoEwq(zip_data_content)
  return result
 def pull(self,inject_version_state:WoEwd=WoEwj,reset_state_before:WoEwd=WoEwj,fetch_all:WoEwd=WoEwF):
  ...
  rm_rf(zip_path)
 @WoEwI
 def presigned_url(pod_name:WoEwB,mode:WoEwB)->WoEwB:
  ...
  if response.status_code>=400:
   raise WoEwP("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise WoEwP("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:WoEwB=WoEwF,three_way:WoEwd=WoEwj):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=WoEwV)
 ...
 def pull(self,inject_version_state:WoEwd=WoEwj,reset_state_before:WoEwd=WoEwj,fetch_all:WoEwd=WoEwF):
  ...
  if remote_ref not in remote_refs:
   raise WoEwP('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=WoEwi(self,"clone_dir",WoEwF)
...
class PodConfigManagerMeta(WoEwN):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=WoEwF
   for manager in cls.CHAIN:
    try:
     tmp=WoEwi(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif WoEwD(tmp,WoEwp)and WoEwD(result,WoEwp):
       result.extend(tmp)
    except WoEwP:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not WoEwF:
    return result
   raise WoEwP('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(WoEwT,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @WoEwk
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise WoEwP('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(WoEwT):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=WoEwV).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=WoEwK([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=WoEwj):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except WoEwP:
   if safe:
    return{}
   raise WoEwP('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(WoEwT):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise WoEwP("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise WoEwP("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:WoEwB,remote:WoEwd,pre_config:Dict[WoEwB,WoEwB])->WoEwd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],**kwargs)->WoEwd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",WoEwj))
 return result
def rename_pod(current_pod_name:WoEwB,new_pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:WoEwd,pre_config:Dict[WoEwB,WoEwB],**kwargs)->List[WoEwB]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],message:WoEwB=WoEwF,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:WoEwB,version:WoEwQ,reset_state:WoEwd,pre_config:Dict[WoEwB,WoEwB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],**kwargs)->List[WoEwB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:WoEwQ,pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:WoEwQ,pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:WoEwQ,inject_version_state:WoEwd,reset_state:WoEwd,commit_before:WoEwd,pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB],**kwargs)->WoEwd:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:WoEwQ,pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB])->List[WoEwB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:WoEwQ,commit:WoEwQ,pod_name:WoEwB,pre_config:Dict[WoEwB,WoEwB])->WoEwB:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:WoEwQ,pod_name:WoEwB,comment:WoEwB,pre_config:Dict[WoEwB,WoEwB]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=WoEwF,squash_commits=WoEwj,comment=WoEwF,three_way=WoEwj,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=WoEwj,reset_state_before=WoEwj,**kwargs):
 pre_config=kwargs.get("pre_config",WoEwF)
 if not pod_name:
  raise WoEwP("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=WoEwj,exclude_from_reset:List[WoEwB]=WoEwF):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise WoEwP("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->WoEwB:
 try:
...
def get_persisted_resource_names(data_dir)->List[WoEwB]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and WoEwq(os.listdir(entry.path))>0:
    names.append(entry.name)
