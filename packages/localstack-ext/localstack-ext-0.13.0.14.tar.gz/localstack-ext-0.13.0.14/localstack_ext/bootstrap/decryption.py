...
class DecryptionHandler:
 decryption_key:Eqyxp
 def __init__(self,decryption_key:Eqyxp):
  self.decryption_key=decryption_key
 def decrypt(self,content)->Eqyxp:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=EqyxW):
  if path and not Eqyxl(path,Eqyxs):
   path=Eqyxs(EqyxS(path,"_path",[]))
  if not path:
   return EqyxW
...
def patch_traceback_lines():
 if EqyxS(traceback.FrameSummary,"_ls_patch_applied",EqyxW):
  return
 @EqyxT
 def line(self):
  try:
 ...
 Eqyxe(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=EqyxV
def patch_inspect_findsource():
 if EqyxS(inspect,"_ls_patch_applied",EqyxW):
  return
 ...
 inspect._ls_patch_applied=EqyxV
