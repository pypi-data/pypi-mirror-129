...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or qjAPQ
 @qjAPv
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return qjAPu(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @qjAPd
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if qjAPz(origins,qjAPC)else origins.get("Items")
  origins_to_create=qjAPp([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if qjAPz(aliases,qjAPC)else aliases.get("Items")
  aliases_to_create=qjAPp(aliases)
  for distr in distrs:
   origins=qjAPp([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=qjAPp(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",qjAPl)
   config["Enabled"]=qjAPk(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if qjAPz(custom_errors,qjAPN)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=qjAPk(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":qjAPl}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=qjAPk(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=qjAPQ):
    parent=config if parent is qjAPQ else parent
    items=parent.get(key)or[]
    if qjAPz(items,qjAPN)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":qjAPn(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if qjAPz(cache_behavs,qjAPN)else cache_behavs)
