...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[FIbjX])[0]
 def get_physical_resource_id(self,attribute=FIbjX,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if FIbju(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return FIbjg(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @FIbjy
 def add_defaults(resource,stack_name:FIbjB):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return FIbjg(SESReceiptRule,self).get_cfn_attribute(attribute)
 @FIbjh
