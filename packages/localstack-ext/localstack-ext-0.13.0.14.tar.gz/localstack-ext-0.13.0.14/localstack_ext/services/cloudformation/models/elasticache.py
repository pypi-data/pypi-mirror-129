...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=VWxvt(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not VWxvB
   return result
  except VWxvL:
   props=self.props
   ...
   if parts[0]in candidates and VWxvc(parts)>1:
