...
SENT_EMAILS=[]
EMAIL_BLACKLIST=LUmSR()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",LUmSz(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=LUmSr,from_name=LUmSr,smtp_host=LUmSr,smtp_user=LUmSr,smtp_pass=LUmSr,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if LUmSu(recipients,LUmSp)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=LUmSr):
 ...
 result["From"]=formataddr((LUmSz(Header(from_name,"utf-8")),from_email))
 return result
