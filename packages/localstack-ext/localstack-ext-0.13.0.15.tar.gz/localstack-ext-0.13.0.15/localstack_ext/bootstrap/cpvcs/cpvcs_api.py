...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=GxaWN,state_files=GxaWl(),version_number=0)
 ...
 with GxaWw(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with GxaWw(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with GxaWw(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:GxaWO,meta_archives:Dict[GxaWO,GxaWO],state_archives:Dict[GxaWO,GxaWO],remote_info:Dict[GxaWO,GxaWO],pod_name:GxaWO):
...
def merge_from_remote(version_space_archive:GxaWO,meta_archives:Dict[GxaWO,GxaWO],state_archives:Dict[GxaWO,GxaWO]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->GxaWM:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:GxaWO):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:GxaWO)->GxaWM:
 ...
 return GxaWi
def list_locally_available_pods(show_remote_or_local:GxaWM=GxaWi)->Set[GxaWO]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:GxaWO,file_name:GxaWO,service:GxaWO,region:GxaWO)->GxaWO:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->GxaWO:
 ...
 return key
def _get_state_file_path(key:GxaWO)->GxaWO:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=GxaWl(GxaWY(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:GxaWO)->Optional[GxaWO]:
...
def get_version_info(version_no:GxaWF)->Dict[GxaWO,GxaWO]:
...
def read_file_from_archive(archive_path:GxaWO,file_name:GxaWO):
 try:
...
def commit(message:GxaWO=GxaWN)->Revision:
 ...
 curr_expansion_point_parent_state_files=GxaWl()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=GxaWN
...
def zip_directories(zip_dest:GxaWO,directories:List[GxaWO])->GxaWO:
 ...
 return zip_dest
def create_version_space_archive()->GxaWO:
...
def get_max_version_no()->GxaWF:
 with GxaWw(config_context.get_max_ver_path())as fp:
  return GxaWF(os.path.basename(fp.readline()))
...
def push_overwrite(version:GxaWF,comment:GxaWO)->GxaWM:
 ...
 _create_metadata_archive(version_node=version_node,overwrite=GxaWi,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return GxaWi
def push(comment:GxaWO=GxaWN,three_way:GxaWM=GxaWd)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=GxaWl(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,get_version_state_archive(new_max_version_no)),state_files=GxaWl(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 ...
 return new_version
def get_version_meta_archive(version_no:GxaWF)->GxaWO:
...
def get_version_state_archive(version_no:GxaWF)->GxaWO:
...
def get_version_metamodel(version_no:GxaWF)->GxaWO:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:GxaWM=GxaWd,overwrite:GxaWM=GxaWd,metamodels_file:GxaWO=GxaWN):
 ...
 with GxaWw(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with GxaWw(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:GxaWF,state_file_refs:Set[StateFileRef],delete_files=GxaWd,archive=GxaWi):
...
def set_active_version(version_no:GxaWF,commit_before=GxaWd)->GxaWM:
 ...
 return GxaWd
def _set_active_version(key:GxaWO):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=GxaWl(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:GxaWF)->Version:
 versions=load_version_references()
 version_ref=GxaWk((version[1]for version in versions if version[0]==version_no),GxaWN)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[GxaWF,GxaWO]]:
 result={}
 with GxaWw(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with GxaWw(symlink,"r")as sp:
    result[GxaWF(os.path.basename(symlink))]=sp.readline()
 return GxaWK(result.items(),key=lambda x:x[0],reverse=GxaWi)
def list_versions()->List[GxaWO]:
 ...
 return result
def list_version_commits(version_no:GxaWF)->List[GxaWO]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:GxaWF,commit_no:GxaWF)->GxaWO:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->GxaWO:
 with GxaWw(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->GxaWO:
 with GxaWw(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->GxaWO:
 with GxaWw(config_context.get_known_ver_path(),"a")as fp:
...
def _is_special_case(obj)->GxaWM:
 return GxaWq(obj,(Queue,FakeBucket))
...
def _merge_two_state_dir_func(**kwargs):
 ...
 if os.path.isfile(dst_state_file_path):
  ...
  mkdir(dst_state_dir)
  persist_object(src_state,dst_state_file_path)
def _merge_expansion_point_with_max(three_way=GxaWd):
 ...
 common_ancestor_state_archive=GxaWN
 tmp_common_ancestor_state_dir=GxaWN
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=GxaWd)
...
def _infer_backend_init(clazz,sf):
 if GxaWq(clazz,GxaWf):
  backend={}
 else:
  constructor=GxaWX(clazz,"__init__",GxaWN)
  ...
  if "region" in sig_args.args:
   ...
   backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->GxaWO:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[GxaWO,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,GxaWN)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(GxaWN,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=GxaWr(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=GxaWr(backend1)
