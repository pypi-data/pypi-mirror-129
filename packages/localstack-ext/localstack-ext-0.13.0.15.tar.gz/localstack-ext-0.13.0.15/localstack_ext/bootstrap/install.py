...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[BvsTM],BvsTM],api_name:BvsTM):
 if not(is_debian()and in_docker()):
  ...
  return
 if BvsTF(packages,BvsTM):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=BvsTz)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=BvsTz)
def check_or_install(command:BvsTM,packages:Union[List[BvsTM],BvsTM],api:BvsTM):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=BvsTz):
  try:
