...
class PodInfo:
 def __init__(self,name=lfTvy,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(lfTvr):
 BACKEND="_none_"
 def __init__(self,pod_name=lfTvy,config=lfTvy):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise lfTvP("Not implemented")
 def delete(self,remote:lfTvi)->lfTvi:
  raise lfTvP("Not implemented")
 def push(self,comment:lfTvW=lfTvy,three_way:lfTvi=lfTvM)->PodInfo:
  raise lfTvP("Not implemented")
 def push_overwrite(self,version:lfTvK,comment:lfTvW=lfTvy):
  pass
 def pull(self,inject_version_state:lfTvi=lfTvM,reset_state_before:lfTvi=lfTvM,fetch_all:lfTvi=lfTvy):
  raise lfTvP("Not implemented")
 def commit(self,message:lfTvW=lfTvy):
  raise lfTvP("Not implemented")
 def inject(self,version:lfTvK,reset_state:lfTvi):
  raise lfTvP("Not implemented")
 def list_versions(self)->List[lfTvW]:
  raise lfTvP("Not implemented")
 def version_info(self,version:lfTvK):
  raise lfTvP("Not implemented")
 def version_metamodel(self,version:lfTvK):
  raise lfTvP("Not implemented")
 def set_version(self,version:lfTvK,inject_version_state:lfTvi,reset_state:lfTvi,commit_before:lfTvi):
  raise lfTvP("Not implemented")
 def list_version_commits(self,version:lfTvK)->List[lfTvW]:
  raise lfTvP("Not implemented")
 def get_commit_diff(self,version:lfTvK,commit:lfTvK)->lfTvW:
  raise lfTvP("Not implemented")
 def register_remote(self,pod_name:lfTvW,ci_pod:lfTvi)->lfTvi:
  raise lfTvP("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->lfTvi:
  raise lfTvP("Not implemented")
 def list_pods(self,fetch_remote:lfTvi)->Set[lfTvW]:
  raise lfTvP("Not implemented")
 ...
 def get(cls,pod_name,pre_config=lfTvy):
  ...
  raise lfTvP('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=lfTvM
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=lfTvp)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=lfTvp
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise lfTvP("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @lfTvV
 def get_state_zip_from_instance(get_content=lfTvM):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise lfTvP("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:lfTvW=lfTvy):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @lfTvV
 def parse_pod_name_from_qualifying_name(qualifying_name:lfTvW)->lfTvW:
 ...
 @lfTvV
 def _get_max_version_for_pod_from_platform(pod_name:lfTvW,auth_headers):
  ...
  remote_max_ver=lfTvK(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with lfTvG(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with lfTvG(meta_archive,"rb")as meta_archive_content:
   ...
   with lfTvG(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=lfTvy)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:lfTvW):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:lfTvi)->lfTvi:
  ...
  if remote:
   pass
  return lfTvM
 def _push_to_remote(self,url:lfTvW):
 ...
 def push_overwrite(self,version:lfTvK,comment:lfTvW=lfTvy):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return lfTvp
 def pull(self,inject_version_state:lfTvi=lfTvM,reset_state_before:lfTvi=lfTvM,fetch_all:lfTvi=lfTvp):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=lfTvM):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(lfTvA(lambda ver:lfTvW(ver),lfTve(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:lfTvK,reset_state:lfTvi):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=lfTvp,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def get_commit_diff(self,version:lfTvK,commit:lfTvK)->lfTvW:
  ...
  return commit_diff
 def register_remote(self,pod_name:lfTvW,ci_pod:lfTvi=lfTvM)->lfTvi:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:lfTvi)->Set[lfTvW]:
  ...
  return result
 @lfTvC
 def upload_content(cls,presigned_url:lfTvW,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise lfTvP("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @lfTvV
 def create_platform_url(request:lfTvW)->lfTvW:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:lfTvW=lfTvy,three_way:lfTvi=lfTvM)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=lfTvp)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(lfTvg(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=lfTvg(zip_data_content)
  return result
 def pull(self,inject_version_state:lfTvi=lfTvM,reset_state_before:lfTvi=lfTvM,fetch_all:lfTvi=lfTvy):
  ...
  rm_rf(zip_path)
 @lfTvV
 def presigned_url(pod_name:lfTvW,mode:lfTvW)->lfTvW:
  ...
  if response.status_code>=400:
   raise lfTvP("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise lfTvP("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:lfTvW=lfTvy,three_way:lfTvi=lfTvM):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=lfTvp)
 ...
 def pull(self,inject_version_state:lfTvi=lfTvM,reset_state_before:lfTvi=lfTvM,fetch_all:lfTvi=lfTvy):
  ...
  if remote_ref not in remote_refs:
   raise lfTvP('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=lfTvq(self,"clone_dir",lfTvy)
...
class PodConfigManagerMeta(lfTvj):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=lfTvy
   for manager in cls.CHAIN:
    try:
     tmp=lfTvq(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif lfTva(tmp,lfTvX)and lfTva(result,lfTvX):
       result.extend(tmp)
    except lfTvP:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not lfTvy:
    return result
   raise lfTvP('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(lfTvr,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @lfTvC
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise lfTvP('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(lfTvr):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=lfTvp).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=lfTvw([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=lfTvM):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except lfTvP:
   if safe:
    return{}
   raise lfTvP('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(lfTvr):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise lfTvP("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise lfTvP("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:lfTvW,remote:lfTvi,pre_config:Dict[lfTvW,lfTvW])->lfTvi:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],**kwargs)->lfTvi:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",lfTvM))
 return result
def rename_pod(current_pod_name:lfTvW,new_pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:lfTvi,pre_config:Dict[lfTvW,lfTvW],**kwargs)->List[lfTvW]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],message:lfTvW=lfTvy,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:lfTvW,version:lfTvK,reset_state:lfTvi,pre_config:Dict[lfTvW,lfTvW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],**kwargs)->List[lfTvW]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:lfTvK,pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:lfTvK,pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:lfTvK,inject_version_state:lfTvi,reset_state:lfTvi,commit_before:lfTvi,pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW],**kwargs)->lfTvi:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:lfTvK,pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW])->List[lfTvW]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:lfTvK,commit:lfTvK,pod_name:lfTvW,pre_config:Dict[lfTvW,lfTvW])->lfTvW:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:lfTvK,pod_name:lfTvW,comment:lfTvW,pre_config:Dict[lfTvW,lfTvW]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=lfTvy,squash_commits=lfTvM,comment=lfTvy,three_way=lfTvM,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=lfTvM,reset_state_before=lfTvM,**kwargs):
 pre_config=kwargs.get("pre_config",lfTvy)
 if not pod_name:
  raise lfTvP("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=lfTvM,exclude_from_reset:List[lfTvW]=lfTvy):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise lfTvP("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->lfTvW:
 try:
...
def get_persisted_resource_names(data_dir)->List[lfTvW]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and lfTvg(os.listdir(entry.path))>0:
    names.append(entry.name)
