...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return WkpSA if e else WkpSE
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->WkpSr:
...
@click.option("--revert",is_flag=WkpSA,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:WkpSi):
...
def _cpvcs_initialized(pod_name:WkpSK)->WkpSi:
...
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_init(name:WkpSK):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=WkpSA,default=WkpSE)
def cmd_cpvcs_delete(name:WkpSK,remote:WkpSi):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_register_remote(name:WkpSK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=WkpSA)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=WkpSA)
def cmd_cpvcs_rename(name:WkpSK,new_name:WkpSK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_commit(message:WkpSK,name:WkpSK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=WkpSA,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=WkpSA,default=WkpSE,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_push(squash:WkpSi,message:WkpSK,name:WkpSK,three_way:WkpSi):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
@click.option("-v","--version",type=WkpSj)
@click.option("-m","--message",required=WkpSE)
def cmd_cpvcs_push_overwrite(version:WkpSj,message:WkpSK,name:WkpSK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=WkpSj,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=WkpSA,default=WkpSE,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_inject(version:WkpSj,reset:WkpSi,name:WkpSK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=WkpSA,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=WkpSA,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:WkpSK,inject:WkpSi,reset:WkpSi):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=WkpSA,default=WkpSE)
def cmd_cpvcs_list_pods(remote:WkpSi):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_versions(name:WkpSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WkpSA,type=WkpSj)
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_version_info(version:WkpSj,name:WkpSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=WkpSj,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_version_metamodel(version:WkpSj,name:WkpSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WkpSA,type=WkpSj,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=WkpSA,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=WkpSA,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=WkpSE,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_set_version(version:WkpSj,inject:WkpSi,reset:WkpSi,commit_before:WkpSi,name:WkpSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_commits(version:WkpSj,name:WkpSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=WkpSA)
@click.option("--commit","-c",required=WkpSA)
@click.option("-n","--name",help="Name of the cloud pod",required=WkpSA)
def cmd_cpvcs_commit_diff(version:WkpSj,commit:WkpSj,name:WkpSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(WkpSr)
  status.stop()
...
def print_table(columns:Dict[WkpSK,WkpSK],rows:List[Dict[WkpSK,Any]],formatters:Dict[WkpSK,Callable[[Any],WkpSK]]=WkpSr):
 from rich.table import Table
 if formatters is WkpSr:
  formatters=WkpSY()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  WkpSO(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->WkpSr:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->WkpSr:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->WkpSr:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->WkpSr:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
