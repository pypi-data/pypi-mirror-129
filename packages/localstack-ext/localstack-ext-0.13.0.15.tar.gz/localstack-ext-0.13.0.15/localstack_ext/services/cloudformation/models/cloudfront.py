...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or jXnDo
 @jXnDL
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return jXnDH(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @jXnDR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if jXnDK(origins,jXnDN)else origins.get("Items")
  origins_to_create=jXnDe([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if jXnDK(aliases,jXnDN)else aliases.get("Items")
  aliases_to_create=jXnDe(aliases)
  for distr in distrs:
   origins=jXnDe([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=jXnDe(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",jXnDS)
   config["Enabled"]=jXnDJ(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if jXnDK(custom_errors,jXnDG)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=jXnDJ(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":jXnDS}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=jXnDJ(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=jXnDo):
    parent=config if parent is jXnDo else parent
    items=parent.get(key)or[]
    if jXnDK(items,jXnDG)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":jXnDP(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if jXnDK(cache_behavs,jXnDG)else cache_behavs)
