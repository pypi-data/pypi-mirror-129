...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=URzMf(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not URzMF
   return result
  except URzMy:
   props=self.props
   ...
   if parts[0]in candidates and URzMk(parts)>1:
