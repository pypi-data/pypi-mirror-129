...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[Dysim])[0]
 def get_physical_resource_id(self,attribute=Dysim,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if DysiK(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return DysiL(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @DysiB
 def add_defaults(resource,stack_name:Dysif):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return DysiL(SESReceiptRule,self).get_cfn_attribute(attribute)
 @DysiJ
