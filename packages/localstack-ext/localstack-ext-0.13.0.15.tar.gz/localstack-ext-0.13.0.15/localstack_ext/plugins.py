...
def api_key_configured()->JoLAn:
 return JoLAx if os.environ.get("LOCALSTACK_API_KEY")else JoLAb
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if JoLAI([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
