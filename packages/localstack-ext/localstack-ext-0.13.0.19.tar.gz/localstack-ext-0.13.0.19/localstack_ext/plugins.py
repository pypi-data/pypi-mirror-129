...
def api_key_configured()->LgNUj:
 return LgNUe if os.environ.get("LOCALSTACK_API_KEY")else LgNUV
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if LgNUt([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
