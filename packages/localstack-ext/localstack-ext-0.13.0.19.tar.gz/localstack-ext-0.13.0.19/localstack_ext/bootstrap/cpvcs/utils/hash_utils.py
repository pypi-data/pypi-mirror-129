...
def random_hash()->mQrYI:
 return mQrYx(random.getrandbits(160))
def compute_file_hash(file_path:mQrYI,accum=mQrYB)->Optional[mQrYI]:
 try:
  with mQrYP(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:mQrYK,state_archive:mQrYI)->mQrYI:
 ...
 m.update(mQrYI(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->mQrYI:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=mQrYT(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with mQrYP(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except mQrYH as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if mQrYG(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(mQrYI(cpvcs_node.revision_number).encode("utf-8"))
 elif mQrYG(cpvcs_node,Version):
  m.update(mQrYI(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
