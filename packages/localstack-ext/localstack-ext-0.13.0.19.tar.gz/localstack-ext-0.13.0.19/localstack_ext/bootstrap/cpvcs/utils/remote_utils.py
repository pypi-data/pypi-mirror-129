...
def extract_meta_and_state_archives(meta_archives:Dict[cxpmy,cxpmy],state_archives:Dict[cxpmy,cxpmy]):
  ...
  for version_no,archive in product_space_archive.items():
   with zipfile.ZipFile(archive)as meta_zip:
    if product_space_archive==meta_archives:
     archive_dest=config_context.get_version_meta_archive_path(version_no=version_no,with_format=cxpma)
    else:
     archive_dest=config_context.get_version_state_archive_path(version_no=version_no,with_format=cxpma)
...
def register_remote(remote_info:Dict[cxpmy,cxpmy]):
 if config_context.is_remotly_managed():
  LOG.warning("Pod is already remotely managed")
  return
 with cxpmu(config_context.get_remote_info_path(),"w")as fp:
...
def merge_version_space(version_space_archive):
 ...
 for version_ref_file in os.listdir(remote_config_context.get_ver_refs_path()):
  ...
  with cxpmu(remote_version_ref_file_path,"r")as fp:
...
def _create_metamodel_helper(obj,width=25,visited:set=cxpmV):
 if obj is cxpmV:
  return obj
 ...
 elif cxpmb(obj,cxpmv):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if cxpmC(result)>width:
   result={"size":cxpmC(result),"items":result[:width]}
 return result
