...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[eqGdF],eqGdF],api_name:eqGdF):
 if not(is_debian()and in_docker()):
  ...
  return
 if eqGdD(packages,eqGdF):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=eqGdb)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=eqGdb)
def check_or_install(command:eqGdF,packages:Union[List[eqGdF],eqGdF],api:eqGdF):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:eqGdF,target_file:eqGdF,timeout:int=eqGdp):
 def do_download(url,print_error=eqGdb):
  try:
