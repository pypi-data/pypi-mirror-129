...
class DecryptionHandler:
 decryption_key:PpgUl
 def __init__(self,decryption_key:PpgUl):
  self.decryption_key=decryption_key
 def decrypt(self,content)->PpgUl:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=PpgUi):
  if path and not PpgUB(path,PpgUq):
   path=PpgUq(PpgUr(path,"_path",[]))
  if not path:
   return PpgUi
...
def patch_traceback_lines():
 if PpgUr(traceback.FrameSummary,"_ls_patch_applied",PpgUi):
  return
 @PpgUG
 def line(self):
  try:
 ...
 PpgUA(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=PpgUJ
def patch_inspect_findsource():
 if PpgUr(inspect,"_ls_patch_applied",PpgUi):
  return
 ...
 inspect._ls_patch_applied=PpgUJ
