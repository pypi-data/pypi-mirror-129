...
class PodInfo:
 def __init__(self,name=kpdIQ,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(kpdIw):
 BACKEND="_none_"
 def __init__(self,pod_name=kpdIQ,config=kpdIQ):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise kpdIJ("Not implemented")
 def delete(self,remote:kpdIS)->kpdIS:
  raise kpdIJ("Not implemented")
 def push(self,comment:kpdIe=kpdIQ,three_way:kpdIS=kpdIi)->PodInfo:
  raise kpdIJ("Not implemented")
 def push_overwrite(self,version:kpdIB,comment:kpdIe=kpdIQ):
  pass
 def pull(self,inject_version_state:kpdIS=kpdIi,reset_state_before:kpdIS=kpdIi,fetch_all:kpdIS=kpdIQ):
  raise kpdIJ("Not implemented")
 def commit(self,message:kpdIe=kpdIQ):
  raise kpdIJ("Not implemented")
 def inject(self,version:kpdIB,reset_state:kpdIS):
  raise kpdIJ("Not implemented")
 def list_versions(self)->List[kpdIe]:
  raise kpdIJ("Not implemented")
 def version_info(self,version:kpdIB):
  raise kpdIJ("Not implemented")
 def version_metamodel(self,version:kpdIB):
  raise kpdIJ("Not implemented")
 def set_version(self,version:kpdIB,inject_version_state:kpdIS,reset_state:kpdIS,commit_before:kpdIS):
  raise kpdIJ("Not implemented")
 def list_version_commits(self,version:kpdIB)->List[kpdIe]:
  raise kpdIJ("Not implemented")
 def get_commit_diff(self,version:kpdIB,commit:kpdIB)->kpdIe:
  raise kpdIJ("Not implemented")
 def register_remote(self,pod_name:kpdIe,ci_pod:kpdIS)->kpdIS:
  raise kpdIJ("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->kpdIS:
  raise kpdIJ("Not implemented")
 def list_pods(self,fetch_remote:kpdIS)->Set[kpdIe]:
  raise kpdIJ("Not implemented")
 ...
 def get(cls,pod_name,pre_config=kpdIQ):
  ...
  raise kpdIJ('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=kpdIi
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=kpdIK)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=kpdIK
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise kpdIJ("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @kpdIL
 def get_state_zip_from_instance(get_content=kpdIi):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise kpdIJ("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:kpdIe=kpdIQ):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @kpdIL
 def parse_pod_name_from_qualifying_name(qualifying_name:kpdIe)->kpdIe:
 ...
 @kpdIL
 def _get_max_version_for_pod_from_platform(pod_name:kpdIe,auth_headers):
  ...
  remote_max_ver=kpdIB(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with kpdIb(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with kpdIb(meta_archive,"rb")as meta_archive_content:
   ...
   with kpdIb(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=kpdIQ)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:kpdIe):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:kpdIS)->kpdIS:
  ...
  if remote:
   pass
  return kpdIi
 def _push_to_remote(self,url:kpdIe):
 ...
 def push_overwrite(self,version:kpdIB,comment:kpdIe=kpdIQ):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return kpdIK
 def pull(self,inject_version_state:kpdIS=kpdIi,reset_state_before:kpdIS=kpdIi,fetch_all:kpdIS=kpdIK):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=kpdIi):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(kpdIm(lambda ver:kpdIe(ver),kpdIM(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:kpdIB,reset_state:kpdIS):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=kpdIK,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def get_commit_diff(self,version:kpdIB,commit:kpdIB)->kpdIe:
  ...
  return commit_diff
 def register_remote(self,pod_name:kpdIe,ci_pod:kpdIS=kpdIi)->kpdIS:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:kpdIS)->Set[kpdIe]:
  ...
  return result
 @kpdIP
 def upload_content(cls,presigned_url:kpdIe,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise kpdIJ("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @kpdIL
 def create_platform_url(request:kpdIe)->kpdIe:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:kpdIe=kpdIQ,three_way:kpdIS=kpdIi)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=kpdIK)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(kpdIx(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=kpdIx(zip_data_content)
  return result
 def pull(self,inject_version_state:kpdIS=kpdIi,reset_state_before:kpdIS=kpdIi,fetch_all:kpdIS=kpdIQ):
  ...
  rm_rf(zip_path)
 @kpdIL
 def presigned_url(pod_name:kpdIe,mode:kpdIe)->kpdIe:
  ...
  if response.status_code>=400:
   raise kpdIJ("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise kpdIJ("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:kpdIe=kpdIQ,three_way:kpdIS=kpdIi):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=kpdIK)
 ...
 def pull(self,inject_version_state:kpdIS=kpdIi,reset_state_before:kpdIS=kpdIi,fetch_all:kpdIS=kpdIQ):
  ...
  if remote_ref not in remote_refs:
   raise kpdIJ('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=kpdIY(self,"clone_dir",kpdIQ)
...
class PodConfigManagerMeta(kpdIc):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=kpdIQ
   for manager in cls.CHAIN:
    try:
     tmp=kpdIY(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif kpdIG(tmp,kpdIa)and kpdIG(result,kpdIa):
       result.extend(tmp)
    except kpdIJ:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not kpdIQ:
    return result
   raise kpdIJ('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(kpdIw,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @kpdIP
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise kpdIJ('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(kpdIw):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=kpdIK).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=kpdIR([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=kpdIi):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except kpdIJ:
   if safe:
    return{}
   raise kpdIJ('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(kpdIw):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise kpdIJ("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise kpdIJ("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:kpdIe,remote:kpdIS,pre_config:Dict[kpdIe,kpdIe])->kpdIS:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],**kwargs)->kpdIS:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",kpdIi))
 return result
def rename_pod(current_pod_name:kpdIe,new_pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:kpdIS,pre_config:Dict[kpdIe,kpdIe],**kwargs)->List[kpdIe]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],message:kpdIe=kpdIQ,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:kpdIe,version:kpdIB,reset_state:kpdIS,pre_config:Dict[kpdIe,kpdIe],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],**kwargs)->List[kpdIe]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:kpdIB,pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:kpdIB,pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:kpdIB,inject_version_state:kpdIS,reset_state:kpdIS,commit_before:kpdIS,pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe],**kwargs)->kpdIS:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:kpdIB,pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe])->List[kpdIe]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:kpdIB,commit:kpdIB,pod_name:kpdIe,pre_config:Dict[kpdIe,kpdIe])->kpdIe:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:kpdIB,pod_name:kpdIe,comment:kpdIe,pre_config:Dict[kpdIe,kpdIe]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=kpdIQ,squash_commits=kpdIi,comment=kpdIQ,three_way=kpdIi,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=kpdIi,reset_state_before=kpdIi,**kwargs):
 pre_config=kwargs.get("pre_config",kpdIQ)
 if not pod_name:
  raise kpdIJ("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=kpdIi,exclude_from_reset:List[kpdIe]=kpdIQ):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise kpdIJ("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->kpdIe:
 try:
...
def get_persisted_resource_names(data_dir)->List[kpdIe]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and kpdIx(os.listdir(entry.path))>0:
    names.append(entry.name)
