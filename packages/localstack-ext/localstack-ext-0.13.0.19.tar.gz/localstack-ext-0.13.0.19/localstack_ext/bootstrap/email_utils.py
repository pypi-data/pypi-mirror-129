...
SENT_EMAILS=[]
EMAIL_BLACKLIST=ATqXk()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",ATqXj(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=ATqXd,from_name=ATqXd,smtp_host=ATqXd,smtp_user=ATqXd,smtp_pass=ATqXd,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if ATqXw(recipients,ATqXP)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=ATqXd):
 ...
 result["From"]=formataddr((ATqXj(Header(from_name,"utf-8")),from_email))
 return result
