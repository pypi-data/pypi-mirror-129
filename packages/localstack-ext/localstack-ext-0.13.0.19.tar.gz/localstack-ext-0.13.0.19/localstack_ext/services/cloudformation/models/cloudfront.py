...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or mJWAP
 @mJWAi
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return mJWAD(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @mJWAG
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if mJWAj(origins,mJWAu)else origins.get("Items")
  origins_to_create=mJWAh([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if mJWAj(aliases,mJWAu)else aliases.get("Items")
  aliases_to_create=mJWAh(aliases)
  for distr in distrs:
   origins=mJWAh([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=mJWAh(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",mJWAB)
   config["Enabled"]=mJWAC(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if mJWAj(custom_errors,mJWAa)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=mJWAC(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":mJWAB}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=mJWAC(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=mJWAP):
    parent=config if parent is mJWAP else parent
    items=parent.get(key)or[]
    if mJWAj(items,mJWAa)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":mJWAk(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if mJWAj(cache_behavs,mJWAa)else cache_behavs)
