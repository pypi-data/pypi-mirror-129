...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[EGkSx])[0]
 def get_physical_resource_id(self,attribute=EGkSx,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if EGkSA(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return EGkSJ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @EGkSl
 def add_defaults(resource,stack_name:EGkSW):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return EGkSJ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @EGkSz
