...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=bRvpF(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not bRvpt
   return result
  except bRvpO:
   props=self.props
   ...
   if parts[0]in candidates and bRvpX(parts)>1:
