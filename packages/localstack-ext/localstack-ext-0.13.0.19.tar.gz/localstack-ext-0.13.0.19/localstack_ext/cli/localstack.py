...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return WyNxs if e else WyNxR
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->WyNxJ:
...
@click.option("--revert",is_flag=WyNxs,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:WyNxB):
...
def _cpvcs_initialized(pod_name:WyNxz)->WyNxB:
...
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_init(name:WyNxz):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=WyNxs,default=WyNxR)
def cmd_cpvcs_delete(name:WyNxz,remote:WyNxB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_register_remote(name:WyNxz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=WyNxs)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=WyNxs)
def cmd_cpvcs_rename(name:WyNxz,new_name:WyNxz):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_commit(message:WyNxz,name:WyNxz):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=WyNxs,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=WyNxs,default=WyNxR,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_push(squash:WyNxB,message:WyNxz,name:WyNxz,three_way:WyNxB):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
@click.option("-v","--version",type=WyNxV)
@click.option("-m","--message",required=WyNxR)
def cmd_cpvcs_push_overwrite(version:WyNxV,message:WyNxz,name:WyNxz):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=WyNxV,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=WyNxs,default=WyNxR,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_inject(version:WyNxV,reset:WyNxB,name:WyNxz):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=WyNxs,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=WyNxs,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:WyNxz,inject:WyNxB,reset:WyNxB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=WyNxs,default=WyNxR)
def cmd_cpvcs_list_pods(remote:WyNxB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_versions(name:WyNxz):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WyNxs,type=WyNxV)
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_version_info(version:WyNxV,name:WyNxz):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=WyNxV,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_version_metamodel(version:WyNxV,name:WyNxz):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WyNxs,type=WyNxV,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=WyNxs,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=WyNxs,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=WyNxR,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_set_version(version:WyNxV,inject:WyNxB,reset:WyNxB,commit_before:WyNxB,name:WyNxz):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_commits(version:WyNxV,name:WyNxz):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=WyNxs)
@click.option("--commit","-c",required=WyNxs)
@click.option("-n","--name",help="Name of the cloud pod",required=WyNxs)
def cmd_cpvcs_commit_diff(version:WyNxV,commit:WyNxV,name:WyNxz):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(WyNxJ)
  status.stop()
...
def print_table(columns:Dict[WyNxz,WyNxz],rows:List[Dict[WyNxz,Any]],formatters:Dict[WyNxz,Callable[[Any],WyNxz]]=WyNxJ):
 from rich.table import Table
 if formatters is WyNxJ:
  formatters=WyNxX()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  WyNxp(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->WyNxJ:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->WyNxJ:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->WyNxJ:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->WyNxJ:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
