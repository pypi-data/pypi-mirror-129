...
def get_fragmented_line(e: Entity) -> str:
    ...
    fragments = []
    for m in matches:
        frag_start = offset + start
        frag_end = offset + m.start()
        if frag_start != frag_end:
            spans.append((frag_start, frag_end))
            fragments.append(e.text[start : m.start()])
            start = m.end()
    frag_start = offset + start
    frag_end = offset + len(e.text)
    if frag_start != frag_end:
        spans.append((frag_start, frag_end))
        fragments.append(e.text[start:])
