...
class Optimization(ABC):
        ...
        for gen in range(self.n_gen):
            ...
            if gen == 0:
                ...
                while len(self.pop) < self.n_pop:
                    ...
                    fom_dct = self.execute_model_with_error_handling(individual, len(self.pop))
                    if all([not np.isnan(value) for value in fom_dct.values()]):
                        self.pop.append(individual)
                        fom_dcts.append(fom_dct)
                # initialize population
            else:
                fom_dcts = [self.execute_model_with_error_handling(individual, index)
                            for index, individual in enumerate(self.pop)]

            # log current computation results
            self.scores = self.calculate_scores(fom_dcts)
            for individual, fom_dct, score in zip(self.pop, fom_dcts, self.scores):
                self.logger.append_to_logger(self.decode_individual(individual), fom_dct, score)

            # display an update
            print(sorted(list(self.scores)))
            ...
            self.pop = self.update_generation(self.pop, self.scores)
            self.logger.save_logger()
            self.save_the_best_individual(self.pop, self.scores, gen)

    def execute_model_with_error_handling(self, individual, index):
    ...
    @abstractmethod
    def save_the_best_individual(self, pop, scores, gen):
    ...
    def initialize_population(self) -> List[List[int]]:
        ...
        """
        return [self.initialize_individual() for _ in range(self.n_pop)]

    @abstractmethod
    def initialize_individual(self) -> List[int]:
        """Abstract method initializing a population of candidate solutions
...
class RoxieGeneticOptimization(GeneticOptimization):
    ...
    def save_the_best_individual(self, pop, scores, gen):
        ...
        """
        pop_sorted = [pop_el for _, pop_el in sorted(zip(scores, pop))]
        model_input_path_temp = self.model_input_path
        self.model_input_path = self.model_input_path.replace('.json', "_%d.json" % gen)
        try:
            self.update_model(pop_sorted[0])
        except:
            print(pop)
            print(scores)
            print(self.geometry.to_dict())
        self.model_input_path = model_input_path_temp

    def initialize_individual(self) -> List[int]:
        if self.config.model_creation_type == 'default':
            ind = RoxieGeneticOptimization.generate_random_chromosome(self.design_variables)
        elif self.config.model_creation_type == 'targeted':
            ind = RoxieGeneticOptimization.generate_random_chromosome(self.design_variables)
        elif self.config.model_creation_type == 'programmable':
            ind = RoxieGeneticOptimization.generate_random_chromosome_programmable_geometry(self.design_variables)
        else:
            raise AttributeError('Model creation type %s not supported!' % self.config.model_creation_type)
        return ind
    ...
    def update_model(self, individual: List[int]) -> None:
        ...
        if self.config.model_creation_type == 'default':
            decoded_chromosome = self.decode_individual_old(individual)
            updated_block_inputs = self.update_model_parameters(decoded_chromosome).to_dict()
        elif self.config.model_creation_type == 'targeted':
            decoded_chromosome = self.decode_individual_old(individual)
            updated_block_inputs = self.update_model_parameters_targeted(decoded_chromosome).to_dict()
        elif self.config.model_creation_type == 'programmable':
    ...
    def decode_individual(self, individual: List[int]) -> Dict:
        if self.config.model_creation_type == 'default':
            return self.decode_individual_old(individual)
        elif self.config.model_creation_type == 'targeted':
...
class ParallelRoxieGeneticOptimization(RoxieGeneticOptimization):
        ...
        for gen in range(self.n_gen):
            ...
            if gen == 0:
                ...
                while len(self.pop) < self.n_pop:
                    individual = self.initialize_individual()
                    fom_dct = self.execute_model_with_error_handling(individual, len(self.pop))
                    if not any([np.isnan(value) for value in fom_dct.values()]):
                        self.pop.append(individual)
                        fom_dcts.append(fom_dct)
                # initialize population
            else:
    ...
    def execute_model_with_error_handling(self, individual, index):
        # index passed to the notebook - this is how it differs from the regular method
        print('\tIndividual:', index)
        try:
            self.update_model(individual)
