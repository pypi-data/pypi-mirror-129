...
class FileTokensStorage(TokensStorage):
    def __init__(self, directory_path=Path.cwd()):
...
class TokenManager:
    ...
    def init(self, code, skip_error=False):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning('Not possible create new token. Maybe there already there?')
                logger.warning(f"Status code: {response.status_code}: "
                               f"{response.json()['title']} {response.json()['hint']}")
                logger.warning(f"The request data was:\n{str(data)}")
            if response.status_code != 200 and not skip_error:
                raise Exception(response.json()["title"] + response.json()["hint"])
            if response.status_code != 200 and skip_error:
                return
            response = response.json()
            self._storage.save_tokens(response["access_token"], response["refresh_token"])
            logger.info("successful init and store tokens in store")
    ...
    def get_access_token(self):
        ...
        if token is None:
            raise NoToken("You need to init tokens with code by 'init' method")
