    ...
    def add_domainwallstarter(self, boundary_normal: np.ndarray = None, turn_boundary: bool = True,
        ...
        for layer in range(self.nlayer):
            ...
            if turn_boundary:
                ...
                dots_forward = [np.dot((p - midpoint_shifted_forward) / np.linalg.norm(p - midpoint_shifted_forward), boundary_normal)
                                for p in points]
                dots_backward = [np.dot((p - midpoint_shifted_backward) / np.linalg.norm(p - midpoint_shifted_backward), boundary_normal)
                                 for p in points]

                spins = [boundary_normal if (0<dots_backward[index])and(d <= 0) else spins[index] for (index, d) in enumerate(dots_forward)]
