...
def excel_to_pdf(input_file, output_file, sht_no):
    # uses win32com.client module
    # input_file = r'D:\Kartik\My Documents\PythonProjects\ProjectParul\Input\RKI_ADMISSION DETAILS_2020_21.xlsx'
    #give your file name with valid path 
    # output_file = r'D:\Kartik\My Documents\PythonProjects\ProjectParul\Input\output.pdf'
    #give valid output file name and path
    app = client.DispatchEx("Excel.Application") # DispatchEx creates separate instance of Excel. Alternatively
    # Dispatch can also be used. It will create another window in same instance.
    app.Interactive = False
    app.Visible = False
    wb = app.Workbooks.Open(input_file)
    ws = wb.Worksheets[sht_no] # sheet number is 0 index based
    try:
...
class TimeKeeper():
    ...
    def log(self):
        ...
        print(f'Runtime (sec): {self.logs[-1] - self.start_time}')
        return self.logs[-1]

    def get_log(self, log_idx):
        print(f'Log runtime (sec): {self.logs[log_idx] - self.start_time}')

    def end(self):
        self.end_time = datetime.datetime.now()
        print(f'Total runtime (sec): {self.end_time - self.start_time}')
