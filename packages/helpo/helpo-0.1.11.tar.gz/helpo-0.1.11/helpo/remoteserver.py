...
class RemoteServer(object):
    ...
    def add_to_rundeck(self, hostname: str,
        ...
        self._update_ssh_credentials(hostname, vault_tenant, vault_token, vault_url)
    ...
    def _update_hostname(self, hostname: str):
        ...
        self._run_command(rf'''"sudo sed -i 's/^127.0.1.1.*$/127.0.1.1 {hostname}/g' /etc/hosts"''')
        self.hostname = hostname

    @logger.catch
    def _add_sudo_user_with_ssh_key(self, username: str, public_ssh_key: str):
        self._run_command(rf'''"sudo useradd -m -s /bin/bash -G sudo {username}"''')
    ...
    def _change_user_password(self, username: str, password: str):
        self._run_command(rf'''"echo -e '{password}\n{password}' | sudo passwd {username}"''')

    @logger.catch
    def _update_ssh_port(self, port: int):
        self._run_command(rf'''"sudo sed -i 's/^.*Port.*$/Port {port}/g' /etc/ssh/sshd_config"''')
    ...
    def _update_ssh_credentials(self, hostname, vault_tenant, vault_token, vault_url):
        try:
            ...
            self._update_users_passwords(('root', 'abukamel'), remote_server_ssh_info['password'])
            ...
            new_ssh_password = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(32))
            self._update_users_passwords(('root', 'abukamel'), new_ssh_password)
    ...
    def print_final_ssh_server_info(self):
        print('Copy and paste the following commands to add server to your local ssh config')
        print('command storm -v || pip3 install stormssh')
        print(rf'storm add --id_file="~/.ssh/id_ed25519" {self.hostname} {self.username}@{self.ip}:{self.port}')
