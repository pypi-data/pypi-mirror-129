...
class process_text:
    ...
    def set_entity_exclusion_list(self, entities):
        self.entities = entities
        if len(self.entities)==0:
            self.entities = ["PERSON", "ORDINAL", "CARDINAL", "DATE", "TIME", "PRODUCT"]

    def set_language_modal(self, modal_name):
        self.nlp = spacy.load(modal_name)
    ...
    def clear_web_url(self, text):
        return re.sub(r"(^((ftp|http|https):\/\/)?(www.)?(?!.*(ftp|http|https|www.))[a-zA-Z0-9_-]+(\.[a-zA-Z]+)+((\/)[\w#]+)*(\/\w+\?[a-zA-Z0-9_]+=\w+(&[a-zA-Z0-9_]+=\w+)*)?$)+", '' , text)
    ...
    def clear_generic_entity(self, text):
        document = self.nlp(text)
        entitites = [ent.text for ent in document.ents if ent.label_ in self.entities]
        for entity in set(entitites):
            pattern = re.sub("'", "", entity)
