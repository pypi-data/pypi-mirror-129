...
logging.basicConfig(filename='/tmp/mlflow_kernel.log',
...
class MLFlowKernel(IPythonKernel):
    ...
    def __init__(self, **kwargs):
        ...
        if self.mlflow_tracking_uri:
            ...
            logging.info('parent run id = ' + str(self.parent_run_id))
    ...
    def do_execute_internal(self, code, silent, store_history, user_expressions, allow_stdin):
        ...
        try:
            ...
            logging.info(str(ex))
        logging.info("Logging artifacts - completed")
...
def capture_iopub_output(iopub_socket, stdouterr_artifact_file, data_artifact_file, is_running):
    ...
    while is_running():
        try:
            ...
            logging.info('msg received ## ' + msg_str)
            ...
            logging.info('Error ZMQError: ' + str(zqe))
            time.sleep(0.02)
        except Exception as ex:
            logging.debug('Could not decode ' + str(msg_str))
    if stdfp:
        stdfp.close()
    if data_text_fp:
        data_text_fp.close()
