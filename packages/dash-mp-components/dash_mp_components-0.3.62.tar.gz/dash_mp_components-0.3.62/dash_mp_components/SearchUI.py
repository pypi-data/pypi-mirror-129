...
- columns (list; optional):
    ...
    \"POINTGROUP\" The \"formatOptions\" is a special property that is
    ...
    link using the value in this column             formatOptions: {
    baseUrl: \"/materials/\" // prepends this string to the column
    value in the links href (e.g. href=\"/materials/mp-100\")
    }           },           {             name: 'Formula',
    selector: 'formula_pretty',             format: 'FORMULA',
    minWidth: '130px' // sets a minimum width for cells in this column
    },           {             name: 'Volume',             selector:
    'volume',             format: 'FIXED_DECIMAL',
    formatOptions: {               decimals: 3 // number of decimals
    to fix to             },             omit: True // hide this
    column initially           },           {             name:
    'Density',             selector: 'density',             format:
    'SIGNIFICANT_FIGURES',             formatOptions: {
    sigFigs: 4 // number of significant figures to use             }
    },           {             name: 'Is Stable',
    selector: 'is_stable',             format: 'BOOLEAN',
    formatOptions: {               truthyLabel: \"yes\", // sets
    display values for truthy and falsy values
    falsyLabel: \"no\"             }           },           {
    name: 'Energy Above Hull',             selector: 'e_above_hull',
    format: 'FIXED_DECIMAL',             formatOptions: {
    decimals: 2 // number of decimals to fix to             },
    units: 'meV/atom', // unit label string to show under column
    header             conversionFactor: 1000, // number by which to
    multiply result values in this columm           },       ].
...
- searchBarHelpItems (list; optional):
    Search examples to include below the search bar input. This will
    display when input is in focus and empty, or when the help button
    is clicked. Expects an array of items with the following
    properties: \"label\": text to display before examples. If only
    \"label\" and no \"examples\", text will render as small.
    \"examples\": array of input examples. These will become clickable
    and will update the input value.  e.g.       [         {
    label: 'Search Examples'         },         {           label:
    'Include at least elements',           examples: ['Li,Fe',
    'Si,O,K']         }       ].
