...
class BuiltinMacros:
    ...
    @meta({'spec': [[Spec.ArityAtLeastOne]],
           ...
           'type': 'macro',
           'forms': ['(with-measure ...)'],
           'docstring': 'with-measure allows you to measure how long it takes for your code to evaluate'})
    def with_measure_macro(tree_slice: list, env: dict, ev_func, token_cls) -> list:
        """
        Builtin 'with-measure' macro implementation
        ...
        return [token_cls(token_cls.Identifier, 'identity'),
                token_cls(token_cls.String, f'Took {round(finished_at - started_at, 2)} s')]
    ...
    def fn_macro(tree_slice: list, env: dict, ev_func, token_cls) -> list:
        ...
        name_token, arguments_form, body_token_or_form = [None] * 3

        if len(tree_slice) == 2:
            arguments_form, body_token_or_form = tree_slice
        elif len(tree_slice) == 3:
            name_token, arguments_form, body_token_or_form = tree_slice
            assert not isinstance(name_token, list), 'fn: name must be a token, not a form'
            assert name_token.type() == token_cls.Identifier, (
                'fn: name needs to be an Identifier'
            )

        ASSERT_IS_INSTANCE_OF(
            arguments_form,
            list, 'fn: arguments form needs to be a Vector')

        but_first = arguments_form[1:]
        ...
        argument_names = [t.dt().origin() for t in but_first]

        ASSERT_NO_DUPLICATES(
            argument_names, 'fn: arguments form could not contain duplicates')

        name = name_token.dt().origin() if name_token else randstr()

        handler = fn.Fn(env, name, ev_func, token_cls, argument_names, body_token_or_form)

        def function_reference(args: list):
            """Lets create closure"""

            return handler.handle(tuple(args))
...
class BuiltinFunctions:
    ...
    @meta({'spec': [[Spec.ArityVariants, [2]],
           ...
           'docstring': 'Allows you to repeat something as many time as you may want'})
    ...
    def filter_func(args: List[datatypes.Base]) -> datatypes.Vector:
        ...
        return datatypes.Vector(
            tuple(filter(lambda e: function.reference()([e]).reference() is True, collection.reference())))
    ...
    def make_hashmap_func(args: List[datatypes.Base]) -> datatypes.HashMap:
        ...
        return datatypes.HashMap(
            tuple((k.hashed(), k, v) for [k, v] in [args[i:i + 2] for i in range(0, len(args), 2)]))
