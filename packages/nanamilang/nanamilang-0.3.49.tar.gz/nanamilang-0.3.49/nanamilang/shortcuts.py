...
def get(lst: tuple, idx: int, _def):
...
def find(st: set, element: Any, _def):
    """Safely find value from set"""

    return get(tuple(filter(lambda e: e == element, st)), 0, None or _def)
...
def ASSERT_COLL_LENGTH_IS(coll: list or tuple, length: int, message: str = '') -> None:
    """ASSERT_COLL_LENGTH_IS: whether collection length equals to or not?"""
    assert isinstance(coll, (list, tuple)), 'ASSERT_COLL_LENGTH_IS: not a list or tuple'
    assert len(coll) == length, message or f'collection length does not equal to {length}'


def ASSERT_COLL_LENGTH_IS_EVEN(coll: list or tuple, message: str = '') -> None:
    """ASSERT_COLL_LENGTH_IS_EVEN: whether collection length is even or not?"""
    assert isinstance(coll, (list, tuple)), 'ASSERT_COLL_LENGTH_IS_EVEN: not a list or tuple'
...
def ASSERT_NO_DUPLICATES(coll: list or tuple, message: str = '') -> None:
    """ASSERT_NO_DUPLICATES: does the list contain duplicates?"""
    assert isinstance(coll, (list, tuple)), 'ASSERT_NO_DUPLICATES: not a list or tuple'
...
def ASSERT_COLL_CONTAINS_ELEMENT(elem: Any, coll: list or tuple, message: str = '') -> None:
    """ASSERT_COLL_CONTAINS_ELEMENT: does the coll contain element?"""
    assert isinstance(coll, (list, tuple)), 'ASSERT_COLL_CONTAINS_ELEMENT: not a list or tuple'
...
def ASSERT_EVERY_COLLECTION_ITEM_EQUALS_TO(coll: list or tuple, to: Any, message: str = '') -> None:
    """ASSERT_EVERY_COLLECTION_ITEM_EQUALS_TO: does every collection item equal to ...?"""
    assert isinstance(coll, (list, tuple)), 'ASSERT_EVERY_COLLECTION_ITEM_EQUALS_TO: not a list or tuple'
    assert len(list(filter(lambda x: x == to, coll))) == len(coll), message or (
        f'every collection item must be equal to {to.__name__}'
    )


def ASSERT_EVERY_COLLECTION_ITEM_IS_CHILD_OF(coll: list or tuple, of: Any, message: str = '') -> None:
    """ASSERT_EVERY_COLLECTION_ITEM_IS_CHILD_OF: does every collection item is a child of ...?"""
    assert isinstance(coll, (list, tuple)), 'ASSERT_EVERY_COLLECTION_ITEM_IS_CHILD_OF: not a list or tuple'
...
def ASSERT_EVERY_COLLECTION_ITEM_IS_INSTANCE_OF(coll: list or tuple, of: Any, message: str = '') -> None:
    """ASSERT_EVERY_COLLECTION_ITEM_IS_INSTANCE_OF: does every collection item is an instance of ...?"""
    assert isinstance(coll, (list, tuple)), 'ASSERT_EVERY_COLLECTION_ITEM_IS_INSTANCE_OF: not a list or tuple'
