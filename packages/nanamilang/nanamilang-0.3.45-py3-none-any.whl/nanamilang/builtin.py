...
class BuiltinMacros:
    ...
    def with_measure_macro(tree_slice: list, env: dict, ev_func, token_cls) -> list:
        ...
        return [token_cls(token_cls.Identifier, 'identity'),
                token_cls(token_cls.String, f'Took {round(finish - start, 2)} second(s) to evaluate')]
    ...
    def fn_macro(tree_slice: list, env: dict, ev_func, token_cls) -> list:
        ...
        if len(tree_slice) == 2:
            ...
            assert fn_name_token.type() == token_cls.Identifier, (
        ...
        handler = fn.Fn(env, fn_name, ev_func, token_cls, fn_argument_names, fn_body_token_or_form)
        ...
        function_reference.meta = {
            'name': fn_name,
            'type': 'function', 'forms': handler.generate_meta__forms(), 'docstring': '',
        }

        _env = {fn_name: datatypes.Function({'function_name': fn_name,
