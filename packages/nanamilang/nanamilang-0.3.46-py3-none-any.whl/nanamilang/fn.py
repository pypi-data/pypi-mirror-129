...
class Fn:
    ...
    def __init__(self,
        ...
        self._env = deepcopy(env)  # Save copy instead of reference to environment
    ...
    def env(self) -> dict:
        """NanamiLang Fn Handler, self._env getter"""

        return self._env

    def generate_meta__forms(self) -> list:
        """NanamiLang Fn Handler, generate that list of strings"""
        ...
        def recursively(dt, dt_items) -> list:
            ...
            for x in dt_items:
                if x.name not in ['Set', 'Vector', 'HashMap']:
                    ...
                    tokens_for_items.append(recursively(x, x.reference_as_tuple()))
            return [self._token_cls('Identifier', f'make-{dt.name.lower()}')] + tokens_for_items

        return recursively(dt_instance, dt_instance.reference_as_tuple())

    def handle(self, args: tuple) -> datatypes.Base:
        """NanamiLang Fn Handler, handle function evaluation"""

        Spec.validate(self._fn_name, args, [[Spec.ArityVariants, [len(self._fn_arg_names)]]])

        copied_body_form = deepcopy(self._fn_body_form)

        arg_values_as_tokens = [self._form_for_arg(arg) for arg in args]
