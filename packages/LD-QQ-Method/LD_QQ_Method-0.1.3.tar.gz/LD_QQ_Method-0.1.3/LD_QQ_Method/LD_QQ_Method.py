...
# 机器人基础属性
class myRobot():
    def __init__(self, myApiUrl, myTTSUrl, myTianXingKey, mymyQQurl):
        global apiUrl
        global TTSUrl
        global TianXingKey
        global addMp3Url
        ...
        imageUrl = mymyQQurl + 'Voice/Origin/'
        originUrl = mymyQQurl + 'Image/'
...
def systemSetting(myThread):
    ...
    if myThread.recMsg == '菜单':
        ...
        global quitGroupList
        timeThread_temp = timeThread('2327541179', False)
...
def Music_Mp3toAMRMain(myThread):
    ...
    for i in musicNameListmp3:
        ...
        Music_Mp3toAMR(i)
        musicNameListAMR.append(i)
        sendMsg = sendMsg + i +'\n'
        msg = i + '以添加完成'
        sendMsg_friend(myThread, myThread.recFromQQ, msg)
    Music_addMusicTXT(musicNameListAMR)
    sendMsg_friend(myThread, myThread.recFromQQ, sendMsg)
