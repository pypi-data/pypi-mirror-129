...
class MerlinBinary:
    ...
    def _determine_colourN(self):
        ...
        if self._hdrfn:
            ...
            with open(self._binfns[0], "rb") as f:
                ...
                frame_size = int(self._Mbhp.header_bytesize + (self._Mbhp.bitdepth_bin*self._detX*self._detY)/8)
                f.seek(-frame_size, 2)
                hdr = f.read(self._Mbhp.header_bytesize) 
                hdr = hdr.split(b',')
                colour1 = int(hdr[11])
    ...
    def _determine_axes(self):
        ...
        if len(self._binfns) == 1 and self._scanXalu is None and self._scanYalu is None and len(self._dmfns) == 0:
            ...
            nframes = int((os.path.getsize(self._binfns[0]) / (self._Mbhp.header_bytesize +
                                                              self._im_bytesize))/self._colourN)
    ...
    def write_hdf5(self, h5fn=None, chunks=None, scan_chunks=(16, 16), im_chunks=(16, 16),
                   compression_opts=4, compression='gzip', repack=True, ow=False, embed_header_file=True,
        ...
        if compression not in compression_modes:
            raise NotImplementedError("'compression' (%s) must be one of: " %(compression) + str(compression_modes))
        if compression == 'gzip':
        ...
        with h5py.File(h5fn, 'w', driver=None) as h5f:
            ...
            fpd_binary_hdr_str_dataset = _create_bin_header_str_dataset(fpd_expt_grp, 
                                                                        ...
                                                                        self._dim_axes, self._dim_units)
            # Create FPD data.
            fpd_dataset = _create_fpd_data_hdf5_dataset(fpd_expt_grp, fpd_ds_opts, 
            ...
            for binfn_index, binfn in enumerate(self._binfns):
                ...
                with open(binfn, "rb") as f:
                    ...
                    if self._hdrfn:
                        ...
                        read_binary_func = partial(_read_header_and_data, fp, 1, self._im_bytesize,
                                                   return_bin_data=(not memmap_possible))
        ...
        for binfn_index, binfn in enumerate(self._binfns):
            ...
            with open(binfn, "rb") as f:
                ...
                if self._hdrfn:
                    ...
                    read_binary_func = partial(_read_header_and_data, fp, self._Mbhp.fmt_ver, self._im_bytesize)
                bin_file_iter = iter(read_binary_func, '')
                for (hdr_str, hdr_params, data_buffer) in bin_file_iter:
                    # process bin_data                       
                    if i == sel_ind:
...
def _check_fpd_file(fpg, min_version=None, max_version=None, raise_exception=True, silent=False):
    '''
...
class DataBrowser:
    def __init__(self, fpgn, nav_im=None, cmap=None, norm='lin',
                 colour_index=None, nav_im_dict=None, fpd_check=True):
        '''
        ...
        if nav_im is None:
            if not treat_as_array:
                ...
                if self.colour_index is not None:
                    nav_im = self.h5f['fpd_expt/fpd_sum_im/data'][..., self.colour_index]
                else:
                ...
                if self.ncolours == 0:
                    ...
                    nav_im = fpd.fpd_processing.sum_im(self.h5f_ds, 16, 16)
                else:
                    #nav_im = self.h5f_ds[:, :, self.colour_index].sum((-2, -1))
                    nav_im = fpd.fpd_processing.sum_im(self.h5f_ds, 16, 16)[..., self.colour_index]
        self.nav_im = nav_im
        ...
        if norm not in norms:
            raise NotImplementedError("'norm' (%s) must be one of: " %(norm) + str(norms))
        if norm == 'lin':
            self.norm = None
        elif norm == 'log':
