...
def blob_log_detect(image, min_radius, max_radius, num_radii, threshold, overlap=0.3, log_scale=True, ref_im=None, sigma=2.0, phase_exponent=0, log_xc_max_r=None, subpix_log=False, subpix_xc=False, subpix_dict=None, fit_hw=2, plot=False):
    '''
    ...
    if ref_im is not None:
        ...
        pks = peak_local_max(c, min_distance=1, threshold_abs=None, threshold_rel=None, exclude_border=True)
...
def strain(r_ref, t_ref, r, t, invert_space=False, origin='top',
           plot=True, pct=0, centred_cmap=True):
    '''
    ...
    if plot:
        ...
        if nd == 0:
            ...
            if centred_cmap:
                vmin, vmax = (np.nanmax(np.abs([vmin, vmax]), 0)[:, None] * np.array([-1, 1])).T
                cmap = 'coolwarm'
            else:
                cmap = None
