...
from instancelib.analysis.base import (BinaryModelMetrics,
                                       MulticlassModelMetrics, label_metrics)
...
def mean_score(pattern: str,
               ...
               **kwargs) -> Tuple[float, List[Tuple[MemoryTextInstance, float]]]:
    ...
    Returns:
        Tuple[float, List[Tuple[LT, float]]]: Mean score for the selected label, generated instances and label scores.
    """
    instances, _ = from_pattern(pattern, **kwargs)
    ...
    return np.mean([p for id, p in predictions_by_label]), predictions_by_label
