...
class BusinessMediaFetcher:
  ...
  def process_all(self):
    try:
      ...
      for item in users:
        user_id = item[0]
        access_token = item[1]
        self.process_business_media(user_id,access_token,self.date_limit)
    except Exception as e:
      self.logger.exception("BusinessMediaFetcher: error while processing all")
      raise e
    finally:
  ...
  ### Main function
  def process_business_media(self, user_id, access_token, date_limit = None):
    try:      
        ...
        while paginate:
          ...
          if response and response.get('data'):
            ...
            min_date = self.parse_and_insert(user_id, data, access_token)
        ...
        if response and response.get('data'):
          has_data = True
          self.parse_and_insert(user_id, response.get('data'), access_token, True)
        return has_data
    except Exception as e:
      self.logger.exception("BusinessMediaFetcher: error while processing %s"%user_id)
      raise e

  
  def process_business_comments(self, media_id, access_token):
    try:
        self.logger.info("##############################################")
        ...
        while paginate:
          ...
          if response and response.get('data'):
            ...
            self.parse_and_insert_comments(media_id, data)
      ...
      self.logger.exception("BusinessMediaFetcher: error while processing comments for %s"%media_id)
      raise e
  ...
  def get_users(self):
    ...
    try:
      ...
      self.logger.exception("BusinessMediaFetcher: error while getting users from DB")
      raise e
    return users

  
  ### Parse and insert a page of media
  def parse_and_insert(self, user_id, data, access_token, is_story=False):
    min_date = None
    if len(data) == 0:
      return
    try: 
      ...
      for media_id in media_ids:
        self.process_business_comments(media_id, access_token)
    except Exception as e:
      self.logger.exception("BusinessMediaFetcher: error while parsing or inserting %s"%media_id)
      raise e
    return min_date

  
  ### Parse and insert a page of comments
  def parse_and_insert_comments(self, media_id, data):
    if len(data) == 0:
      return
    try:
      with self.utils.get_conn('ig', False) as conn:
      ...
      self.logger.exception("BusinessMediaFetcher: error while parsing or inserting comments for %s"%media_id)
      raise e
    return True
