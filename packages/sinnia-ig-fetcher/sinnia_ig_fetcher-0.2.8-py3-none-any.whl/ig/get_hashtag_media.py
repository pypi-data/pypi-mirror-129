...
class HashtagsMediaFetcher:
  ...
  def process_all(self):  
    try:
      ...
      for item in hashtags:
        ...
        hashtag_id = item[2]
        access_token = item[3]
        self.process_hashtag_media(conn, user_id, hashtag, hashtag_id,       
                                   access_token, self.date_limit)
      conn.close()
    except:
  ...
  def process_hashtag_media(self, conn, user_id, hashtag, hashtag_id, 
                            access_token, date_limit=None):
    ...
    try:
      ...
      # Insert data
      self.parse_and_insert(conn, all_data, hashtag)
    except:
  ...
  def parse_and_insert(self, conn, data, hashtag):      
    cursor = conn.cursor()
    cursor.execute('SET character_set_connection=utf8mb4;')
    for item in data:
...
def config_fetcher_as_module():
  ...
  print('HashtagsMediaFetcher loading conf from resource_package='
        + resource_package + ' resource_path=' + resource_path)
  conf_file = pkg_resources.resource_filename(resource_package, resource_path)
  print('HashtagsMediaFetcher loading conf ' + str(conf_file))  return (logger, conf_file)
