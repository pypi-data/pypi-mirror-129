...
class BusinessMediaInsightsFetcher:
  ...
  def __init__(self, args = None):
    try:    
      if is_imported:
        ...
        print('BusinessMediaInsightsFetcher init as main')
  ...
  def process_all(self):
    ...
    try:    
      ...
      for item in media:
        ...
        media_story = item[3]
        access_token = item[4]
        self.process_business_media_insights(user_id, media_id, media_type, access_token)
        if media_story == 1:
          self.process_business_media_insights(user_id, media_id, 'story', access_token)
    except Exception as e:
      self.logger.exception('BusinessMediaInsightsFetcher: error processing all')
      raise e
    finally:
  ...
  def process_user(self, user_id):
    ...
    try:    
      ...
      self.logger.exception('BusinessMediaInsights: error processing user')
      raise e
    finally:
      self.logger.info('Total time: %s' % (datetime.now() - exec_start_time))

    
  def process_business_media_insights(self, user_id, media_id, media_type, access_token):
    ...
    if response:
      try:
          self.parse_and_insert(user_id, media_id, response.get('data'))
      except Exception as e:
          self.logger.exception("BusinessMediaInsights: error processomg user")
          raise e
  ...
  def get_media(self, user_id=None, days=5):
    media = list()
    try:      
      with self.utils.get_conn('ig', False) as conn:
        if user_id is None:
          ...
          fields = {"user_id":  user_id, "days": days}
      ...
      self.logger.exception("BusinessMediaInsights: error while getting media from DB")
      raise e
    return media


  def parse_and_insert(self, user_id, media_id, metrics):  
    try:
      with self.utils.get_conn('ig', False) as conn:
        cursor = conn.cursor()  
      ...
      self.logger.exception("BusinessMediaInsights: error while parse_and_insert for %s"%user_id)
      raise e
  

### If used as main: set Stream+File log handlers and local config file
def config_fetcher_as_main():
  print('BusinessMediaInsightsFetcher setting Stream+File log handlers and local config file.')
  ...
  # add file handler
  log_file = 'logs/get_business_media_insights.log'
