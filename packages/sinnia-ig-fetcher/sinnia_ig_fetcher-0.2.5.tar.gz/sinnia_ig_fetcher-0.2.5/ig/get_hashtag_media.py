...
class HashtagsMediaFetcher:
  ...
  def process_hashtag_media(self, conn, user_id, hashtag, hashtag_id, 
    ...
    self.logger.debug("Getting limit media for '%s' (date %s)" % (hashtag, self.date_limit))
    limit_media = self.get_limit_media(conn, hashtag, self.date_limit)
    try:
      all_data = []
      ...
      print("Count after top_media for %s: %s" % (hashtag, len(all_data)))
      ...
      print("Count after recent_media for %s: %s" % (hashtag, len(all_data)))
      # Insert data
  ...
  def get_hashtag_id(self, conn, user_id, hashtag, access_token):
    ...
    if response and response["data"]!=None and len(response["data"])>0:
      ...
      print("Updated hashtag id for '%s': %s" % (hashtag, str(hashtag_id)))
    return hashtag_id
  ...
  def parse_and_insert(self, conn, data, hashtag):      
    ...
    for item in data:
      ...
      conn.commit()
    print("Inserted %s media for hashtag '%s'" % (len(data), hashtag))
