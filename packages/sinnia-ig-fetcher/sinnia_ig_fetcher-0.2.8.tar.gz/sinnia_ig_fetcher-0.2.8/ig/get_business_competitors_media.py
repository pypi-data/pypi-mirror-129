  ...
  def process_all(self):
    try:
      ...
      for item in users:
        ...
        access_token = item[2]
        self.process_competitors_media(conn, user_id, competitor_name, access_token)
      conn.close()
    except:
...
#   r = requests.post(url, body)
  ...
  ### Main function
  def process_competitors_media(self, conn, user_id, competitor, access_token):
    try:
      self.logger.info("##############################################")
...
def config_fetcher_as_module():
  ...
  print('CompetitorsMediaFetcher loading conf from resource_package='
        + resource_package + ' resource_path=' + resource_path)
  conf_file = pkg_resources.resource_filename(resource_package, resource_path)
  print('CompetitorsMediaFetcher loading conf ' + str(conf_file))
