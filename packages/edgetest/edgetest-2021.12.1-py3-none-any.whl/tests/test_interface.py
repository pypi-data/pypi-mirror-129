...
def test_cli_reqs(mock_popen, mock_cpopen, mock_builder):
    ...
    assert mock_popen.call_args_list == [
        call(
            (f"{str(loc)}/.edgetest/myupgrade/bin/python", "-m", "pip", "install", "."),
            stdout=-1,
            universal_newlines=True,
        ),
        call(
            (
                f"{str(loc)}/.edgetest/myupgrade/bin/python",
                "-m",
                "pip",
                "install",
                "myupgrade",
        ...
        call(
            (
                f"{str(loc)}/.edgetest/all-requirements/bin/python",
                "-m",
                "pip",
                "install",
                ".",
    ...
    assert mock_cpopen.call_args_list == [
        call(
            (f"{str(loc)}/.edgetest/myupgrade/bin/python", "-m", "pytest"),
            universal_newlines=True,
        ),
        call(
            (f"{str(loc)}/.edgetest/all-requirements/bin/python", "-m", "pytest"),
            universal_newlines=True,
        ),
    ]
