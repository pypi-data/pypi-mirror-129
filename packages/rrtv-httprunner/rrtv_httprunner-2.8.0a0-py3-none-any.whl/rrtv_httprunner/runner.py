...
class HttpRunner(object):
    ...
    def build_api_testcase_config(self, _id, config: TConfig):
        ...
        variables = json.dumps(config.variables)
        parameters = json.dumps(globalvar.get_value("parameters", None)) if globalvar.get_value("parameters",
                                                                                                None) is not None else ""
        export = json.dumps(config.export)
        datasource = json.dumps(config.datasource)
    ...
    def build_project_meta(self, _id):
        ...
        rd.str_set("Python::Off::Datasource", json.dumps(self.__project_meta.datasource))
        ...
        def debugtalk():
            ...
            if os.path.exists(path):
                with open(path, 'r', encoding='utf-8') as f:
                    debugtalk_py = f.read()
                    rd.str_set("Python::Off::Debugtalk", json.dumps(debugtalk_py))
                ...
                for path in self.__project_meta.custom_path:
                    ...
                    if os.path.exists(whole_path):
                        with open(whole_path, 'r', encoding='utf-8') as f:
                            custom_py = f.read()
                            if custom_py != "":
                                rd.hash_set("Python::Off::Custom", path, json.dumps(custom_py))
    ...
    def build_api_testcase_step(self, _id, parent_id, teststeps: List[TStep], cite: bool):
        ...
        for index, step in enumerate(teststeps):
            ...
            if step.testcase is None:
                api = ""
                middleware_begin = json.dumps(step.begin)
                begin_hooks = json.dumps(step.begin_hooks)
                setup_hooks = json.dumps(step.setup_hooks)
                url = step.request.url if step.request.url is not None else ""
                request = json.dumps(step.request.__dict__)
                name = step.name
                extract = json.dumps(step.extract)
                tear_down_hooks = json.dumps(step.teardown_hooks)
                validate = json.dumps(step.validators)
                middleware_end = json.dumps(step.end)
                end_hooks = json.dumps(step.end_hooks)
                md5 = md5Step(api, sequence, middleware_begin, begin_hooks, setup_hooks, url, request, name, extract,
                              tear_down_hooks,
                              validate, middleware_end, end_hooks)
                md5_list.append(md5)
                # 判断该用例库中是否存在 用例不存在新增
                check = db.query("select * from t_api_testcase_step where md5=%s;", md5)
                if check is None:
                    sql = "insert into t_api_testcase_step (case_id, name ,api, testcase, sequence, middleware_begin, begin_hooks, setup_hooks, url, request, extract, tear_down_hooks, validate, middleware_end, end_hooks,md5, created_by, updated_by) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                    db.insert(sql, (
                        _id, name, api, parent_id, sequence, middleware_begin, begin_hooks, setup_hooks, url, request,
                        extract,
