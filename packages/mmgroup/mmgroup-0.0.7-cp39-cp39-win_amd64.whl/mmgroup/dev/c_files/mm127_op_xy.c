...
static const uint32_t TABLE24_START[4] = {
   MM_OP127_OFS_X, MM_OP127_OFS_Z, MM_OP127_OFS_Y
};
...
// %%EXPORT px
void mm_op127_omega(uint_mmv_t *v, uint32_t d)
...
// 0, -1, \Omega or -\Omega.
{
    ...
    for (i0 = 0; i0 < 8; i0 += 4) {
        ...
        for (i1 = 0; i1 < 2048; ++i1) {
            // %%FOR j in range(V24_INTS_USED)
            pv[0] ^=  0x7f7f7f7f7f7f7f7fULL;
            pv[1] ^=  0x7f7f7f7f7f7f7f7fULL;
            pv[2] ^=  0x7f7f7f7f7f7f7f7fULL;
            // %%END FOR
            pv[3] = 0;
            pv +=  4;      
        }
...
// %%EXPORT px
void mm_op127_y_tag_A(uint_mmv_t *v, uint32_t f)
{
        ...
        mask[1][0] = 0x7f7f7f7f7f7f7f7fULL;
        // %%END IF
        // %%IF i < V24_INTS_USED - 1 or 24 % INT_FIELDS == 0
        mask[1][1] = 0x7f7f7f7f7f7f7f7fULL;
        // %%END IF
        // %%IF i < V24_INTS_USED - 1 or 24 % INT_FIELDS == 0
        mask[1][2] = 0x7f7f7f7f7f7f7f7fULL;
        // %%END IF
        for (i = 0; i < 3; ++i) {
            x = f >> (i << 3);
            // %%MMV_UINT_SPREAD x, x
