...
class Properties(object):
    ...
    def _string(self, prop, notr=None):
        ...
        return QtWidgets.QApplication.translate(
            self.uiname, text, prop.get("comment"), -1
        )
    ...
    def _palette(self, prop):
        palette = self.factory.createQObject(
            "QPalette", "palette", (), is_attribute=False
        )
    ...
    def _brush(self, prop):
        ...
        if brushstyle in (
            ...
            brush = self.factory.createQObject(
                "QBrush", "brush", (gradient,), is_attribute=False
            )
        else:
            color = self._color(prop[0])
            brush = self.factory.createQObject(
                "QBrush", "brush", (color,), is_attribute=False
            )
    ...
    def _sizepolicy(self, prop, widget):
        ...
        sizePolicy = self.factory.createQObject(
            "QSizePolicy", "sizePolicy", (hsizetype, vsizetype), is_attribute=False
        )
