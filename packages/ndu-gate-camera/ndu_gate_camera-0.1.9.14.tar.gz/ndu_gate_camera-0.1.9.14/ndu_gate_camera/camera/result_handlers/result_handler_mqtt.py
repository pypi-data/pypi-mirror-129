...
class ResultHandlerMqtt:
    ...
    def update_camera_configuration(self, obj):
        try:
            if list(obj['data'].keys())[0].endswith('.json') and list(obj['data'].keys())[0].startswith('custom') is False:
    ...
    def post_frame(self, f):
        ...
        try:
            ...
            response = requests.post('http://' + self.host + ':9090/api/v1/' + self.access_token + '/attributes',
            ...
            for result in results:
                ...
                for key in data:
                    if data[key] is not None:
                        # if key == "frame":
                        #     #self.send_attribute(device, results)
                        #     #self.send_frame(device, data)
                        #     #await asyncio.run(self.send_frame(device, data))
                        #     #asyncio.create_task(self.send_frame(device, data))
                        #     self.save_result([{"data": {"frame": data[key]}}], device=device, data_type='attribute')
                        # else:
