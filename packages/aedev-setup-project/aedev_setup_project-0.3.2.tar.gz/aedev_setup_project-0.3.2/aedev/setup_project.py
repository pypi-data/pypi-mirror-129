...
.. hint::
    ...
    :mod:`grm <aedev.git_repo_manager>` tool with the `info` action command line argument).
...
namespaces `aedev <https://gitlab.com/aedev-group/projects>` and `ae <https://gitlab.com/ae-group/projects>`.
...
the order of preference - the first has the highest priority, the last the lowest):
    ...
    2. provide a `setup-hook module <aedev.setup_hook>`.
...
def _load_requirements(pev: PevType):
    ...
    pev['setup_require'] = [_ for _ in ('ae_base', 'aedev_setup_project') if _ != package_name]
