...
class Optimizer:
    ...
    def _progress_factor(self):
        ...
        if self.maximum_evaluations:
            prog.append(self.evaluations_count / self.maximum_evaluations)
        return max(prog)  
    ...
    def _stopping_criteria(self):
        ...
        if self.maximum_stalled_iterations:
            ...
            it_stall = np.size(np.where(f_hist==f_hist[-1]))
            if it_stall >= self.maximum_stalled_iterations:
                if self.monitoring == 'basic':
        ...
        if self.maximum_stalled_evaluations:
            f_hist = np.array([r[2].f for r in self.results.cHistory])
            it_stall = np.size(np.where(f_hist==f_hist[-1]))
            eval_stall = self.evaluations_count - self.results.cHistory[-it_stall][1]
            if eval_stall >= self.maximum_stalled_evaluations:
                if self.monitoring == 'basic':
