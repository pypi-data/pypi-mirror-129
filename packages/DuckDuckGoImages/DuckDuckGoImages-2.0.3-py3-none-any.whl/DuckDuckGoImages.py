...
def download(query, folder='.', max_urls=None, thumbnails=False, parallel=False, shuffle=False, remove_folder=False, licence=ALL):
    if thumbnails:
        urls = get_image_thumbnails_urls(query, licence)
    else:
        urls = get_image_urls(query, licence)
...
def get_image_urls(query, licence):
    token = _fetch_token(query)
    return _fetch_search_urls(query, token, licence)

def get_image_thumbnails_urls(query, licence):
    token = _fetch_token(query)
    return _fetch_search_urls(query, token, licence, what="thumbnail")
...
def _fetch_search_urls(query, token, URL="https://duckduckgo.com/", what="image", licence=ALL):
    query = {
        "vqd": token,
        "q": query,
        "l": "wt-wt",
    ...
    if licence and licence in _licences:
