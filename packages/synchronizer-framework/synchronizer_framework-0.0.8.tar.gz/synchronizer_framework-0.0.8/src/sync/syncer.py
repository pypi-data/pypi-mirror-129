    ...
    def _update_linked_entity(
        ...
        def is_dirty(entity):
            try:
                ...
                logging.warn(f"asked to read {mapping.dirty_attribute_name} from {entity} but no such attribute exists")
                raise e

        if mapping.dirty_mode == DirtyMode.UPDATE and not is_dirty(from_entity):
            return operations
        ...
        for to_entity in others:
            ...
            if mapping.dirty_mode == DirtyMode.DO_NOT_OVERWRITE and is_dirty(to_entity):
                continue
