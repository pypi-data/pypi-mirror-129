...
def model_service_ga_build(data, config_params):
    ...
    logger.info("insert_tables count_times:" + str((end - start).seconds * 1000))
    ...
    logger.info("update_subgraph_by_sql count_times:" + str((end - start).seconds * 1000))
    graphNodes = []
    # 开始时间
    start = datetime.datetime.now()
    ...
    logger.info("graphNodes.append count_times:" + str((end - start).seconds * 1000))
    graphEdges = []
    # 开始时间
    start = datetime.datetime.now()
    ...
    logger.info("graphEdges.append count_times:" + str((end - start).seconds * 1000))
    GraphSet = {}
    GraphSet["pathList"] = {}
    GraphSet["columns"] = []
    GraphSet["rowList"] = []
...
def insert_vertexes_data(subgraph_name, graphNodes):
    pass
...
def convert_def_node(graphNodes):
    ...
    for graphNode in graphNodes:
        ...
        # schema = defdata.iloc[:, :2].columns.values.tolist()
        # field_data = defdata.iloc[:, :2].values.tolist()
