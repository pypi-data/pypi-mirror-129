    ...
    def dda_ms1_and_ms2(ms1, ms2, rt, mz_exp):
        ...
        for scan1 in ms1:
            if scan1.scan_time[0] > rt:
                break
        for scan2 in ms2:
            if (scan2.selected_precursors[0]['mz'] > mz_exp - 0.015) & (
                    scan2.selected_precursors[0]['mz'] < mz_exp + 0.015) & (scan2.scan_time[0] > rt):
                break
        spec1, spec2 = pd.Series(data=scan1.i, index=scan1.mz, name=scan1.scan_time[0]), pd.Series(data=scan2.i,
                                                                                                   index=scan2.mz,
                                                                                                   name=scan2.scan_time[
                                                                                                       0])
        return spec1, spec2
...
def fold_change_filter(path, fold_change=5, area_threhold=500):
    ...
    blk_files = [file for file in area_files if 'blank' in file or 'Control' in file or 'control' in file]
    ...
    area_files_sample = [file for file in area_files if
                         'blank' not in file and 'control' not in file and 'Control' not in file]
    for i in tqdm(range(len(area_files_sample)), desc='Fold change processing'):
        print(area_files_sample[i])
        ###基于峰面积的对比拿到比较数据
        sample = pd.read_excel(area_files_sample[i], index_col='Unnamed: 0')
        compare = pd.concat((sample, final_blk), axis=1)
        compare_result1 = compare[compare[compare.columns[0]]
                                  > fold_change * compare['blk']].sort_values(by=compare.columns[0], ascending=False)
        ...
        new_name = area_files_sample[i].replace('_final_area', 'unique_cmps')  ### 文件输出名称
