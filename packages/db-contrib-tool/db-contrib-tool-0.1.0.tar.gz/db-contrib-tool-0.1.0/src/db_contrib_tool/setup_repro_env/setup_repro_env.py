...
from db_contrib_tool.plugin import PluginInterface, Subcommand
...
class SetupReproEnv(Subcommand):
    ...
    def get_urls(self, version: str, buildvariant_name: Optional[str] = None) -> EvgURLInfo:
        ...
        evg_version = evergreen_conn.get_evergreen_version(
            multiversionconstants.EVERGREEN_PROJECTS, self.evg_api, version
        )
        if evg_version is None:
            ...
            evg_version = evergreen_conn.get_evergreen_version(
                multiversionconstants.EVERGREEN_PROJECTS, self.evg_api, commit_hash
            )
        if evg_version is None:
            return EvgURLInfo()
        ...
        for url in [artifacts_url, binaries_url, symbols_url, python_venv_url]:
            if url is not None:
                ...
                try:
                    ...
                    LOGGER.warning(
                        "Setting up tarball failed with error, retrying once...", error=err
                    )
                    time.sleep(1)
                    try_download(url)
