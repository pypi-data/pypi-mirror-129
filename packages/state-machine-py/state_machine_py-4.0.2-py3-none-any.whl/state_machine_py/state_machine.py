    ...
    def leave(self, line):
        ...
        if key in self._transition_dict:
            ...
            if self.state.name in self._transition_dict:
                current_state_dict = self._transition_dict[self.state.name]

                if edge_name in current_state_dict:
                    next_state_name = current_state_dict[edge_name]

                    if self.verbose:
                        print(
                            f"[state_machine] Leave {self.state.name} {edge_name} {next_state_name}")
