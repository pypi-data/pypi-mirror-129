        ...
        def _set_referer(r):
            if isinstance(r,scrapy.Item):
                ...
                if response.headers.get('Set-Cookie'):
                    # headers[b'cookie'] = '; '+response.headers['Set-Cookie'].decode("utf-8") if headers.get('cookie').decode("utf-8") else response.headers['Set-Cookie'].decode("utf-8")
                    headers['cookie'] = headers.get('cookie','').decode("utf-8")+'; '+response.headers.get('Set-Cookie','').decode("utf-8") \
                        if headers.get('cookie','').decode("utf-8") else response.headers.get('Set-Cookie','').decode("utf-8")
