...
class geomoptCls(object):
    ...
    def gau_geomopt(self, Gaussianbin):
        ...
        for args in args2pass:
            if 'mndokeywords' in args:
                fkw = args.split('=')[1]
    ...
    def ase_opt(self, fmax, steps, optimizer='LBFGS'):
        ...
        xyz_final_file = 'final_geo.xyz'
        for i in range(len(self.atoms)):
            traj = f'mol_{i+1}_geo.traj'
            calculator = MLatomCalculator(i)
            ...
            io.write(xyz_final_file, self.atoms[i], format='extxyz', plain=True, append=True)
    ...
    def ase_freq(self):
        ...
        for i in range(nmol):
            ...
            if re.search('aiqm1', ''.join(args2pass), flags=re.IGNORECASE):
                import AIQM1
                charges, mults = AIQM1.AIQM1Cls(args2pass).forward(i)
                mult = mults[0]
            else:   
...
def write_gau_inp(imol, element, coord, charge, spin, task='opt'):
    ...
    # ts   -> cmd = #p opt(ts,noeigen,nomicro) external='python ...'
    # irc  -> cmd = #p irc(maxpoints=20) external='python ...'
    if   task == 'opt':
        cmd = "#p opt(nomicro) external='%s %s/geomopt.py'\n" % (pythonbin, mlatomdir)
    elif task == 'freq':
        cmd = "#p freq external='%s %s/geomopt.py'\n" % (pythonbin, mlatomdir)
    #elif task == 'ts':
    #    cmd = "#p opt(ts,noeigen,nomicro) external='%s %s/geomopt.py'\n" % (pythonbin, mlatomdir)
    #elif task == 'irc':
    #    cmd = "#p irc(maxpoints=20) external='%s %s/geomopt.py'\n" % (pythonbin, mlatomdir)
...
class args(object):
    ...
    def parse(cls, argsraw):
        if 'geomopt' in ' '.join(argsraw).lower():
            cls.opttask = 'opt'
        elif 'freq' in ' '.join(argsraw).lower():
            cls.opttask = 'freq'
        deadlist1 = []; deadlist2 = []; deadlist3 = []
        for arg in argsraw:
            flagmatch1 = re.search('(^hyperopt)|(^setname=)|(^learningcurve$)|(^lcntrains)|(^lcnrepeats)|(^deltalearn)|(^yb=)|(^yt=)|(^yestt=)|(^nlayers=)|(^selfcorrect)|(^geomopt)|(^freq)|(^mlprog)|(^initBetas)', arg, flags=re.UNICODE | re.MULTILINE | re.DOTALL | re.IGNORECASE)
