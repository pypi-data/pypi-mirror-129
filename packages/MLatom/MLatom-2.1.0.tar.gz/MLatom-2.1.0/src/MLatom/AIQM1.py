...
class AIQM1Cls(object):
    ...
    def predict(self, device, d4, sae, optmol):
        
        numbers, coordinates = get_xyz(self.xyzfile)
        charges, mults, odm2_energies, odm2_grads, odm2_hesses = run_odm2_calculation(self.mndobin, numbers, coordinates, self.hesscalc, self.mndokw, optmol)
        ...
        for i in range(len(numbers)):
            ...
            delta_energy = torch.stack([e for e in delta_energies], dim=0).mean(0)
    ...
    def forward(self, optmol=None):
        # optmol is used for geomopt
        if self.level == 'dft':
            sae = self.dftsae
        elif self.level == 'cc':
            sae = self.ccsae
        self.define_aev()
        self.load_models()
        charges, mults = self.predict(self.device, self.d4, sae, optmol)
        return charges, mults
...
def run_odm2_calculation(mndobin, numbers, coordinates, hesscalc, mndokw, optmol):
    charges = np.full(len(numbers), 0)
    mults = np.full(len(numbers), 1)
    energies = []; grads = []; hesses = []
    if mndokw: 
        mndokeywords, charges, mults = read_mndokw(mndokw, optmol)
    ...
    for i in range(len(mndokeywords)):
        ...
        if status:
            ...
            if imult == 0:
                ...
                mults[i] = imult
    if optmol is not None:
        mndokeywords = [mndokeywords[optmol]]
        charges = [charges[optmol]]
        mults = [mults[optmol]]
