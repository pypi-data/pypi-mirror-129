...
module A_KRR
  ...
  use dataset,       only : arrayOfArrays, Nprops, NgradXYZmax!, Yest, NitemsTot
...
subroutine init_KRR()
!==============================================================================
  use A_KRR_kernel,  only : getSigma, getC, getD, getNN
  use optionsModule, only : option
...
!==============================================================================
  ...
  if (option%calcVal) then
    ...
    do i=1, NestIndices
      Yest(indicesForEst(i)) = Yest_KRR(indicesForEst(i), alpha(:,1))
    end do
    !$OMP END PARALLEL DO
  end if
