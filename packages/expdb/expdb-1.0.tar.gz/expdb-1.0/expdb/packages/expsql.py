"""Exporting tables from databases in SQL file format

This module allows the user to export tables (single, multiple, all)
existing in a database as a `.sql` filetype.
"""
...
class SQL:
    """Class for exporting tables from a database in SQL format
    ...
    def exportone(self, table: str, path: str = "~") -> bool:
        ...
        """
        try:
            PATH = os.path.join(os.path.expanduser(path), f"{self.database}.{table}.sql")
            COMMAND = f"mysqldump -u{self.username} -p{self.password} {self.database} {table} > {PATH}"
            os.system(COMMAND)
            return True
    ...
    def exportdb(self, path: str = "~") -> bool:
        """Exports the database as a whole
        ...
        """
        try:
            PATH = os.path.join(os.path.expanduser(path), f"{self.database}.sql")
            COMMAND = f"mysqldump -u{self.username} -p{self.password} {self.database} > {PATH}"
            os.system(COMMAND)
            return True
