"""Exporting tables from databases in CSV file format

This module allows the user to export tables (single, multiple, all)
existing in a database as a `.csv` filetype.
"""
...
class CSV:
    """Class for exporting tables from a database in CSV format
    ...
    Methods
    -------
    exportone(table, path = "~", delimiter = ",")
        Export a single table from the database
    exportmany(tables, path = "~", delimiter = ",")
        Export multiple tables from the database
    exportall(path = "~", delimiter = ",")
    ...
    def exportone(self, table: str, path: str = "~", delimiter: str = ",") -> bool:
        ...
        delimiter : str, optional
            Delimiter for separating data (default is `,`)
        ...
        try:
            ...
            PATH = os.path.join(os.path.expanduser(path), f"{self.database}.{table}.csv")
            with open(PATH, mode = "w", newline = "") as export_file:
                WRITER = csv.writer(export_file, delimiter = delimiter)
    ...
    def exportmany(self, tables: List[str], path: str = "~", delimiter: str = ",") -> bool:
    ...
    def exportall(self, path: str = "~", delimiter: str = ",") -> bool:
