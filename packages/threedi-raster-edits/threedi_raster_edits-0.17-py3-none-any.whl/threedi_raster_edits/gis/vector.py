...
class Vector:
    ...
    def set_table(self):
        ...
        if not hasattr(self, "_table"):
            ...
            for feature in Progress(self, "table", quiet=False):
    ...
    def intersects(self, geometry, fids, method="intersect", quiet=True):
        ...
        for fid in Progress(fids, method, quiet):
            feature = self[fid]
            if method == "intersect":
...
def create_index(layer, quiet=True, path=None):
    ...
    layer.ResetReading()
    for feature in Progress(layer, "Rtree Index", quiet):
        if feature:
...
def multiparts_to_singleparts(in_layer: ogr.Layer, quiet=True):
    ...
    in_layer.ResetReading()
    for count, feature in Progress(in_layer, "Multi2single", quiet, enum=True):
        items = feature.items()
        geometry = feature.GetGeometryRef()
...
def difference(
    ...
    in_layer.ResetReading()
    for in_feature in Progress(in_layer, "Difference"):
        ...
        if in_feature:
            ...
            difference = None
            for loop, mask_feature in Progress(
                mask_features,
