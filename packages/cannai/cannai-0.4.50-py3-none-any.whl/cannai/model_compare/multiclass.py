...
color_list = ["g", "r", "b", "y", "m", "c","k", "deeppink", "darkseagreen", "tomato", "darkslategray","darkred","yellow","aqua","indigo"]
...
def multiclass_base(C_mod, key_list, tl, addtional_model):
    ...
    if len(key_list) > 15:
        raise ImportError("maximum number of keys inputs is 15, but " + str(len(key_list)) + " was input")
    try:
...
def multiclass_base_group_sub(C_mod, group_list):
    ...
    if len(group_list) > 15:
        raise ImportError("maximum number of keys inputs is 15, but " + str(len(group_list)) + " was input")
    try:
        if C_mod.iscannai != True:
            raise ImportError("C_mod.iscannai is not true")
    except AttributeError:
