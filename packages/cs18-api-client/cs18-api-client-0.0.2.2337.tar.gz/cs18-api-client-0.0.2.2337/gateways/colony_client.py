...
class Colony:
    ...
    def remove_repository_from_space(self, wait_for_cache_refresh=False, deployment_type: str = None):
        method_url = self._api_root.spaces.space_repositories(space_name=self.space, deployment_type=deployment_type)
        resp = self.session.delete(url=method_url)
        GatewayUtils.handle_response(response=resp, return_codes=[200])
