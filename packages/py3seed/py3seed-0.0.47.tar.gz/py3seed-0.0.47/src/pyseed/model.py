...
from typing import no_type_check, Dict, Type, Callable, get_origin, get_args, get_type_hints, Set, Any, ForwardRef
...
class ModelMeta(ABCMeta):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):
        ...
        def _validate_annotation(field_name, field_type, level):
            """ Validate field definition. """
            # Update max level
            nonlocal max_list_level
            max_list_level = max(max_list_level, level)
            ...
            if origin is None:
                _validate_simple_annotation(field_name, field_type, level)
            # Dict
            elif origin is dict:
                ...
                #
                _validate_simple_annotation(field_name, v_type, level)
            # List
            elif origin is list:
                l_type = get_args(field_type)[0]
                if l_type is not field_type:  # Prevent deep loop for self-referencing, i.e, f: List[Self]
                    _validate_simple_annotation(field_name, l_type, level, True)
            else:
                raise SchemaError(f'{field_name}: {field_type} is not supported')

        def _validate_simple_annotation(field_name, field_type, level, is_in_list=False):
            """ Inner method to validate built-in types, SimpleEnums or sub models. """
            # ForwardRef is used for self-referencing, i.e, List['Statistic'] -> List[ForwardRef('Statistic')]
            # ForwardRef will be replaced by its inner class after first reference, however, we still need to invoke
            # model.update_forward_refs() to make sure all the ForwardRef is replaced.
            if isinstance(field_type, ForwardRef):
                pass
            elif isinstance(field_type, SimpleEnumMeta):
                enum_types = set()
                ...
                for a_n, a_t in get_type_hints(field_type).items():
                    if a_t is field_type:  # Prevent deep loop for self-referencing, i.e, f: Self
                        continue
                    #
                    _validate_annotation(a_n, a_t, level + (1 if is_in_list else 0))
        ...
        if namespace.get('__qualname__') != 'BaseModel':
            ...
            for ann_name, ann_type in annotations.items():
                ...
                if ann_name.startswith('__'):
                    ...
                    continue
                # Validate
                _validate_annotation(ann_name, ann_type, 0)
                # Create model field
        ...
        # Too many nested list will cause UI generation and business logic too complicated, so raise an error to prevent
        if max_list_level >= 3:
            raise SchemaError(f'{name} has too many nested list: {max_list_level}')
        # Create class
