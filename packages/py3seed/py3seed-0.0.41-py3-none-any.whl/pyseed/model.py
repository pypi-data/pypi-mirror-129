    ...
    def __new__(mcs, name, bases, namespace, **kwargs):
        ...
        def _validate_annotation(field_name, field_type, level):
            ...
            # List
                ...
                if l_type in AUTHORIZED_TYPES + [field_type]:  # Skip level of simple types and self-referencing
                    pass
                else:
                    level += 1
                #
                _validate_simple_annotation(field_name, l_type, level)
            else:
                raise SchemaError(f'{field_name}: {field_type} is not supported')

        def _validate_simple_annotation(field_name, field_type, level):
            """ Inner method to validate built-in types, SimpleEnums or sub models. """
            print(f'{field_name} with {field_type} and type {type(field_type)}')
            if isinstance(field_type, SimpleEnumMeta):
                ...
                for a_n, a_t in get_type_hints(field_type).items():
                    _validate_annotation(a_n, a_t, level)
