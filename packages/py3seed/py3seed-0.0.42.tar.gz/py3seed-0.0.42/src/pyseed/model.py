...
class ModelMeta(ABCMeta):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):
        ...
        #
        for base_ in reversed(bases):
            if issubclass(base_, BaseModel) and base_ is not BaseModel:
                fields.update(deepcopy(base_.__fields__))
        #
        max_list_level = 0
        ...
        def _validate_simple_annotation(field_name, field_type, level, is_in_list=False):
            ...
            if isinstance(field_type, ForwardRef):  # ForwardRef is used for self-referencing
                pass
        ...
        if namespace.get('__qualname__') != 'BaseModel':
            ...
            for ann_name, ann_type in annotations.items():
                ...
                if isinstance(ann_type, ForwardRef):
                    ann_type = globals()[ann_type.__forward_arg__]
                # Create model field
