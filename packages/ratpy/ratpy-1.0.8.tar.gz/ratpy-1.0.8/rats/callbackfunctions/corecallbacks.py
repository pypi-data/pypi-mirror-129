...
from rats.modules.session_data_tracker import SessionDataTracker
...
from rats.modules.RATS_CONFIG import *
...
def preprocessdata(click):
    ...
    for filename in os.listdir(str(packagepath / 'cache')):
        if '.txt' in filename:
            list_of_names.append(filename.split(".txt")[0])

    session_data = SessionDataTracker(data_directory = str(packagepath)+cachepath, parser=rats_parser.RatsParser)
    session_data.scan_for_files()
    session_data.parse_data_files()
    session_data.compare_data_files()
    session_data.save_session_data()

    tabledata = session_data.data_files[['file', 'log']]
    children = dash_table.DataTable(
        ...
        style_data_conditional=[{
            'if': {
                'filter_query': "{log} contains 'erroneous'",
                'column_id': 'log'
            ...
            {
                'if': {
                    'filter_query': "{log} contains 'differs'",
                    'column_id': 'log'
            ...
            {
                'if': {
                    'filter_query': "{log} contains 'No errors'",
                    'column_id': 'log'
...
def shutdown(n_clicks):
    if n_clicks is None:
        ...
        print('Rats Data Processing says closed!')
