...
@LastEditTime: 2021-11-26 11:55:12
...
class CmsBaseModel():
    ...
    def get_cms_info_list(self, place_id, page_size, page_index, order_by="id desc", app_id="", act_id=0, is_cache=True):
        """
        :description: 位置信息列表
        :param place_id：位置标识
        :param page_size：页大小
        ...
        if is_cache == True:
            page_list, total = CmsInfoModel(context=self).get_cache_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params,dependency_key=f"cms_info_list:placeid_{place_id}",cache_expire=600)
        else:
            page_list, total = CmsInfoModel(context=self).get_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
