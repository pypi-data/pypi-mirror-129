...
class baselayer:
    ...
    def backprop(self, neuroncost, alpha):
        ...
        self.preweights = [[self.weights[t][i] - self.weights[t][i] * alpha * neuroncost[i] for i in range(len(neuroncost))] for t in range(len(self.weights))]
    ...
    def loadwb(self, wb):
        self.weights = wb[0]
        self.biases = wb[1]
        if all(not all(isinstance(t, int) or isinstance(t, float) for t in i) for i in self.weights) or not all(isinstance(i, int) or isinstance(i, float) for i in self.biases):
...
class twoDpoolinglayer:
                ...
                for y in range(self.poolingdimensions[1]):
                    for x in range(self.poolingdimensions[0]):
                        ...
                        if self.activationtype == "lin":
                            ...
                            temp += Decimal(self.weights[y][x]) * neuroncost[i][t] * e ** float(self.inputs[i][t]) / (e ** float(self.inputs[i][t]) + 1) ** 2
                        elif self.activationtype == "relu":
    ...
    def update(self):
        self.weights = [[self.weights[i][t] + self.preweightchange[i][t] for t in range(self.poolingdimensions[0])]for i in range(self.poolingdimensions[1])]