...
class endlayer:
    ...
    def out(self, inputs, trueval):
        ...
        if self.costcal == None:
            self.cost = [(self.outs[i] - Decimal(trueval[i])) / len(self.outs) for i in range(len(self.outs))]
        else:
            self.cost = [(self.costcal(self.outs[i], Decimal(trueval[i]))) / len(self.outs) for i in range(len(self.outs))]
        if self.activationtype == "sig":
            self.cost = [float(self.cost[i]) * e ** float(self.outs[i]) / (e ** float(self.outs[i]) + 1) ** 2 for i in range(len(self.outs))]
        ...
        '''elif self.activationtype == "softmax":
            self.cost = [- self.cost[i] for i in range(len(self.cost))]'''
