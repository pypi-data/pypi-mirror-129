...
class FTWClient:
    ...
    async def team_add_user_or_update_role(self, team_id: int, discord_id: int, role: UserTeamRole):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{self.ftw_host}/api/v1/team/user/join", json=request_body) as resp:
                if resp.status != 200:
                    print(f"Failed ({resp.status}) to add user to team", request_body)
                    print(await resp.json())
    ...
    async def server_get_with_id(self, server_id: int):
        async with aiohttp.ClientSession() as session:
            session.headers.add("Authorization", f"{self.ftw_api_key}")
    ...
    async def server_rent(self, match_id: int, dcid: int, gametype: GameType, rcon: str, password: str, ttl_hours: int) -> int:
        request_body = {
            'match_id': match_id,
            'dcid': dcid,
