    ...
    def f_call(*args, **kwargs):
        for i in range(3):
            try:
                ...
                if i<2:
                    logger.error(f'API request got exception: {e}', exc_info=True)
                    time.sleep(1)
                else:
                    raise e
    return f_call
...
class OperetoClient(object):
    ...
    def _call_rest_api(self, method, url, data={}, error=None, timeout=(15, 30), **kwargs):
    ...
    def get_process_flow(self, pid=None):
        ...
        * *pid* (`string`) -- Identifier of an existing process
        ...
        """
        pid = self._get_pid(pid)
        return self._call_rest_api('get', '/processes/'+pid+'/flow', timeout=(15, 60), error='Failed to fetch process information')
    ...
    def get_process_rca(self, pid=None):
        ...
        * *pid* (`string`) -- Identifier of an existing process
        ...
        """
        pid = self._get_pid(pid)
        return self._call_rest_api('get', '/processes/'+pid+'/rca', timeout=(15, 60), error='Failed to fetch process information')
