...
def naive_content_type(fname: str):
    """
    ...
    ctypes = ['binary/octet-stream', 'image/png', 'image/jpeg', 'image/gif',
              'image/bmp', 'image/tiff', 'image/svg+xml', 'text/plain',
              'text/rtf', 'application/msword', 'application/zip',
              'audio/mpeg', 'application/pdf', 'application/x-gzip',
    ...
    for itor in ctypes:
        ...
        if fname.endswith(flag):
            return itor
    if fname.endswith('svg') or fname.endswith('xml'):
