...
def upload_s3_file(file_name, oss_path, *args, **kwargs):
    ...
    s3_client = boto3.client('s3')
    try:
        response = s3_client.upload_file(file_name, bucket, object_name, *args, **kwargs)
...
def get_object_uri_by_s3_uri(s3_uri):
    bucket_name, object_name, _ = split_path(s3_uri, protocol=protocol)
    location = boto3.client('s3').get_bucket_location(Bucket=bucket_name)['LocationConstraint']
    url = f"https://s3.{location}.amazonaws.com.cn/{bucket_name}/{object_name}"
    return url
