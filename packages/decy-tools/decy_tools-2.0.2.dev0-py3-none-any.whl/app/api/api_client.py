...
class ApiClient:
    ...
    def _make_api_call_base(self, http_method, uri, data=None):
        ...
        if data:
            response = session_method(url, data=data, timeout=5)
        else:
            response = session_method(url, timeout=5)
        return response
    ...
    def make_api_call(self, http_method, uri, data=None):
        ...
        if response.ok:
            try:
                ...
                self._logger.critical(response.text)
                return response.text
        elif response.status_code in range(400, 600):
            self._logger.critical(response.text)
            self._logger.critical(response.message)
            sys.exit()
