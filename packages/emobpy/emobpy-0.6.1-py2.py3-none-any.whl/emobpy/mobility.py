...
import numpy as np
import pandas as pd
from .constants import OPERATORS, WEEKS, RULE, CWD
from .tools import (check_for_new_function_name, _add_column_datetime, cmp, mobility_progress_bar, display_all)
...
class Mobility:
    ...
    def _select_tour(self):
        ...
        if self.Rnd_week_trips != 0:
            ...
            logger.debug("No trip day: %d", self.days)
    ...
    def run(self):
        ...
        logger.debug("###################################################")
        logger.debug("===================================================")
        logger.debug("New profile running: %s", self.name)
        logger.debug("===================================================")
        logger.debug("###################################################")
        ...
        if not self.profile.empty:
            ...
            logger.info(
                """Empty profile. ...   This occurs either because a high probability of zero trips per day or/and selected few hours for the time series (couple of days only). Make sure you change seed value from seed.txt file or add a random int as seed arg to set_seed(seed) in your script to generate different seudo-random values."""
            )
        ...
        logger.info("Elapsed time (min): " + str(self.total_time))
