...
class FfmpegProcess:
    def __init__(self, command, ffmpeg_loglevel="panic"):
        """
    ...
    def run(self, progress_handler=None, ffmpeg_output_file=None):
        ...
        duration_secs = int(input_file_info.get_duration())
        print(f"DURATION: {duration_secs}")
        ...
        try:
            while self._process.poll() is None:
                ...
                if "out_time_ms" in line:
                    print(f"Line: {line}")
                    seconds_processed = int(line.strip()[12:]) / 1000
                    print(f"Processed: {seconds_processed}")
                    seconds_increase = seconds_processed - previous_seconds_processed
                    print(f"INCREASE: {seconds_increase}")
