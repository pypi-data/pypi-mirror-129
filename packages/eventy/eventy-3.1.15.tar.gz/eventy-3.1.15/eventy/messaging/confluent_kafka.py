...
from confluent_kafka import Producer, Consumer, Message, TopicPartition, KafkaError
...
class CKStore(RecordStore):
    ...
    def _on_assign(consumer: Consumer, partitions: list[TopicPartition]) -> None:
        partitions_str = ', '.join(f'{p.topic}[{p.partition}]:{p.offset} (err:{p.error})' for p in partitions)
        logger.info(f"ASSIGNED partitions for consumer {consumer}: " + partitions_str)

    @staticmethod
    def _on_revoke(consumer: Consumer, partitions: list[TopicPartition]) -> None:
        partitions_str = ', '.join(f'{p.topic}[{p.partition}]:{p.offset} (err:{p.error})' for p in partitions)
        logger.info(f"REVOKED partitions for consumer {consumer}: " + partitions_str)

    @staticmethod
    def _on_lost(consumer: Consumer, partitions: list[TopicPartition]) -> None:
        partitions_str = ', '.join(f'{p.topic}[{p.partition}]:{p.offset} (err:{p.error})' for p in partitions)
        logger.info(f"LOST partitions for consumer {consumer}: " + partitions_str)
    ...
    def read(
        ...
        try:
            partitions_str = ', '.join(f'{p.topic}[{p.partition}]:{p.offset} (err:{p.error})' for p in self.consumer.assignment())
        ...
        for message in messages:
            if message.error():
                logger.debug(f"Message read: {message.topic()}[{message.partition()}]:{message.offset()}")
                if message.error().code() == KafkaError._PARTITION_EOF:
                    logger.info(f"Received Kafka EOF message {message.error()}.")
                else:
