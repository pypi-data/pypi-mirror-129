...
from sqlalchemy import Integer, SmallInteger, BigInteger, String, Float, Boolean, \
    Date
...
def detect_aws_type(df: pd.DataFrame) -> Dict:
    """
        ...
        Examples:
            >>> from rcd_pyutils.pandas_manager import detect_aws_type
...
def get_ser_aws_type(ser: pd.Series) -> Union[
    Type[SmallInteger], Type[Integer], Type[BigInteger], Type[Float], Type[Date], Type[Boolean], String]:
    # print(f"Parsing {ser.name} with pandas type: {ser.dtype}...")
    if (ser.dtype == pd.Int64Dtype()) | np.issubdtype(ser.dtype, int):
        return get_int_type(ser.abs().max())
    elif np.issubdtype(ser.dtype, float):
        return Float
    elif np.issubdtype(ser.dtype, np.datetime64):
        return Date
    elif np.issubdtype(ser.dtype, np.bool_):
        return Boolean
    elif np.issubdtype(ser.dtype, np.string_) | np.issubdtype(ser.dtype, np.object_):
