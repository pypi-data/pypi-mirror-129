...
def __getpass__(prompt: str, char=tuple('*'), mnbe: bool = False):
    """a"""
    ...
    while True:
            ...
            if mega:
                ...
                for i, ctrl_coded in enumerate(control1):
                    if ctrl_coded in ch_backup:
                        ...
                        if control2[i] == 'X' and len(buf) >= 1 and platform.startswith('win'): #if Ctrl + X was pressed
                            clpbCopy(str(buf))
                            buf, written, char_group = __delete__(buf)
                        elif control2[i] == 'C' and len(buf) >= 1: #if Ctrl + C was pressed
                            clpbCopy(str(buf))
                        elif control2[i] == 'V' and platform.startswith('win'): #if Ctrl + V was pressed
        ...
        if ch == '\r' and do == 0: #if Enter was pressed
            ...
            if ch_backup == "[b'\\x08']" or ch_backup == "[b'\\x7f']" or ch_backup == "['\\x08']" or ch_backup == "['\\x7f']": # if ch is backspace
                if written > 0:
