...
class ODataServiceNode(OdataNode):
    ...
    def _get_named_child(self, name: str, namespace_uri: str = None,
        ...
        if namespace_uri is None:
            products = req_product_by_name(self, name)
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            if isinstance(item, tuple):
                return self.__retrieve_child_from_tuple(item)
            if isinstance(item, str):
                return self._get_named_child(item)
            if isinstance(item, ODataCustomQuery):
                return [ODataProductNode(self, p['Id'], data=p)
                        for p in item.apply_query(self)]
    ...
    def __truediv__(self, other):
        if isinstance(other, tuple):
            if len(other) == 2:
                return self.__retrieve_child_from_tuple(other)
        if isinstance(other, str):
            return self._get_named_child(other)
        if isinstance(other, ODataCustomQuery):
            return [ODataProductNode(self, p['Id'], data=p)
                    for p in other.apply_query(self)]
        raise DrbException('Invalid type for a DrbNode bracket: '
                           f'{other.__class__}')
...
class ODataProductNode(OdataNode):
    ...
    def has_impl(self, impl: type) -> bool:
        return impl == io.BufferedIOBase or impl == io.BytesIO
