...
def download_file_kernel(fname,url):
    ...
    try:
        if global_dict['wget']:
            os.system(f'wget {url} -t2 -T 300 -O "{fname}"')
        elif global_dict['aria2']:
            tname = fname.parent / ('temp_'+fname.name)
            os.system(f'aria2c {url} -o "{tname}"')
            aname = tname.parent / (tname.name+'.aria2')
            time.sleep(0.001)
            if not aname.is_file():
                os.rename(tname,fname)
                attr['exists'] = True
        elif global_dict['nodownload']:
            #建立空文件 不进行下载
            open(fname,'wb').close()
            attr['exists'] = True
        else:
...
@click.option('--nodownload',default=False,help='不进行真实的下载',is_flag=True)
@click.option('--force',default=False,help='强制下载,忽略parsed参数',is_flag=True)
def main(args,parallel,url,include,exclude,ftypes,decode_name,convert_links,one,sort,include_html,wget,include_html_re,aria2,recursive,force,nodownload):
    if not url:
        url = args[0]
    ...
    global_dict['nodownload'] = nodownload
