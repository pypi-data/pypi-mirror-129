...
from telegram.ext import (CallbackContext, CallbackQueryHandler,
                          ...
                          MessageHandler, PicklePersistence, PollAnswerHandler,
                          PollHandler, TypeHandler, Updater)

import digitalguide
from digitalguide.pattern import (EMOJI_PATTERN, JA_PATTERN, NEIN_PATTERN,
                                  WEITER_PATTERN, WOHIN_PATTERN)
        ...
        for handler in handlers:
            if handler["handler"] == "MessageHandler":
                if handler["filter"] == "regex":
                    ...
                    newHandler = MessageHandler(
                        digitalguide.rasa_filter.FilterRasa(handler["intent"]), actions[handler["action"]])
                elif handler["filter"] == "uhrzeit":
                    newHandler = MessageHandler(
                        digitalguide.uhrzeit_filter.FilterUhrzeit(), actions[handler["action"]])
                else:
