...
def readActions(format_inhalt_list, messanger="Telegram", asset_url=""):
    ...
    for format, inhalt in format_inhalt_list:
        ...
        if format == "Textnachricht":
            ...
            match = re.search(regex, inhalt)
            if match:
                mp3_filename = os.path.join("assets/", match.group('file'))
                ogg_filename = mp3_filename.replace(".mp3", ".ogg")
                subprocess.run(["ffmpeg", '-i', mp3_filename, "-ac", "1", '-acodec', 'libopus', ogg_filename, '-y'])

                if messanger=="Telegram":
                    action["file"] = ogg_filename
                else:
                    action["url"] = os.path.join(asset_url, ogg_filename)
