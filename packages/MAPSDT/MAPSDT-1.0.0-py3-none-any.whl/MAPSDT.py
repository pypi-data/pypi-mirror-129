...
def MAPSDT(df, target_names:list, tree=None, split_portion=0.3, max_depth=5, Genetic_Progrmmaing=False, init_size=50, max_generations=50,
           save=True,
           visualizing=True):
    config = {'data': df, 'target_names': target_names, 'split_portion': split_portion, 'max_depth': max_depth,
    ...
    train_data, test_data, attribute = preprocessingData(df, target_names, split_portion)
    ...
    if visualizing:
        dtset = list(map(lambda x:x.dataset, tree.leaf))
        for i in dtset:
            leaves = list(map(lambda x:x.Decision.replace(x.Decision,config['target_names'][int(x.Decision)]), i))
