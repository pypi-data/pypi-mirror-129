...
class Http():
    ...
    def Build(self):
        self.appList = [
            ...
            ('/{0}/devices'.format(self.srv_name), DevProc, dict(sql=self.sql)),
            ('/{0}/devices/(\w+)'.format(self.srv_name), DevProcID, dict(sql=self.sql)),
        ]
        ...
        if 'HTTP_THREAD' in self.config:
            httpServer.start(self.config['HTTP_THREAD'])
        else:
            httpServer.start(1)
