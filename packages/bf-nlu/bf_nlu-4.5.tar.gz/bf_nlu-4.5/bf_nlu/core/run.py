...
from bf_nlu import model, server
...
from bf_nlu.core.interpreter import NaturalLanguageInterpreter
...
def create_http_input_channels(
    ...
    if credentials_file:
        all_credentials = bf_nlu.utils.io.read_config_file(credentials_file)
    else:
        all_credentials = {}
...
def _create_single_channel(channel, credentials) -> Any:
    ...
    if channel in BUILTIN_CHANNELS:
        ...
        try:
            input_channel_class = bf_nlu.utils.common.class_from_module_path(channel)
...
def configure_app(
    ...
    """Run the agent."""
    from bf_nlu import server

    bf_nlu.core.utils.configure_file_logging(logger, log_file)
...
def serve_application(
    ...
    bf_nlu.utils.common.update_sanic_log_level(log_file)
    ...
    app.run(
        ...
        workers=bf_nlu.core.utils.number_of_sanic_workers(
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `bf_nlu.core.run` directly is no longer supported. "
        "Please use `bf_nlu run` to start a bf_nlu server or `bf_nlu shell` to chat with "
