...
from bf_nlu.constants import MINIMUM_COMPATIBLE_VERSION, DOCS_BASE_URL, DOCS_URL_POLICIES

from bf_nlu.core import utils
from bf_nlu.core.constants import USER_INTENT_BACK, USER_INTENT_RESTART
from bf_nlu.core.actions.action import (
    ACTION_LISTEN_NAME,
    ACTION_BACK_NAME,
    ACTION_RESTART_NAME,
)
from bf_nlu.core.domain import Domain
from bf_nlu.core.events import SlotSet, ActionExecuted, ActionExecutionRejected, Event
from bf_nlu.core.exceptions import UnsupportedDialogueModelError
from bf_nlu.core.featurizers import MaxHistoryTrackerFeaturizer
...
from bf_nlu.core import registry
...
class PolicyEnsemble:
    versioned_packages = ["bf_nlu", "tensorflow", "sklearn"]
    ...
    def load_metadata(cls, path) -> Any:
        ...
        metadata = json.loads(bf_nlu.utils.io.read_file(os.path.abspath(metadata_path)))
        return metadata
    ...
    def ensure_model_compatibility(metadata, version_to_check=None) -> None:
        ...
        model_version = metadata.get("bf_nlu", "0.0.0")
        if version.parse(model_version) < version.parse(version_to_check):
            raise UnsupportedDialogueModelError(
                "The model version is too old to be "
                "loaded by this bf_nlu Core instance. "
                "Either retrain the model, or run with "
                "an older version. "
                "Model version: {} Instance version: {} "
                "Minimal compatible version: {}"
                "".format(model_version, bf_nlu.__version__, version_to_check),
                model_version,
            )
