...
from bf_nlu.nlu import components, utils  # pytype: disable=pyi-error
from bf_nlu.nlu.components import Component, ComponentBuilder  # pytype: disable=pyi-error
from bf_nlu.nlu.config import bf_nluNLUModelConfig, component_config_from_pipeline
from bf_nlu.nlu.persistor import Persistor
...
class Metadata:
    ...
    def load(model_dir: Text):
        ...
        try:
            ...
            data = bf_nlu.utils.io.read_json_file(metadata_file)
...
class Trainer:
    ...
    def _build_pipeline(
        self, cfg: bf_nluNLUModelConfig, component_builder: ComponentBuilder
    ) -> List[Component]:
    ...
    def persist(
        ...
        bf_nlu.utils.io.create_directory(dir_name)
...
class Interpreter:
    ...
    def ensure_model_compatibility(
        ...
        model_version = metadata.get("bf_nlu_version", "0.0.0")
        if version.parse(model_version) < version.parse(version_to_check):
            raise UnsupportedModelError(
                "The model version is too old to be "
                "loaded by this bf_nlu NLU instance. "
                "Either retrain the model, or run with "
                "an older version. "
                "Model version: {} Instance version: {}"
                "".format(model_version, bf_nlu.__version__)
            )
    ...
    def load(
        ...
        Args:
            ...
            component_builder: The
                :class:`bf_nlu.nlu.components.ComponentBuilder` to use.
