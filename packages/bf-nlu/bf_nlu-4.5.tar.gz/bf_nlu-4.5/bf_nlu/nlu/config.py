...
def load(
    ...
    if config is not None:
        try:
            file_config = bf_nlu.utils.io.read_config_file(config)
...
def _load_from_dict(config: Dict, **kwargs: Any) -> "bf_nluNLUModelConfig":
    if kwargs:
        config.update(kwargs)
    return bf_nluNLUModelConfig(config)
