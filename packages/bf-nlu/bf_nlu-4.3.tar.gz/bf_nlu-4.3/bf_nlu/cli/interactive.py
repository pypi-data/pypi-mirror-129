...
from bf_nlu.constants import DEFAULT_MODELS_PATH, DEFAULT_ENDPOINTS_PATH
...
def add_subparser(
    ...
    interactive_core_parser = interactive_subparsers.add_parser(
        ...
        "for a bf_nlu Core model by chatting. Uses the 'RegexInterpreter', i.e. "
        "`/<intent>` input format.",
    )
    ...
    if args.model is None:
        ...
        if not (zipped_model and os.path.exists(zipped_model)):
            utils.print_error_and_exit(
                ...
                f"found at path '{args.model}'.  Use 'bf_nlu train' to train a model."
            )
