...
from bf_nlu.core import constants, jobs, training
...
from bf_nlu.core.interpreter import NaturalLanguageInterpreter, RegexInterpreter
from bf_nlu.core.lock_store import LockStore, InMemoryLockStore
from bf_nlu.core.nlg import NaturalLanguageGenerator
from bf_nlu.core.policies.ensemble import PolicyEnsemble, SimplePolicyEnsemble
from bf_nlu.core.policies.memoization import MemoizationPolicy
from bf_nlu.core.policies.policy import Policy
from bf_nlu.core.processor import MessageProcessor
from bf_nlu.core.tracker_store import (
...
from bf_nlu.utils.common import raise_warning, update_sanic_log_level
...
def _load_and_set_updated_model(
    ...
    if nlu_path:
        from bf_nlu.core.interpreter import bf_nluNLUInterpreter

        interpreter = bf_nluNLUInterpreter(model_directory=nlu_path)
    else:
        interpreter = (
    ...
    async with model_server.session() as session:
        try:
            ...
            async with session.request(
                ...
                bf_nlu.utils.io.unarchive(await resp.read(), model_directory)
                logger.debug(
...
class Agent:
    ...
    def handle_channels(
        ...
        from bf_nlu.core import run

        raise_warning(
            "Using `handle_channels` is deprecated. "
            "Please use `bf_nlu.run(...)` or see "
            "`bf_nlu.core.run.configure_app(...)` if you want to implement "
        ...
        app.run(
            ...
            workers=bf_nlu.core.utils.number_of_sanic_workers(self.lock_store),
        )
    ...
    async def visualize(
        ...
        from bf_nlu.core.training.visualization import visualize_stories
        from bf_nlu.core.training.dsl import StoryFileReader
