...
from bf_nlu.utils.common import raise_warning, arguments_of
...
from bf_nlu.core.agent import Agent, load_agent
...
from bf_nlu.core.domain import InvalidDomain
from bf_nlu.core.events import Event
...
from bf_nlu.core.trackers import DialogueStateTracker, EventVerbosity
...
def create_app(
    ...
    async def train(request: Request) -> HTTPResponse:
        ...
        bf_nlu.utils.io.write_text_file(rjs["config"], config_path)

        if "nlu" in rjs:
            nlu_path = os.path.join(temp_dir, "nlu.md")
            bf_nlu.utils.io.write_text_file(rjs["nlu"], nlu_path)

        if "stories" in rjs:
            stories_path = os.path.join(temp_dir, "stories.md")
            bf_nlu.utils.io.write_text_file(rjs["stories"], stories_path)

        domain_path = DEFAULT_DOMAIN_PATH
        if "domain" in rjs:
            domain_path = os.path.join(temp_dir, "domain.yml")
            bf_nlu.utils.io.write_text_file(rjs["domain"], domain_path)
        ...
        try:
            ...
            from bf_nlu import train as train_model
    ...
    def validate_request(rjs):
        ...
        if "nlu" not in rjs and "stories" not in rjs:
            raise ErrorResponse(
                400,
                "BadRequest",
                "To train a bf_nlu model you need to specify at least one type of "
        ...
        if "stories" in rjs and "domain" not in rjs:
            raise ErrorResponse(
                400,
                "BadRequest",
                "To train a bf_nlu model with story training data, you also need to "
                "specify the `domain`.",
    ...
    async def evaluate_stories(request: Request):
        ...
        stories = bf_nlu.utils.io.create_temporary_file(request.body, mode="w+b")
        use_e2e = bf_nlu.utils.endpoints.bool_arg(request, "e2e", default=False)
    ...
    async def evaluate_intents(request: Request):
        ...
        nlu_data = bf_nlu.utils.io.create_temporary_file(request.body, mode="w+b")
