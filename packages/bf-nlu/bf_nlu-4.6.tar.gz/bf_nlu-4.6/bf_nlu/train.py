...
from bf_nlu.constants import DEFAULT_MODELS_PATH, DEFAULT_CORE_SUBDIRECTORY_NAME
...
async def _train_async_internal(
    ...
    """Trains a bf_nlu model (Core and NLU). Use only from `train_async`.
    ...
    if stories.is_empty():
        print_warning("No stories present. Just a bf_nlu NLU model will be trained.")
    ...
    if nlu_data.is_empty():
        print_warning("No NLU data present. Just a bf_nlu Core model will be trained.")
...
async def _train_nlu_with_validated_data(
    ...
    with ExitStack() as stack:
        ...
        _, nlu_model, _ = await bf_nlu.nlu.train(
            config,
            file_importer,
            _train_path,
