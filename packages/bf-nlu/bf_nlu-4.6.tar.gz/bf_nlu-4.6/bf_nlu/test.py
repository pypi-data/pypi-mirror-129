...
from bf_nlu.cli.utils import print_error, print_warning
...
def test_core_models_in_directory(
    ...
    from bf_nlu.core.test import compare_models_in_dir, plot_core_results
...
def _get_sanitized_model_directory(model_directory: Text) -> Text:
    """Adjusts the `--model` argument of `bf_nlu test core` when called with `--evaluate-model-directory`.

    By default bf_nlu uses the latest model for the `--model` parameter. However, for `--evaluate-model-directory` we
...
def test_core(
    ...
    import bf_nlu.model
    from bf_nlu.core.interpreter import RegexInterpreter, NaturalLanguageInterpreter
    ...
    try:
        unpacked_model = bf_nlu.model.get_model(model)
    ...
    core_path, nlu_path = bf_nlu.model.get_model_subdirectories(unpacked_model)

    if not core_path:
        print_error(
            "Unable to test: could not find a Core model. Use 'bf_nlu train' to train a "
            "bf_nlu model and provide it via the '--model' argument."
        )
    ...
    kwargs = utils.minimal_kwargs(
        additional_arguments, bf_nlu.core.test, ["stories", "agent"]
    )

    loop = asyncio.get_event_loop()
    loop.run_until_complete(
        bf_nlu.core.test(stories, _agent, out_directory=output, **kwargs)
    )
...
def test_nlu(
    ...
    from bf_nlu.model import get_model
...
def compare_nlu_models(
    ...
    from bf_nlu.core.test import plot_nlu_results
...
def perform_nlu_cross_validation(
    ...
    nlu_config = bf_nlu.nlu.config.load(config)
    data = bf_nlu.nlu.training_data.load_data(nlu)
