...
def md_format_message(text, intent, entities) -> Text:
    from bf_nlu.nlu.training_data.formats import MarkdownWriter, MarkdownReader
...
class Event:
    ...
    a conversation and tell the :class:`bf_nlu.core.trackers.DialogueStateTracker`
    ...
    def metadata(self) -> Dict[Text, Any]:
        # Needed for compatibility with bf_nlu versions <1.4.0. Previous versions
        # of bf_nlu serialized trackers using the pickle module. For the moment,
        # bf_nlu still supports loading these serialized trackers with pickle,
