...
from bf_nlu.cli import utils
from bf_nlu.cli.utils import bcolors
...
def read_global_config() -> Dict[Text, Any]:
    ...
    try:
        return bf_nlu.utils.io.read_config_file(GLOBAL_USER_CONFIG_PATH)
    except Exception:
...
def set_log_level(log_level: Optional[int] = None):
    """Set log level of bf_nlu and Tensorflow either to the provided log level or
...
def write_global_config_value(name: Text, value: Any) -> None:
    ...
    try:
        ...
        c[name] = value
        bf_nlu.core.utils.dump_obj_as_yaml_to_file(GLOBAL_USER_CONFIG_PATH, c)
