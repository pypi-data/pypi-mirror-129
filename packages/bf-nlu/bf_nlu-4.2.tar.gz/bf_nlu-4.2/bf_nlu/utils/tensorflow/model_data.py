...
from bf_nlu.utils.tensorflow.constants import BALANCED, SEQUENCE
...
class bf_nluModelData:
    ...
    def add_features(self, key: Text, features: List[np.ndarray]):
        ...
        if key in self.data:
            raise ValueError(f"Key '{key}' already exists in bf_nluModelData.")
    ...
    def _split_by_label_ids(
        ...
        for label_id in unique_label_ids:
            ...
            label_data.append(
                bf_nluModelData(self.label_key, self._data_for_ids(data, matching_ids))
            )
        return label_data
    ...
    def _check_label_key(self):
        if self.label_key is not None and (
            ...
            raise ValueError(f"Key '{self.label_key}' not in bf_nluModelData.")
    ...
    def _convert_train_test_split(
        ...
        return (
            bf_nluModelData(self.label_key, data_train),
            bf_nluModelData(self.label_key, data_val),
        )
