...
from bf_nlu.constants import DEFAULT_DATA_PATH
...
def _add_data_convert_parsers(
    ...
    from bf_nlu.nlu import convert
...
def split_nlu_data(args: argparse.Namespace) -> None:
    from bf_nlu.nlu.training_data.loading import load_data
    from bf_nlu.nlu.training_data.util import get_file_format
...
def validate_files(args: argparse.Namespace, stories_only: bool = False) -> None:
    ...
    if not all_good:
        bf_nlu.cli.utils.print_error_and_exit("Project validation completed with errors.")
