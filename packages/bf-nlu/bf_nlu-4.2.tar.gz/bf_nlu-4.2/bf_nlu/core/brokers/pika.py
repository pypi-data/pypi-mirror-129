...
class PikaEventBroker(EventBroker):
    ...
    def _get_message_properties(
        ...
        The `app_id` property is set to the value of `self.bf_nlu_environment` if
        ...
        Returns:
            `pika.spec.BasicProperties` with the `bf_nlu_ENVIRONMENT` environment variable
        ...
        if self.bf_nlu_environment:
            kwargs["app_id"] = self.bf_nlu_environment
...
def create_rabbitmq_ssl_options(
    ...
    if client_certificate_path and client_key_path:
        ...
        ssl_context = bf_nlu.server.create_ssl_context(
