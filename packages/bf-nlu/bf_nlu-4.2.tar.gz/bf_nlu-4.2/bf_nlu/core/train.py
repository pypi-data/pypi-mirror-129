...
async def train(
    ...
    from bf_nlu.core import config, utils
    from bf_nlu.core.utils import AvailableEndpoints
...
async def get_no_of_stories(story_file: Text, domain: Text) -> int:
    ...
    from bf_nlu.core.domain import TemplateDomain
...
async def do_compare_training(
    ...
    bf_nlu.utils.io.dump_obj_as_json_to_file(training_stories_per_model_file, story_range)
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `bf_nlu.core.train` directly is no longer supported. Please use "
        "`bf_nlu train` to train a combined Core and NLU model or `bf_nlu train core` "
