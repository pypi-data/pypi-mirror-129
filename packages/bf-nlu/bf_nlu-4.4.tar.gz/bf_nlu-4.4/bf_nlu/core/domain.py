...
import bf_nlu.core.constants
from bf_nlu.utils.common import (
    raise_warning,
    lazy_property,
    sort_list_of_dicts_by_first_key,
)
import bf_nlu.utils.io
from bf_nlu.cli.utils import bcolors, wrap_with_color
...
from bf_nlu.core.actions import action  # pytype: disable=pyi-error
from bf_nlu.core.actions.action import Action  # pytype: disable=pyi-error
...
from bf_nlu.core.events import SlotSet, UserUttered
from bf_nlu.core.slots import Slot, UnfeaturizedSlot, CategoricalSlot
from bf_nlu.utils.endpoints import EndpointConfig
from bf_nlu.utils.validation import InvalidYamlFileError, validate_yaml_schema
...
if typing.TYPE_CHECKING:
    from bf_nlu.core.trackers import DialogueStateTracker
...
class Domain:
    ...
    def from_file(cls, path: Text) -> "Domain":
        return cls.from_yaml(bf_nlu.utils.io.read_file(path))
    ...
    def from_yaml(cls, yaml: Text) -> "Domain":
        ...
        data = bf_nlu.utils.io.read_yaml(yaml)
    ...
    def from_dict(cls, data: Dict) -> "Domain":
        ...
        if "templates" in data:
            raise_warning(
                ...
                "no longer work in future versions of bf_nlu. Please replace "
    ...
    def _get_session_config(session_config: Dict) -> SessionConfig:
        ...
        if session_expiration_time is None:
            raise_warning(
                ...
                "session expiration time of 60 minutes in bf_nlu version 2.0 if not "
    ...
    def persist_specification(self, model_path: Text) -> None:
        ...
        bf_nlu.utils.io.create_directory_for_file(domain_spec_path)

        metadata = {"states": self.input_states}
        bf_nlu.utils.io.dump_obj_as_json_to_file(domain_spec_path, metadata)
    ...
    def load_specification(cls, path: Text) -> Dict[Text, Any]:
        ...
        specification = json.loads(bf_nlu.utils.io.read_file(metadata_path))
    ...
    def cleaned_domain(self) -> Dict[Text, Any]:
        ...
        for slot in domain_data["slots"].values():  # pytype: disable=attribute-error
            ...
            if slot["auto_fill"]:
                del slot["auto_fill"]
            if slot["type"].startswith("bf_nlu.core.slots"):
    ...
    def _actions_for_domain_warnings(self) -> List[Text]:
        ...
        from bf_nlu.core.actions.action import (  # pytype: disable=pyi-error
    ...
    def check_missing_templates(self) -> None:
        ...
        utterances = [
            ...
            if a.startswith(bf_nlu.core.constants.UTTER_PREFIX)
        ]
