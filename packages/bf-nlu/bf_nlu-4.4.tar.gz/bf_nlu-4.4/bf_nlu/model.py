...
from bf_nlu.cli.utils import print_success, create_output_path
...
from bf_nlu.core.utils import get_dict_hash
from bf_nlu.exceptions import ModelNotFound
from bf_nlu.utils.common import TempDirectoryPath
...
FINGERPRINT_bf_nlu_VERSION_KEY = "version"
...
SECTION_CORE = Section(
    ...
    relevant_keys=[
        ...
        FINGERPRINT_bf_nlu_VERSION_KEY,
    ],
)
SECTION_NLU = Section(
    name="NLU model",
...
def get_model_subdirectories(
    ...
    Args:
        unpacked_model_path: Path to unpacked bf_nlu model.
...
def create_package_bf_nlu(
    training_directory: Text,
    output_filename: Text,
    fingerprint: Optional[Fingerprint] = None,
) -> Text:
    """Create a zipped bf_nlu model from trained model files.
...
async def model_fingerprint(file_importer: "TrainingDataImporter") -> Fingerprint:
    ...
    return {
        ...
        FINGERPRINT_bf_nlu_VERSION_KEY: bf_nlu.__version__,
    }
...
def fingerprint_from_path(model_path: Text) -> Fingerprint:
    ...
    if os.path.isfile(fingerprint_path):
        return bf_nlu.utils.io.read_json_file(fingerprint_path)
    else:
        return {}
...
def persist_fingerprint(output_path: Text, fingerprint: Fingerprint):
    ...
    bf_nlu.utils.io.dump_obj_as_json_to_file(path, fingerprint)
...
def should_retrain(
    ...
    Returns:
        A FingerprintComparisonResult object indicating whether bf_nlu Core and/or bf_nlu NLU needs
...
def package_model(
    ...
    create_package_bf_nlu(train_path, output_directory, fingerprint)

    print_success(
        "Your bf_nlu model is trained and saved at '{}'.".format(
