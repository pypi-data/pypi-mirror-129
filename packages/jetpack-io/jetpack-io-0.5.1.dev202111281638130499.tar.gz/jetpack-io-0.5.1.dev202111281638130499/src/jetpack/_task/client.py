...
class Client:
    ...
    def create_task(
        ...
        crete_task_reponse = cast(
            remote_pb2.CreateTaskResponse,
            self.get_stub().CreateTask(request),
        )
    ...
    async def create_task_async(
        ...
        crete_task_reponse = await cast(
            ...
            self.get_async_stub().CreateTask(request),
        )
        return Future(self, uuid.UUID(crete_task_reponse.task_id))
    ...
    def _build_create_task_request(
        ...
        task_proto_obj = remote_pb2.Task(
            qualified_symbol=task.name(),
            encoded_args=encoded_args,
            hostname=os.environ.get("HOSTNAME", ""),  # k8s sets this
            target_time=Timestamp(seconds=int(task.target_time)),
        )
