...
class Candle_analysis:
    ...
    def filter_technicals(self, df) -> pd.DataFrame:
        return pd.DataFrame(
            {
                "open": list(df["Open Price"]),
                "close": list(df["Close Price"]),
                "high": list(df["High Price"]),
                "low": list(df["Low Price"]),
                "date": list(df["Date"]),
            }
        )

    def find_candles(self, symbol, from_date, to_date) -> pd.DataFrame:
        data = self.filter_technicals(
            self.retrieve_stocks.stock_data(symbol, from_date, to_date)
        )
        candles_found = [[] for _ in range(len(data))]
        for candle in self.candle_patterns():
            candle_weights = getattr(talib, candle)(
