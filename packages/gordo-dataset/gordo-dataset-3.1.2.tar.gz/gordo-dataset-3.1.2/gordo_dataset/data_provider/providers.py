...
import typing
from typing import Dict, Union, Any, Optional, Tuple, cast
...
from gordo_dataset.utils import capture_args, capture_args_ext

from gordo_dataset.sensor_tag import SensorTag, Tag, extract_tag_name
...
def load_series_from_multiple_providers(
    ...
    tag_list: typing.List[Union[str, SensorTag]],
    dry_run: typing.Optional[bool] = False,
    **kwargs,
) -> typing.Iterable[Tuple[pd.Series, Tag]]:
    """
    ...
    """
    readers_to_tags = cast(
        typing.Dict[GordoBaseDataProvider, typing.List[Union[str, SensorTag]]],
    ...
    for tag_reader, readers_tags in readers_to_tags.items():
        if readers_tags:
            ...
            for series, sensor_tag in tag_reader.load_series(
...
class DataLakeProvider(GordoBaseDataProvider):
    ...
    def get_metadata(self):
        storage = self._get_storage_instance()
        return {"storage_name": storage.name}
