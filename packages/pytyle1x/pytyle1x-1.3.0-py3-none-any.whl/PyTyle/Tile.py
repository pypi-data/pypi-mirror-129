...
class Tile:
	...
	def _change_margins(self, marChange, intMarChange):
		...
		if intMar != None and mar != None:
			self.help_set_margins(
				max(0, intMar + intMarChange),
				max(0, mar + marChange)
			)
	...
	# Set margins to values
	def help_set_margins(self, internal, external):
		self.state.set('internal_margin', internal)
		self.state.set('margin', external)
		self._tile()
	...
	def tile(self):
		...
		# Restore margins via awful hack
		intMar = State.recover_data('internal_margin') or 0
		mar = State.recover_data('margin') or 0
		if intMar or mar:
			self.help_set_margins(intMar, mar)
			State.save_data('internal_margin', None)
			State.save_data('margin', None)
	...
	def cycle_tiler(self):
		for i in range(len(Config.misc('tilers'))):
			if Config.misc('tilers')[i] is self.__class__.__name__:
				# get margins for safekeeping
				intMar = self.state.get('internal_margin') or 0
				mar = self.state.get('margin') or 0

				# save using State's (brand new) _DATA dict. State should not be used like this.
				State.save_data('internal_margin', intMar)
				State.save_data('margin', mar)
