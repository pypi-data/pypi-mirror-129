...
from typing import Callable, Tuple, Any, Iterable, List
...
def most_common(iterable: Iterable[Any]) -> List[Any]:
    """
    Returns a list of the most common elements in the given iterable.
    Arguments:
    iterable: Iterable[Any] - The iterable to work on
    """
    common = Counter(iterable).most_common()
    count = common[0][1]
    only_most_common = list(filter(lambda x: x[1] == count, common))
    return [element[0] for element in only_most_common]

def least_common(iterable: Iterable[Any]) -> List[Any]:
    """
    Returns a list of the least common element in the given iterable.
    Arguments:
    iterable: Iterable[Any] - The iterable to work on
    """
    uncommon = list(reversed(Counter(iterable).most_common()))
    count = uncommon[0][1]
    only_least_common = list(filter(lambda x: x[1] == count, uncommon))
    return [element[0] for element in only_least_common]
