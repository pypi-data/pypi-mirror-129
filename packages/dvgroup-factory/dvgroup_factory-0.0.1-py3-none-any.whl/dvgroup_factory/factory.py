...
class Factory:
    ...
    def info(self) -> str:
        s = "Класс DVGroupServices\n"
        s += 'Создание экземпляра:\n ins = DVGroupServices(vault_url=url, vault_token=token)\n'
    ...
    def comon_factory__(self, service_name, factory, *args, **kwargs):
    ...
    def clickhouse_client(self, **kwargs):
        if len(self.__services["clickhouse_client"]) == 0 or (("new" in kwargs) and (kwargs["new"])):
            scrts = self._get_secrets("clickhouse")
            cnfg = self.get_config_from_consul("clickhouse")
            if "new" in kwargs.keys():
                del kwargs["new"]
    ...
    def kafka_producer(self, **kwargs):
        if len(self.__services["kafka_producer"]) == 0 or (("new" in kwargs) and (kwargs["new"])):
            if "new" in kwargs.keys():
                del kwargs["new"]
            scrts = self._get_secrets("kafka")
            cnfg = self.get_config_from_consul("kafka")
    ...
    def azure_container_client_old(self, **kwargs):
        if len(self.__services["azure_container_client"]) == 0 or (("new" in kwargs) and (kwargs["new"])):
            if "new" in kwargs.keys():
                del kwargs["new"]
            scrts = self._get_secrets("ms-azure-se")
            cnfg = self.get_config_from_consul("ms-azure-se")
    ...
    def azure_container_client(self,**kwargs):
        ...
        #srvc = ContainerClient(account_url=self.config["ms-azure-se"]["account_url"], AccountName=self.secrets["ms-azure-se"]["AccountName"],
        #                       AccountKey=self.secrets["ms-azure-se"]["AccountKey"], **kwargs)
