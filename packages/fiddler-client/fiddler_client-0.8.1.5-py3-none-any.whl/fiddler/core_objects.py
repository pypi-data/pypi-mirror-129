...
class Column:
    ...
    def violation_of_nullable(self, value, fall_back=None):
        if fall_back:
            if self.name in fall_back:
                value = None if value in fall_back[self.name] else value

        if self.is_nullable is not None and self.is_nullable is False:
            return Column._value_is_na_or_none(value)
        return False

    def check_violation(self, value, fall_back=None):
        if self.violation_of_nullable(value, fall_back):
