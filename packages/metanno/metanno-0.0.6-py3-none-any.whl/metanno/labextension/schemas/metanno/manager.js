...
var toastError = function toastError(error_string) {
  //INotification.error(`Message: ${e.message} at ${parseInt(lineStr)-1}:${parseInt(columnStr)-1}`);
  _jupyterlab_toastify.INotification.error( /*#__PURE__*/_react.default.createElement("div", null, error_string.split("\n").map(function (line) {
  ...
  _defineProperty(this, "try_catch_exec", function (fn) {
    return function () {
      try {
        ...
        var _2 = _slicedToArray(_toConsumableArray(e.stack.matchAll(/<anonymous>:(\d+):(\d+)/gm))[0], 3),
            _ = _2[0],
            lineStr = _2[1],
            columnStr = _2[2];

        if (_this.sourcemap !== null) {
          var source_line_str = _this.source_code_py.split("\n")[_this.sourcemap[parseInt(lineStr) - 1][0][2]].trim();

          toastError("Error: ".concat(e.message, " at \n").concat(source_line_str));
        } else {
          toastError("Error: ".concat(e.message));
        }
      }
    };
  });
