...
class Distribution:
    ...
    def get_assignment_probability(self, a: Assignment):
        """
        Get the probability of a full assignment under the distribution.
        """
        assert (asg.is_full_assignment(a, self.all_nodes))
        return self.distribution[a]

    def get_assignments_probability(self, assignments: List[Assignment]):
        """
        Get the sum of the probabilities of a list of full assignments under the distribution.
        """
        probability = 0.0
        for a in assignments:
            probability += self.get_assignment_probability(a)
        return probability

    def get_part_assignment_probability(self, a: Assignment):
        """
        Get the probability of a partial assignment under the distribution, that is, the sum of the probabilities of
        all full assignments agreeing with the partial assignment.
        """
        return self.get_assignments_probability(asg.sum_out(a, self.all_nodes))

    def get_marginal_probability(self, node: Node):
        return self.get_assignments_probability(asg.get_node_assignments(node, self.all_nodes))
    ...
    def print_formatted(self, format_string: str, printer=print, precision=DEFAULT_PRECISION):
        """
        Print distribution in the formats specified by the format string: (M) marginal node probabilities,
