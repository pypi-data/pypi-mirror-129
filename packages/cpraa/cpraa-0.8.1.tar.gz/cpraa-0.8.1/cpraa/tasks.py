...
def ls_constraints(ls: LinearSolver, semantics: List[Semantics]):
    ...
    for sem in semantics:
        try:
            ...
            raise LinearConstraintError("No linear constraints available (or implemented) for semantics '"
                                        + str(sem) + "'") from None
...
def get_constraints_from_semantics(semantics: List[Semantics], si: SolverInstance):
    semantics_constraints_timer = Timer(output_string="Generating constraints for all semantics took")
    constraints = []
    for sem in semantics:
...
def get_model(si: SolverInstance, semantics: List[Semantics], constraints=None):
    ...
    if not constraints:
        constraints = []
    constraints.extend(get_constraints_from_semantics(semantics, si))
...
def get_n_distributions(si: SolverInstance, semantics: List[Semantics], n: int) -> List[Distribution]:
    """
    Compute up to `n` distributions satisfying the semantics' constraints.

    :param si: the interface to the smt solvers
    :param semantics: a list of semantics
    :param n: the maximum number of distributions to compute
    :return: one distribution if one exists
    """
    solver = si.get_solver()
    new_constraint = smt.And(get_constraints_from_semantics(semantics, si))
    distributions = []
    num_dist = 0
    while num_dist < n:
        solver.add_assertion(new_constraint)
        sat = solver.solve()
        if sat:
            ...
            num_dist += 1
            new_constraint = smt.Not(si.distribution_to_constraint(distribution))
        else:
            break
...
def get_all_satisfying_labelings(si: SolverInstance, lab: LabelingScheme, semantics: List[Semantics]) -> Set[Labeling]:
    ...
    """
    constraints = get_constraints_from_semantics(semantics, si)
    solver = si.get_solver()
