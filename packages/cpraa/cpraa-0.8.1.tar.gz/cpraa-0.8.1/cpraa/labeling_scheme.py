...
import z3
from pysmt.shortcuts import GE, LE, Real, And, GT, LT, Equals, Or, Not
...
class ThresholdClassical(LabelingScheme):
    ...
    def get_constraints(self, node: Node, si: SolverInstance):
        ...
        out_constraint = And(LT(node_prob, Real(self.t_in)), parent_out_constraint)
...
class Conservative(LabelingScheme):
    """
    A node is labeled
     - 'in' if it holds with probability 1,
     - 'out' if it is attacked with probability 1, and
     - 'undec' in all other cases.
    """
    def get_label(self, node: Node, distribution: Distribution):
        prob = distribution.get_marginal_probability(node)
        if prob == 1.0:
            ...
            no_attacker: Assignment = tuple([(attacker, False) for attacker in node.get_parents()])
            if distribution.get_part_assignment_probability(no_attacker) == 0:
                return l_out
            else:
                return l_undec
    ...
    def get_constraints(self, node: Node, si: SolverInstance):
        node_prob = si.get_node_symbol(node)
        in_constraint = GE(node_prob, Real(1))

        no_attacker: Assignment = tuple([(attacker, False) for attacker in node.get_parents()])
        no_attacker_prob = si.get_symbol(no_attacker)
        out_constraint = Equals(no_attacker_prob, Real(0))

        undec_constraint = Not(Or(in_constraint, out_constraint))
        return [in_constraint, out_constraint, undec_constraint]
