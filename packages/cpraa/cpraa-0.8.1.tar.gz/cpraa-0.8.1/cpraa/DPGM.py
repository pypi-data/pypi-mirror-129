...
def parents(nodes: List[Node], edge_type=DEFAULT_EDGE_TYPE) -> Set[Node]:
    """
    ...
    parents_set = set()
    for node in nodes:
        parents_set.union(node.get_parents(edge_type))
    return parents_set


def children(nodes: List[Node], edge_type=DEFAULT_EDGE_TYPE) -> Set[Node]:
    """
    Return the union of the nodes' children w.r.t. to a an edge type
    :param nodes: A list of nodes
    :param edge_type: The type of the considered edges
    :return: The sorted list of the parents
    """
    children_set = set()
    for node in nodes:
        children_set.union(node.get_children(edge_type))
    return children_set
