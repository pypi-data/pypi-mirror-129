...
def is_full_assignment(assignment: Assignment, all_nodes: List[Node]):
    """
    Returns true if all nodes in the list are assigned in the given assignment and vice versa.
    """
    return set([node for (node, _) in assignment]) == set(all_nodes)
