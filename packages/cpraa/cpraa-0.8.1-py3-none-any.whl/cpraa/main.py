...
#  - parse aspartix format
#  - handle not implemented Error in main
...
def main():
    ...
    base_group.add_argument("-f", "--file",
                            help="the argumentation framework as .tgf file")
    ...
    base_group.add_argument("-a", "--arguments", nargs="+", default=[], metavar="NODE",
                            help="one or more nodes from the AF to be used in one of the tasks")
    base_group.add_argument("-OD", "--one_distribution", action="store_true",
                            help="compute one distribution satisfying the semantics")
    base_group.add_argument("-ND", "--n_distributions", nargs="?", metavar="N", type=int, const=MAX_NUM_DISTRIBUTIONS,
                            help="compute up to N distributions satisfying the semantics "
                                 "(default: N=" + str(MAX_NUM_DISTRIBUTIONS) + ")")
    ...
    base_group.add_argument("-b", "--backend_solver", metavar="SOLVER", type=str,
                            help="the solver used as backend. Options are all SMT solvers available via pySMT ("
                                 ...
                                 + Z3_LEGACY + ")")
    base_group.add_argument("-smt", "--smt_file",
                            help="add additional constraints from a file in SMT-LIB format")
    ...
    info_group.add_argument("-p", "--output_precision", metavar="PRECISION", type=int, default=5,
                            help="precision of floats that are output (default: up to 6 decimal places)")
    ...
    if args.n_distributions is not None:
        if not af:
            arg_parser.error("an AF must be provided to compute distributions")
        if solver == Z3_LEGACY or solver in linear_solvers:
            arg_parser.error("task is only available via pysmt solvers")
        nd_timer = Timer(output_string="Computing up to " + str(args.n_distributions) + " distributions took")
        print("Computing up to " + str(args.n_distributions) + " distributions satisfying the following semantics:",
              repr_list(semantics))

        si = get_smt_solver(si, af, solver)
        distributions = tasks.get_n_distributions(si, semantics, args.n_distributions)
        num_dist = len(distributions)
        if num_dist > 0:
            for i in range(num_dist):
                print()
                print(str(i+1) + "/" + str(args.n_distributions) + ":")
                distributions[i].print_formatted(args.distribution_output_format, precision=args.output_precision)
            if num_dist < args.n_distributions:
                print("No further distributions satisfying the given semantics exist.")
        else:
            print("No distribution satisfying the given semantics exists.")
        nd_timer.stop()
    ...
    if args.skeptical_acceptance_all is not None:
        ...
        for argument in af.get_nodes():
            if solver == Z3_LEGACY:
                acceptable, counterexample = tasks.check_skeptical_acceptance_z3(z3i, semantics, argument=argument,
                                                                                 threshold=threshold)
            else:
                acceptable, counterexample = tasks.check_skeptical_acceptance(si, semantics, argument=argument,
                                                                              threshold=threshold)
            if acceptable:
                accepted_args.append(argument)
