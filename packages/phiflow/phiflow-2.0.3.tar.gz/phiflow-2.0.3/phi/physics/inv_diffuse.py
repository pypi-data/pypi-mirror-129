...
def generate_heat_example(*shape, bounds: Box = None):
    ...
    positions = (math.random_uniform(shape.batch, batch(components=10), channel(vector=shape.spatial_rank)) - 0.5) * bounds.size * 0.8 + bounds.size * 0.5
    for i in range(10):
        position = positions.components[i]
        half_size = math.random_uniform(shape.batch, channel(vector=shape.spatial_rank)) * 10
