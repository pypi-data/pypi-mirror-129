...
async def attach_metadata(device):
    ...
    try:
        if hasattr(device, '_hw_version_major_property') and hasattr(device, '_hw_version_minor_property') and hasattr(device, '_hw_version_variant_property'):
            ...
            try:
                metadata = db.get_product(device._product)
...
def format_errors(odrv, clear=False):
    ...
    for name, axis in axes:
        ...
        lines += dump_item("  ", 'procedure result', axis, 'procedure_result', lambda x: decode_enum(x, odrive.enums.ProcedureResult))
    
    if hasattr(odrv, 'n_issues') and odrv.n_issues == 0:
        lines.append("internal issues: " + RichText("none", *STYLE_GOOD))
    if hasattr(odrv, 'n_issues') and odrv.n_issues > 0:
        lines.append("internal issues: " + RichText(str(odrv.n_issues), *STYLE_BAD))
