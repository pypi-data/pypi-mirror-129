...
def check_for_updates():
    ...
    if check_updates and time.time() - last_check_time > ONE_DAY:
        ...
        class GetLatestVersion(QThread):
            ...
            def run(self):
                try:
                    request = requests.get('https://bo.dashenglab.com/api/open/client/version')
                    data = request.text
        ...
        def compare_versions(latest):
            ...
            if version(latest) <= version(current) or \
                    latest == skipped:
                return
            notif = Notification(title=i18n.t("orange.update.title"),
                                 text=i18n.t("orange.update.text").format(current, latest),
                                 accept_button_label=i18n.t("orange.update.accept"),
                                 reject_button_label=i18n.t("orange.update.reject"),
            ...
            def handle_click(role):
                ...
                if role == notif.AcceptRole:
                    QDesktopServices.openUrl(QUrl("https://bw.dashenglab.com/5fbb9705a6e5e67080638ac6.html"))
...
class OMain(Main):
    ...
    def show_splash_message(self, message: str, color=QColor("#000080")):
