...
def buildCli(consoleBaseClass = None):
    ...
    from game import Heartbeat, peer, mobile_instance, NanoText

    import sys
    sys.__peer_shell_stdout = sys.stdout

    if consoleBaseClass is None:
        from .system import ConsoleBusiness as consoleBaseClass
...
class PythonWinCompartment:
    class PyWinConsoleBusiness: # XXX: (ConsoleBusiness): so much for a base class.
    ...
    @classmethod
    def EmbedFromOptions(self, xxx_todo_changeme, async = True):
