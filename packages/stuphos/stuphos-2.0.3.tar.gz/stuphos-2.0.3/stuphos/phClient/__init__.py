...
class filesystem:
    '''
    client.upload(filesystem(args).structure,
                  path = 'mount/point')
...
class apiClient:
    ...
    def __init__(self, username, password, host = HOST, protocol = 'https', runtime = None, **kwd):
        ...
        if protocol == 'https':
            self.server = ServerProxy(endpoint, context = ssl.SSLContext())
        else:
            self.server = ServerProxy(endpoint)
    ...
    def login(self, playerName, password):
        return self.session(self, playerName, password)
    ...
    class session:
        ...
        def __init__(self, client, playerName, password, sessionId = None):
            self.client = client
...
class nativeSession:
    ...
    def __init__(self, frame, host, playerName, password):
        task = frame.task
        ...
        def connect():
            self._session = client.login(playerName, password)
            return self
