...
def init_parser(hub):
    if "parser" not in hub.config.ARGS:
        ...
        hub.config.PARSER = argparse.ArgumentParser()
...
def subparsers(hub, raw: Dict[str, Any], cli: str) -> bool:
    ...
    if not subs:
        return True
    hub.config.ARGS["sub"] = hub.config.PARSER.add_subparsers(
        dest="_subparser_"
    )
...
def setup(hub, raw_cli: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for arg in _keys(raw_cli):
        ...
        if action is None:
            ...
            action = hub.config.PARSER._registry_get("action", action)

        if isinstance(action, str):
            signature = inspect.signature(
                hub.config.PARSER._registry_get("action", action).__init__
            )
        else:
...
def parse(
    ...
    if only_parse_known_arguments:
        ...
        opts = hub.config.PARSER.parse_args(args, namespace)
        opts_dict = opts.__dict__
