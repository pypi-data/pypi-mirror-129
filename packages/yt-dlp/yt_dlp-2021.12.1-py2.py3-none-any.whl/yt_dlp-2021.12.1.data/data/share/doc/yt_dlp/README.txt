...
-   Based on YOUTUBE-DL 2021.06.06 COMMIT/379F52A and YOUTUBE-DLC
    2020.11.11-3 COMMIT/98E248F: You get all the features and patches of
...
-   NEW AND FIXED EXTRACTORS: Many new extractors have been added and a
    lot of exisiting ones have been fixed. See the changelog or the list
    of supported sites
...
certain formats. Licensed under GPLv2+ * PYCRYPTODOMEX - For decrypting
AES-128 HLS streams and various other data. Licensed under BSD2 *
...
Licensed under MIT * ATOMICPARSLEY - For embedding thumbnail in mp4/m4a
if mutagen is not present. Licensed under GPLv2+ * RTMPDUMP - For
downloading rtmp streams. ffmpeg will be used as a fallback. Licensed
under GPLv2+ * MPLAYER or MPV - For downloading rstp streams. ffmpeg
will be used as a fallback. Licensed under GPLv2+ * PHANTOMJS - Used in
extractors where javascript needs to be run. Licensed under BSD3 *
...
j, l, q can be used for converting to Bytes, Json (flag # for
pretty-printing), a comma seperated List (flag # for \n
newline-seperated) and a string Quoted for the terminal (flag # to split
a list into different arguments), respectively 1. UNICODE NORMALIZATION:
The format type U can be used for NFC unicode normalization. The
alternate form flag (#) changes the normalization to NFD and the
...
-   acodec: Audio Codec (opus > vorbis > aac > mp4a > mp3 > eac3 > ac3 >
    dts > other > unknown)
