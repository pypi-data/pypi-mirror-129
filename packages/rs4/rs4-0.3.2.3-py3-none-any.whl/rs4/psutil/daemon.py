...
def get_pidfile (path, procname):
	if procname:
		return os.path.join (path, f'.{procname.split ("/")[-1]}.pid')
...
class Daemonizer:
	def __init__(self, chdir="/", procname = None, umask=0o22, lockpath = None):
		...
		self.pidfile = get_pidfile (self.lockpath, procname)
...
def status (lockpath, procname = None):
	pidfile = get_pidfile (lockpath, procname)
