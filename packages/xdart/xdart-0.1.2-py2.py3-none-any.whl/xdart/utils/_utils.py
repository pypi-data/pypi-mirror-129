...
def get_1D_data(h5_file, arch_ids=None, static=True):
    ...
    print(scan_name, str(h5_file))

    df1 = pd.DataFrame(columns=('idx', 'intensity', 'tth', 'q'))
    with catch_h5py_file(sphere.data_file, 'r') as file:
        if arch_ids is None:
            arch_ids = sphere.arches.index

        for nn, idx in enumerate(arch_ids):
            try:
