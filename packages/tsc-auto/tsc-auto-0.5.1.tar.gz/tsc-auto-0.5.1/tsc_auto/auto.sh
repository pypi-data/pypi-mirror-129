...
spy=set_gpu.py  # set_gpu.py脚本所在目录绝对路径
# function 与mac不兼容
...
if [ $help ]; then
  ...
  echo -e '-h, --help\t查看帮助'
  echo -e '说明: 使用gpu必须在对应conda环境下执行, 深度学习框架要安装在默认路径, py脚本路径和cuda路径需要适配'
  exit 0
fi
...
if [ $deep ]; then  # 配置环境变量
  ...
  export LD_LIBRARY_PATH=~/anaconda3/lib:$cudap  # cuda 路径根据需要修改
