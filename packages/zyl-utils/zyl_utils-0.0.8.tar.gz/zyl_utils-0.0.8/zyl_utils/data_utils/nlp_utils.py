...
class MyTokenizer:
    def __init__(self):
        # 把连号‘-’分开
        self.sentence_tokenizer_zh = self._cut_paragraph_to_sentence_zh
        self.sentence_tokenizer_en = self._cut_paragraph_to_sentence_en().tokenize

        self.word_piece_tokenizer_zh = self._cut_sentence_to_word_piece_zh
        self.word_piece_tokenizer_en = self._cut_sentence_to_word_piece_en().tokenize

    def _cut_paragraph_to_sentence_zh(self, para: str, drop_empty_line=True, strip=True, deduplicate=False):
        """
        Args:
           para: 输入文本
    ...
    def _cut_paragraph_to_sentence_en(self):
    ...
    def _cut_sentence_to_word_piece_zh(self, sentence: str):
    ...
    def _cut_sentence_to_word_piece_en(self):
        from nltk import WordPunctTokenizer

        return WordPunctTokenizer()

    def cut_sentence_to_word_piece(self, sentence: str):
        if langid.classify(sentence)[0] == 'zh':
            return self.word_piece_tokenizer_zh(sentence)
        else:
            return self.word_piece_tokenizer_en(sentence)

    def cut_paragraph_to_sentences(self, paragraph: str):
        if langid.classify(paragraph)[0] == 'zh':
            return self.sentence_tokenizer_zh(paragraph)
        else:
            return self.sentence_tokenizer_en(paragraph)
