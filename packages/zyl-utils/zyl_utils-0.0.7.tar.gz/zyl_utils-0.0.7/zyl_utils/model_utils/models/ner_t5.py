...
from zyl_utils.model_utils.metrics import ner_metric
...
class NerT5:
    ...
    def train(self, train_data: pd.DataFrame, eval_data: pd.DataFrame):
        # deal with dt
        train_data = NerT5._deal_with_df(train_data)
        eval_data = NerT5._deal_with_df(eval_data)
    ...
    def _eval_entity_recognition(model, eval_data: pd.DataFrame, check_in_input_text: bool, delimiter='|',
        ...
        for prefix in set(prefixes):
            ...
            res_df = ner_metric.entity_recognition_v2(y_true, y_pred, pos_neg_ratio=pos_neg_ratio,
                                                      self_metric=self_metric)
            eval_res[prefix] = res_df

        print(f'sum report:')
        res_df = ner_metric.entity_recognition_v2(revised_target_texts, revised_pred_target_texts,
