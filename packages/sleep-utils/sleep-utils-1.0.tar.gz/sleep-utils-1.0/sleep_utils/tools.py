...
def hypno_summary(hypno):
    """
    summarizes the hypnogram parameters such as
        - % S1/S2/SWS/REM/Wake
        ...
    """
    from pprint import pprint
    hypno = np.array(hypno)
    sleep_stages = {'WASO':0, 'S1':1, 'S2':2, 'SWS':3, 'REM':4}
    stages = sleep_stages.copy()
    hypno_sleep = hypno[np.where(hypno!=0)[0][0]:np.where(hypno!=0)[0][-2]]
    for stage, num in sleep_stages.items():
        stages[stage] = f'{np.mean(hypno_sleep==num)*100:.1f}%'
    # stages['Wake'] = f'{np.mean(hypno==sleep_stages["Wake"])*100:.1f}%'
    pprint(stages)
