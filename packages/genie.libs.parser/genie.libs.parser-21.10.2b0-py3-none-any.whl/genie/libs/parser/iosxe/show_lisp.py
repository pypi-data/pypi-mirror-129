...
class ShowLispInstanceIdDNStatisticsSchema(MetaParser):
    ...
    schema = {
        'lisp_id': {
            int: {
                'instance_id': {
                    16777214: { # Value other than 16777214 is not accepted
                        'iaf_count': int,
...
class ShowLispInstanceIdDNStatistics(ShowLispInstanceIdDNStatisticsSchema):
    ...
    ''' Parser for
        ...
        * show lisp instance-id 16777214 dn statistics
    '''
    cli_command = ['show lisp {lisp_id} instance-id 16777214 dn statistics',
                   'show lisp instance-id 16777214 dn statistics']

    def cli(self, output=None, lisp_id=None):
        if output is None:
            if lisp_id:
                output = self.device.execute(self.cli_command[0].format(lisp_id=lisp_id))
            else:
                output = self.device.execute(self.cli_command[1])
        ret_dict = {}
