            ...
            for i in change:
                ...
                if "last_modified" in i["field"] or u"最近更新时间" in i["field"] or isinstance(i["field"],
                    ...
                    "field"] or (not i['old'] and not i["new"]) or i['old'] == i['new'] or "id" in i["field"] or "ID" in \
                        i["field"]:
                    continue
                if isinstance(i["old"], dict) or isinstance(i["new"], dict):
                    continue

                changestr = u"由%s更新为%s" % (map_nullfalse(i["old"]), map_nullfalse(i["new"]))

                # 比较两个数据是否相同
                try:

                    # 强制类型转换为float
                    old_one = map_nullfalse(i["old"])

                    new_one = map_nullfalse(i["new"])

                    print("old", old_one, type(old_one))
                    print("new", new_one, type(new_one))
                    if float(old_one) == float(new_one):
                        continue
                    ...
                    try:
                        ...
                        new_time = None
                        if "T" in i["old"]:
                            old_time = datetime.datetime.strptime(i["old"], '%Y-%m-%dT%H:%M:%S')
                        else:
                            old_time = datetime.datetime.strptime(i["old"], '%Y-%m-%d %H:%M')

                        if "T" in i["new"]:
                            new_time = datetime.datetime.strptime(i["new"], '%Y-%m-%dT%H:%M:%S')
                        else:
                            new_time = datetime.datetime.strptime(i["new"], '%Y-%m-%d %H:%M')

                        if new_time.strftime('%Y-%m-%d %H:%M') == old_time.strftime('%Y-%m-%d %H:%M'):
                            continue
                        else:
                            save_history(instance, user=current_request.user, action=changestr, field_name=i["field"])
                    except Exception as err:
