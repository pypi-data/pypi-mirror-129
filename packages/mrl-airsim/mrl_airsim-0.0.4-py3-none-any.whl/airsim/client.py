...
class VehicleClient:
    ...
    def simGetFastImage(self, camera_name, vehicle_name=''):
        response_raw = self.client.call('simGetFastImage', camera_name, vehicle_name)
    ...
    def simSetVehiclePoseWorld(self, pose, ignore_collison, vehicle_name = ''):
        self.client.call('simSetVehiclePoseWorld', pose, ignore_collison, vehicle_name)

    def simGetVehiclePoseWorld(self, vehicle_name=''):
        pose = self.client.call('simGetVehiclePoseWorld', vehicle_name)
        return Pose.from_msgpack(pose)

    def simGetHeadingToSpline(self, spline_name, lookahead, is_reverse=False, follow_tangent=False, vehicle_name=''):
        heading = self.client.call('simGetHeadingToSpline', spline_name, lookahead, is_reverse, follow_tangent, vehicle_name)
        return Quaternionr.from_msgpack(heading)

    def simGetSplineClosestPoints(self, spline_name, ref_points, skip_orientation=False, vehicle_name=''):
        spline_info = self.client.call('simGetSplineClosestPoints', spline_name, ref_points, skip_orientation, vehicle_name)
        return [Pose.from_msgpack(p) for p in spline_info]

    def simGetSplineConformity(self, spline_name, ref_points, decay=1.0, skip_para=False, vehicle_name=''):
        spline_info = self.client.call('simGetSplineConformity', spline_name, ref_points, decay, skip_para, vehicle_name)
        return [Vector3r.from_msgpack(p) for p in spline_info]

    def simGetSplineDefinition(self, spline_name, resolution=1.0, vehicle_name=''):
        spline_info = self.client.call('simGetSplineDefinition', spline_name, resolution, vehicle_name)
        return [Pose.from_msgpack(p) for p in spline_info]

    def simGetVehicleTerrain(self, vehicle_name=''):
        return self.client.call('simGetVehicleTerrain', vehicle_name)

    def resetToLandscape(self, vehicle_name=''):
        is_success = self.client.call('resetToLandscape', vehicle_name)
        return is_success

    def resetToVolume(self, volume_name, steer_noise_range=90.0, vehicle_name=''):
        is_success = self.client.call('resetToVolume', volume_name, steer_noise_range, vehicle_name)
        return is_success

    def resetToSpline(self, spline_name, offset_noise, yaw_noise, is_reverse=False, vehicle_name=''):
        is_success = self.client.call('resetToSpline', spline_name, offset_noise, yaw_noise, is_reverse, vehicle_name)
        return is_success

    def simGetImageObjectDetections(self, request, max_detection_range=50.0, include_image=False, vehicle_name=''):
        result = self.client.call('simGetImageObjectDetections', request, max_detection_range, include_image, vehicle_name)
        img_detections = ImageObjectDetections.from_msgpack(result)
        img_detections.detections = [ObjectDetection.from_msgpack(d) for d in img_detections.detections]
        for d in img_detections.detections:
            d.box = [Vector3r.from_msgpack(p) for p in d.box]
        return img_detections

    def simSpawnMovingActor(self, path, location, direction, velocity):
        return self.client.call('simSpawnMovingActor', path, location, direction, velocity)
...
class CarClient(VehicleClient, object):
    ...
    def getCarStateWorld(self, vehicle_name = ''):
        state_raw = self.client.call('getCarStateWorld', vehicle_name)
        return CarState.from_msgpack(state_raw)

    def getCarStateWorldNp(self, vehicle_name = ''):
        s = self.getCarStateWorld(vehicle_name)
        pos = np.array([s.kinematics_estimated.position.x_val,
                        s.kinematics_estimated.position.y_val,
                        s.kinematics_estimated.position.z_val])
        quat = np.array([s.kinematics_estimated.orientation.x_val, s.kinematics_estimated.orientation.y_val,
                         s.kinematics_estimated.orientation.z_val, s.kinematics_estimated.orientation.w_val])
        return pos, quat, s
