...
from tensorflow.keras import regularizers                           # for manipulating Tensorflow models
import tensorflow_addons as tfa                                     # for more advanced loss and activation functions                                                            
...
def train_classifier(df_train,PROJECT_FOLDER,img_path, model, params):
    def my_preprocess (x):
        ...
        x = tfa.image.random_cutout(images = x, mask_size = (80,80), seed = 111)
    ...
    dense1 = layers.Dense(units = 512, kernel_regularizer=regularizers.l2(0.001), activation=tfa.activations.mish) (flatten) #mish => https://arxiv.org/abs/1908.08681
    ...
    opt = tfa.optimizers.RectifiedAdam(lr=learning_rate) #RAdam => https://arxiv.org/pdf/1908.03265.pdf
    
    classifier_model = Model(inputs=model.input, outputs=output, name="classifier_model")
    classifier_model.compile(loss="categorical_crossentropy",optimizer=opt,metrics=[keras.metrics.TopKCategoricalAccuracy(k=1,name = 'top1'),keras.metrics.TopKCategoricalAccuracy(k=5,name='top5')])
    ...
    if continue_training :
        classifier_model.fit(training_generator, epochs=epochs, validation_data=validation_generator ,callbacks=callbacks,verbose=1,class_weight=class_weights)
