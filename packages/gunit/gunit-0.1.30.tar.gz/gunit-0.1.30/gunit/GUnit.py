    ...
    def junit(self):
        ...
        for suitei, suite in enumerate(suites):
            ...
            for testi in range(testn):
                name = re.sub(r'"', '', suite.tests[testi])
                message = re.sub(r'"', '', suite.msgs[testi])
                fail = True if suite.msgs[testi] == 'true' else False
                elapsed = suite.times[testi]
                fline = suite.lines[testi]

                err_msg = ""
                if fail:
                    err_msg = f"Affirmation failed at {suite_name}:{fline} with the following message: {message}"

                tc = TestCase(name, suite_name, float(elapsed), "", err_msg, allow_multiple_subelements=True)
