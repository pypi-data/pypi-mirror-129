...
if __name__ == "__main__":
    ...
    # df_mini_scree_plots = pd.read_csv(res_path / "random_nmf" / f"mini_scree_plots.csv")
    df_mini_scree_plots = pd.DataFrame(index=[], columns=[])

    for itrial in range(40):
        nb_clusters = np.random.randint(low=5, high=50)
        min_corr = np.random.uniform(low=0.85, high=0.95)
        max_corr = np.random.uniform(low=0.05, high=0.15)
        h_avg_csize = np.random.randint(low=10, high=20)
        w_avg_csize = np.random.randint(low=10, high=50)
        eps = np.random.uniform(low=0.05, high=0.20)
        generated_M = generate_nmf_reconstruction(n_comp=nb_clusters,
                                                  ...
                                                  epsilon=eps)
        generated_M.to_csv(res_path / "random_nmf" / f"M_nc{nb_clusters}_corrmin{round(min_corr,2)}_"
                                       f"corrmax{round(max_corr,2)}_noise{round(eps,2)}.csv",
                           float_format="%.4f")
        df_scree_plot = nmf.generate_scree_plot(generated_M, ncmin=5, ncmax=50)
        df_mini_scree_plots = nmf.add_examples_from_scree_plot(df_mini_scree_plots, df_scree_plot, nb_clusters)
        df_mini_scree_plots.to_csv(res_path / "random_nmf" / f"mini_scree_plots.csv")
