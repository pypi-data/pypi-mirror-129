...
class SmartDRLAgent:
    ...
    def make_day_trade(self,actions,accountid,demo=True):
        app = api_connect(demo=demo)
        ticker_list = [x.upper() for x in self.ticker_list]
        stock_data = get_stock_info(app,ticker_list,accountid)
        trade_log = pd.DataFrame({"tic":[],"action":[],"quantity":[]})
        actions = [int((x * 10) - 5) for x in actions[0]]
        print(stock_data)
        for i,tic in enumerate(ticker_list):
            quantity = actions[i]
            current_quantity = stock_data[tic][1]
            print(quantity,current_quantity)
            if quantity == current_quantity or quantity == 0 or (current_quantity > 5 or current_quantity <-5):
