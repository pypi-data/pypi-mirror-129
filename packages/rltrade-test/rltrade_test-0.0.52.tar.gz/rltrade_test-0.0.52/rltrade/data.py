...
class IBKRDownloader:
    ...
    def download_min_data(self,app:IBapi,id,ticker,end:str):
        def run_loop():
	            app.run()
        thread = threading.Thread(target=run_loop,daemon=True)
        thread.start()
        while True:
            if isinstance(app.nextorderId, int):
                ...
                print('waiting for connection')
                time.sleep(1)
        app.reqHistoricalData(id,stock_contract(ticker) ,end+" 23:00:00",
                            "2 D",'1 min','TRADES',0,2,False,[])
        time.sleep(5)
        app.nextorderId += 1
        df = app.get_df()
        df['tic'] = ticker
    ...
    def fetch_min_data(self,demo=True):
        ...
        for tic in self.ticker_list:
            ...
            for end in self.dates[1:]:
                self.cnt += 1
                try:
                    temp_df = self.download_min_data(app,self.cnt,tic,end)
                    df = df.append(temp_df)
                except:
                    print("Not able to download",tic)
                    not_downloaded.append(tic)
    ...
    def download_todays_min_data(self,app:IBapi,id:int,ticker,end:str,duration:str):
        def run_loop():
	            app.run()
        thread = threading.Thread(target=run_loop,daemon=True)
        thread.start()
        while True:
            if isinstance(app.nextorderId, int):
                ...
                print('waiting for connection')
                time.sleep(1)
        app.reqHistoricalData(id,stock_contract(ticker) ,"",
                            duration,'1 min','TRADES',0,2,True,[])
        time.sleep(5)
        app.nextorderId += 1
        df = app.get_df()
        df['tic'] = ticker
    ...
    def fetch_todays_min_data(self,end,duration,demo=True):
        df = pd.DataFrame()
        not_downloaded = list()
        print("connecting to server...")
        app = api_connect(demo=demo)
        for tic in self.ticker_list:
            self.cnt +=1
            try:
                temp_df = self.download_todays_min_data(app,self.cnt,tic,end,duration)
                df = df.append(temp_df)
            except:
                print("Not able to download",tic)
                not_downloaded.append(tic)
...
class FeatureEngineer:
    ...
    def add_half_hour_time(self,df:pd.DataFrame):
        df['half_hour_time'] = (df.groupby('tic')['date']
