...
class IBKRDownloader:
    ...
    def fetch_min_data(self,demo=True):
        ...
        for tic in self.ticker_list:
            ...
            for end in self.dates[1:]:
                cnt += 1
                try:
                    temp_df = self.download_min_data(app,cnt,tic,end)
                    df = df.append(temp_df)
                except:
                    print("Not able to download",tic)
                    not_downloaded.append(tic)
    ...
    def download_todays_min_data(self,app:IBapi,id:int,ticker,end:str,duration:str):
        ...
        while True:
            if isinstance(app.nextorderId, int):
                ...
                print('waiting for connection')
                time.sleep(1)
        app.reqHistoricalData(id,stock_contract(ticker) ,end,
    ...
    def fetch_todays_min_data(self,end,duration,demo=True):
        ...
        for tic in self.ticker_list:
            cnt +=1
            try:
                temp_df = self.download_todays_min_data(app,cnt,tic,end,duration)
                df = df.append(temp_df)
            except:
                print("Not able to download",tic)
                not_downloaded.append(tic)
