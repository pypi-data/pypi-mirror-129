...
class IBKRDownloader:
    ...
    def download_data(self,app:IBapi,id,ticker,sec,exchange):
        ...
        app.reqHistoricalData(id,cnt,self.end_date+" 20:59:00",
    ...
    def download_min_data(self,app:IBapi,id,ticker,sec,exchange,end:str):
        ...
        if self.symbol_type == 'stock':
            ...
            cnt = future_contract(ticker,secType=sec,exchange=exchange)
        app.reqHistoricalData(id,cnt ,end+" 20:59:00",
    ...
    def download_tic_data(self,app:IBapi,id,ticker,sec,exchange,start):
        def run_loop():
                app.run()
        thread = threading.Thread(target=run_loop,daemon=True)
        thread.start()
        ...
        if self.symbol_type == 'stock':
            ...
            cnt = future_contract(ticker,secType=sec,exchange=exchange)
        app.reqHistoricalTicks(id,cnt,start,"",1000,"TRADES",1,True,[])
        app.nextorderId += 1
        df = app.get_tic_df()
        df['tic'] = ticker
        df['sec'] = sec
    ...
    def fetch_tick_data(self):
        ...
        for i,tic in enumerate(self.ticker_list):
            ...
            sec = self.sec_types[i]
            exchange = self.exchanges[i]
            start = self.start_date + " 14:00:00"
            end = self.end_date + " 20:59:00"
            try:
                while (pd.to_datetime(start,format="%Y%m%d %H:%M:%S")
                        <= pd.to_datetime(end,format="%Y%m%d %H:%M:%S")):
                    temp_df = self.download_tic_data(app,i,tic,sec,exchange,start)
                    df = df.append(temp_df)
