...
class IBapi(EWrapper, EClient):
    ...
    def symbolSamples(self, reqId: int, contractDescriptions: list):
        ...
        for description in contractDescriptions:
            print(description.contract.symbol)
            print(description.contract.localSymbol)
...
def future_contract(symbol,secType="FUT",exchange="GLOBEX",currency="USD"):
    ''' custom function to create future contrace '''
    contract = Contract()
    # contract.symbol = symbol
...
def buy_stock(app,ticker,sectype,exchange,quantity,symbol_type):
...
def sell_stock(app,ticker,sectype,exchange,quantity,symbol_type):
    def run_loop():
	    app.run()
...
def get_stock_info(app,ticker_list,sec_types,exchanges,accountid,demo=True,symbol_type='stock'):
    def run_loop():
	    app.run()
    ...
    for i,tic in enumerate(ticker_list):
        if symbol_type == 'stock':
            contract = stock_contract(str(tic),secType=sec_types[i],exchange=exchanges[i])
        elif symbol_type == 'future':
            print("fut")
            contract = future_contract(str(tic),secType=sec_types[i],exchange=exchanges[i])
