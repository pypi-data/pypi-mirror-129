...
class SmartDRLAgent:
    ...
    def make_trade(self,actions,accountid,symbol_type,demo=True):
    ...
    def make_day_trade(self,actions,weights,accountid,symbol_type,demo=True):
        app = api_connect(demo=demo)
        ticker_list = [x.upper() for x in actions.index]
        stock_data = get_stock_info(app,ticker_list,accountid)
        trade_log = pd.DataFrame({"tic":[],"action":[],"quantity":[]})
        ...
        for i,tic in enumerate(ticker_list):
            ...
            if quantity == current_quantity or quantity == 0:
                ...
                if current_quantity <= 0:
                    ...
                    buy_stock(app,tic,to_buy,symbol_type)
                ...
                if current_quantity >=0:
                    ...
                    sell_stock(app,tic,to_sell,symbol_type)
                elif current_quantity < 0:
                    if current_quantity > quantity:
                        print(f"No Action {tic}")
                        trade_log.loc[len(trade_log.index)]  = [tic,"hold","-"]
