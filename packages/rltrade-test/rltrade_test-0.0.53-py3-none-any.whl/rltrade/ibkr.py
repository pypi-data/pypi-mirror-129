...
def futures_contract(symbol,secType="FUT+CONTFUT",exchange="GLOBEX",currency="USD"):
...
def buy_stock(app,ticker,quantity,symbol_type):
    ...
    if symbol_type == 'stock':
        cont = stock_contract(ticker)
    elif symbol_type == 'future':
        cont = futures_contract(ticker)

    app.placeOrder(app.nextorderId, cont, order)
    app.nextorderId += 1
    time.sleep(2)

def sell_stock(app,ticker,quantity,symbol_type):
    def run_loop():
	    app.run()
