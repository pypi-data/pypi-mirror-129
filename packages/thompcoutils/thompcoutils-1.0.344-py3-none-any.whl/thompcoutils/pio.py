...
class LightSensor:
    """
    Defines a Light Sensor that is built from an RC circuit.  It takes a certain amount of time for the capacitor to charge.
     If it charges to above the threshold in the min_time, the light detector says it is on.
     If it fails to charge in the min_time, it is said to be off
    """

    def __init__(self, pin_number, min_time):
        """
        Constructs a LightSensor
        :param pin_number: the pin the circuit is attached to
        :param min_time: the minimum time required to charge the capacitor for the circuit to be considered on
        """
    ...
    def check_for_light(self):
        """
        Checks for light. This resets the pin, grounding the capacitor, then sets back to input and waits the min_time
        ...
        while read_value == GPIO.LOW and diff < self.min_time:
            read_value = GPIO.input(self.pin_number)
            diff = time.time() - current_time
            logger.debug('difference is {}, read value is {}'.format(diff, read_value))
        if diff >= self.min_time or GPIO.input(self.pin_number) == GPIO.LOW:
            return 0
        else:
            return 1
...
class OnboardLed(Led):
    onboard_led : Optional["OnboardLed"] = None

    def __init__(self, board_type: Optional["PiBoardType"]):
        if OnboardLed.onboard_led is not None:
            return

        if board_type == PiBoardType.Zero:
            print('running with pin 47')
            pin = Led(47)
        else:
            raise Exception('Board type {} not handled'.format(board_type))

        Led.__init__(self, pin)
        OnboardLed.onboard_led = self
