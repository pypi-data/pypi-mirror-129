...
class Watchdog(threading.Thread):
    ...
    def add_parent(self, parent, package=None):
        self.parents[parent]['time'] = datetime.datetime.now()
        self.parents[parent]['package'] = package
        ...
        while self.is_running:
            ...
            if len(self.parents) == 0:
                ...
                for key in self.parents:
                    time_diff = now - self.parents[key]["time"]
                    if time_diff.seconds > self.check_period:
                        self.timeout_function(self.parents[key]["package"])
