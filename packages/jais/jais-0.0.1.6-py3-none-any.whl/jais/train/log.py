...
from jais.__main__ import CNF0
...
class TrainingLogger:
    def __init__(self,
        ...
        self.logs_dir = CNF0.paths.logs_dir if logs_dir is None else logs_dir
        if self.logger_name:
            if self.logger_name in ['wb', 'wandb']:
                import wandb
                wb_kwargs = {'dir': self.logs_dir, 'save_code': True}
...
class RichProgress:
    ...
    def update_epoch(self, row: list) -> None:
        ...
        def typecast(x): return f"{x:.4f}" if isinstance(x, float) else str(x)
