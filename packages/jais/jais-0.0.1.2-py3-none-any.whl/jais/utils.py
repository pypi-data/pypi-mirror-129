...
@yaml_object(yaml)
class ProjectDirSetter:
    """Custom tag `!rootdir` loader class for yaml file."""

    yaml_tag = u'!projectdir'
...
def get_logger(name: str,
               ...
               logs_conf_filepath: Union[str, Path] = ROOT_DIR/"configs/logs.conf",
...
def load_default_configs() -> Tuple[edict, logging.Logger]:
    ...
    # Load configurations
    CNF0 = load_config(f"{ROOT_DIR}/configs/default.yaml")

    # Set logger settings
    LOG_FILENAME = os.getenv('JAIS_LOG_FILENAME')
    if LOG_FILENAME is None:
        LOG_FILENAME = f"{CNF0.log.filename_prefix}@{time.time()}.log"
        os.environ['JAIS_LOG_FILENAME'] = LOG_FILENAME

    LOG = get_logger(name=CNF0.log.name, 
                     logs_dir=CNF0.paths.logs_dir,
                     log_filename=LOG_FILENAME,
                     logs_conf_filepath=f"{ROOT_DIR}/configs/logs.conf",
                     keep_n_recent_logs=CNF0.log.keep_n_recent_logs
                     )
