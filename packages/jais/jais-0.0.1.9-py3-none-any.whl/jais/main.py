...
class ComplexCLI(click.MultiCommand):
    ...
    def get_command(self, ctx, name: str) -> click.core.Command:
        ...
        try:
            ...
            mod = __import__(f"{CNF.project_name}.{name}.cli", None, None, ["cli"])
        except ImportError:
            return
...
def cli(ctx):
    if ctx.invoked_subcommand is None:
        ...
        click.echo(f"""Welcome to {click.style(CNF['project_name'], fg='yellow')} CLI.

    Type `{click.style(f"{CNF['project_name']} --help", fg='yellow')}` for usage details
    """)
