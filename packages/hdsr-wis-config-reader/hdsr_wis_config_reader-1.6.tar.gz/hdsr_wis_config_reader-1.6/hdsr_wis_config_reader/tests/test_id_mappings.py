...
def test_read_idmaps_without_fews_config():
    oppvl_path = WIS_CONFIG_TEST_DIR / "IdMapFiles" / "IdOPVLWATER.xml"
    df_oppvl = IdMappingCollection.get_idmap_df(idmap=IdMapChoices.idmap_opvl_water, file_path=oppvl_path)
    assert len(df_oppvl) == 6050

    oppvl_hymos_path = WIS_CONFIG_TEST_DIR / "IdMapFiles" / "IdOPVLWATER_HYMOS.xml"
    df_oppvl_hymos = IdMappingCollection.get_idmap_df(idmap=IdMapChoices.idmap_opvl_water, file_path=oppvl_hymos_path)
    assert len(df_oppvl_hymos) == 2360
