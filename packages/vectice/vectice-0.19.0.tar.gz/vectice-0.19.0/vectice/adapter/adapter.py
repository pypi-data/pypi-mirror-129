...
class Adapter(AbstractAdapter):
    ...
    def _save_attachments(self, artifact_name: Optional[str], attachments: Attachments):
        try:
            ...
            self._client._post_attachments(f"/metadata/entityfiles/modelversion/{version_id}", files=attachments.files)
        except Exception as e:
    ...
    def list_model_versions_dataframe(self, model_id: int, page_index=Page.index, page_size=Page.size):
        ...
        return pd.merge(df_model, merged_entities, left_index=True, right_index=True)
