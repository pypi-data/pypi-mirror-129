...
def convert_sagemaker_rbai_bbox(
    prediction_dir: str,
    tasks: Dict,
    ...
    for task in tasks:
        ...
        for det in detection["prediction"]:
            ...
            return_task = {
                "taskId": task["taskId"],
                "priority": score_sum / score_count,
                "labels": labels,
            }
...
def write_line_to_lst(
    img_path: str, boxes: np.ndarray, ids: np.ndarray, idx: int
):
    """
...
def convert_redbrick_to_sagemaker(
    result: Dict, cache_dir: str
    ...
    for _, task in enumerate(tqdm.tqdm(result)):
        ...
        for label_idx, label in enumerate(task["labels"]):
            ...
            if class_ in taxonomy:
                ...
                class_id = class_id_count
                class_id_count += 1
                taxonomy[class_] = class_id
