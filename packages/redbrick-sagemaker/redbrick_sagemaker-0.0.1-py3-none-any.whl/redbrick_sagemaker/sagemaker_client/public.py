...
class SagemakerClient:
    ...
    def __init__(
        ...
        self.sagemaker_session: sagemaker.Session = sagemaker.Session(
            self.boto_session
        )
    ...
    def upload_to_s3(self) -> None:
        ...
        self.sagemaker_session.upload_data(
            path=os.path.join(
                self.cache_dir, "annotation_redbrick_validation.rec"
            ),
            bucket=self.bucket,
    ...
    def launch_tuning_job(self, total_training_samples: int):
        ...
        self.tuning_job_name = f"redbrick-{str(uuid.uuid4())}"[0:32]
    ...
    def transform(self):
        ...
        in_path = (
            f"s3://{self.bucket}/"
            + f"{self.bucket_prefix}/{self.s3_image_inference}"
        )
    ...
    def best_training_job(self) -> Tuple[bool, str]:
        ...
        tuner = sagemaker.tuner.HyperparameterTuner.attach(
