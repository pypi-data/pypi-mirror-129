...
class CAPM(Markovitz):
    ...
    def beta(self, Ri):
        ...
        if len(Ri) != len(Rm):
            raise ValueError('传入的数据长度不等于市场数据长度!')
...
if __name__ == '__main__':
    ...
    # 某种风险资产的收益率时间序列
    Ri=capm.data[capm.data.name == '紫光国微']['pctChg']
        
    # 按照定义计算beta系数 --> 返回beta值            
    pprint(capm.beta(Ri=Ri))

    # 回归估计beta系数 --> 返回包含alpha和beta的字典
    pprint(capm.ls_beta(Ri=Ri))

    # 所有风险资产分别按照【回归估计beta系数、回归估计截距项alpha的值和按照定义计算beta系数】 --> 返回一个DataFrame
    pprint(capm.all_beta())

    # 证券特征线，show=False --> 保存为svg格式矢量图
    capm.scl(name='紫光国微',show=True)

    # 证券市场线，show=False --> 保存为svg格式矢量图
    capm.sml(show=True)
