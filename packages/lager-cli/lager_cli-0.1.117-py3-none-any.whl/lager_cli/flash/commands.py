...
def do_flash(session, gateway, hexfile, binfile, preverify, verify, run=False):
    """
...
def flash(ctx, gateway, hexfile, binfile, run, preverify, verify):
    """
    ...
    resp = do_flash(session, gateway, hexfile, binfile, preverify, verify, run)
    stream_output(resp)
