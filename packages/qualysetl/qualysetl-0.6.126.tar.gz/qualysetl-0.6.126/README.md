...
```text
    ...
    Operation - qetl_manage_user -u [path] -e etl_host_list_detection
...
```text
    Operation - qetl_manage_user -u [path] -e etl_asset_inventory -d 1970-01-01T00:00:00Z
     ...
     - sqlite database will contain one table:  
       1) Q_Asset_Inventory - Asset Inventory of Asset Last Updated -d 'DATE' to now 
          ...
          Q_Asset_Inventory_Software_Unique  - Unique List of Software derived from 
                                               Q_Asset_Inventory including Lifecycle Info if JSON cell enabled.
          Q_Asset_Inventory_Software_AssetId - Unique List of AssetId to Software table derived from 
                                               Q_Asset_Inventory for correlation of Software to Asset.
```
...
#                                                      in excel. Will also truncate database cells. 
#                                                      Use the option False to not truncate any data.
# asset_inventory_present_csv_cell_as_json: True     # To enable json in complex cells instead of indexed list. (faster)
...
------------ | -------------
   ...
   qetl_home/data/asset_inventory_software_unique.csv | Asset Inventory Software Unique CSV derived from Asset Inventory 
   qetl_home/data/asset_inventory_software_assetid.csv | Asset Inventory Software AssetId CSV derived from Asset Inventory
...
Start qetl_setup_python_venv - Thu 17 Jun 2021 08:39:41 PM PDT
  ...
  3) prepare_opt_qetl_env_dirs
    ...
    usage:        qetl_setup_python_venv [/opt/qetl]
...
0.6.123 | 2021-12-01 19:00 ET | Updated asset inventory features to include tables Q_Asset_Inventory_Software_Assetid (Asset ID to Software) and Q_Asset_Inventory_Software_Unique ( unique list of software and lifecycle info found in asset inventory ).  These tables are useful to create views of unique software, unique software -> server.
0.6.123 | 2021-12-01 19:00 ET | Updated qetl_manage_user credential handling to support -p prompt for cred, -s accept json creds from stdin, -m accept creds exported to environment. 
