...
def get_slapp_files_matching(pattern: str) -> List[str]:
    return sorted(glob.glob(join(SLAPP_DATA_FOLDER, pattern)))


def get_latest_slapp_files_matching(pattern: str) -> Optional[str]:
    files = get_slapp_files_matching(pattern)
    if files:
        return files[-1]
    else:
        return None
