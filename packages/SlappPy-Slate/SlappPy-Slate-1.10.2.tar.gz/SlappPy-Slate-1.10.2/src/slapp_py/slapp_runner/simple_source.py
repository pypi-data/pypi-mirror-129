...
class SimpleSource:
    ...
    def get_linked_date_display(self) -> str:
        ...
        otherwise return its truncated text only."""
        link = self.url
        text = truncate(escape_characters(self.date or ''), 16, '…')
        return f"[{text}]({link})" if link else text

    def get_linked_name_display(self) -> str:
        """Return a markdown link with the truncated source name if available,
        otherwise return its truncated text only."""
        link = self.url
    ...
    def truncated_name(self):
        ...
        return truncate(escape_characters(f"{(self.date or '')}-{self.tournament_name}"), 100, '…')
