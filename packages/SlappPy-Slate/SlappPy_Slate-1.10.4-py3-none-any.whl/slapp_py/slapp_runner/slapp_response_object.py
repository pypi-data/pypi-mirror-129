...
class SlappResponseObject:
    ...
    def get_simple_sources(self, sources: Union[Player, Team, List[UUID]]) -> List[SimpleSource]:
        ...
        result.sort(key=lambda s: s.name)  # Sorting by name guaranteed - all sources should have a name.
        return result
    ...
    def get_grouped_sources_text(self, sources: Union[Player, Team, List[UUID]]) -> List[str]:
        ...
        for tourney, simple_sources in groups.items():
            ...
            group_value = (', '.join([simple_source.get_linked_date_display() for simple_source in simple_sources])
                           if group_key
                           else '\n'.join([simple_source.get_linked_name_display() for simple_source in simple_sources])
                           )
