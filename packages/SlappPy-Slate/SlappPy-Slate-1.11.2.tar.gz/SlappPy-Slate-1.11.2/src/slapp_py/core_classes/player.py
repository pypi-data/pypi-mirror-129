...
class Player:
    ...
    @property
    def get_latest_plus_membership(self) -> Optional[PlusMembership]:
        """Order PlusMembership by date, get the last (most recent) or None"""
        return (sorted(self.plus_membership, key=lambda pm: pm.date)[-1]) if len(self.plus_membership) else None
