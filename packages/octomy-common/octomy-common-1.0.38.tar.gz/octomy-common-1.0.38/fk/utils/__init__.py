...
def get_package_file_content(filename, do_debug=False):
    """Look for file relative to package root and return it's content"""
    content = ""
    app_base_path: str = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../"))
    script_path: str = os.path.abspath(os.path.dirname(__file__))
    full_filename = os.path.join(app_base_path, filename)
    if do_debug:
        logger.warn(f"app_base_path={app_base_path}")
        logger.warn(f"script_path={script_path}")
        logger.warn(f"filename={filename}")
        logger.warn(f"__name__={__name__}")
        logger.warn(f"__file__={__file__}")
        logger.warn(f"full_filename={full_filename}")
    if pkg_resources.resource_exists(__name__, filename):
        logger.info(f"Fetching {filename} from package")
        content = pkg_resources.resource_string(__name__, filename).decode("utf-8").strip()
    elif os.path.exists(full_filename):
        logger.info(f"Fetching {full_filename} from dir")
