...
def findele(element,list):
    try:
        ...
        if cheele(element,list) == True:
            handlelist = list.copy()
            if element == '':
                update = 'a'
            else:
                update = ''
            while element in handlelist:
                eleindex = handlelist.index(element)
                indexnum.append(eleindex)
                handlelist.remove(element)
                handlelist.insert(eleindex,update)
            for reele in indexnum:
                handlelist.insert(reele,element)
            return indexnum
