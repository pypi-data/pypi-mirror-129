...
import uritemplate
from heaobject.root import is_primitive, is_primitive_list, is_heaobject_dict_list, is_heaobject_dict
...
def expected_one(fixtures: Dict[str, List[Dict[str, Any]]], coll: str, wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
    ...
    for action, action_name, rel in ((wstl_builder.find_action(a.name), a.name, a.rel) for a in get_actions_):
        ...
        _set_collection_template(action, collection, fixtures[coll][0], 1, rel)
    return [collection]
...
def _create_template(d: Dict[str, List[Dict[str, Any]]], exclude=('id',)):
...
def _set_collection_template(action, collection_doc, f, len_fixtures, rel):
    """
    ...
    def template_data_generator():
        for i in action['inputs']:
            if 'add' in targets:
                ...
                nm = i['name']
                val = f.get(i['section'], None) if 'section' in i else f.get(nm)
...
def _data_append_part(data_, x, y, extra=None):
    if not extra:
        extra = {}
    data_.append({
...
def _action_to_template_data(action, f):
    return [{'name': d['name'],
         ...
         'readOnly': d.get('readOnly', False),
         'required': d.get('required', False),
         'value': f.get(d['name'], None)} for d in action['inputs']]
