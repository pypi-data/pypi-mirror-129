...
def run(project_slug: str,
    ...
    Two-tuples are assumed to be for GET requests.
...
ConnectionString = {mongodb_connection_string}
            ...
            swagger.add_routes([web.get(r[0], r[1]) for r in routes if len(r) == 2])
            swagger.add_routes([r[0](r[1], r[2]) for r in routes if len(r) != 2])
            web.run_app(app)
        done = True
    finally:
        if not done:
