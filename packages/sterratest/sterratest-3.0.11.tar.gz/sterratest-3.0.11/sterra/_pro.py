...
class babilities:
    def __init__(self, ld, pa, io = 10000, dd = False, pt = 0, sz=None) -> dict:
        ...
        self.sz = len(ld) if sz == None else sz
        ...
        for d in self.ld:
            ...
            if d['name'] != None:
                if re.findall(repv, d['name']) != []:
                    p = babilities.coef(p, c=1, v=100)
            
            if re.findall(repv, d['username']) != []:
                p = babilities.coef(p, c=1, v=100)
            p = babilities.coef(p, c=3, v=(100 if d['is_private'] else 0))
            ...
            p = babilities.coef(p, c=1, v=100) if not d['is_verified'] else babilities.coef(p, c=3, v=(100 if SUB > 60 else 0))
            ...
            oa = {'%': str(T if T >= 10 else '0'+str(T))}
            for k in range(len(list(d.keys()))):
                oa[list(d.keys())[k]] = list(d.values())[k]
            o.append(oa)
        return o
