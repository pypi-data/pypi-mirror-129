...
class babilities:
    ...
    def personnal(self):
        ...
        for D in self.ld:
            ...
            if d['biography'] != None:
                if re.findall(repv, d['biography']) != []:
                    p = babilities.coef(p, c=2, v=100)

            if d['name'] != None:
                if re.findall(repv, d['name']) != []:
                    p = babilities.coef(p, c=2, v=100)

            if re.findall(repv, d['username']) != []:
                p = babilities.coef(p, c=2, v=100)
            p = babilities.coef(p, c=4, v=(100 if d['is_private'] else 0))
            p = babilities.coef(p, c=5, v=(100 - (int(d['followers'] if d['followers'] < self.io else self.io) * 100) / self.io))
            p = babilities.coef(p, c=1, v=(100 - (int(d['following'] if d['following'] < 1500 else 1500) * 100) / 1500))
            ...
            p = babilities.coef(p, c=2, v=(100 if d['posts'] > 50 and SUB > 92 else 0))
...
ld = _con.verter('/usr/local/lib/python3.9/site-packages/sterra/export/rchivesaetprovna & rchivesaetprovna_comparaison.Dec-01-2021;22-56-23.xlsx')
#ld = [{'%': '0-12', 'id': 5760166621, 'username': 'ninatypique', 'name': 'nina :)🐠', 'link': 'https://www.instagram.com/ninatypique/', 'biography': 'lyon ✨\nnina@babylone-agency.com \n@ninakipik', 'is_private': 'False', 'followers': 27456, 'following': 594, 'posts': 90, 'external_url': 'http://21buttons.com/ninatypique/', 'is_business_account': 'False', 'is_professional_account': 'True', 'is_verified': 'False', 'business_address_json': 'None', 'business_category_name': 'None', 'business_contact_method': 'None', 'business_email': 'None', 'business_phone_number': 'None', 'connected_fb_page': 'None', 'mutual_followed_by_count': 8, 'has_ar_effects': 'False', 'has_channel': 'False', 'has_clips': 'True', 'has_guides': 'False', 'hide_like_and_view_counts': 'False', 'is_joined_recently': 'False'}]
probas = babilities(ld=ld, pa='personnal').d()
for k in probas:
    print(k['%'], k['username'])